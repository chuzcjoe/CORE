...
RenderCube::RenderCube(core::vulkan::VulkanContext* context,
...
void RenderCube::Render(VkCommandBuffer command_buffer, VkExtent2D extent) {
...
void RenderCube::UpdateUniformBuffer(const glm::mat4& model, const glm::mat4& view,
...
std::vector<core::vulkan::BindingInfo> RenderCube::GetBindingInfo() const {
  return {{0, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, VK_SHADER_STAGE_VERTEX_BIT}};
}

const std::vector<uint32_t> RenderCube::LoadVertexShader() const {
...
const std::vector<uint32_t> RenderCube::LoadFragmentShader() const {
  static const std::vector<uint32_t> shader_code =
...
std::vector<VkVertexInputBindingDescription> RenderCube::GetVertexBindingDescriptions() const {
...
std::vector<VkVertexInputAttributeDescription> RenderCube::GetVertexAttributeDescriptions() const {
...
void RenderCube::BuildGeometry() {
  const float h = 0.5f;
  // 6 faces, each with a distinct base color so rotation is visually obvious.
  const glm::vec3 face_colors[6] = {
      {0.92f, 0.30f, 0.30f},  // +X red
      {0.30f, 0.92f, 0.30f},  // -X green
      {0.30f, 0.55f, 0.95f},  // +Y blue
      {0.95f, 0.85f, 0.30f},  // -Y yellow
      {0.85f, 0.40f, 0.90f},  // +Z magenta
      {0.30f, 0.90f, 0.90f},  // -Z cyan
  };
  ...
  for (int f = 0; f < 6; ++f) {
    ...
    for (int c = 0; c < 4; ++c) {
      vertices_.push_back({face_corners[f][c], normals[f], face_colors[f]});
    }

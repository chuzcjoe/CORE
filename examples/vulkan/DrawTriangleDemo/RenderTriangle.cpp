...
RenderTriangle::RenderTriangle(core::vulkan::VulkanContext* context,
                               core::vulkan::VulkanRenderPass* render_pass)
    : core::vulkan::VulkanRender(context, render_pass) {
  CreateVertexBuffer();
}
...
std::vector<core::vulkan::BindingInfo> RenderTriangle::GetBindingInfo() const {
  return {{0, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, VK_SHADER_STAGE_VERTEX_BIT}};
}

const std::vector<uint32_t> RenderTriangle::LoadVertexShader() const {
...
const std::vector<uint32_t> RenderTriangle::LoadFragmentShader() const {
  static const std::vector<uint32_t> shader_code =
...
std::vector<VkVertexInputBindingDescription> RenderTriangle::GetVertexBindingDescriptions() const {
...
std::vector<VkVertexInputAttributeDescription> RenderTriangle::GetVertexAttributeDescriptions()
    const {
...
void RenderTriangle::CreateVertexBuffer() {
...
void RenderTriangle::UpdateUniformBuffer(const int width, const int height) {

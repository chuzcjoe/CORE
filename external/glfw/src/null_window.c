...
GLFWbool _glfwCreateWindowNull(_GLFWwindow* window, const _GLFWwndconfig* wndconfig,
  ...
  if (wndconfig->mousePassthrough) _glfwSetWindowMousePassthroughNull(window, GLFW_TRUE);
  ...
  if (window->monitor) {
    ...
    if (wndconfig->visible) {
      _glfwShowWindowNull(window);
      if (wndconfig->focused) _glfwFocusWindowNull(window);
    }
  }
...
void _glfwSetWindowMonitorNull(_GLFWwindow* window, _GLFWmonitor* monitor, int xpos, int ypos,
...
void _glfwSetWindowSizeLimitsNull(_GLFWwindow* window, int minwidth, int minheight, int maxwidth,
                                  int maxheight) {
...
void _glfwGetWindowFrameSizeNull(_GLFWwindow* window, int* left, int* top, int* right,
                                 int* bottom) {
...
GLFWbool _glfwWindowMaximizedNull(_GLFWwindow* window) { return window->null.maximized; }
...
GLFWbool _glfwFramebufferTransparentNull(_GLFWwindow* window) { return window->null.transparent; }
...
float _glfwGetWindowOpacityNull(_GLFWwindow* window) { return window->null.opacity; }
...
GLFWbool _glfwRawMouseMotionSupportedNull(void) { return GLFW_TRUE; }

void _glfwShowWindowNull(_GLFWwindow* window) { window->null.visible = GLFW_TRUE; }
...
void _glfwFocusWindowNull(_GLFWwindow* window) {
  ...
  if (previous) {
    ...
    if (previous->monitor && previous->autoIconify) _glfwIconifyWindowNull(previous);
  }

  _glfwInputWindowFocus(window, GLFW_TRUE);
}

GLFWbool _glfwWindowFocusedNull(_GLFWwindow* window) { return _glfw.null.focusedWindow == window; }

GLFWbool _glfwWindowIconifiedNull(_GLFWwindow* window) { return window->null.iconified; }

GLFWbool _glfwWindowVisibleNull(_GLFWwindow* window) { return window->null.visible; }
...
GLFWbool _glfwCreateStandardCursorNull(_GLFWcursor* cursor, int shape) { return GLFW_TRUE; }
...
const char* _glfwGetClipboardStringNull(void) { return _glfw.null.clipboardString; }

EGLenum _glfwGetEGLPlatformNull(EGLint** attribs) { return 0; }
...
int _glfwGetKeyScancodeNull(int key) { return _glfw.null.scancodes[key]; }
...
VkResult _glfwCreateWindowSurfaceNull(VkInstance instance, _GLFWwindow* window,

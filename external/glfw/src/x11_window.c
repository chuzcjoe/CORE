...
static GLFWbool waitForAnyEvent(double* timeout) {
  ...
  struct pollfd fds[] = {[XLIB_FD] = {ConnectionNumber(_glfw.x11.display), POLLIN},
  ...
  while (!XPending(_glfw.x11.display)) {
    ...
    for (int i = 1; i < sizeof(fds) / sizeof(fds[0]); i++) {
      if (fds[i].revents & POLLIN) return GLFW_TRUE;
    }
  }
...
static GLFWbool waitForVisibilityNotify(_GLFWwindow* window) {
  ...
  while (!XCheckTypedWindowEvent(_glfw.x11.display, window->x11.handle, VisibilityNotify, &dummy)) {
...
static int getWindowState(_GLFWwindow* window) {
  ...
  if (_glfwGetWindowPropertyX11(window->x11.handle, _glfw.x11.WM_STATE, _glfw.x11.WM_STATE,
...
static Bool isSelectionEvent(Display* display, XEvent* event, XPointer pointer) {
  if (event->xany.window != _glfw.x11.helperWindowHandle) return False;
...
static int translateKey(int scancode) {
  ...
  if (scancode < 0 || scancode > 255) return GLFW_KEY_UNKNOWN;
...
static void updateWindowMode(_GLFWwindow* window) {
  if (window->monitor) {
    if (_glfw.x11.xinerama.available && _glfw.x11.NET_WM_FULLSCREEN_MONITORS) {
      sendEventToWM(window, _glfw.x11.NET_WM_FULLSCREEN_MONITORS, window->monitor->x11.index,
    ...
    if (_glfw.x11.NET_WM_STATE && _glfw.x11.NET_WM_STATE_FULLSCREEN) {
      ...
      XChangeWindowAttributes(_glfw.x11.display, window->x11.handle, CWOverrideRedirect,
                              &attributes);
    ...
    if (!window->x11.transparent) {
      ...
      XChangeProperty(_glfw.x11.display, window->x11.handle, _glfw.x11.NET_WM_BYPASS_COMPOSITOR,
    ...
    if (_glfw.x11.NET_WM_STATE && _glfw.x11.NET_WM_STATE_FULLSCREEN) {
      sendEventToWM(window, _glfw.x11.NET_WM_STATE, _NET_WM_STATE_REMOVE,
                    _glfw.x11.NET_WM_STATE_FULLSCREEN, 0, 1, 0);
    } else {
      XSetWindowAttributes attributes;
...
static uint32_t decodeUTF8(const char** s) {
  ...
  static const uint32_t offsets[] = {0x00000000u, 0x00003080u, 0x000e2080u,
...
static char* convertLatin1toUTF8(const char* source) {
  ...
  for (sp = source; *sp; sp++) tp += _glfwEncodeUTF8(tp, *sp);
...
static void updateCursorImage(_GLFWwindow* window) {
  if (window->cursorMode == GLFW_CURSOR_NORMAL || window->cursorMode == GLFW_CURSOR_CAPTURED) {
    ...
    XDefineCursor(_glfw.x11.display, window->x11.handle, _glfw.x11.hiddenCursorHandle);
  }
}
...
static void captureCursor(_GLFWwindow* window) {
  XGrabPointer(_glfw.x11.display, window->x11.handle, True,
               ...
               GrabModeAsync, window->x11.handle, None, CurrentTime);
}
...
static GLFWbool createNativeWindow(_GLFWwindow* window, const _GLFWwndconfig* wndconfig,
  ...
  window->x11.handle =
      XCreateWindow(_glfw.x11.display, _glfw.x11.root, xpos, ypos, width, height,
                    0,      // Border width
                    depth,  // Color depth
  ...
  // Declare the WM protocols supported by GLFW
  {
    ...
    XSetWMProtocols(_glfw.x11.display, window->x11.handle, protocols,
  ...
  // Declare our PID
  {
    ...
    XChangeProperty(_glfw.x11.display, window->x11.handle, _glfw.x11.NET_WM_PID, XA_CARDINAL, 32,
                    PropModeReplace, (unsigned char*)&pid, 1);
  }

  if (_glfw.x11.NET_WM_WINDOW_TYPE && _glfw.x11.NET_WM_WINDOW_TYPE_NORMAL) {
    Atom type = _glfw.x11.NET_WM_WINDOW_TYPE_NORMAL;
    XChangeProperty(_glfw.x11.display, window->x11.handle, _glfw.x11.NET_WM_WINDOW_TYPE, XA_ATOM,
  ...
  // Announce support for Xdnd (drag and drop)
  {
    ...
    XChangeProperty(_glfw.x11.display, window->x11.handle, _glfw.x11.XdndAware, XA_ATOM, 32,
...
static Atom writeTargetToProperty(const XSelectionRequestEvent* request) {
  ...
  if (request->target == _glfw.x11.TARGETS) {
    ...
    const Atom targets[] = {_glfw.x11.TARGETS, _glfw.x11.MULTIPLE, _glfw.x11.UTF8_STRING,
                            XA_STRING};

    XChangeProperty(_glfw.x11.display, request->requestor, request->property, XA_ATOM, 32,
  ...
  if (request->target == _glfw.x11.MULTIPLE) {
    ...
    Atom* targets;
    const unsigned long count = _glfwGetWindowPropertyX11(
        request->requestor, request->property, _glfw.x11.ATOM_PAIR, (unsigned char**)&targets);
    ...
    for (unsigned long i = 0; i < count; i += 2) {
      ...
      if (j < formatCount) {
        XChangeProperty(_glfw.x11.display, request->requestor, targets[i + 1], targets[i], 8,
                        PropModeReplace, (unsigned char*)selectionString, strlen(selectionString));
      } else
        targets[i + 1] = None;
    }

    XChangeProperty(_glfw.x11.display, request->requestor, request->property, _glfw.x11.ATOM_PAIR,
  ...
  if (request->target == _glfw.x11.SAVE_TARGETS) {
    ...
    XChangeProperty(_glfw.x11.display, request->requestor, request->property, _glfw.x11.NULL_, 32,
  ...
  for (int i = 0; i < formatCount; i++) {
    if (request->target == formats[i]) {
      ...
      XChangeProperty(_glfw.x11.display, request->requestor, request->property, request->target, 8,
                      PropModeReplace, (unsigned char*)selectionString, strlen(selectionString));
...
static const char* getSelectionString(Atom selection) {
  ...
  *selectionString = NULL;
  ...
  for (size_t i = 0; i < targetCount; i++) {
    ...
    XConvertSelection(_glfw.x11.display, selection, targets[i], _glfw.x11.GLFW_SELECTION,
    ...
    if (notification.xselection.property == None) continue;
    ...
    XGetWindowProperty(_glfw.x11.display, notification.xselection.requestor,
                       notification.xselection.property, 0, LONG_MAX, True, AnyPropertyType,
                       &actualType, &actualFormat, &itemCount, &bytesAfter, (unsigned char**)&data);
    ...
    if (actualType == _glfw.x11.INCR) {
      ...
      for (;;) {
        while (!XCheckIfEvent(_glfw.x11.display, &dummy, isSelPropNewValueNotify,
...
static void acquireMonitor(_GLFWwindow* window) {
  if (_glfw.x11.saver.count == 0) {
    ...
    XGetScreenSaver(_glfw.x11.display, &_glfw.x11.saver.timeout, &_glfw.x11.saver.interval,
...
static void processEvent(XEvent* event) {
  ...
  if (XFindContext(_glfw.x11.display, event->xany.window, _glfw.x11.context, (XPointer*)&window) !=
      0) {
  ...
  switch (event->type) {
    ...
    case KeyPress: {
      ...
      if (window->x11.ic) {
        ...
        if (diff == event->xkey.time || (diff > 0 && diff < ((Time)1 << 31))) {
          if (keycode) _glfwInputKey(window, key, keycode, GLFW_PRESS, mods);
        ...
        if (!filtered) {
          ...
          count = Xutf8LookupString(window->x11.ic, &event->xkey, buffer, sizeof(buffer) - 1, NULL,
                                    &status);
          ...
          if (status == XLookupChars || status == XLookupBoth) {
            const char* c = chars;
            chars[count] = '\0';
            while (c - chars < count) _glfwInputChar(window, decodeUTF8(&c), mods, plain);
          }

          if (chars != buffer) _glfw_free(chars);
        }
      } else {
        KeySym keysym;
        ...
        if (codepoint != GLFW_INVALID_CODEPOINT) _glfwInputChar(window, codepoint, mods, plain);
      }
    ...
    case ConfigureNotify: {
      ...
      if (!event->xany.send_event && window->x11.parent != _glfw.x11.root) {
        ...
        Window dummy;
        XTranslateCoordinates(_glfw.x11.display, window->x11.parent, _glfw.x11.root, xpos, ypos,
                              &xpos, &ypos, &dummy);
    ...
    case ClientMessage: {
      ...
      if (event->xclient.message_type == _glfw.x11.WM_PROTOCOLS) {
        ...
        if (list) {
          count = _glfwGetWindowPropertyX11(_glfw.x11.xdnd.source, _glfw.x11.XdndTypeList, XA_ATOM,
        ...
        if (_glfw.x11.xdnd.format) {
          if (_glfw.x11.xdnd.version >= 1) time = event->xclient.data.l[2];

          // Request the chosen format from the source window
          XConvertSelection(_glfw.x11.display, _glfw.x11.XdndSelection, _glfw.x11.xdnd.format,
                            _glfw.x11.XdndSelection, window->x11.handle, time);
        ...
        XTranslateCoordinates(_glfw.x11.display, _glfw.x11.root, window->x11.handle, xabs, yabs,
                              &xpos, &ypos, &dummy);
    ...
    case SelectionNotify: {
      if (event->xselection.property == _glfw.x11.XdndSelection) {
        ...
        const unsigned long result =
            _glfwGetWindowPropertyX11(event->xselection.requestor, event->xselection.property,
...
//
unsigned long _glfwGetWindowPropertyX11(Window window, Atom property, Atom type,
                                        unsigned char** value) {
  Atom actualType;
  int actualFormat;
  unsigned long itemCount, bytesAfter;

  XGetWindowProperty(_glfw.x11.display, window, property, 0, LONG_MAX, False, type, &actualType,
...
void _glfwPushSelectionToManagerX11(void) {
  XConvertSelection(_glfw.x11.display, _glfw.x11.CLIPBOARD_MANAGER, _glfw.x11.SAVE_TARGETS, None,
                    _glfw.x11.helperWindowHandle, CurrentTime);
...
void _glfwCreateInputContextX11(_GLFWwindow* window) {
  ...
  window->x11.ic = XCreateIC(_glfw.x11.im, XNInputStyle, XIMPreeditNothing | XIMStatusNothing,
                             XNClientWindow, window->x11.handle, XNFocusWindow, window->x11.handle,
  ...
  if (window->x11.ic) {
    ...
    if (XGetICValues(window->x11.ic, XNFilterEvents, &filter, NULL) == NULL) {
      XSelectInput(_glfw.x11.display, window->x11.handle, attribs.your_event_mask | filter);
    }
  }
}
...
void _glfwDestroyWindowX11(_GLFWwindow* window) {
  if (_glfw.x11.disabledCursorWindow == window) enableCursor(window);
...
void _glfwSetWindowTitleX11(_GLFWwindow* window, const char* title) {
  if (_glfw.x11.xlib.utf8) {
    Xutf8SetWMProperties(_glfw.x11.display, window->x11.handle, title, title, NULL, 0, NULL, NULL,
                         NULL);
  }

  XChangeProperty(_glfw.x11.display, window->x11.handle, _glfw.x11.NET_WM_NAME,
                  _glfw.x11.UTF8_STRING, 8, PropModeReplace, (unsigned char*)title, strlen(title));

  XChangeProperty(_glfw.x11.display, window->x11.handle, _glfw.x11.NET_WM_ICON_NAME,
                  _glfw.x11.UTF8_STRING, 8, PropModeReplace, (unsigned char*)title, strlen(title));
...
void _glfwSetWindowIconX11(_GLFWwindow* window, int count, const GLFWimage* images) {
  if (count) {
    ...
    XChangeProperty(_glfw.x11.display, window->x11.handle, _glfw.x11.NET_WM_ICON, XA_CARDINAL, 32,
                    PropModeReplace, (unsigned char*)icon, longCount);

    _glfw_free(icon);
  } else {
    XDeleteProperty(_glfw.x11.display, window->x11.handle, _glfw.x11.NET_WM_ICON);
  }
...
void _glfwGetWindowPosX11(_GLFWwindow* window, int* xpos, int* ypos) {
  ...
  XTranslateCoordinates(_glfw.x11.display, window->x11.handle, _glfw.x11.root, 0, 0, &x, &y,
                        &dummy);
...
void _glfwSetWindowSizeX11(_GLFWwindow* window, int width, int height) {
  if (window->monitor) {
    if (window->monitor->window == window) acquireMonitor(window);
  } else {
...
void _glfwSetWindowSizeLimitsX11(_GLFWwindow* window, int minwidth, int minheight, int maxwidth,
                                 int maxheight) {
  int width, height;
...
void _glfwGetWindowFrameSizeX11(_GLFWwindow* window, int* left, int* top, int* right, int* bottom) {
  ...
  if (window->monitor || !window->decorated) return;

  if (_glfw.x11.NET_FRAME_EXTENTS == None) return;
    ...
    while (!XCheckIfEvent(_glfw.x11.display, &event, isFrameExtentsEvent, (XPointer)window)) {
      if (!waitForX11Event(&timeout)) {
        _glfwInputError(GLFW_PLATFORM_ERROR,
                        "X11: The window manager has a broken _NET_REQUEST_FRAME_EXTENTS "
                        "implementation; please report this issue");
        return;
      }
    }
  }

  if (_glfwGetWindowPropertyX11(window->x11.handle, _glfw.x11.NET_FRAME_EXTENTS, XA_CARDINAL,
                                (unsigned char**)&extents) == 4) {
    if (left) *left = extents[0];
    if (top) *top = extents[2];
    if (right) *right = extents[1];
    if (bottom) *bottom = extents[3];
  }
...
void _glfwGetWindowContentScaleX11(_GLFWwindow* window, float* xscale, float* yscale) {
  if (xscale) *xscale = _glfw.x11.contentScaleX;
  if (yscale) *yscale = _glfw.x11.contentScaleY;
}
...
void _glfwRestoreWindowX11(_GLFWwindow* window) {
  ...
  if (_glfwWindowIconifiedX11(window)) {
    ...
    if (_glfw.x11.NET_WM_STATE && _glfw.x11.NET_WM_STATE_MAXIMIZED_VERT &&
...
void _glfwMaximizeWindowX11(_GLFWwindow* window) {
  ...
  if (_glfwWindowVisibleX11(window)) {
    ...
    Atom* states = NULL;
    unsigned long count = _glfwGetWindowPropertyX11(window->x11.handle, _glfw.x11.NET_WM_STATE,
                                                    XA_ATOM, (unsigned char**)&states);

    // NOTE: We don't check for failure as this property may not exist yet
    //       and that's fine (and we'll create it implicitly with append)

    Atom missing[2] = {_glfw.x11.NET_WM_STATE_MAXIMIZED_VERT,
                       _glfw.x11.NET_WM_STATE_MAXIMIZED_HORZ};
...
void _glfwRequestWindowAttentionX11(_GLFWwindow* window) {
  ...
  sendEventToWM(window, _glfw.x11.NET_WM_STATE, _NET_WM_STATE_ADD,
                _glfw.x11.NET_WM_STATE_DEMANDS_ATTENTION, 0, 1, 0);
}
...
void _glfwSetWindowMonitorX11(_GLFWwindow* window, _GLFWmonitor* monitor, int xpos, int ypos,
                              int width, int height, int refreshRate) {
  if (window->monitor == monitor) {
    if (monitor) {
      if (monitor->window == window) acquireMonitor(window);
    } else {
      if (!window->resizable) updateNormalHints(window, width, height);
...
GLFWbool _glfwWindowMaximizedX11(_GLFWwindow* window) {
  ...
  const unsigned long count = _glfwGetWindowPropertyX11(window->x11.handle, _glfw.x11.NET_WM_STATE,
                                                        XA_ATOM, (unsigned char**)&states);
...
GLFWbool _glfwWindowHoveredX11(_GLFWwindow* window) {
  ...
  while (w) {
    ...
    const Bool result =
        XQueryPointer(_glfw.x11.display, w, &root, &w, &rootX, &rootY, &childX, &childY, &mask);
...
void _glfwSetWindowDecoratedX11(_GLFWwindow* window, GLFWbool enabled) {
  ...
  XChangeProperty(_glfw.x11.display, window->x11.handle, _glfw.x11.MOTIF_WM_HINTS,
                  _glfw.x11.MOTIF_WM_HINTS, 32, PropModeReplace, (unsigned char*)&hints,
...
void _glfwSetWindowFloatingX11(_GLFWwindow* window, GLFWbool enabled) {
  ...
  if (_glfwWindowVisibleX11(window)) {
    ...
    sendEventToWM(window, _glfw.x11.NET_WM_STATE, action, _glfw.x11.NET_WM_STATE_ABOVE, 0, 1, 0);
  } else {
    Atom* states = NULL;
    const unsigned long count = _glfwGetWindowPropertyX11(
        window->x11.handle, _glfw.x11.NET_WM_STATE, XA_ATOM, (unsigned char**)&states);
    ...
    if (enabled) {
      ...
      for (i = 0; i < count; i++) {
        if (states[i] == _glfw.x11.NET_WM_STATE_ABOVE) break;
      }

      if (i == count) {
        XChangeProperty(_glfw.x11.display, window->x11.handle, _glfw.x11.NET_WM_STATE, XA_ATOM, 32,
                        PropModeAppend, (unsigned char*)&_glfw.x11.NET_WM_STATE_ABOVE, 1);
      }
    } else if (states) {
      for (unsigned long i = 0; i < count; i++) {
        if (states[i] == _glfw.x11.NET_WM_STATE_ABOVE) {
          states[i] = states[count - 1];
          XChangeProperty(_glfw.x11.display, window->x11.handle, _glfw.x11.NET_WM_STATE, XA_ATOM,
                          32, PropModeReplace, (unsigned char*)states, count - 1);
          break;
        }
      }
    }
...
void _glfwSetWindowMousePassthroughX11(_GLFWwindow* window, GLFWbool enabled) {
  ...
  if (enabled) {
    ...
    XShapeCombineRegion(_glfw.x11.display, window->x11.handle, ShapeInput, 0, 0, region, ShapeSet);
    XDestroyRegion(region);
  } else {
    XShapeCombineMask(_glfw.x11.display, window->x11.handle, ShapeInput, 0, 0, None, ShapeSet);
  }
}
...
float _glfwGetWindowOpacityX11(_GLFWwindow* window) {
  ...
  if (XGetSelectionOwner(_glfw.x11.display, _glfw.x11.NET_WM_CM_Sx)) {
    ...
    if (_glfwGetWindowPropertyX11(window->x11.handle, _glfw.x11.NET_WM_WINDOW_OPACITY, XA_CARDINAL,
                                  (unsigned char**)&value)) {
...
void _glfwSetWindowOpacityX11(_GLFWwindow* window, float opacity) {
  ...
  XChangeProperty(_glfw.x11.display, window->x11.handle, _glfw.x11.NET_WM_WINDOW_OPACITY,
                  XA_CARDINAL, 32, PropModeReplace, (unsigned char*)&value, 1);
}
...
void _glfwGetCursorPosX11(_GLFWwindow* window, double* xpos, double* ypos) {
  ...
  XQueryPointer(_glfw.x11.display, window->x11.handle, &root, &child, &rootX, &rootY, &childX,
                &childY, &mask);
...
const char* _glfwGetScancodeNameX11(int scancode) {
  if (!_glfw.x11.xkb.available) return NULL;
  ...
  if (keysym == NoSymbol) return NULL;

  const uint32_t codepoint = _glfwKeySym2Unicode(keysym);
  if (codepoint == GLFW_INVALID_CODEPOINT) return NULL;
...
GLFWbool _glfwCreateCursorX11(_GLFWcursor* cursor, const GLFWimage* image, int xhot, int yhot) {
  ...
  if (!cursor->x11.handle) return GLFW_FALSE;
...
void _glfwDestroyCursorX11(_GLFWcursor* cursor) {
  if (cursor->x11.handle) XFreeCursor(_glfw.x11.display, cursor->x11.handle);
}
...
void _glfwSetClipboardStringX11(const char* string) {
  ...
  XSetSelectionOwner(_glfw.x11.display, _glfw.x11.CLIPBOARD, _glfw.x11.helperWindowHandle,
                     CurrentTime);
...
const char* _glfwGetClipboardStringX11(void) { return getSelectionString(_glfw.x11.CLIPBOARD); }
...
EGLenum _glfwGetEGLPlatformX11(EGLint** attribs) {
  ...
  if (_glfw.egl.EXT_platform_base && _glfw.egl.EXT_platform_x11) return EGL_PLATFORM_X11_EXT;
...
void _glfwGetRequiredInstanceExtensionsX11(char** extensions) {
  if (!_glfw.vk.KHR_surface) return;

  if (!_glfw.vk.KHR_xcb_surface || !_glfw.x11.x11xcb.handle) {
    if (!_glfw.vk.KHR_xlib_surface) return;
  }
...
GLFWbool _glfwGetPhysicalDevicePresentationSupportX11(VkInstance instance, VkPhysicalDevice device,
                                                      uint32_t queuefamily) {
  VisualID visualID = XVisualIDFromVisual(DefaultVisual(_glfw.x11.display, _glfw.x11.screen));
  ...
  if (_glfw.vk.KHR_xcb_surface && _glfw.x11.x11xcb.handle) {
    ...
    return vkGetPhysicalDeviceXlibPresentationSupportKHR(device, queuefamily, _glfw.x11.display,
                                                         visualID);
  }
}
...
VkResult _glfwCreateWindowSurfaceX11(VkInstance instance, _GLFWwindow* window,
  ...
  if (_glfw.vk.KHR_xcb_surface && _glfw.x11.x11xcb.handle) {
    ...
    if (err) {
      _glfwInputError(GLFW_PLATFORM_ERROR, "X11: Failed to create Vulkan XCB surface: %s",
    ...
    if (err) {
      _glfwInputError(GLFW_PLATFORM_ERROR, "X11: Failed to create Vulkan X11 surface: %s",
                      _glfwGetVulkanResultString(err));
    }
...
GLFWAPI void glfwSetX11SelectionString(const char* string) {
  ...
  XSetSelectionOwner(_glfw.x11.display, _glfw.x11.PRIMARY, _glfw.x11.helperWindowHandle,
                     CurrentTime);

  if (XGetSelectionOwner(_glfw.x11.display, _glfw.x11.PRIMARY) != _glfw.x11.helperWindowHandle) {
    _glfwInputError(GLFW_PLATFORM_ERROR, "X11: Failed to become owner of primary selection");
  }
}

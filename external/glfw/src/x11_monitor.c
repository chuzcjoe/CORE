...
static GLFWvidmode vidmodeFromModeInfo(const XRRModeInfo* mi, const XRRCrtcInfo* ci) {
  ...
  _glfwSplitBPP(DefaultDepth(_glfw.x11.display, _glfw.x11.screen), &mode.redBits, &mode.greenBits,
                &mode.blueBits);
...
void _glfwPollMonitorsX11(void) {
  if (_glfw.x11.randr.available && !_glfw.x11.randr.monitorBroken) {
    ...
    _glfwInputMonitor(_glfwAllocMonitor("Display", widthMM, heightMM), GLFW_CONNECTED,
                      _GLFW_INSERT_FIRST);
  }
}
...
void _glfwSetVideoModeX11(_GLFWmonitor* monitor, const GLFWvidmode* desired) {
  if (_glfw.x11.randr.available && !_glfw.x11.randr.monitorBroken) {
    ...
    if (native) {
      ...
      XRRSetCrtcConfig(_glfw.x11.display, sr, monitor->x11.crtc, CurrentTime, ci->x, ci->y, native,
...
void _glfwRestoreVideoModeX11(_GLFWmonitor* monitor) {
  if (_glfw.x11.randr.available && !_glfw.x11.randr.monitorBroken) {
    ...
    XRRSetCrtcConfig(_glfw.x11.display, sr, monitor->x11.crtc, CurrentTime, ci->x, ci->y,
                     monitor->x11.oldMode, ci->rotation, ci->outputs, ci->noutput);
...
void _glfwGetMonitorWorkareaX11(_GLFWmonitor* monitor, int* xpos, int* ypos, int* width,
  ...
  if (_glfw.x11.NET_WORKAREA && _glfw.x11.NET_CURRENT_DESKTOP) {
    ...
    if (_glfwGetWindowPropertyX11(_glfw.x11.root, _glfw.x11.NET_CURRENT_DESKTOP, XA_CARDINAL,
      ...
      if (extentCount >= 4 && *desktop < extentCount / 4) {
        ...
        if (areaX + areaWidth > globalX + globalWidth) areaWidth = globalX - areaX + globalWidth;
  ...
  if (xpos) *xpos = areaX;
  if (ypos) *ypos = areaY;
  if (width) *width = areaWidth;
  if (height) *height = areaHeight;
}
  ...
  *count = 0;
  ...
  if (_glfw.x11.randr.available && !_glfw.x11.randr.monitorBroken) {
    ...
    for (int i = 0; i < oi->nmode; i++) {
      ...
      for (j = 0; j < *count; j++) {
        if (_glfwCompareVideoModes(result + j, &mode) == 0) break;
      }
...
GLFWbool _glfwGetVideoModeX11(_GLFWmonitor* monitor, GLFWvidmode* mode) {
  if (_glfw.x11.randr.available && !_glfw.x11.randr.monitorBroken) {
    ...
    _glfwSplitBPP(DefaultDepth(_glfw.x11.display, _glfw.x11.screen), &mode->redBits,
...
GLFWbool _glfwGetGammaRampX11(_GLFWmonitor* monitor, GLFWgammaramp* ramp) {
  if (_glfw.x11.randr.available && !_glfw.x11.randr.gammaBroken) {
    ...
    XF86VidModeGetGammaRamp(_glfw.x11.display, _glfw.x11.screen, ramp->size, ramp->red, ramp->green,
                            ramp->blue);
    return GLFW_TRUE;
  } else {
...
void _glfwSetGammaRampX11(_GLFWmonitor* monitor, const GLFWgammaramp* ramp) {
  if (_glfw.x11.randr.available && !_glfw.x11.randr.gammaBroken) {
    ...
    XF86VidModeSetGammaRamp(_glfw.x11.display, _glfw.x11.screen, ramp->size,
                            (unsigned short*)ramp->red, (unsigned short*)ramp->green,

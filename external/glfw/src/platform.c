...
GLFWbool _glfwSelectPlatform(int desiredID, _GLFWplatform* platform) {
  ...
  if (desiredID != GLFW_ANY_PLATFORM && desiredID != GLFW_PLATFORM_WIN32 &&
      desiredID != GLFW_PLATFORM_COCOA && desiredID != GLFW_PLATFORM_WAYLAND &&
      desiredID != GLFW_PLATFORM_X11 && desiredID != GLFW_PLATFORM_NULL) {
  ...
  if (desiredID == GLFW_ANY_PLATFORM) {
    ...
    if (count == 1) return supportedPlatforms[0].connect(supportedPlatforms[0].ID, platform);
...
GLFWAPI int glfwPlatformSupported(int platformID) {
  ...
  if (platformID != GLFW_PLATFORM_WIN32 && platformID != GLFW_PLATFORM_COCOA &&
      platformID != GLFW_PLATFORM_WAYLAND && platformID != GLFW_PLATFORM_X11 &&

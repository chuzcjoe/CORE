...
GLFWbool _glfwInitVulkan(int mode) {
  ...
  if (_glfw.vk.available) return GLFW_TRUE;
  ...
  if (_glfw.hints.init.vulkanLoader)
    ...
    if (!_glfw.vk.handle) _glfw.vk.handle = _glfwLoadLocalVulkanLoaderCocoa();
    ...
    _glfw.vk.GetInstanceProcAddr = (PFN_vkGetInstanceProcAddr)_glfwPlatformGetModuleSymbol(
        _glfw.vk.handle, "vkGetInstanceProcAddr");
...
void _glfwTerminateVulkan(void) {
  if (_glfw.vk.handle) _glfwPlatformFreeModule(_glfw.vk.handle);
}
...
GLFWAPI const char** glfwGetRequiredInstanceExtensions(uint32_t* count) {
  ...
  *count = 0;
  ...
  if (!_glfwInitVulkan(_GLFW_REQUIRE_LOADER)) return NULL;

  if (!_glfw.vk.extensions[0]) return NULL;
...
GLFWAPI GLFWvkproc glfwGetInstanceProcAddress(VkInstance instance, const char* procname) {
  ...
  if (strcmp(procname, "vkGetInstanceProcAddr") == 0) return (GLFWvkproc)vkGetInstanceProcAddr;

  proc = (GLFWvkproc)vkGetInstanceProcAddr(instance, procname);
  if (!proc) {
    if (_glfw.vk.handle) proc = (GLFWvkproc)_glfwPlatformGetModuleSymbol(_glfw.vk.handle, procname);
  }

  return proc;
}

GLFWAPI int glfwGetPhysicalDevicePresentationSupport(VkInstance instance, VkPhysicalDevice device,
  ...
  if (!_glfwInitVulkan(_GLFW_REQUIRE_LOADER)) return GLFW_FALSE;
...
GLFWAPI VkResult glfwCreateWindowSurface(VkInstance instance, GLFWwindow* handle,
  ...
  *surface = VK_NULL_HANDLE;
  ...
  if (!_glfwInitVulkan(_GLFW_REQUIRE_LOADER)) return VK_ERROR_INITIALIZATION_FAILED;
  ...
  if (window->context.client != GLFW_NO_API) {
    _glfwInputError(GLFW_INVALID_VALUE,
                    "Vulkan: Window surface creation requires the window to have the client API "
                    "set to GLFW_NO_API");

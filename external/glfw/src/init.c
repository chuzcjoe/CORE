...
static _GLFWinitconfig _glfwInitHints = {
    ...
    .vulkanLoader = NULL,
    .ns = {.menubar = GLFW_TRUE, .chdir = GLFW_TRUE},
    .x11 =
        {
...
//
static void* defaultAllocate(size_t size, void* user) { return malloc(size); }

// The deallocation function used when no custom allocator is set
//
static void defaultDeallocate(void* block, void* user) { free(block); }
...
static void terminate(void) {
  ...
  while (_glfw.windowListHead) glfwDestroyWindow((GLFWwindow*)_glfw.windowListHead);

  while (_glfw.cursorListHead) glfwDestroyCursor((GLFWcursor*)_glfw.cursorListHead);

  for (i = 0; i < _glfw.monitorCount; i++) {
    _GLFWmonitor* monitor = _glfw.monitors[i];
    if (monitor->originalRamp.size) _glfw.platform.setGammaRamp(monitor, &monitor->originalRamp);
...
void _glfw_free(void* block) {
  if (block) _glfw.allocator.deallocate(block, _glfw.allocator.user);
}
...
GLFWAPI int glfwGetError(const char** description) {
  ...
  if (error) {
    ...
    if (description && code) *description = error->description;
  }

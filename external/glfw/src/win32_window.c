...
static void updateWindowStyles(const _GLFWwindow* window) {
  ...
  if (_glfwIsWindows10Version1607OrGreaterWin32()) {
    AdjustWindowRectExForDpi(&rect, style, FALSE, getWindowExStyle(window),
...
static void maximizeWindowManually(_GLFWwindow* window) {
  ...
  SetWindowPos(window->win32.handle, HWND_TOP, rect.left, rect.top, rect.right - rect.left,
               rect.bottom - rect.top, SWP_NOACTIVATE | SWP_NOZORDER | SWP_FRAMECHANGED);
}
  ...
  switch (uMsg) {
    case WM_MOUSEACTIVATE: {
      ...
      if (HIWORD(lParam) == WM_LBUTTONDOWN) {
        if (LOWORD(lParam) != HTCLIENT) window->win32.frameAction = GLFW_TRUE;
      }
    ...
    case WM_INPUT: {
      ...
      if (GetRawInputData(ri, RID_INPUT, _glfw.win32.rawInput, &size, sizeof(RAWINPUTHEADER)) ==
          (UINT)-1) {
    ...
    case WM_SIZE: {
      ...
      const GLFWbool maximized =
          wParam == SIZE_MAXIMIZED || (window->win32.maximized && wParam != SIZE_RESTORED);
    ...
    case WM_DROPFILES: {
      ...
      for (i = 0; i < count; i++) _glfw_free(paths[i]);
      _glfw_free(paths);
...
static int createNativeWindow(_GLFWwindow* window, const _GLFWwndconfig* wndconfig,
  ...
  if (!_glfw.win32.mainWindowClass) {
    ...
    wc.hIcon = LoadImageW(GetModuleHandleW(NULL), L"GLFW_ICON", IMAGE_ICON, 0, 0,
  ...
  window->win32.handle = CreateWindowExW(exStyle, MAKEINTATOM(_glfw.win32.mainWindowClass),
                                         wideTitle, style, frameX, frameY, frameWidth, frameHeight,
...
GLFWbool _glfwCreateWindowWin32(_GLFWwindow* window, const _GLFWwndconfig* wndconfig,
...
void _glfwSetWindowSizeLimitsWin32(_GLFWwindow* window, int minwidth, int minheight, int maxwidth,
                                   int maxheight) {
  RECT area;
...
void _glfwGetWindowFrameSizeWin32(_GLFWwindow* window, int* left, int* top, int* right,
                                  int* bottom) {
  RECT rect;
  int width, height;
...
void _glfwSetWindowMonitorWin32(_GLFWwindow* window, _GLFWmonitor* monitor, int xpos, int ypos,
  ...
  if (window->monitor) {
    ...
    SetWindowPos(window->win32.handle, after, rect.left, rect.top, rect.right - rect.left,
...
GLFWbool _glfwFramebufferTransparentWin32(_GLFWwindow* window) {
  ...
  if (!IsWindows8OrGreater()) {
    ...
    //       contents is blended additively with the previous frame instead
    //       of replacing it
    if (FAILED(DwmGetColorizationColor(&color, &opaque)) || opaque) return GLFW_FALSE;
  }
...
void _glfwPollEventsWin32(void) {
  ...
  if (handle) {
    ...
    if (window) {
      int i;
      const int keys[4][2] = {{VK_LSHIFT, GLFW_KEY_LEFT_SHIFT},
      ...
      for (i = 0; i < 4; i++) {
        ...
        if (window->keys[key] != GLFW_PRESS) continue;
  ...
  if (window) {
    ...
    // The re-center is required in order to prevent the mouse cursor stopping at the edges of the
    // screen.
...
void _glfwSetCursorModeWin32(_GLFWwindow* window, int mode) {
  if (_glfwWindowFocusedWin32(window)) {
    if (mode == GLFW_CURSOR_DISABLED) {
      _glfwGetCursorPosWin32(window, &_glfw.win32.restoreCursorPosX,
...
GLFWbool _glfwCreateCursorWin32(_GLFWcursor* cursor, const GLFWimage* image, int xhot, int yhot) {
  ...
  if (!cursor->win32.handle) return GLFW_FALSE;
...
GLFWbool _glfwCreateStandardCursorWin32(_GLFWcursor* cursor, int shape) {
  ...
  cursor->win32.handle =
      LoadImageW(NULL, MAKEINTRESOURCEW(id), IMAGE_CURSOR, 0, 0, LR_DEFAULTSIZE | LR_SHARED);
...
VkResult _glfwCreateWindowSurfaceWin32(VkInstance instance, _GLFWwindow* window,
  ...
  if (err) {
    _glfwInputError(GLFW_PLATFORM_ERROR, "Win32: Failed to create Vulkan surface: %s",

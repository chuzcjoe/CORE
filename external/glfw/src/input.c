...
#define GLFW_MOD_MASK                                                                       \
  (GLFW_MOD_SHIFT | GLFW_MOD_CONTROL | GLFW_MOD_ALT | GLFW_MOD_SUPER | GLFW_MOD_CAPS_LOCK | \
   GLFW_MOD_NUM_LOCK)
...
static _GLFWmapping* findMapping(const char* guid) {
  ...
  for (i = 0; i < _glfw.mappingCount; i++) {
    if (strcmp(_glfw.mappings[i].guid, guid) == 0) return _glfw.mappings + i;
  }
...
static _GLFWmapping* findValidMapping(const _GLFWjoystick* js) {
  ...
  if (mapping) {
    ...
    for (i = 0; i <= GLFW_GAMEPAD_BUTTON_LAST; i++) {
      if (!isValidElementForJoystick(mapping->buttons + i, js)) return NULL;
    }

    for (i = 0; i <= GLFW_GAMEPAD_AXIS_LAST; i++) {
      if (!isValidElementForJoystick(mapping->axes + i, js)) return NULL;
    }
  }
...
static GLFWbool parseMapping(_GLFWmapping* mapping, const char* string) {
  ...
  while (*c) {
    ...
    for (i = 0; i < sizeof(fields) / sizeof(fields[0]); i++) {
      ...
      if (fields[i].element) {
        ...
        length = strlen(name);
        if (strncmp(c, name, length) != 0) return GLFW_FALSE;
      }
  ...
  for (i = 0; i < 32; i++) {
    if (mapping->guid[i] >= 'A' && mapping->guid[i] <= 'F') mapping->guid[i] += 'a' - 'A';
  }
...
void _glfwInputChar(_GLFWwindow* window, uint32_t codepoint, int mods, GLFWbool plain) {
  ...
  if (plain) {
    if (window->callbacks.character) window->callbacks.character((GLFWwindow*)window, codepoint);
  }
}
...
//
_GLFWjoystick* _glfwAllocJoystick(const char* name, const char* guid, int axisCount,
                                  int buttonCount, int hatCount) {
  int jid;
  _GLFWjoystick* js;

  for (jid = 0; jid <= GLFW_JOYSTICK_LAST; jid++) {
    if (!_glfw.joysticks[jid].allocated) break;
  }
...
GLFWAPI void glfwSetInputMode(GLFWwindow* handle, int mode, int value) {
  ...
  switch (mode) {
    case GLFW_CURSOR: {
      if (value != GLFW_CURSOR_NORMAL && value != GLFW_CURSOR_HIDDEN &&
          value != GLFW_CURSOR_DISABLED && value != GLFW_CURSOR_CAPTURED) {
...
GLFWAPI const char* glfwGetKeyName(int key, int scancode) {
  ...
  if (key != GLFW_KEY_UNKNOWN) {
    ...
    if (key != GLFW_KEY_KP_EQUAL && (key < GLFW_KEY_KP_0 || key > GLFW_KEY_KP_ADD) &&
...
GLFWAPI void glfwSetCursorPos(GLFWwindow* handle, double xpos, double ypos) {
  ...
  if (xpos != xpos || xpos < -DBL_MAX || xpos > DBL_MAX || ypos != ypos || ypos < -DBL_MAX ||
      ypos > DBL_MAX) {
...
GLFWAPI GLFWcursor* glfwCreateStandardCursor(int shape) {
  ...
  if (shape != GLFW_ARROW_CURSOR && shape != GLFW_IBEAM_CURSOR && shape != GLFW_CROSSHAIR_CURSOR &&
      shape != GLFW_POINTING_HAND_CURSOR && shape != GLFW_RESIZE_EW_CURSOR &&
      shape != GLFW_RESIZE_NS_CURSOR && shape != GLFW_RESIZE_NWSE_CURSOR &&
      shape != GLFW_RESIZE_NESW_CURSOR && shape != GLFW_RESIZE_ALL_CURSOR &&
...
GLFWAPI void glfwDestroyCursor(GLFWcursor* handle) {
  ...
  // Make sure the cursor is not being used by any window
  {
    ...
    for (window = _glfw.windowListHead; window; window = window->next) {
      if (window->cursor == cursor) glfwSetCursor((GLFWwindow*)window, NULL);
    }
  }
...
GLFWAPI int glfwJoystickPresent(int jid) {
  ...
  js = _glfw.joysticks + jid;
  if (!js->connected) return GLFW_FALSE;
...
GLFWAPI const float* glfwGetJoystickAxes(int jid, int* count) {
  ...
  *count = 0;
  ...
  js = _glfw.joysticks + jid;
  if (!js->connected) return NULL;

  if (!_glfw.platform.pollJoystick(js, _GLFW_POLL_AXES)) return NULL;
...
GLFWAPI const unsigned char* glfwGetJoystickButtons(int jid, int* count) {
  ...
  *count = 0;
  ...
  if (!_glfw.platform.pollJoystick(js, _GLFW_POLL_BUTTONS)) return NULL;
...
GLFWAPI const char* glfwGetJoystickName(int jid) {
  ...
  if (!_glfw.platform.pollJoystick(js, _GLFW_POLL_PRESENCE)) return NULL;
...
GLFWAPI int glfwUpdateGamepadMappings(const char* string) {
        ...
        if (parseMapping(&mapping, line)) {
          ...
          if (previous)
            *previous = mapping;
            ...
            _glfw.mappingCount++;
            _glfw.mappings =
                _glfw_realloc(_glfw.mappings, sizeof(_GLFWmapping) * _glfw.mappingCount);
  ...
  for (jid = 0; jid <= GLFW_JOYSTICK_LAST; jid++) {
    ...
    if (js->connected) js->mapping = findValidMapping(js);
  }
...
GLFWAPI int glfwJoystickIsGamepad(int jid) {
  ...
  if (!_glfw.platform.pollJoystick(js, _GLFW_POLL_PRESENCE)) return GLFW_FALSE;
...
GLFWAPI int glfwGetGamepadState(int jid, GLFWgamepadstate* state) {
  ...
  if (!_glfw.platform.pollJoystick(js, _GLFW_POLL_ALL)) return GLFW_FALSE;
  ...
  for (i = 0; i <= GLFW_GAMEPAD_BUTTON_LAST; i++) {
    ...
    if (e->type == _GLFW_JOYSTICK_AXIS) {
      ...
      if (e->axisOffset < 0 || (e->axisOffset == 0 && e->axisScale > 0)) {
        if (value >= 0.f) state->buttons[i] = GLFW_PRESS;
      } else {
        if (value <= 0.f) state->buttons[i] = GLFW_PRESS;
      }
      ...
      if (js->hats[hat] & bit) state->buttons[i] = GLFW_PRESS;
...
GLFWAPI void glfwSetTime(double time) {
  ...
  _glfw.timer.offset =
      _glfwPlatformGetTimerValue() - (uint64_t)(time * _glfwPlatformGetTimerFrequency());
}

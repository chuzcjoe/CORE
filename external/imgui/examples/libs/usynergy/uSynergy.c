...
static void sTrace(uSynergyContext *context, const char *text) {
  ...
  if (context->m_traceFunc != 0L) context->m_traceFunc(context->m_cookie, text);
}
...
**/
static void sAddUInt8(uSynergyContext *context, uint8_t value) { *context->m_replyCur++ = value; }
...
static void sSendMouseCallback(uSynergyContext *context) {
  ...
  if (context->m_mouseCallback == 0L) return;

  // Send callback
  context->m_mouseCallback(context->m_cookie, context->m_mouseX, context->m_mouseY,
...
**/
static void sSendKeyboardCallback(uSynergyContext *context, uint16_t key, uint16_t modifiers,
                                  uSynergyBool down, uSynergyBool repeat) {
  // Skip if no callback is installed
  if (context->m_keyboardCallback == 0L) return;
...
static void sSendJoystickCallback(uSynergyContext *context, uint8_t joyNum) {
  ...
  // Skip if no callback is installed
  if (context->m_joystickCallback == 0L) return;

  // Send callback
  sticks = context->m_joystickSticks[joyNum];
  context->m_joystickCallback(context->m_cookie, joyNum, context->m_joystickButtons[joyNum],
...
static void sProcessMessage(uSynergyContext *context, const uint8_t *message) {
  ...
  if (memcmp(message + 4, "Synergy", 7) == 0) {
    ...
    //		1 uint32:	The sequence number. It's zero, because this message is always coming from the
    //server? 		1 uint32:	The total size of the remaining 'string' (as per the Synergy %s string
    //format (which is 1 uint32 for size followed by a char buffer (not necessarily null
...
static void sUpdateContext(uSynergyContext *context) {
  ...
  int num_received = 0;
  int packlen = 0;
  if (context->m_receiveFunc(context->m_cookie, context->m_receiveBuffer + context->m_receiveOfs,
                             receive_size, &num_received) == USYNERGY_FALSE) {
    /* Receive failed, let's try to reconnect */
    char buffer[128];
    sprintf(buffer,
            "Receive failed (%d bytes asked, %d bytes received), trying to reconnect in a second",
            receive_size, num_received);
    sTrace(context, buffer);
  ...
  if (num_received == 0) context->m_sleepFunc(context->m_cookie, 500);
  ...
  for (;;) {
    ...
    if (packlen + 4 > context->m_receiveOfs) break;

    /* Process message */
    sProcessMessage(context, context->m_receiveBuffer);

    /* Move packet to front of buffer */
    memmove(context->m_receiveBuffer, context->m_receiveBuffer + packlen + 4,
  ...
  if (packlen > USYNERGY_RECEIVE_BUFFER_SIZE) {
    ...
    char buffer[128];
    sprintf(buffer, "Oversized packet: '%c%c%c%c' (length %d)", context->m_receiveBuffer[4],
            context->m_receiveBuffer[5], context->m_receiveBuffer[6], context->m_receiveBuffer[7],
            packlen);
    sTrace(context, buffer);
    ...
    while (num_received != packlen) {
      ...
      int to_receive =
          buffer_left < USYNERGY_RECEIVE_BUFFER_SIZE ? buffer_left : USYNERGY_RECEIVE_BUFFER_SIZE;
      int ditch_received = 0;
      if (context->m_receiveFunc(context->m_cookie, context->m_receiveBuffer, to_receive,
                                 &ditch_received) == USYNERGY_FALSE) {
        /* Receive failed, let's try to reconnect */
...
void uSynergyUpdate(uSynergyContext *context) {
  if (context->m_connected) {
    ...
    /* Try to connect */
    if (context->m_connectFunc(context->m_cookie)) context->m_connected = USYNERGY_TRUE;
  }
}
...
void uSynergySendClipboard(uSynergyContext *context, const char *text) {
  ...
  sAddUInt32(context,
             4 + 4 + 4 + text_length); /* Rest of message size: numFormats, format, length, data */

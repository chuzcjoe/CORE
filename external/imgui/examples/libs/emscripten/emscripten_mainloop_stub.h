...
//   - It conveys at a glance that this is a no-BS framework, it won't take your main loop away from
//   you.
//   - It is generally nice and elegant.
// - However, comes Emscripten... it is a wonderful and magical tech but it requires a "main loop"
// function.
// - Only some of our examples would run on Emscripten. Typically the ones rendering with GL or WGPU
// ones.
// - I tried to refactor those examples but felt it was problematic that other examples didn't
// follow the
//   same layout. Why would the SDL+GL example be structured one way and the SGL+DX11 be structured
//   differently? Especially as we are trying hard to convey that using a Dear ImGui backend in an
//   *existing application* should requires only a few dozens lines of code, and this should be
//   consistent and symmetrical for all backends.
// - So the next logical step was to refactor all examples to follow that layout of using a "main
// loop" function.

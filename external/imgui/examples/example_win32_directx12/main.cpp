...
int main(int, char**) {
  ...
  // (current version of the backend will only allocate one descriptor, future versions will need to
  // allocate more)
  ...
  // Before 1.91.6: our signature was using a single descriptor. From 1.92, specifying
  // SrvDescriptorAllocFn/SrvDescriptorFreeFn will be required to benefit from new features.
  // ImGui_ImplDX12_Init(g_pd3dDevice, APP_NUM_FRAMES_IN_FLIGHT, DXGI_FORMAT_R8G8B8A8_UNORM,
  ...
  while (!done) {
    ...
    // HRESULT hr = g_pSwapChain->Present(0, g_SwapChainTearingSupport ? DXGI_PRESENT_ALLOW_TEARING
    // : 0); // Present without vsync
...
bool CreateDeviceD3D(HWND hWnd) {
...

  {
    ...
    if (dxgiFactory->CreateSwapChainForHwnd(g_pd3dCommandQueue, hWnd, &sd, nullptr, nullptr,
                                            &swapChain1) != S_OK)
      return false;

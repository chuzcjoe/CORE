...
void android_main(struct android_app* app) {
  ...
  while (true) {
    ...
    while (ALooper_pollOnce(g_Initialized ? 0 : -1, nullptr, &out_events, (void**)&out_data) >= 0) {
      // Process one event
      if (out_data != nullptr) out_data->process(app, out_data);
...
void Init(struct android_app* app) {
  ...
  // This is mostly boilerplate code for EGL...
  {
    ...
    g_EglContext =
        eglCreateContext(g_EglDisplay, egl_config, EGL_NO_CONTEXT, egl_context_attributes);
  ...
  // (android/app/src/main/assets), we use our GetAssetData() helper to retrieve them.

  // We load the default font with increased size to improve readability on many devices with "high"
  // DPI.
  // FIXME: Put some effort into DPI awareness.
  // Important: when calling AddFontFromMemoryTTF(), ownership of font_data is transferred by Dear
  // ImGui by default (deleted is handled by Dear ImGui), unless we set FontDataOwnedByAtlas=false
  // in ImFontConfig
...
void MainLoopStep() {
  ...
  // Our state
  // (we use static, which essentially makes the variable globals, as a convenience to keep the
  // example code easy to follow)
...
static int GetAssetData(const char* filename, void** outData) {
  ...
  AAsset* asset_descriptor =
      AAssetManager_open(g_App->activity->assetManager, filename, AASSET_MODE_BUFFER);
  if (asset_descriptor) {

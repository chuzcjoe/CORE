...
// engine/app.
// - Unlike other backends, the user must call the function ImGui_ImplSDLGPU_PrepareDrawData()
// BEFORE issuing a SDL_GPURenderPass containing ImGui_ImplSDLGPU_RenderDrawData.
...
int main(int, char**) {
  ...
  SDL_GPUDevice* gpu_device = SDL_CreateGPUDevice(
      SDL_GPU_SHADERFORMAT_SPIRV | SDL_GPU_SHADERFORMAT_DXIL | SDL_GPU_SHADERFORMAT_METALLIB, true,
      nullptr);
  ...
  // font rendering.
  // - Read 'docs/FONTS.md' for more instructions and details. If you like the default font but want
  // it to scale better, consider using the 'ProggyVector' from the same author!
  ...
  while (!done) {
    ...
    // all inputs to dear imgui, and hide them from your application based on those two flags. [If
    // using SDL_MAIN_USE_CALLBACKS: call ImGui_ImplSDL3_ProcessEvent() from your SDL_AppEvent()
    // function]
    SDL_Event event;

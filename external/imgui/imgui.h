...
// - Add '#define IMGUI_DEFINE_MATH_OPERATORS' before including this file (or in imconfig.h) to
// access courtesy maths operators for ImVec2 and ImVec4.
...
// IMGUI_API is used for core imgui functions, IMGUI_IMPL_API is used for the default backends files
// (imgui_impl_xxx.h) Using dear imgui via a shared library is not recommended: we don't guarantee
// backward nor forward ABI compatibility + this is a call-heavy library and function call overhead
// adds up.
#ifndef IMGUI_API
#define IMGUI_API
#endif
...
// Read comments above DebugCheckVersionAndDataLayout() for details.
...
// (MSVC provides an equivalent mechanism via SAL Annotations but it requires the macros in a
// different
//  location. e.g. #include <sal.h> + void myprintf(_Printf_format_string_ const char* format, ...),
//  and only works when using Code Analysis, rather than just normal compiling).
// (see https://github.com/ocornut/imgui/issues/8871 for a patch to enable this for MSVC's Code
// Analysis)
...
#pragma clang diagnostic ignored \
    "-Wreserved-identifier"  // warning: identifier '_Xxx' is reserved because it starts with '_'
...
typedef unsigned int
    ImGuiID;  // A unique ID used by widgets (typically the result of hashing a stack of string)
...
struct ImGuiIO;       // Main configuration and I/O between your application and ImGui (also see:
                      // ImGuiPlatformIO)
...
struct ImGuiMultiSelectIO;   // Structure to interact with a BeginMultiSelect()/EndMultiSelect()
                             // block
struct ImGuiOnceUponAFrame;  // Helper for running a block of code not more than once a frame
struct ImGuiPayload;         // User data payload for drag and drop operations
struct ImGuiPlatformIO;  // Interface between platform/renderer backends and ImGui (e.g. Clipboard,
                         // IME hooks). Extends ImGuiIO. In docking branch, this gets extended to
                         // support multi-viewports.
struct ImGuiPlatformImeData;           // Platform IME data for io.PlatformSetImeDataFn() function.
struct ImGuiSelectionBasicStorage;     // Optional helper to store multi-selection state + apply
...
struct ImGuiViewport;  // A Platform Window (always only one in 'master' branch), in the future may
                       // represent Platform Monitor

// Enumerations
// - We don't use strongly typed enums much because they add constraints (can't extend in private
// code, can't store typed in bit fields, extra casting on iteration)
...
//   - In Visual Studio: CTRL+comma ("Edit.GoToAll") can follow symbols inside comments, whereas
//   CTRL+F12 ("Edit.GoToImplementation") cannot.
//   - In Visual Studio w/ Visual Assist installed: ALT+G ("VAssistX.GoToImplementation") can also
//   follow symbols inside comments.
//   - In VS Code, CLion, etc.: CTRL+click can follow symbols inside comments.
enum ImGuiDir : int;  // -> enum ImGuiDir              // Enum: A cardinal direction (Left, Right,
                      // Up, Down)
enum ImGuiKey : int;  // -> enum ImGuiKey              // Enum: A key identifier (ImGuiKey_XXX or
                      // ImGuiMod_XXX value)
enum ImGuiMouseSource : int;     // -> enum ImGuiMouseSource      // Enum; A mouse input source
                                 // identifier (Mouse, TouchScreen, Pen)
enum ImGuiSortDirection : ImU8;  // -> enum ImGuiSortDirection    // Enum: A sorting direction
                                 // (ascending or descending)
typedef int ImGuiCol;  // -> enum ImGuiCol_             // Enum: A color identifier for styling
typedef int
    ImGuiCond;  // -> enum ImGuiCond_            // Enum: A condition for many Set*() functions
typedef int ImGuiDataType;     // -> enum ImGuiDataType_        // Enum: A primary data type
typedef int ImGuiMouseButton;  // -> enum ImGuiMouseButton_     // Enum: A mouse button identifier
...
typedef int ImGuiTableBgTarget;  // -> enum ImGuiTableBgTarget_   // Enum: A color target for
                                 // TableSetBgColor()

// Flags (declared as int to allow using as flags without overhead, and to not pollute the top of
// this file)
// - Tip: Use your programming IDE navigation facilities on the names in the _central column_ below
// to find the actual flags/enum lists!
//   - In Visual Studio: CTRL+comma ("Edit.GoToAll") can follow symbols inside comments, whereas
...
typedef int ImGuiColorEditFlags;  // -> enum ImGuiColorEditFlags_  // Flags: for ColorEdit4(),
                                  // ColorPicker4() etc.
...
typedef int ImGuiHoveredFlags;    // -> enum ImGuiHoveredFlags_    // Flags: for IsItemHovered(),
                                  // IsWindowHovered() etc.
typedef int ImGuiInputFlags;      // -> enum ImGuiInputFlags_      // Flags: for Shortcut(),
                                  // SetNextItemShortcut()
typedef int ImGuiInputTextFlags;  // -> enum ImGuiInputTextFlags_  // Flags: for InputText(),
                                  // InputTextMultiline()
typedef int ImGuiItemFlags;  // -> enum ImGuiItemFlags_       // Flags: for PushItemFlag(), shared
                             // by all items
typedef int
    ImGuiKeyChord;  // -> ImGuiKey | ImGuiMod_XXX    // Flags: for IsKeyChordPressed(), Shortcut()
                    // etc. an ImGuiKey optionally OR-ed with one or more ImGuiMod_XXX values.
typedef int ImGuiListClipperFlags;  // -> enum ImGuiListClipperFlags_// Flags: for ImGuiListClipper
typedef int ImGuiPopupFlags;        // -> enum ImGuiPopupFlags_      // Flags: for OpenPopup*(),
                                    // BeginPopupContext*(), IsPopupOpen()
typedef int
    ImGuiMultiSelectFlags;         // -> enum ImGuiMultiSelectFlags_// Flags: for BeginMultiSelect()
typedef int ImGuiSelectableFlags;  // -> enum ImGuiSelectableFlags_ // Flags: for Selectable()
typedef int ImGuiSliderFlags;      // -> enum ImGuiSliderFlags_     // Flags: for DragFloat(),
                                   // DragInt(), SliderFloat(), SliderInt() etc.
typedef int ImGuiTabBarFlags;      // -> enum ImGuiTabBarFlags_     // Flags: for BeginTabBar()
typedef int ImGuiTabItemFlags;     // -> enum ImGuiTabItemFlags_    // Flags: for BeginTabItem()
typedef int ImGuiTableFlags;       // -> enum ImGuiTableFlags_      // Flags: For BeginTable()
typedef int
    ImGuiTableColumnFlags;       // -> enum ImGuiTableColumnFlags_// Flags: For TableSetupColumn()
typedef int ImGuiTableRowFlags;  // -> enum ImGuiTableRowFlags_   // Flags: For TableNextRow()
typedef int ImGuiTreeNodeFlags;  // -> enum ImGuiTreeNodeFlags_   // Flags: for TreeNode(),
...
typedef unsigned int ImWchar32;    // A single decoded U32 character/code point. We encode them as
                                   // multi bytes UTF-8 when used in strings.
typedef unsigned short ImWchar16;  // A single decoded U16 character/code point. We encode them as
                                   // multi bytes UTF-8 when used in strings.
#ifdef IMGUI_USE_WCHAR32  // ImWchar [configurable type: override in imconfig.h with '#define
                          // IMGUI_USE_WCHAR32' to support Unicode planes 1-16]
...
// - Most users are likely to use this store an item INDEX but this may be used to store a
// POINTER/ID as well. Read comments near ImGuiMultiSelectIO for details.
typedef ImS64 ImGuiSelectionUserData;

// Callback and functions types
typedef int (*ImGuiInputTextCallback)(
    ImGuiInputTextCallbackData* data);  // Callback function for ImGui::InputText()
...
typedef void (*ImGuiMemFreeFunc)(
    void* ptr, void* user_data);  // Function signature for ImGui::SetAllocatorFunctions()

// ImVec2: 2D vector used to store positions, sizes etc. [Compile-time configurable type]
// - This is a frequently used type in the API. Consider using IM_VEC2_CLASS_EXTRA to create
// implicit cast from/to our preferred type.
// - Add '#define IMGUI_DEFINE_MATH_OPERATORS' before including this file (or in imconfig.h) to
// access courtesy maths operators for ImVec2 and ImVec4.
...
// ImTextureID = backend specific, low-level identifier for a texture uploaded in GPU/graphics
// system. [Compile-time configurable type]
// - When a Rendered Backend creates a texture, it store its native identifier into a ImTextureID
// value.
//   (e.g. Used by DX11 backend to a `ID3D11ShaderResourceView*`; Used by OpenGL backends to store
//   `GLuint`;
...
//   - To use something other than a 64-bit value: add '#define ImTextureID MyTextureType*' in your
//   imconfig.h file.
//   - This can be whatever to you want it to be! read the FAQ entry about textures for details.
//   - You may decide to store a higher-level structure containing texture, sampler, shader etc.
//   with various
//     constructors if you like. You will need to implement ==/!= operators.
// History:
// - In v1.91.4 (2024/10/08): the default type for ImTextureID was changed from 'void*' to 'ImU64'.
// This allowed backends requirig 64-bit worth of data to build on 32-bit architectures. Use
// intermediary intptr_t cast and read FAQ if you have casting warnings.
// - In v1.92.0 (2025/06/11): added ImTextureRef which carry either a ImTextureID either a pointer
// to internal texture atlas. All user facing functions taking ImTextureID changed to ImTextureRef
#ifndef ImTextureID
typedef ImU64 ImTextureID;  // Default: store up to 64-bits (any pointer or integer). A majority of
...
// - When a texture is created by user code (e.g. custom images), we directly stores the low-level
// ImTextureID.
//   Because of this, when displaying your own texture you are likely to ever only manage
//   ImTextureID values on your side.
// - When a texture is created by the backend, we stores a ImTextureData* which becomes an
// indirection
...
//   to be frequently useful to the end-user, and it would be erroneously called by many legacy
//   code.
// - If you want to bind the current atlas when using custom rectangle, you can use
// io.Fonts->TexRef.
// - Binding generators for languages such as C (which don't have constructors), should provide a
// helper, e.g.
//      inline ImTextureRef ImTextureRefFromID(ImTextureID tex_id) { ImTextureRef tex_ref = {
//      ._TexData = NULL, .TexID = tex_id }; return tex_ref; }
// In 1.92 we changed most drawing functions using ImTextureID to use ImTextureRef.
// We intentionally do not provide an implicit ImTextureRef -> ImTextureID cast operator because it
// is technically lossy to convert ImTextureRef to ImTextureID before rendering.
IM_MSVC_RUNTIME_CHECKS_OFF
struct ImTextureRef {
  ImTextureRef() {
    _TexData = NULL;
  ...
  inline ImTextureID GetTexID()
      const;  // == (_TexData ? _TexData->TexID : _TexID) // Implemented below in the file.

  // Members (either are set, never both!)
  ImTextureData* _TexData;  //      A texture, generally owned by a ImFontAtlas. Will convert to
                            //      ImTextureID during render loop, after texture has been uploaded.
  ImTextureID _TexID;  // _OR_ Low-level backend texture identifier, if already uploaded or created
                       // by user/app. Generally provided to e.g. ImGui::Image() calls.
};
IM_MSVC_RUNTIME_CHECKS_RESTORE

//-----------------------------------------------------------------------------
// [SECTION] Dear ImGui end-user API functions
// (Note that ImGui:: being a namespace, you can add extra ImGui:: functions in your own separate
// file. Please don't modify imgui source files!)
//-----------------------------------------------------------------------------

namespace ImGui {
// Context creation and access
// - Each context create its own ImFontAtlas by default. You may instance one yourself and pass it
// to CreateContext() to share a font atlas between contexts.
...
//   for each static/DLL boundary you are calling from. Read "Context and Memory Allocators" section
//   of imgui.cpp for details.
...
// Main
IMGUI_API ImGuiIO& GetIO();  // access the ImGuiIO structure (mouse/keyboard/gamepad inputs, time,
                             // various configuration options/flags)
IMGUI_API ImGuiPlatformIO&
GetPlatformIO();  // access the ImGuiPlatformIO structure (mostly hooks/functions to connect to
...
IMGUI_API void
EndFrame();  // ends the Dear ImGui frame. automatically called by Render(). If you don't need to
             // render data (skipping rendering) you may call EndFrame() without Render()... but
             // you'll have wasted CPU already! If you don't need to render, better to not create
             // any windows and not call NewFrame() at all!
IMGUI_API void Render();  // ends the Dear ImGui frame, finalize the draw data. You can then get
                          // call GetDrawData().
IMGUI_API ImDrawData*
GetDrawData();  // valid after Render() and until the next call to NewFrame(). Call
                // ImGui_ImplXXXX_RenderDrawData() function in your Renderer Backend to render.

// Demo, Debug, Information
IMGUI_API void ShowDemoWindow(
    bool* p_open =
        NULL);  // create Demo window. demonstrate most ImGui features. call this to learn about the
                // library! try to make it always available in your application!
IMGUI_API void ShowMetricsWindow(
    bool* p_open = NULL);  // create Metrics/Debugger window. display Dear ImGui internals: windows,
                           // draw commands, various internal state, etc.
IMGUI_API void ShowDebugLogWindow(
    bool* p_open =
        NULL);  // create Debug Log window. display a simplified log of important dear imgui events.
IMGUI_API void ShowIDStackToolWindow(
    bool* p_open = NULL);  // create Stack Tool window. hover items with mouse to query information
...
GetVersion();  // get the compiled version string e.g. "1.80 WIP" (essentially the value for
...
// - You may append multiple times to the same window during the same frame by calling Begin()/End()
// pairs multiple times.
...
//   return value! [Important: due to legacy reason, Begin/End and BeginChild/EndChild are
//   inconsistent with all other functions
//    such as BeginMenu/EndMenu, BeginPopup/EndPopup, etc. where the EndXXX call should only be
//    called if the corresponding BeginXXX function returned true. Begin and BeginChild are the only
//    odd ones out. Will be fixed in a future update.]
...
// - Before 1.90 (November 2023), the "ImGuiChildFlags child_flags = 0" parameter was "bool border =
// false".
//   This API is backward compatible with old code, as we guarantee that ImGuiChildFlags_Borders ==
//   true. Consider updating your old code:
...
// - Specifying ImGuiChildFlags_AutoResizeX or ImGuiChildFlags_AutoResizeY makes the sizing
// automatic based on child contents.
//   Combining both ImGuiChildFlags_AutoResizeX _and_ ImGuiChildFlags_AutoResizeY defeats purpose of
//   a scrolling region and is NOT recommended.
// - BeginChild() returns false to indicate the window is collapsed or fully clipped, so you may
// early out and omit submitting
//   anything to the window. Always call a matching EndChild() for each BeginChild() call,
//   regardless of its return value. [Important: due to legacy reason, Begin/End and
//   BeginChild/EndChild are inconsistent with all other functions
//    such as BeginMenu/EndMenu, BeginPopup/EndPopup, etc. where the EndXXX call should only be
//    called if the corresponding BeginXXX function returned true. Begin and BeginChild are the only
//    odd ones out. Will be fixed in a future update.]
...
IMGUI_API bool IsWindowHovered(
    ImGuiHoveredFlags flags =
        0);  // is current window hovered and hoverable (e.g. not blocked by a popup/modal)? See
             // ImGuiHoveredFlags_ for options. IMPORTANT: If you are trying to check whether your
             // mouse should be dispatched to Dear ImGui or to your underlying app, you should not
             // use this function! Use the 'io.WantCaptureMouse' boolean for that! Refer to FAQ
             // entry "How can I tell whether to dispatch mouse/keyboard to Dear ImGui or my
...
IMGUI_API ImVec2 GetWindowPos();  // get current window position in screen space (IT IS UNLIKELY YOU
                                  // EVER NEED TO USE THIS. Consider always using
                                  // GetCursorScreenPos() and GetContentRegionAvail() instead)
IMGUI_API ImVec2
GetWindowSize();  // get current window size (IT IS UNLIKELY YOU EVER NEED TO USE THIS. Consider
                  // always using GetCursorScreenPos() and GetContentRegionAvail() instead)
IMGUI_API float GetWindowWidth();   // get current window width (IT IS UNLIKELY YOU EVER NEED TO USE
                                    // THIS). Shortcut for GetWindowSize().x.
IMGUI_API float GetWindowHeight();  // get current window height (IT IS UNLIKELY YOU EVER NEED TO
...
IMGUI_API void SetNextWindowSize(
    const ImVec2& size, ImGuiCond cond = 0);  // set next window size. set axis to 0.0f to force an
...
IMGUI_API void SetNextWindowSizeConstraints(
    ...
    void* custom_callback_data =
        NULL);  // set next window size limits. use 0.0f or FLT_MAX if you don't want limits. Use -1
                // for both min and max of same axis to preserve current size (which itself is a
                // constraint). Use callback to apply non-trivial programmatic constraints.
...
IMGUI_API void SetNextWindowCollapsed(
    bool collapsed, ImGuiCond cond = 0);  // set next window collapsed state. call before Begin()
IMGUI_API void
...
IMGUI_API void SetNextWindowBgAlpha(
    float alpha);  // set next window background color alpha. helper to easily override the Alpha
                   // component of ImGuiCol_WindowBg/ChildBg/PopupBg. you may also use
...
IMGUI_API void SetWindowCollapsed(
    bool collapsed, ImGuiCond cond = 0);  // (not recommended) set current window collapsed state.
...
// Windows Scrolling
// - Any change of Scroll will be applied at the beginning of next frame in the first call to
// Begin().
// - You may instead use SetNextWindowScroll() prior to calling Begin() to avoid this delay, as an
// alternative to using SetScrollX()/SetScrollY().
...
//  - PushFont(font, style.FontSizeBase * 2.0f)  // Change font and set size to be twice bigger than
//  current size.
//  - PushFont(font, font->LegacySize)           // Change font and set size to size passed to
//  AddFontXXX() function. Same as pre-1.92 behavior.
// *IMPORTANT* before 1.92, fonts had a single size. They can now be dynamically be adjusted.
//  - In 1.92 we have REMOVED the single parameter version of PushFont() because it seems like the
//  easiest way to provide an error-proof transition.
//  - PushFont(font) before 1.92 = PushFont(font, font->LegacySize) after 1.92          // Use
//  default font size as passed to AddFontXXX() function.
...
//  - CORRECT:   PushFont(NULL, style.FontSizeBase)         // use current unscaled size    == does
//  nothing
//  - CORRECT:   PushFont(NULL, style.FontSizeBase * 2.0f)  // use current unscaled size x2 == make
//  text twice bigger
//  - INCORRECT: PushFont(NULL, GetFontSize())              // INCORRECT! using size after global
//  factors already applied == GLOBAL SCALING FACTORS WILL APPLY TWICE!
//  - INCORRECT: PushFont(NULL, GetFontSize() * 2.0f)       // INCORRECT! using size after global
//  factors already applied == GLOBAL SCALING FACTORS WILL APPLY TWICE!
IMGUI_API void PushFont(ImFont* font,
                        float font_size_base_unscaled);  // Use NULL as a shortcut to keep current
                                                         // font. Use 0.0f to keep current size.
IMGUI_API void PopFont();
IMGUI_API ImFont* GetFont();  // get current font
IMGUI_API float
GetFontSize();  // get current scaled font size (= height in pixels). AFTER global scale factors
                // applied. *IMPORTANT* DO NOT PASS THIS VALUE TO PushFont()! Use
                // ImGui::GetStyle().FontSizeBase to get value before global scale factors.
IMGUI_API ImFontBaked* GetFontBaked();  // get current font bound at current size // ==
                                        // GetFont()->GetFontBaked(GetFontSize())
...
IMGUI_API void PushStyleVarX(ImGuiStyleVar idx,
                             float val_x);  // modify X component of a style ImVec2 variable. "
IMGUI_API void PushStyleVarY(ImGuiStyleVar idx,
                             float val_y);  // modify Y component of a style ImVec2 variable. "
IMGUI_API void PopStyleVar(int count = 1);
IMGUI_API void PushItemFlag(ImGuiItemFlags option,
...
IMGUI_API float
CalcItemWidth();  // width of item given pushed settings and current cursor position. NOT
                  // necessarily the width of last item unlike most 'Item' functions.
IMGUI_API void PushTextWrapPos(
    float wrap_local_pos_x = 0.0f);  // push word-wrapping position for Text*() commands. < 0.0f: no
...
IMGUI_API ImVec2 GetFontTexUvWhitePixel();  // get UV coordinate for a white pixel, useful to draw
...
IMGUI_API ImU32
GetColorU32(ImU32 col, float alpha_mul = 1.0f);  // retrieve given color with style alpha applied,
                                                 // packed as a 32-bit value suitable for ImDrawList
...
//    functions. -> this is the preferred way forward.
//    - Window-local coordinates:   SameLine(offset), GetCursorPos(), SetCursorPos(),
//    GetCursorStartPos(), PushTextWrapPos()
//    - Window-local coordinates:   GetContentRegionMax(), GetWindowContentRegionMin(),
//    GetWindowContentRegionMax() --> all obsoleted. YOU DON'T NEED THEM.
// - GetCursorScreenPos() = GetCursorPos() + GetWindowPos(). GetWindowPos() is almost only ever
// useful to convert from window-local to absolute coordinates. Try not to use it.
IMGUI_API ImVec2 GetCursorScreenPos();  // cursor position, absolute coordinates. THIS IS YOUR BEST
                                        // FRIEND (prefer using this rather than GetCursorPos(),
                                        // also more useful to work with ImDrawList API).
IMGUI_API void SetCursorScreenPos(
    const ImVec2& pos);  // cursor position, absolute coordinates. THIS IS YOUR BEST FRIEND.
IMGUI_API ImVec2
GetContentRegionAvail();  // available space from current position. THIS IS YOUR BEST FRIEND.
IMGUI_API ImVec2 GetCursorPos();  // [window-local] cursor position in window-local coordinates.
...
IMGUI_API ImVec2
GetCursorStartPos();  // [window-local] initial cursor position, in window-local coordinates. Call
...
// ID stack/scopes
// Read the FAQ (docs/FAQ.md or http://dearimgui.com/faq) for more details about how ID are handled
// in dear imgui.
// - Those questions are answered and impacted by understanding of the ID stack system:
//   - "Q: Why is my widget not reacting when I click on it?"
//   - "Q: How can I have widgets with an empty label?"
//   - "Q: How can I have multiple widgets with the same label?"
// - Short version: ID are hashes of the entire ID stack. If you are creating widgets in a loop you
// most likely
//   want to push a unique identifier (e.g. object pointer, loop index) to uniquely differentiate
//   them.
...
IMGUI_API void SeparatorText(
    const char* label);  // currently: formatted text with a horizontal line
...
IMGUI_API bool SmallButton(
    const char* label);  // button with (FramePadding.y == 0) to easily embed within text
...
IMGUI_API void ProgressBar(float fraction, const ImVec2& size_arg = ImVec2(-FLT_MIN, 0),
                           const char* overlay = NULL);
IMGUI_API void
Bullet();  // draw a small circle + keep the cursor on the same line. advance cursor x position by
...
IMGUI_API bool TextLinkOpenURL(
    const char* label,
    const char* url = NULL);  // hyperlink text button, automatically open file/url when clicked

// Widgets: Images
// - Read about ImTextureID/ImTextureRef  here:
// https://github.com/ocornut/imgui/wiki/Image-Loading-and-Displaying-Examples
// - 'uv0' and 'uv1' are texture coordinates. Read about them from the same link above.
// - Image() pads adds style.ImageBorderSize on each side, ImageButton() adds style.FramePadding on
// each side.
// - ImageButton() draws a background based on regular Button() color + optionally an inner
// background if specified.
// - An obsolete version of Image(), before 1.91.9 (March 2025), had a 'tint_col' parameter which is
...
IMGUI_API bool Combo(const char* label, int* current_item, const char* const items[],
...
// by default and can go off-bounds. Use ImGuiSliderFlags_AlwaysClamp to always clamp.
// - For all the Float2/Float3/Float4/Int2/Int3/Int4 versions of every function, note that a 'float
// v[X]' function argument is the same as 'float* v',
//   the array syntax is just a way to document the number of elements that are expected to be
//   accessible. You can pass address of your first element out of a contiguous set, e.g.
//   &myvector.x
...
// increase value by 1). For keyboard/gamepad navigation, minimum speed is Max(v_speed,
// minimum_step_at_given_precision).
// - Use v_min < v_max to clamp edits to given limits. Note that CTRL+Click manual input can
// override those limits if ImGuiSliderFlags_AlwaysClamp is not used.
...
// - Legacy: Pre-1.78 there are DragXXX() function signatures that take a final `float power=1.0f'
// argument instead of the `ImGuiSliderFlags flags=0' argument.
//   If you get a warning converting a float to ImGuiSliderFlags, read
//   https://github.com/ocornut/imgui/issues/3361
IMGUI_API bool DragFloat(const char* label, float* v, float v_speed = 1.0f, float v_min = 0.0f,
                         float v_max = 0.0f, const char* format = "%.3f",
                         ImGuiSliderFlags flags = 0);  // If v_min >= v_max we have no bound
IMGUI_API bool DragFloat2(const char* label, float v[2], float v_speed = 1.0f, float v_min = 0.0f,
                          float v_max = 0.0f, const char* format = "%.3f",
                          ImGuiSliderFlags flags = 0);
IMGUI_API bool DragFloat3(const char* label, float v[3], float v_speed = 1.0f, float v_min = 0.0f,
                          float v_max = 0.0f, const char* format = "%.3f",
                          ImGuiSliderFlags flags = 0);
IMGUI_API bool DragFloat4(const char* label, float v[4], float v_speed = 1.0f, float v_min = 0.0f,
                          float v_max = 0.0f, const char* format = "%.3f",
                          ImGuiSliderFlags flags = 0);
IMGUI_API bool DragFloatRange2(const char* label, float* v_current_min, float* v_current_max,
                               ...
                               ImGuiSliderFlags flags = 0);
IMGUI_API bool DragInt(const char* label, int* v, float v_speed = 1.0f, int v_min = 0,
                       int v_max = 0, const char* format = "%d",
                       ImGuiSliderFlags flags = 0);  // If v_min >= v_max we have no bound
IMGUI_API bool DragInt2(const char* label, int v[2], float v_speed = 1.0f, int v_min = 0,
                        int v_max = 0, const char* format = "%d", ImGuiSliderFlags flags = 0);
IMGUI_API bool DragInt3(const char* label, int v[3], float v_speed = 1.0f, int v_min = 0,
                        int v_max = 0, const char* format = "%d", ImGuiSliderFlags flags = 0);
IMGUI_API bool DragInt4(const char* label, int v[4], float v_speed = 1.0f, int v_min = 0,
                        int v_max = 0, const char* format = "%d", ImGuiSliderFlags flags = 0);
IMGUI_API bool DragIntRange2(const char* label, int* v_current_min, int* v_current_max,
...
// 1; etc.
// - Format string may also be set to NULL or use the default format ("%f" or "%d").
// - Legacy: Pre-1.78 there are SliderXXX() function signatures that take a final `float power=1.0f'
// argument instead of the `ImGuiSliderFlags flags=0' argument.
//   If you get a warning converting a float to ImGuiSliderFlags, read
//   https://github.com/ocornut/imgui/issues/3361
...
IMGUI_API bool SliderScalar(const char* label, ImGuiDataType data_type, void* p_data,
                            ...
                            ImGuiSliderFlags flags = 0);
IMGUI_API bool SliderScalarN(const char* label, ImGuiDataType data_type, void* p_data,
                             int components, const void* p_min, const void* p_max,
                             const char* format = NULL, ImGuiSliderFlags flags = 0);
IMGUI_API bool VSliderFloat(const char* label, const ImVec2& size, float* v, float v_min,
...
IMGUI_API bool VSliderInt(const char* label, const ImVec2& size, int* v, int v_min, int v_max,
                          const char* format = "%d", ImGuiSliderFlags flags = 0);
IMGUI_API bool VSliderScalar(const char* label, const ImVec2& size, ImGuiDataType data_type,
...
// misc/cpp/imgui_stdlib.h and comments in imgui_demo.cpp.
...
IMGUI_API bool InputScalarN(const char* label, ImGuiDataType data_type, void* p_data,
...
IMGUI_API bool TreeNodeEx(const void* ptr_id, ImGuiTreeNodeFlags flags, const char* fmt, ...)
    IM_FMTARGS(3);
IMGUI_API bool TreeNodeExV(const char* str_id, ImGuiTreeNodeFlags flags, const char* fmt,
                           va_list args) IM_FMTLIST(3);
IMGUI_API bool TreeNodeExV(const void* ptr_id, ImGuiTreeNodeFlags flags, const char* fmt,
                           va_list args) IM_FMTLIST(3);
...
IMGUI_API void SetNextItemOpen(
    bool is_open, ImGuiCond cond = 0);  // set next TreeNode/CollapsingHeader open state.
IMGUI_API void SetNextItemStorageID(
    ImGuiID storage_id);  // set id to use for open/close storage (default to same as item id).
...
// - This enables standard multi-selection/range-selection idioms (CTRL+Mouse/Keyboard,
// SHIFT+Mouse/Keyboard, etc.) in a way that also allow a clipper to be used.
// - ImGuiSelectionUserData is often used to store your item index within the current view (but may
...
// - TreeNode() is technically supported but... using this correctly is more complicated. You need
// some sort of linear/random access to your tree,
//   which is suited to advanced trees setups already implementing filters and clipper. We will work
//   simplifying the current demo.
// - 'selection_size' and 'items_count' parameters are optional and used by a few features. If they
// are costly for you to compute, you may avoid them.
IMGUI_API ImGuiMultiSelectIO* BeginMultiSelect(ImGuiMultiSelectFlags flags, int selection_size = -1,
                                               int items_count = -1);
IMGUI_API ImGuiMultiSelectIO* EndMultiSelect();
IMGUI_API void SetNextItemSelectionUserData(ImGuiSelectionUserData selection_user_data);
IMGUI_API bool
IsItemToggledSelection();  // Was the last item selection state toggled? Useful if you need the
                           // per-item information _before_ reaching EndMultiSelect(). We only
                           // returns toggle _event_ in order to handle clipping correctly.

// Widgets: List Boxes
// - This is essentially a thin wrapper to using BeginChild/EndChild with the
// ImGuiChildFlags_FrameStyle flag for stylistic changes + displaying a label.
// - If you don't need a label you can probably simply use BeginChild() with the
// ImGuiChildFlags_FrameStyle flag for the same result.
...
// available for convenience purpose. This is analogous to how Combos are created.
// - Choose frame width:   size.x > 0.0f: custom  /  size.x < 0.0f or -FLT_MIN: right-align   /
// size.x = 0.0f (default): use current ItemWidth
// - Choose frame height:  size.y > 0.0f: custom  /  size.y < 0.0f or -FLT_MIN: bottom-align  /
// size.y = 0.0f (default): arbitrary default height which can fit ~7 items
IMGUI_API bool BeginListBox(const char* label,
...
IMGUI_API void PlotLines(const char* label, float (*values_getter)(void* data, int idx), void* data,
...
IMGUI_API void PlotHistogram(const char* label, const float* values, int values_count,
                             ...
                             float scale_min = FLT_MAX, float scale_max = FLT_MAX,
                             ImVec2 graph_size = ImVec2(0, 0), int stride = sizeof(float));
IMGUI_API void PlotHistogram(const char* label, float (*values_getter)(void* data, int idx),
...
// - SetTooltip() is more or less a shortcut for the 'if (BeginTooltip()) { Text(...); EndTooltip();
// }' idiom (with a subtlety that it discard any previously submitted tooltip)
...
IMGUI_API void SetTooltip(const char* fmt, ...)
    IM_FMTARGS(1);  // set a text-only tooltip. Often used after a ImGui::IsItemHovered() check.
...
// BeginTooltip())' idiom.
// - SetItemTooltip() is a shortcut for the 'if (IsItemHovered(ImGuiHoveredFlags_ForTooltip)) {
// SetTooltip(...); }' idiom.
// - Where 'ImGuiHoveredFlags_ForTooltip' itself is a shortcut to use
// 'style.HoverFlagsForTooltipMouse' or 'style.HoverFlagsForTooltipNav' depending on active input
// type. For mouse it defaults to 'ImGuiHoveredFlags_Stationary | ImGuiHoveredFlags_DelayShort'.
...
IMGUI_API void SetItemTooltip(const char* fmt, ...)
    IM_FMTARGS(1);  // set a text-only tooltip if preceding item was hovered. override any previous
                    // call to SetTooltip().
...
//  - IMPORTANT: Popup identifiers are relative to the current ID stack, so OpenPopup and BeginPopup
//  generally needs to be at the same level of the stack.
//    This is sometimes leading to confusing mistakes. May rework this in the future.
//  - BeginPopup(): query popup state, if open start appending into the window. Call EndPopup()
//  afterwards if returned true. ImGuiWindowFlags are forwarded to the window.
//  - BeginPopupModal(): block every interaction behind the window, cannot be closed by user, add a
//  dimming background, has a title bar.
...
IMGUI_API bool BeginPopupModal(const char* name, bool* p_open = NULL,
                               ImGuiWindowFlags flags = 0);  // return true if the modal is open,
...
//  need some options).
//  - Use ImGuiPopupFlags_NoOpenOverExistingPopup to avoid opening a popup if there's already one at
//  the same level. This is equivalent to e.g. testing for !IsAnyPopupOpen() prior to OpenPopup().
//  - Use IsWindowAppearing() after BeginPopup() to tell if a window just opened.
//  - IMPORTANT: Notice that for OpenPopupOnItemClick() we exceptionally default flags to 1 (==
//  ImGuiPopupFlags_MouseButtonRight) for backward compatibility with older API taking 'int
//  mouse_button = 1' parameter
...
IMGUI_API void OpenPopupOnItemClick(
    ...
    ImGuiPopupFlags popup_flags =
        1);  // helper to open popup when clicked on last item. Default to
             // ImGuiPopupFlags_MouseButtonRight == 1. (note: actually triggers on the mouse
             // _released_ event to be consistent with popup behaviors)
IMGUI_API void CloseCurrentPopup();  // manually close the popup we have begin-ed into.

// Popups: open+begin combined functions helpers
//  - Helpers to do OpenPopup+BeginPopup where the Open action is triggered by e.g. hovering an item
//  and right-clicking.
//  - They are convenient to easily create context menus, hence the name.
//  - IMPORTANT: Notice that BeginPopupContextXXX takes ImGuiPopupFlags just like OpenPopup() and
//  unlike BeginPopup(). For full consistency, we may add ImGuiWindowFlags to the
//  BeginPopupContextXXX functions in the future.
//  - IMPORTANT: Notice that we exceptionally default their flags to 1 (==
//  ImGuiPopupFlags_MouseButtonRight) for backward compatibility with older API taking 'int
//  mouse_button = 1' parameter, so if you add other flags remember to re-add the
...
//  - IsPopupOpen(): return true if the popup is open at the current BeginPopup() level of the popup
//  stack.
...
//  - IsPopupOpen() with ImGuiPopupFlags_AnyPopupId + ImGuiPopupFlags_AnyPopupLevel: return true if
//  any popup is open.
...
//    into a column.
//    - If you are using tables as a sort of grid, where every column is holding the same type of
//    contents,
//      you may prefer using TableNextColumn() instead of TableNextRow() + TableSetColumnIndex().
//      TableNextColumn() will automatically wrap-around into the next row if needed.
//    - IMPORTANT: Comparatively to the old Columns() API, we need to call TableNextColumn() for the
//    first column!
//    - Summary of possible call flow:
//        - TableNextRow() -> TableSetColumnIndex(0) -> Text("Hello 0") -> TableSetColumnIndex(1) ->
//        Text("Hello 1")  // OK
//        - TableNextRow() -> TableNextColumn()      -> Text("Hello 0") -> TableNextColumn()      ->
//        Text("Hello 1")  // OK
//        -                   TableNextColumn()      -> Text("Hello 0") -> TableNextColumn()      ->
//        Text("Hello 1")  // OK: TableNextColumn() automatically gets to next row!
...
IMGUI_API void TableSetupScrollFreeze(
    int cols, int rows);  // lock columns/rows so they stay visible when scrolled.
IMGUI_API void TableHeader(const char* label);  // submit one header cell manually (rarely used)
IMGUI_API void TableHeadersRow();  // submit a row with headers cells based on data provided to
                                   // TableSetupColumn() + submit context menu
IMGUI_API void
TableAngledHeadersRow();  // submit a row with angled headers for every column with the
                          // ImGuiTableColumnFlags_AngledHeader flag. MUST BE FIRST ROW.

// Tables: Sorting & Miscellaneous functions
// - Sorting: call TableGetSortSpecs() to retrieve latest sort specs for the table. NULL when not
// sorting.
//   When 'sort_specs->SpecsDirty == true' you should sort your data. It will be true when sorting
//   specs have changed since last call, or the first time. Make sure to set 'SpecsDirty = false'
...
TableGetSortSpecs();  // get latest sort specs for the table (NULL if not sorting).  Lifetime: don't
                      // hold on this pointer over multiple frames or past any subsequent call to
                      // BeginTable().
...
IMGUI_API ImGuiTableColumnFlags TableGetColumnFlags(
    int column_n =
        -1);  // return column flags so you can query their Enabled/Visible/Sorted/Hovered status
              // flags. Pass -1 to use current column.
IMGUI_API void TableSetColumnEnabled(
    int column_n,
    bool v);  // change user accessible enabled/disabled state of a column. Set to false to hide the
              // column. User can use the context menu to change this themselves (right-click in
              // headers, or right-click in columns body with ImGuiTableFlags_ContextMenuInBody)
IMGUI_API int
TableGetHoveredColumn();  // return hovered column. return -1 when table is not hovered. return
                          // columns_count if the unused space at the right of visible columns is
                          // hovered. Can also use (TableGetColumnFlags() &
                          // ImGuiTableColumnFlags_IsHovered) instead.
IMGUI_API void TableSetBgColor(ImGuiTableBgTarget target, ImU32 color,
                               int column_n = -1);  // change the color of a cell, row, or column.
...
// Tab Bars, Tabs
// - Note: Tabs are automatically created by the docking system (when in 'docking' branch). Use this
// to create tab bars/tabs yourself.
...
IMGUI_API void SetTabItemClosed(
    const char*
        tab_or_docked_window_label);  // notify TabBar or Docking system of a closed tab/window
                                      ...
                                      // bars). For tab-bar: call after BeginTabBar() and before Tab
...
IMGUI_API bool SetDragDropPayload(
    ...
    ImGuiCond cond = 0);  // type is a user defined string of maximum 32 characters. Strings
                          // starting with '_' are reserved for dear imgui internal types. Data is
                          // copied and held by imgui. Return true when payload has been accepted.
IMGUI_API void
...
GetDragDropPayload();  // peek directly into the current payload from anywhere. returns NULL when
                       // drag and drop is finished or inactive. use ImGuiPayload::IsDataType() to
                       // test for the payload type.

// Disabling [BETA API]
// - Disable all user interactions and dim items visuals (applying style.DisabledAlpha over current
// colors)
...
// of boolean expressions (as a micro-optimization: if you have tens of thousands of
// BeginDisabled(false)/EndDisabled() pairs, you might want to reformulate your code to avoid making
// those calls)
...
// Focus, Activation
IMGUI_API void
SetItemDefaultFocus();  // make last item the default focused item of a newly appearing window.
...
IMGUI_API void SetNavCursorVisible(
    bool visible);  // alter visibility of keyboard/gamepad cursor. by default: show when using an
                    // arrow key, hide when clicking with mouse.

// Overlapping mode
IMGUI_API void
SetNextItemAllowOverlap();  // allow next item to be overlapped by a subsequent item. Useful with
                            // invisible buttons, selectable, treenode covering an area where
                            // subsequent items may need to be added. Note that both Selectable()
...
IMGUI_API bool IsItemClicked(
    ImGuiMouseButton mouse_button =
        0);  // is the last item hovered and mouse clicked on? (**)  == IsMouseClicked(mouse_button)
             // && IsItemHovered()Important. (**) this is NOT equivalent to the behavior of e.g.
             // Button(). Read comments in function definition.
...
IsItemDeactivated();  // was the last item just made inactive (item was previously active). Useful
                      // for Undo/Redo patterns with widgets that require continuous editing.
IMGUI_API bool
IsItemDeactivatedAfterEdit();  // was the last item just made inactive and made a value change when
                               ...
                               // patterns with widgets that require continuous editing. Note that
...
// Viewports
// - Currently represents the Platform Window created by the application which is hosting our Dear
// ImGui windows.
// - In 'docking' branch with multi-viewport enabled, we extend this concept to have multiple active
// viewports.
...
GetBackgroundDrawList();  // this draw list will be the first rendered one. Useful to quickly draw
...
IMGUI_API void ColorConvertRGBtoHSV(float r, float g, float b, float& out_h, float& out_s,
                                    float& out_v);
IMGUI_API void ColorConvertHSVtoRGB(float h, float s, float v, float& out_r, float& out_g,
                                    float& out_b);
...
// - (legacy: before v1.87, we used ImGuiKey to carry native/user indices as defined by each
// backends. This was obsoleted in 1.87 (2022-02) and completely removed in 1.91.5 (2024-11). See
// https://github.com/ocornut/imgui/issues/4921)
// - (legacy: any use of ImGuiKey will assert when key < 512 to detect passing legacy native/user
// indices)
IMGUI_API bool IsKeyDown(ImGuiKey key);  // is key being held.
IMGUI_API bool IsKeyPressed(
    ImGuiKey key, bool repeat = true);       // was key pressed (went from !Down to Down)? if
                                             // repeat=true, uses io.KeyRepeatDelay / KeyRepeatRate
IMGUI_API bool IsKeyReleased(ImGuiKey key);  // was key released (went from Down to !Down)?
IMGUI_API bool IsKeyChordPressed(
    ImGuiKeyChord
        key_chord);  // was key chord (mods + key) pressed, e.g. you can pass 'ImGuiMod_Ctrl |
                     // ImGuiKey_S' as a key-chord. This doesn't do any routing or focus check,
...
IMGUI_API const char* GetKeyName(
    ImGuiKey key);  // [DEBUG] returns English name of the key. Those names are provided for
                    // debugging purpose and are not meant to be saved persistently nor compared.
IMGUI_API void SetNextFrameWantCaptureKeyboard(
    bool want_capture_keyboard);  // Override io.WantCaptureKeyboard flag next frame (said flag is
                                  // left for your application to handle, typically when true it
                                  // instructs your app to ignore inputs). e.g. force capture
                                  // keyboard when your widget is being hovered. This is equivalent
                                  // to setting "io.WantCaptureKeyboard = want_capture_keyboard";
                                  // after the next NewFrame() call.

// Inputs Utilities: Shortcut Testing & Routing [BETA]
// - ImGuiKeyChord = a ImGuiKey + optional ImGuiMod_Alt/ImGuiMod_Ctrl/ImGuiMod_Shift/ImGuiMod_Super.
//       ImGuiKey_C                          // Accepted by functions taking ImGuiKey or
//       ImGuiKeyChord arguments ImGuiMod_Ctrl | ImGuiKey_C          // Accepted by functions taking
//       ImGuiKeyChord arguments
//   only ImGuiMod_XXX values are legal to combine with an ImGuiKey. You CANNOT combine two ImGuiKey
//   values.
// - The general idea is that several callers may register interest in a shortcut, and only one
// owner gets it.
//      Parent   -> call Shortcut(Ctrl+S)    // When Parent is focused, Parent gets the shortcut.
//        Child1 -> call Shortcut(Ctrl+S)    // When Child1 is focused, Child1 gets the shortcut
//        (Child1 overrides Parent shortcuts) Child2 -> no call                  // When Child2 is
//        focused, Parent gets the shortcut.
//   The whole system is order independent, so if Child1 makes its calls before Parent, results will
//   be identical. This is an important property as it facilitate working with foreign code or
//   larger codebase.
// - To understand the difference:
//   - IsKeyChordPressed() compares mods and call IsKeyPressed() -> function has no side-effect.
//   - Shortcut() submits a route, routes are resolved, if it currently can be routed it calls
//   IsKeyChordPressed() -> function has (desirable) side-effects as it can prevents another call
//   from getting the route.
...
//   e.g. Button(...); SetItemKeyOwner(ImGuiKey_MouseWheelY); to make hovering/activating a button
//   disable wheel for scrolling.
// - Reminder ImGuiKey enum include access to mouse buttons and gamepad, so key ownership can apply
// to them.
// - Many related features are still in imgui_internal.h. For instance, most IsKeyXXX()/IsMouseXXX()
// functions have an owner-id-aware version.
IMGUI_API void SetItemKeyOwner(
    ImGuiKey key);  // Set key owner to last item ID if it is hovered or active. Equivalent to 'if
...
IMGUI_API bool IsMouseClicked(
    ImGuiMouseButton button, bool repeat = false);  // did mouse button clicked? (went from !Down to
                                                    // Down). Same as GetMouseClickedCount() == 1.
IMGUI_API bool IsMouseReleased(
    ImGuiMouseButton button);  // did mouse button released? (went from Down to !Down)
IMGUI_API bool IsMouseDoubleClicked(
    ImGuiMouseButton
        button);  // did mouse button double-clicked? Same as GetMouseClickedCount() == 2. (note
                  // that a double-click will also report IsMouseClicked() == true)
IMGUI_API bool IsMouseReleasedWithDelay(
    ImGuiMouseButton button,
    float delay);  // delayed mouse release (use very sparingly!). Generally used with 'delay >=
                   // io.MouseDoubleClickTime' + combined with a 'io.MouseClickedLastCount==1' test.
                   // This is a very rarely used UI idiom, but some apps use this: e.g. MS Explorer
                   // single click on an icon to rename.
IMGUI_API int GetMouseClickedCount(
    ImGuiMouseButton button);  // return the number of successive mouse-clicks at the time where a
                               // click happen (otherwise 0).
...
IMGUI_API bool IsMousePosValid(
    const ImVec2* mouse_pos = NULL);  // by convention we use (-FLT_MAX,-FLT_MAX) to denote that
                                      // there is no mouse available
IMGUI_API bool
IsAnyMouseDown();  // [WILL OBSOLETE] is any mouse button held? This was designed for backends, but
                   // prefer having backend maintain a mask of held mouse buttons, because upcoming
                   // input queue system will make this invalid.
...
GetMousePosOnOpeningCurrentPopup();  // retrieve mouse position at the time of opening popup we have
                                     // BeginPopup() into (helper to avoid user backing that value
                                     // themselves)
...
IMGUI_API ImVec2 GetMouseDragDelta(
    ...
    float lock_threshold =
        -1.0f);  // return the delta from the initial clicking position while the mouse button is
                 ...
                 // moves past a distance threshold at least once (uses io.MouseDraggingThreshold if
                 // lock_threshold < 0.0f)
IMGUI_API void ResetMouseDragDelta(ImGuiMouseButton button = 0);  //
IMGUI_API ImGuiMouseCursor
GetMouseCursor();  // get desired mouse cursor shape. Important: reset in ImGui::NewFrame(), this is
...
IMGUI_API void SetNextFrameWantCaptureMouse(
    bool want_capture_mouse);  // Override io.WantCaptureMouse flag next frame (said flag is left
                               // for your application to handle, typical when true it instructs
                               // your app to ignore inputs). This is equivalent to setting
                               // "io.WantCaptureMouse = want_capture_mouse;" after the next
                               // NewFrame() call.
...
// - Important: default value "imgui.ini" is relative to current working dir! Most apps will want to
...
// Debug Utilities
// - Your main debugging friend is the ShowMetricsWindow() function, which is also accessible from
// Demo->Tools->Metrics Debugger
IMGUI_API void DebugTextEncoding(const char* text);
IMGUI_API void DebugFlashStyleColor(ImGuiCol idx);
IMGUI_API void DebugStartItemPicker();
IMGUI_API bool DebugCheckVersionAndDataLayout(
    const char* version_str, size_t sz_io, size_t sz_style, size_t sz_vec2, size_t sz_vec4,
...
IMGUI_API void DebugLog(const char* fmt, ...)
    IM_FMTARGS(1);  // Call via IMGUI_DEBUG_LOG() for maximum stripping in caller code!
...
// SetCurrentContext() + SetAllocatorFunctions()
//   for each static/DLL boundary you are calling from. Read "Context and Memory Allocators" section
//   of imgui.cpp for more details.
IMGUI_API void SetAllocatorFunctions(ImGuiMemAllocFunc alloc_func, ImGuiMemFreeFunc free_func,
                                     void* user_data = NULL);
...
// (Those are per-window flags. There are shared flags in ImGuiIO: io.ConfigWindowsResizeFromEdges
// and io.ConfigWindowsMoveFromTitleBarOnly)
enum ImGuiWindowFlags_ {
  ...
  ImGuiWindowFlags_NoCollapse =
      1 << 5,  // Disable user collapsing window by double-clicking on it. Also referred to as
  ...
  ImGuiWindowFlags_NoNavFocus = 1 << 17,   // No focusing toward this window with keyboard/gamepad
                                           // navigation (e.g. skipped by CTRL+TAB)
  ImGuiWindowFlags_UnsavedDocument =
      1 << 18,  // Display a dot next to the title. When used in a tab/docking context, tab is
                // selected when clicking the X + closure is not assumed (will wait for user to stop
                // submitting the tab). Otherwise closure is assumed when pressing the X, so if you
...
// Obsolete names
  ...
  ImGuiWindowFlags_NavFlattened =
      1 << 29,  // Obsoleted in 1.90.9: Use ImGuiChildFlags_NavFlattened in BeginChild() call.
  ImGuiWindowFlags_AlwaysUseWindowPadding =
      1 << 30,  // Obsoleted in 1.90.0: Use ImGuiChildFlags_AlwaysUseWindowPadding in BeginChild()
                // call.
#endif
};

// Flags for ImGui::BeginChild()
// (Legacy: bit 0 must always correspond to ImGuiChildFlags_Borders to be backward compatible with
// old API using 'bool border = false'.) About using AutoResizeX/AutoResizeY flags:
...
// - Size measurement for a given axis is only performed when the child window is within visible
// boundaries, or is just appearing.
//   - This allows BeginChild() to return false when not within boundaries (e.g. when scrolling),
//   which is more optimal. BUT it won't update its auto-size while clipped.
//     While not perfect, it is a better default behavior as the always-on performance gain is more
//     valuable than the occasional "resizing after becoming visible again" glitch.
//   - You may also use ImGuiChildFlags_AlwaysAutoResize to force an update even when child window
//   is not in view.
//     HOWEVER PLEASE UNDERSTAND THAT DOING SO WILL PREVENT BeginChild() FROM EVER RETURNING FALSE,
//     disabling benefits of coarse clipping.
enum ImGuiChildFlags_ {
  ImGuiChildFlags_None = 0,
  ImGuiChildFlags_Borders = 1 << 0,  // Show an outer border and enable WindowPadding. (IMPORTANT:
                                     // this is always == 1 == true for legacy reason)
  ImGuiChildFlags_AlwaysUseWindowPadding =
      1 << 1,  // Pad with style.WindowPadding even if no border are drawn (no padding by default
               // for non-bordered child windows because it makes more sense)
  ImGuiChildFlags_ResizeX =
      1 << 2,  // Allow resize from right border (layout direction). Enable .ini saving (unless
  ...
  ImGuiChildFlags_AlwaysAutoResize =
      1 << 6,  // Combined with AutoResizeX/AutoResizeY. Always measure size even when child is
               // hidden, always return true, always disable clipping optimization! NOT RECOMMENDED.
  ImGuiChildFlags_FrameStyle = 1 << 7,  // Style the child window like a framed item: use FrameBg,
                                        // FrameRounding, FrameBorderSize, FramePadding instead of
                                        // ChildBg, ChildRounding, ChildBorderSize, WindowPadding.
  ImGuiChildFlags_NavFlattened =
      1 << 8,  // [BETA] Share focus scope, allow keyboard/gamepad navigation to cross over parent
               // border to this child or between sibling child windows.

// Obsolete names
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
  ImGuiChildFlags_Border =
      ImGuiChildFlags_Borders,  // Renamed in 1.91.1 (August 2024) for consistency.
#endif
};
...
enum ImGuiItemFlags_ {
  ...
  ImGuiItemFlags_NoTabStop = 1 << 0,  // false    // Disable keyboard tabbing. This is a "lighter"
                                      // version of ImGuiItemFlags_NoNav.
  ImGuiItemFlags_NoNav = 1 << 1,      // false    // Disable any form of focusing (keyboard/gamepad
                                      // directional navigation and SetKeyboardFocusHere() calls).
  ImGuiItemFlags_NoNavDefaultFocus = 1 << 2,  // false    // Disable item being a candidate for
                                              // default focus (e.g. used by title bar items).
  ImGuiItemFlags_ButtonRepeat =
      1 << 3,  // false    // Any button-like behavior will have repeat mode enabled (based on
               // io.KeyRepeatDelay and io.KeyRepeatRate values). Note that you can also call
               // IsItemActive() after any button to tell if it is being held.
  ...
  ImGuiItemFlags_AllowDuplicateId =
      1 << 5,  // false    // Allow submitting an item with the same identifier as an item already
               // submitted this frame without triggering a warning tooltip if
               // io.ConfigDebugHighlightIdConflicts is set.
};

// Flags for ImGui::InputText()
// (Those are per-item flags. There are shared flags in ImGuiIO: io.ConfigInputTextCursorBlink and
...
enum ImGuiInputTextFlags_ {
  ...
  ImGuiInputTextFlags_EnterReturnsTrue =
      1 << 6,  // Return 'true' when Enter is pressed (as opposed to every time the value was
               // modified). Consider using IsItemDeactivatedAfterEdit() instead!
  ImGuiInputTextFlags_EscapeClearsAll =
      1 << 7,  // Escape key clears content if not empty, and deactivate otherwise (contrast to
               // default behavior of Escape to revert)
  ImGuiInputTextFlags_CtrlEnterForNewLine =
      1 << 8,  // In multi-line mode, validate with Enter, add new line with Ctrl+Enter (default is
               // opposite: validate with Ctrl+Enter, add line with Enter).
  ...
  ImGuiInputTextFlags_ParseEmptyRefVal = 1 << 13,  // InputFloat(), InputInt(), InputScalar() etc.
                                                   // only: parse empty string as zero value.
  ImGuiInputTextFlags_DisplayEmptyRefVal =
      1 << 14,  // InputFloat(), InputInt(), InputScalar() etc. only: when value is zero, do not
                // display it. Generally used with ImGuiInputTextFlags_ParseEmptyRefVal.
  ...
  ImGuiInputTextFlags_ElideLeft =
      1 << 17,  // When text doesn't fit, elide left side to ensure right side stays visible. Useful
                // for path/filenames. Single-line only!
  ...
  ImGuiInputTextFlags_CallbackAlways = 1 << 20,  // Callback on each iteration. User code may query
  ...
  ImGuiInputTextFlags_CallbackResize =
      1 << 22,  // Callback on buffer capacity changes request (beyond 'buf_size' parameter value),
                ...
                // BufSize in the callback and NEED to honor it. (see misc/cpp/imgui_stdlib.h for an
                // example of using this)
  ImGuiInputTextFlags_CallbackEdit =
      1 << 23,  // Callback on any edit. Note that InputText() already returns true on edit + you
                // can always use IsItemEdited(). The callback is useful to manipulate the
                // underlying buffer while focus is active.

  // Multi-line Word-Wrapping [BETA]
  // - Not well tested yet. Please report any incorrect cursor movement, selection behavior etc. bug
  // to https://github.com/ocornut/imgui/issues/3237.
  // - Wrapping style is not ideal. Wrapping of long words/sections (e.g. words larger than total
  // available width) may be particularly unpleasing.
  ...
  //   Ballpark estimate of cost on my 2019 desktop PC: for a 100 KB text buffer: +~0.3 ms
  //   (Optimized) / +~1.0 ms (Debug build). The CPU cost is very roughly proportional to text
  //   length, so a 10 KB buffer should cost about ten times less.
  ImGuiInputTextFlags_WordWrap = 1 << 24,  // InputTextMultine(): word-wrap lines that are too long.

  // Obsolete names
  // ImGuiInputTextFlags_AlwaysInsertMode  = ImGuiInputTextFlags_AlwaysOverwrite   // [renamed
  // in 1.82] name was not matching behavior
};
...
enum ImGuiTreeNodeFlags_ {
  ...
  ImGuiTreeNodeFlags_OpenOnDoubleClick =
      1 << 6,  // Open on double-click instead of simple click (default for multi-select unless any
               // _OpenOnXXX behavior is set explicitly). Both behaviors may be combined.
  ImGuiTreeNodeFlags_OpenOnArrow =
      1 << 7,  // Open when clicking on the arrow part (default for multi-select unless any
               // _OpenOnXXX behavior is set explicitly). Both behaviors may be combined.
  ImGuiTreeNodeFlags_Leaf =
      1 << 8,  // No collapsing, no arrow (use as a convenience for leaf nodes).
  ImGuiTreeNodeFlags_Bullet =
      1 << 9,  // Display a bullet instead of arrow. IMPORTANT: node can still be marked open/close
               // if you don't set the _Leaf flag!
  ...
  ImGuiTreeNodeFlags_SpanAvailWidth =
      1 << 11,  // Extend hit box to the right-most edge, even if not framed. This is not the
                // default in order to allow adding other items on the same line without using
                // AllowOverlap mode.
  ...
  ImGuiTreeNodeFlags_SpanAllColumns = 1 << 14,  // Frame will span all columns of its container
  ...
  ImGuiTreeNodeFlags_NavLeftJumpsToParent =
      1 << 17,  // Nav: left arrow moves back to parent. This is processed in TreePop() when there's
                // an unfullfilled Left nav request remaining.
  ...
  ImGuiTreeNodeFlags_DrawLinesFull =
      1 << 19,  // Horizontal lines to child nodes. Vertical line drawn down to TreePop() position:
                // cover full contents. Faster (for large trees).
  ImGuiTreeNodeFlags_DrawLinesToNodes =
      1 << 20,  // Horizontal lines to child nodes. Vertical line drawn down to bottom-most child
                // node. Slower (for large trees).

#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
  ImGuiTreeNodeFlags_NavLeftJumpsBackHere =
      ImGuiTreeNodeFlags_NavLeftJumpsToParent,                           // Renamed in 1.92.0
...
// - To be backward compatible with older API which took an 'int mouse_button = 1' argument instead
// of 'ImGuiPopupFlags flags',
//   we need to treat small flags values as a mouse button index, so we encode the mouse button in
//   the first few bits of the flags. It is therefore guaranteed to be legal to pass a mouse button
//   index in ImGuiPopupFlags.
// - For the same reason, we exceptionally default the ImGuiPopupFlags argument of
// BeginPopupContextXXX functions to 1 instead of 0.
//   IMPORTANT: because the default parameter is 1 (==ImGuiPopupFlags_MouseButtonRight), if you rely
//   on the default parameter and want to use another flag, you need to pass in the
...
enum ImGuiPopupFlags_ {
  ...
  ImGuiPopupFlags_NoReopen =
      1 << 5,  // For OpenPopup*(), BeginPopupContext*(): don't reopen same popup if already open
               // (won't reposition, won't reinitialize navigation)
  // ImGuiPopupFlags_NoReopenAlwaysNavInit = 1 << 6,   // For OpenPopup*(), BeginPopupContext*():
  // focus and initialize navigation even when not reopening.
  ImGuiPopupFlags_NoOpenOverExistingPopup =
      1 << 7,  // For OpenPopup*(), BeginPopupContext*(): don't open if there's already a popup at
               // the same level of the popup stack
  ImGuiPopupFlags_NoOpenOverItems = 1 << 8,  // For BeginPopupContextWindow(): don't return true
  ...
  ImGuiPopupFlags_AnyPopupLevel = 1 << 11,  // For IsPopupOpen(): search/test at any level of the
...
enum ImGuiSelectableFlags_ {
  ...
  ImGuiSelectableFlags_NoAutoClosePopups = 1
                                           << 0,  // Clicking this doesn't close parent popup window
                                                  // (overrides ImGuiItemFlags_AutoClosePopups)
  ImGuiSelectableFlags_SpanAllColumns = 1 << 1,   // Frame will span all columns of its container
                                                  // table (text will still fit in current column)
  ImGuiSelectableFlags_AllowDoubleClick = 1 << 2,  // Generate press events on double clicks too
  ImGuiSelectableFlags_Disabled = 1 << 3,          // Cannot be selected, display grayed out text
  ImGuiSelectableFlags_AllowOverlap =
      1 << 4,  // (WIP) Hit testing to allow subsequent widgets to overlap this one
  ImGuiSelectableFlags_Highlight = 1 << 5,    // Make the item be displayed as if it is hovered
  ImGuiSelectableFlags_SelectOnNav = 1 << 6,  // Auto-select when moved into, unless Ctrl is held.
                                              // Automatic when in a BeginMultiSelect() block.

#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
  ImGuiSelectableFlags_DontClosePopups =
      ImGuiSelectableFlags_NoAutoClosePopups,  // Renamed in 1.91.0
...
enum ImGuiTabBarFlags_ {
  ...
  ImGuiTabBarFlags_NoCloseWithMiddleMouseButton =
      1 << 3,  // Disable behavior of closing tabs (that are submitted with p_open != NULL) with
               // middle mouse button. You may handle this behavior manually on user's side with if
  ...
  ImGuiTabBarFlags_DrawSelectedOverline = 1
                                          << 6,  // Draw selected overline markers over selected tab

  // Fitting/Resize policy
  ImGuiTabBarFlags_FittingPolicyMixed =
      1 << 7,  // Shrink down tabs when they don't fit, until width is style.TabMinWidthShrink, then
  ...
  ImGuiTabBarFlags_FittingPolicyMask_ = ImGuiTabBarFlags_FittingPolicyMixed |
  ...
  ImGuiTabBarFlags_FittingPolicyResizeDown =
      ImGuiTabBarFlags_FittingPolicyShrink,  // Renamed in 1.92.2
#endif
};
...
enum ImGuiTabItemFlags_ {
  ...
  ImGuiTabItemFlags_Leading = 1 << 6,  // Enforce the tab position to the left of the tab bar (after
                                       // the tab list popup button)
  ImGuiTabItemFlags_Trailing =
      1
      << 7,  // Enforce the tab position to the right of the tab bar (before the scrolling buttons)
  ImGuiTabItemFlags_NoAssumedClosure =
      1 << 8,  // Tab is selected when trying to close + closure is not immediately assumed (will
               // wait for user to stop submitting the tab). Otherwise closure is assumed when
...
enum ImGuiFocusedFlags_ {
  ...
  ImGuiFocusedFlags_AnyWindow =
      1 << 2,  // Return true if any window is focused. Important: If you are trying to tell how to
               // dispatch your low-level inputs, do NOT use this. Use 'io.WantCaptureMouse'
               // instead! Please read the FAQ!
  ImGuiFocusedFlags_NoPopupHierarchy =
      1 << 3,  // Do not consider popup hierarchy (do not treat popup emitter as parent of popup)
...
// Note: if you are trying to check whether your mouse should be dispatched to Dear ImGui or to your
...
enum ImGuiHoveredFlags_ {
  ...
  ImGuiHoveredFlags_RootWindow = 1 << 1,  // IsWindowHovered() only: Test from root window (top most
                                          // parent of the current hierarchy)
  ImGuiHoveredFlags_AnyWindow =
      1 << 2,  // IsWindowHovered() only: Return true if any window is hovered
  ImGuiHoveredFlags_NoPopupHierarchy =
      1 << 3,  // IsWindowHovered() only: Do not consider popup hierarchy (do not treat popup
  ...
  // docking hierarchy (treat dockspace host as parent of docked window) (when used with
  ...
  // ImGuiHoveredFlags_AllowWhenBlockedByModal     = 1 << 6,   // Return true even if a modal popup
  // window is normally blocking access to this item/window. FIXME-TODO: Unavailable yet.
  ImGuiHoveredFlags_AllowWhenBlockedByActiveItem =
      1 << 7,  // Return true even if an active item is blocking access to this item/window. Useful
               // for Drag and Drop patterns.
  ImGuiHoveredFlags_AllowWhenOverlappedByItem =
      1 << 8,  // IsItemHovered() only: Return true even if the item uses AllowOverlap mode and is
               // overlapped by another hoverable item.
  ImGuiHoveredFlags_AllowWhenOverlappedByWindow =
      1 << 9,  // IsItemHovered() only: Return true even if the position is obstructed or overlapped
               // by another window.
  ImGuiHoveredFlags_AllowWhenDisabled =
      1 << 10,  // IsItemHovered() only: Return true even if the item is disabled
  ImGuiHoveredFlags_NoNavOverride = 1
                                    << 11,  // IsItemHovered() only: Disable using keyboard/gamepad
  ...
  // - this is a shortcut to pull flags from 'style.HoverFlagsForTooltipMouse' or
  // 'style.HoverFlagsForTooltipNav' where you can reconfigure desired behavior.
  //   e.g. 'TooltipHoveredFlagsForMouse' defaults to 'ImGuiHoveredFlags_Stationary |
  ...
  ImGuiHoveredFlags_Stationary =
      1 << 13,  // Require mouse to be stationary for style.HoverStationaryDelay (~0.15 sec) _at
                // least one time_. After this, can move on same item/window. Using the stationary
                // test tends to reduces the need for a long delay.
  ImGuiHoveredFlags_DelayNone =
      1 << 14,  // IsItemHovered() only: Return true immediately (default). As this is the default
                // you generally ignore this.
  ImGuiHoveredFlags_DelayShort =
      1 << 15,  // IsItemHovered() only: Return true after style.HoverDelayShort elapsed (~0.15 sec)
                // (shared between items) + requires mouse to be stationary for
                // style.HoverStationaryDelay (once per item).
  ImGuiHoveredFlags_DelayNormal =
      1 << 16,  // IsItemHovered() only: Return true after style.HoverDelayNormal elapsed (~0.40
                // sec) (shared between items) + requires mouse to be stationary for
                // style.HoverStationaryDelay (once per item).
  ImGuiHoveredFlags_NoSharedDelay =
      1 << 17,  // IsItemHovered() only: Disable shared delay system where moving from one item to
                // the next keeps the previous timer for a short time (standard for tooltips with
                // long delays)
};
...
enum ImGuiDragDropFlags_ {
  ...
  ImGuiDragDropFlags_SourceNoPreviewTooltip =
      1 << 0,  // Disable preview tooltip. By default, a successful call to BeginDragDropSource
               // opens a tooltip so you can display a preview or description of the source
               // contents. This flag disables this behavior.
  ImGuiDragDropFlags_SourceNoDisableHover =
      1 << 1,  // By default, when dragging we clear data so that IsItemHovered() will return false,
               // to avoid subsequent user code submitting tooltips. This flag disables this
  ...
  ImGuiDragDropFlags_PayloadNoCrossContext = 1 << 6,  // Hint to specify that the payload may not be
                                                      // copied outside current dear imgui context.
  ...
  ImGuiDragDropFlags_AcceptBeforeDelivery =
      1
      << 10,  // AcceptDragDropPayload() will returns true even before the mouse button is released.
  ...
  ImGuiDragDropFlags_SourceAutoExpirePayload =
      ImGuiDragDropFlags_PayloadAutoExpire,  // Renamed in 1.90.9
#endif
};
...
enum ImGuiKey : int {
  ...
  ImGuiKey_LeftShift,
  ImGuiKey_LeftAlt,
  ImGuiKey_LeftSuper,  // Also see ImGuiMod_Ctrl, ImGuiMod_Shift, ImGuiMod_Alt, ImGuiMod_Super
                       // below!
  ImGuiKey_RightCtrl,
  ImGuiKey_RightShift,
  ImGuiKey_RightAlt,
  ...
  ImGuiKey_GamepadFaceLeft,  // X           | Y       | Square   | Tap: Toggle Menu. Hold: Windowing
                             // mode (Focus/Move/Resize windows)
  ...
  ImGuiKey_GamepadDpadDown,   // D-pad Down  | "       | "        | Move / Tweak / Resize Window (in
                              // Windowing mode)
  ImGuiKey_GamepadL1,  // L Bumper    | L       | L1       | Tweak Slower / Focus Previous (in
                       // Windowing mode)
  ImGuiKey_GamepadR1,  // R Bumper    | R       | R1       | Tweak Faster / Focus Next (in Windowing
                       // mode)
  ...
  ImGuiKey_GamepadLStickLeft,   //             |         |          | [Analog] Move Window (in
                                //             Windowing mode)
  ImGuiKey_GamepadLStickRight,  //             |         |          | [Analog] Move Window (in
                                //             Windowing mode)
  ImGuiKey_GamepadLStickUp,     //             |         |          | [Analog] Move Window (in
                                //             Windowing mode)
  ImGuiKey_GamepadLStickDown,   //             |         |          | [Analog] Move Window (in
                                //             Windowing mode)
  ...
  // - Any functions taking a ImGuiKeyChord parameter can binary-or those with regular keys, e.g.
  ...
  //   but may be accessed via standard key API such as IsKeyPressed(), IsKeyReleased(), querying
  //   duration etc.
  // - Code polling every key (e.g. an interface to detect a key press for input mapping) might want
  // to ignore those
  //   and prefer using the real keys (e.g. ImGuiKey_LeftCtrl, ImGuiKey_RightCtrl instead of
  //   ImGuiMod_Ctrl).
  // - In theory the value of keyboard modifiers should be roughly equivalent to a logical or of the
  // equivalent left/right keys.
  //   In practice: it's complicated; mods are often provided from different sources. Keyboard
  //   layout, IME, sticky keys and backends tend to interfere and break that equivalence. The safer
...
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
  ImGuiKey_COUNT = ImGuiKey_NamedKey_END,  // Obsoleted in 1.91.5 because it was extremely
                                           // misleading (since named keys don't start at 0 anymore)
...
// (and for upcoming extended versions of IsKeyPressed(), IsMouseClicked(), Shortcut(),
// SetKeyOwner(), SetItemKeyOwner() that are still in imgui_internal.h) Don't mistake with
// ImGuiInputTextFlags! (which is for ImGui::InputText() function)
enum ImGuiInputFlags_ {
  ImGuiInputFlags_None = 0,
  ImGuiInputFlags_Repeat =
      1 << 0,  // Enable repeat. Return true on successive repeats. Default for legacy
               // IsKeyPressed(). NOT Default for legacy IsMouseClicked(). MUST BE == 1.

  // Flags for Shortcut(), SetNextItemShortcut()
  // - Routing policies: RouteGlobal+OverActive >> RouteActive or RouteFocused (if owner is active
  // item) >> RouteGlobal+OverFocused >> RouteFocused (if in focused window stack) >> RouteGlobal.
  // - Default policy is RouteFocused. Can select only 1 policy among all available.
  ImGuiInputFlags_RouteActive = 1 << 10,  // Route to active item only.
  ImGuiInputFlags_RouteFocused =
      1 << 11,  // Route to windows in the focus stack (DEFAULT). Deep-most focused window takes
                // inputs. Active item takes inputs over deep-most focused window.
  ...
  ImGuiInputFlags_RouteOverFocused = 1 << 14,  // Option: global route: higher priority than focused
                                               // route (unless active item in focused route).
  ImGuiInputFlags_RouteOverActive =
      1 << 15,  // Option: global route: higher priority than active item. Unlikely you need to use
                // that: will interfere with every active items, e.g. CTRL+A registered by InputText
                // will be overridden by this. May not be fully honored as user/internal code is
                // likely to always assume they can access keys when active.
  ImGuiInputFlags_RouteUnlessBgFocused =
      1 << 16,  // Option: global route: will not be applied if underlying background/void is
                // focused (== no Dear ImGui windows are focused). Useful for overlay applications.
  ImGuiInputFlags_RouteFromRootWindow = 1 << 17,  // Option: route evaluated from the point of view
                                                  // of root window rather than current window.

  // Flags for SetNextItemShortcut()
  ImGuiInputFlags_Tooltip = 1 << 18,  // Automatically display a tooltip when hovering item [BETA]
...
enum ImGuiConfigFlags_ {
  ...
  ImGuiConfigFlags_NavEnableGamepad = 1 << 1,  // Master gamepad navigation enable flag. Backend
  ...
  ImGuiConfigFlags_NoMouseCursorChange =
      1 << 5,  // Instruct backend to not alter mouse cursor shape and visibility. Use if the
               ...
               // SetMouseCursor() to change mouse cursor. You may want to honor requests from imgui
               // by reading GetMouseCursor() yourself instead.
  ImGuiConfigFlags_NoKeyboard =
      1 << 6,  // Instruct dear imgui to disable keyboard inputs and interactions. This is done by
  ...
  ImGuiConfigFlags_NavEnableSetMousePos =
      1 << 2,  // [moved/renamed in 1.91.4] -> use bool io.ConfigNavMoveSetMousePos
  ImGuiConfigFlags_NavNoCaptureKeyboard =
      1 << 3,  // [moved/renamed in 1.91.4] -> use bool io.ConfigNavCaptureKeyboard
#endif
};
...
enum ImGuiBackendFlags_ {
  ...
  ImGuiBackendFlags_HasMouseCursors = 1
                                      << 1,  // Backend Platform supports honoring GetMouseCursor()
  ...
  ImGuiBackendFlags_HasSetMousePos =
      1 << 2,  // Backend Platform supports io.WantSetMousePos requests to reposition the OS mouse
               // position (only used if io.ConfigNavMoveSetMousePos is set).
  ...
  ImGuiBackendFlags_RendererHasTextures =
      1 << 4,  // Backend Renderer supports ImTextureData requests to create/update/destroy
               // textures. This enables incremental texture updates and texture reloads. See
               // https://github.com/ocornut/imgui/blob/master/docs/BACKENDS.md for instructions on
               // how to upgrade your custom backend.
};
...
enum ImGuiCol_ {
  ...
  ImGuiCol_TabSelectedOverline,  // Tab horizontal overline, when tab-bar is focused & tab is
                                 // selected
...
// - The enum only refers to fields of ImGuiStyle which makes sense to be pushed/popped inside UI
// code.
...
enum ImGuiButtonFlags_ {
  ...
  ImGuiButtonFlags_MouseButtonMask_ = ImGuiButtonFlags_MouseButtonLeft |
                                      ...
                                      ImGuiButtonFlags_MouseButtonMiddle,  // [Internal]
  ImGuiButtonFlags_EnableNav =
      1
      << 3,  // InvisibleButton(): do not disable navigation/tabbing. Otherwise disabled by default.
};
...
enum ImGuiColorEditFlags_ {
  ...
  ImGuiColorEditFlags_NoTooltip = 1
                                  << 6,  //              // ColorEdit, ColorPicker, ColorButton:
  ...
  ImGuiColorEditFlags_NoDragDrop =
      1 << 9,  //              // ColorEdit: disable drag and drop target. ColorButton: disable drag
               //              and drop source.
  ...
  // ImGuiColorEditFlags_AlphaOpaque to use old behavior.
  // - The new flags may be combined better and allow finer controls.
  ImGuiColorEditFlags_AlphaOpaque =
      1 << 11,  //              // ColorEdit, ColorPicker, ColorButton: disable alpha in the
                //              preview,. Contrary to _NoAlpha it may still be edited when calling
                //              ColorEdit4()/ColorPicker4(). For ColorButton() this does the same as
                //              _NoAlpha.
  ImGuiColorEditFlags_AlphaNoBg =
      1 << 12,  //              // ColorEdit, ColorPicker, ColorButton: disable rendering a
                //              checkerboard background behind transparent color.
  ImGuiColorEditFlags_AlphaPreviewHalf =
      1 << 13,  //              // ColorEdit, ColorPicker, ColorButton: display half opaque / half
                //              transparent preview.

  // User Options (right-click on widget to change some of them).
  ImGuiColorEditFlags_AlphaBar =
      1
      << 16,  //              // ColorEdit, ColorPicker: show vertical alpha bar/gradient in picker.
  ImGuiColorEditFlags_HDR =
      1 << 19,  //              // (WIP) ColorEdit: Currently only disable 0.0f..1.0f limits in RGBA
  ...
  ImGuiColorEditFlags_Float =
      1
      << 24,  // [DataType]   // ColorEdit, ColorPicker, ColorButton: _display_ values formatted as
  ...
  // [Internal] Masks
  ImGuiColorEditFlags_AlphaMask_ = ImGuiColorEditFlags_NoAlpha | ImGuiColorEditFlags_AlphaOpaque |
                                   ImGuiColorEditFlags_AlphaNoBg |
                                   ImGuiColorEditFlags_AlphaPreviewHalf,
...
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
  ImGuiColorEditFlags_AlphaPreview =
      0,  // [Removed in 1.91.8] This is the default now. Will display a checkerboard unless
          // ImGuiColorEditFlags_AlphaNoBg is set.
#endif
  // ImGuiColorEditFlags_RGB = ImGuiColorEditFlags_DisplayRGB, ImGuiColorEditFlags_HSV =
  // ImGuiColorEditFlags_DisplayHSV, ImGuiColorEditFlags_HEX = ImGuiColorEditFlags_DisplayHex  //
  // [renamed in 1.69]
};

// Flags for DragFloat(), DragInt(), SliderFloat(), SliderInt() etc.
// We use the same sets of flags for DragXXX() and SliderXXX() functions as the features are the
// same and it makes it easier to swap them. (Those are per-item flags. There is shared behavior
// flag too: ImGuiIO: io.ConfigDragClickToInputText)
enum ImGuiSliderFlags_ {
  ImGuiSliderFlags_None = 0,
  ImGuiSliderFlags_Logarithmic = 1
                                 << 5,  // Make the widget logarithmic (linear otherwise). Consider
  ...
  ImGuiSliderFlags_WrapAround = 1 << 8,  // Enable wrapping around from max to min and from min to
                                         // max. Only supported by DragXXX() functions for now.
  ImGuiSliderFlags_ClampOnInput =
      1 << 9,  // Clamp value to min/max bounds when input manually with CTRL+Click. By default
               // CTRL+Click allows going out of bounds.
  ImGuiSliderFlags_ClampZeroRange =
      1 << 10,  // Clamp even if min==max==0.0f. Otherwise due to legacy reason DragXXX functions
                // don't clamp with those values. When your clamping limits are dynamic you almost
                // always want to use it.
  ImGuiSliderFlags_NoSpeedTweaks =
      1 << 11,  // Disable keyboard modifiers altering tweak speed. Useful if you want to alter
...
// Those values are guaranteed to be stable and we frequently use 0/1 directly. Named enums provided
// for convenience.
...
enum ImGuiMouseCursor_ {
  ...
  ImGuiMouseCursor_Progress,    // When waiting for something to process/load, but application is
                                // still interactive.
  ImGuiMouseCursor_NotAllowed,  // When hovering something with disallowed interaction. Usually a
                                // crossed circle.
  ImGuiMouseCursor_COUNT
};

// Enumeration for AddMouseSourceEvent() actual source of Mouse Input data.
// Historically we use "Mouse" terminology everywhere to indicate pointer data, e.g. MousePos,
// IsMousePressed(), io.AddMousePosEvent() But that "Mouse" data can come from different source
// which occasionally may be useful for application to know about. You can submit a change of
// pointer type using io.AddMouseSourceEvent().
enum ImGuiMouseSource : int {
  ImGuiMouseSource_Mouse = 0,    // Input is coming from an actual mouse.
  ImGuiMouseSource_TouchScreen,  // Input is coming from a touch screen (no hovering prior to
                                 // initial press, less precise initial press aiming, dual-axis
                                 // wheeling possible).
  ImGuiMouseSource_Pen,  // Input is coming from a pressure/magnetic pen (often used in conjunction
...
// - Important! Sizing policies have complex and subtle side effects, much more so than you would
// expect.
...
//    - Fixed Columns (if any) will generally obtain their requested width (unless the table cannot
//    fit them all).
...
// - When ScrollX is on:
//    - Table defaults to ImGuiTableFlags_SizingFixedFit -> all Columns defaults to
...
enum ImGuiTableFlags_ {
  ...
  ImGuiTableFlags_RowBg =
      1
      << 6,  // Set each RowBg color with ImGuiCol_TableRowBg or ImGuiCol_TableRowBgAlt (equivalent
             // of calling TableSetBgColor with ImGuiTableBgFlags_RowBg0 on each row manually)
  ...
  ImGuiTableFlags_NoBordersInBody =
      1 << 11,  // [ALPHA] Disable vertical borders in columns Body (borders will always appear in
                // Headers). -> May move to style
  ImGuiTableFlags_NoBordersInBodyUntilResize =
      1 << 12,  // [ALPHA] Disable vertical borders in columns Body until hovered for resize
                // (borders will always appear in Headers). -> May move to style Sizing Policy (read
                // above for defaults)
  ImGuiTableFlags_SizingFixedFit = 1
                                   << 13,  // Columns default to _WidthFixed or _WidthAuto (if
                                           // resizable or not resizable), matching contents width.
  ...
  ImGuiTableFlags_NoHostExtendX =
      1 << 16,  // Make outer width auto-fit to columns, overriding outer_size.x value. Only
                // available when ScrollX/ScrollY are disabled and Stretch columns are not used.
  ImGuiTableFlags_NoHostExtendY =
      1 << 17,  // Make outer height stop exactly at outer_size.y (prevent auto-extending table past
                // the limit). Only available when ScrollX/ScrollY are disabled. Data below the
  ...
  ImGuiTableFlags_NoClip =
      1 << 20,  // Disable clipping rectangle for every individual columns (reduce draw command
                // count, items will be able to overflow into other columns). Generally incompatible
                // with TableSetupScrollFreeze(). Padding
  ImGuiTableFlags_PadOuterX = 1 << 21,  // Default if BordersOuterV is on. Enable outermost padding.
  ...
  ImGuiTableFlags_ScrollX =
      1 << 24,  // Enable horizontal scrolling. Require 'outer_size' parameter of BeginTable() to
                // specify the container size. Changes default sizing policy. Because this creates a
                // child window, ScrollY is currently generally recommended when using ScrollX.
  ImGuiTableFlags_ScrollY = 1 << 25,  // Enable vertical scrolling. Require 'outer_size' parameter
  ...
  ImGuiTableFlags_SortTristate =
      1 << 27,  // Allow no sorting, disable default sorting. TableGetSortSpecs() may return specs
                // where (SpecsCount == 0). Miscellaneous
...
enum ImGuiTableColumnFlags_ {
  ...
  ImGuiTableColumnFlags_Disabled =
      1 << 0,  // Overriding/master disable flag: hide column, won't show in context menu (unlike
               // calling TableSetColumnEnabled() which manipulates the user accessible state)
  ImGuiTableColumnFlags_DefaultHide = 1 << 1,  // Default as a hidden/disabled column.
  ImGuiTableColumnFlags_DefaultSort = 1 << 2,  // Default as a sorting column.
  ImGuiTableColumnFlags_WidthStretch =
      1 << 3,  // Column will stretch. Preferable with horizontal scrolling disabled (default if
               // table sizing policy is _SizingStretchSame or _SizingStretchProp).
  ImGuiTableColumnFlags_WidthFixed =
      1 << 4,  // Column will not stretch. Preferable with horizontal scrolling enabled (default if
               // table sizing policy is _SizingFixedFit and table is resizable).
  ImGuiTableColumnFlags_NoResize = 1 << 5,  // Disable manual resizing.
  ImGuiTableColumnFlags_NoReorder = 1
                                    << 6,  // Disable manual reordering this column, this will also
                                           // prevent other columns from crossing over this column.
  ImGuiTableColumnFlags_NoHide = 1 << 7,   // Disable ability to hide/disable this column.
  ImGuiTableColumnFlags_NoClip = 1 << 8,   // Disable clipping for this column (all NoClip columns
                                           // will render in a same draw command).
  ImGuiTableColumnFlags_NoSort = 1 << 9,   // Disable ability to sort on this field (even if
  ...
  ImGuiTableColumnFlags_NoHeaderLabel =
      1 << 12,  // TableHeadersRow() will submit an empty label for this column. Convenient for some
                // small columns. Name will still appear in context menu or in angled headers. You
                // may append into this cell by calling TableSetColumnIndex() right after the
                // TableHeadersRow() call.
  ...
  ImGuiTableColumnFlags_IndentDisable =
      1 << 17,  // Ignore current Indent value when entering cell (default for columns > 0).
                // Indentation changes _within_ the cell will still be honored.
  ImGuiTableColumnFlags_AngledHeader = 1
                                       << 18,  // TableHeadersRow() will submit an angled header row
                                               // for this column. Note this will add an extra row.

  // Output status flags, read-only via TableGetColumnFlags()
  ImGuiTableColumnFlags_IsEnabled =
      1 << 24,  // Status: is enabled == not hidden by user/api (referred to as "Hide" in
...
// The purpose of the two row/columns layers is to let you decide if a background color change
// should override or blend with the existing color. When using ImGuiTableFlags_RowBg on the table,
// each row has the RowBg0 color automatically set for odd/even rows. If you set the color of RowBg0
// target, your color will override the existing RowBg0 color. If you set the color of RowBg1 or
// ColumnBg1 target, your color will blend over the RowBg0 color.
enum ImGuiTableBgTarget_ {
  ImGuiTableBgTarget_None = 0,
  ImGuiTableBgTarget_RowBg0 = 1,  // Set row background color 0 (generally used for background,
...
struct ImGuiTableSortSpecs {
  ...
  int SpecsCount;   // Sort spec count. Most often 1. May be > 1 when ImGuiTableFlags_SortMulti is
                    // enabled. May be == 0 when ImGuiTableFlags_SortTristate is enabled.
...
struct ImGuiTableColumnSortSpecs {
  ...
  ImS16 SortOrder;  // Index within parent ImGuiTableSortSpecs (always stored in order starting from
                    // 0, tables sorted on a single criteria will always have a 0 here)
  ImGuiSortDirection
      SortDirection;  // ImGuiSortDirection_Ascending or ImGuiSortDirection_Descending
...
//-----------------------------------------------------------------------------
// - You generally do NOT need to care or use this ever. But we need to make it available in imgui.h
// because some of our public structures are relying on it.
// - We use std-like naming convention here, which is a little unusual for this codebase.
// - Important: clear() frees memory, resize(0) keep the allocated buffer. We use resize(0) a lot to
// intentionally recycle allocated buffers across frames and amortize our costs.
...
//   used by dear imgui can be safely initialized by a zero-memset.
//-----------------------------------------------------------------------------
...
struct ImVector {
  ...
  inline ImVector<T>& operator=(const ImVector<T>& src) {
    clear();
    resize(src.Size);
    if (Data && src.Data) memcpy(Data, src.Data, (size_t)Size * sizeof(T));
    return *this;
  }
  inline ~ImVector() {
...
struct ImGuiStyle {
  // Font scaling
  // - recap: ImGui::GetFontSize() == FontSizeBase * (FontScaleMain * FontScaleDpi *
  // other_scaling_factors)
  float FontSizeBase;   // Current base font size before external global factors are applied. Use
                        // PushFont(NULL, size) to modify. Use ImGui::GetFontSize() to obtain scaled
                        // value.
  float FontScaleMain;  // Main global scale factor. May be set by application once, or exposed to
                        // end-user.
  float FontScaleDpi;   // Additional global scale factor from viewport/monitor contents scale. When
  ...
  float WindowBorderHoverPadding;  // Hit-testing extent outside/inside resizing border. Also extend
                                   // determination of hovered window. Generally meaningfully larger
                                   // than WindowBorderSize to make it easy to reach borders.
  ImVec2 WindowMinSize;  // Minimum window size. This is a global setting. If you want to constrain
  ...
  ImVec2 ItemInnerSpacing;  // Horizontal and vertical spacing between within elements of a composed
                            // widget (e.g. a slider and its label).
  ImVec2 CellPadding;  // Padding within a table cell. Cellpadding.x is locked for entire table.
  ...
  ImVec2 TouchExtraPadding;  // Expand reactive bounding box for touch-based system where touch
                             // position is not accurate enough. Unfortunately we don't sort widgets
                             // so priority on overlap will always be given to the first widget. So
  ...
  float TabMinWidthBase;  // Minimum tab width, to make tabs larger than their contents. TabBar
                          // buttons are not affected.
  float TabMinWidthShrink;                 // Minimum tab width after shrinking, when using
                                           // ImGuiTabBarFlags_FittingPolicyMixed policy.
  float TabCloseButtonMinWidthSelected;    // -1: always visible. 0.0f: visible when hovered. >0.0f:
                                           // visible when hovered if minimum width.
  float TabCloseButtonMinWidthUnselected;  // -1: always visible. 0.0f: visible when hovered. >0.0f:
                                           // visible when hovered if minimum width. FLT_MAX: never
                                           // show close button when unselected.
  float TabBarBorderSize;  // Thickness of tab-bar separator, which takes on the tab active color to
                           // denote focus.
  float
  ...
  ImVec2 ButtonTextAlign;  // Alignment of button text when button is larger than text. Defaults to
                           // (0.5f, 0.5f) (centered).
  ImVec2 SelectableTextAlign;  // Alignment of selectable text. Defaults to (0.0f, 0.0f) (top-left
                               // aligned). It's generally important to keep this left-aligned if
                               // you want to lay multiple items on a same line.
  float SeparatorTextBorderSize;  // Thickness of border in SeparatorText()
  ImVec2 SeparatorTextAlign;    // Alignment of text within the separator. Defaults to (0.0f, 0.5f)
                                // (left aligned, center).
  ImVec2 SeparatorTextPadding;  // Horizontal offset of text from each edge of the separator +
                                // spacing on other axis. Generally small values. .y is recommended
                                // to be == FramePadding.y.
  ImVec2 DisplayWindowPadding;  // Apply to regular windows: amount which we enforce to keep visible
                                // when moving near edges of your screen.
  ImVec2
      DisplaySafeAreaPadding;  // Apply to every windows, menus, popups, tooltips: amount where we
  ...
  float MouseCursorScale;      // Scale software rendered mouse cursor (when io.MouseDrawCursor is
                               // enabled). We apply per-monitor DPI scaling over this scale. May be
                               // removed later.
  ...
  bool AntiAliasedLinesUseTex;  // Enable anti-aliased lines/borders using textures where possible.
                                ...
                                // point/nearest filtering). Latched at the beginning of the frame
  ...
  // Behaviors
  // (It is possible to modify those fields mid-frame if specific behavior need it, unlike e.g.
  // configuration fields in ImGuiIO)
  float HoverStationaryDelay;  // Delay for IsItemHovered(ImGuiHoveredFlags_Stationary). Time
                               // required to consider mouse stationary.
  float HoverDelayShort;   // Delay for IsItemHovered(ImGuiHoveredFlags_DelayShort). Usually used
  ...
  IMGUI_API void ScaleAllSizes(
      float scale_factor);  // Scale all spacing/padding/thickness values. Do not scale fonts.
...
//-----------------------------------------------------------------------------
// Also see ImGui::GetPlatformIO() and ImGuiPlatformIO struct for OS/platform related functions:
// clipboard, IME etc.
//-----------------------------------------------------------------------------

// [Internal] Storage used by IsKeyDown(), IsKeyPressed() etc functions.
// If prior to 1.87 you used io.KeysDownDuration[] (which was marked as internal), you should use
// GetKeyData(key)->DownDuration and *NOT* io.KeysData[key]->DownDuration.
struct ImGuiKeyData {
  bool Down;           // True for if key is down
  float DownDuration;  // Duration the key has been down (<0.0f: not pressed, 0.0f: just pressed,
                       // >0.0f: time held)
...
struct ImGuiIO {
  ...
  ImGuiConfigFlags ConfigFlags;    // = 0              // See ImGuiConfigFlags_ enum. Set by
                                   // user/application. Keyboard/Gamepad navigation options, etc.
  ...
  ImVec2 DisplayFramebufferScale;  // = (1, 1)         // Main display density. For retina display
                                   // where window coordinates are different from framebuffer
                                   // coordinates. This will affect font density + will end up in
  ...
  float IniSavingRate;  // = 5.0f           // Minimum time between saving positions/sizes to .ini
                        // file, in seconds.
  const char* IniFilename;  // = "imgui.ini"    // Path to .ini file (important: default "imgui.ini"
                            // is relative to current working dir!). Set NULL to disable automatic
                            // .ini loading/saving or if you want to manually call
  ...
  // Font system
  ImFontAtlas* Fonts;  // <auto>           // Font atlas: load, rasterize and pack one or more fonts
                       // into a single texture.
  ImFont* FontDefault;        // = NULL           // Font to use on NewFrame(). Use NULL to uses
                              // Fonts->Fonts[0].
  bool FontAllowUserScaling;  // = false          // [OBSOLETE] Allow user scaling text of
                              // individual window with CTRL+Wheel.

  // Keyboard/Gamepad Navigation options
  bool ConfigNavSwapGamepadButtons;  // = false          // Swap Activate<>Cancel (A<>B) buttons,
                                     // matching typical "Nintendo/Japanese style" gamepad layout.
  bool ConfigNavMoveSetMousePos;     // = false          // Directional/tabbing navigation teleports
                                     // the mouse cursor. May be useful on TV/console systems where
                                     // moving a virtual mouse is difficult. Will update io.MousePos
                                     // and set io.WantSetMousePos=true.
  bool ConfigNavCaptureKeyboard;     // = true           // Sets io.WantCaptureKeyboard when
                                     // io.NavActive is set.
  bool ConfigNavEscapeClearFocusItem;  // = true           // Pressing Escape can clear focused item
                                       // + navigation id/highlight. Set to false if you want to
                                       // always keep highlight on.
  bool ConfigNavEscapeClearFocusWindow;  // = false          // Pressing Escape can clear focused
                                         // window as well (super set of
                                         // io.ConfigNavEscapeClearFocusItem).
  bool ConfigNavCursorVisibleAuto;    // = true           // Using directional navigation key makes
                                      // the cursor visible. Mouse click hides the cursor.
  bool ConfigNavCursorVisibleAlways;  // = false          // Navigation cursor is always visible.

  // Miscellaneous options
  // (you can visualize and interact with all options in 'Demo->Configuration')
  bool MouseDrawCursor;  // = false          // Request ImGui to draw a mouse cursor for you (if you
                         // are on a platform without a mouse cursor). Cannot be easily renamed to
                         // 'io.ConfigXXX' because this is frequently used by backend
                         // implementations.
  bool ConfigMacOSXBehaviors;  // = defined(__APPLE__) // Swap Cmd<>Ctrl keys + OS X style text
                               // editing cursor movement using Alt instead of Ctrl, Shortcuts using
                               // Cmd/Super instead of Ctrl, Line/Text Start and End using
                               // Cmd+Arrows instead of Home/End, Double click selects by word
                               // instead of selecting whole text, Multi-selection in lists uses
                               // Cmd/Super instead of Ctrl.
  bool ConfigInputTrickleEventQueue;  // = true           // Enable input queue trickling: some
                                      // types of events submitted during the same frame (e.g.
                                      // button down + up) will be spread over multiple frames,
  ...
  bool ConfigInputTextEnterKeepActive;  // = false          // [BETA] Pressing Enter will keep item
                                        // active and select contents (single-line only).
  ...
  bool ConfigWindowsResizeFromEdges;  // = true           // Enable resizing of windows from their
                                      ...
                                      // ImGuiBackendFlags_HasMouseCursors for better mouse cursor
                                      // feedback. (This used to be a per-window
  ...
  bool ConfigWindowsCopyContentsWithCtrlC;  // = false      // [EXPERIMENTAL] CTRL+C copy the
                                            // contents of focused window into the clipboard.
                                            // Experimental because: (1) has known issues with
                                            // nested Begin/End pairs (2) text output quality varies
                                            // (3) text output is in submission order rather than
                                            // spatial order.
  bool ConfigScrollbarScrollByPage;  // = true           // Enable scrolling page by page when
                                     // clicking outside the scrollbar grab. When disabled, always
                                     // scroll to clicked location. When enabled, Shift+Click
                                     // scrolls to clicked location.
  ...
  // Inputs Behaviors
  // (other variables, ones which are expected to be tweaked within UI code, are exposed in
  // ImGuiStyle)
  float MouseDoubleClickTime;     // = 0.30f          // Time for a double-click, in seconds.
  float MouseDoubleClickMaxDist;  // = 6.0f           // Distance threshold to stay in to validate a
                                  // double-click, in pixels.
  float MouseDragThreshold;  // = 6.0f           // Distance threshold before considering we are
                             // dragging.
  float KeyRepeatDelay;      // = 0.275f         // When holding a key/button, time before it starts
  ...
  //    - Recovery after a programming error (native code or scripting language - the later tends to
  //    facilitate iterating on code while running).
  ...
  // - Functions that support error recovery are using IM_ASSERT_USER_ERROR() instead of
  // IM_ASSERT().
  // - By design, we do NOT allow error recovery to be 100% silent. One of the three options needs
  // to be checked!
  // - Always ensure that on programmers seats you have at minimum Asserts or Tooltips enabled when
  // making direct imgui API calls!
  //   Otherwise it would severely hinder your ability to catch and correct mistakes!
  // Read https://github.com/ocornut/imgui/wiki/Error-Handling for details.
  // - Programmer seats: keep asserts (default), or disable asserts and keep error tooltips (new and
  // nice!)
  // - Non-programmer seats: maybe disable asserts, but make sure errors are resurfaced (tooltips,
  // visible log entries, use callback etc.)
  // - Recovery after error/exception: record stack sizes with ErrorRecoveryStoreState(), disable
  // assert, set log callback (to e.g. trigger high-level breakpoint), recover with
  // ErrorRecoveryTryToRecoverState(), restore settings.
  bool ConfigErrorRecovery;  // = true       // Enable error recovery support. Some errors won't be
                             // detected and lead to direct crashes if recovery is disabled.
  bool ConfigErrorRecoveryEnableAssert;  // = true       // Enable asserts on recoverable error. By
                                         // default call IM_ASSERT() when returning from a failing
                                         // IM_ASSERT_USER_ERROR()
  bool ConfigErrorRecoveryEnableDebugLog;  // = true       // Enable debug log output on recoverable
                                           // errors.
  bool ConfigErrorRecoveryEnableTooltip;   // = true       // Enable tooltip on recoverable errors.
                                           // The tooltip include a way to enable asserts if they
                                           // were disabled.

  // Option to enable various debug tools showing buttons that will call the IM_DEBUG_BREAK() macro.
  // - The Item Picker tool will be available regardless of this being enabled, in order to maximize
  // its discoverability.
  // - Requires a debugger being attached, otherwise IM_DEBUG_BREAK() options will appear to crash
  // your application.
  //   e.g. io.ConfigDebugIsDebuggerPresent = ::IsDebuggerPresent() on Win32, or refer to
  //   ImOsIsDebuggerPresent() imgui_test_engine/imgui_te_utils.cpp for a Unix compatible version.
  bool ConfigDebugIsDebuggerPresent;  // = false          // Enable various tools calling
                                      // IM_DEBUG_BREAK().
  ...
  // regardless of return value of BeginXXX()
  // - This is inconsistent with other BeginXXX functions and create confusion for many users.
  // - We expect to update the API eventually. In the meanwhile we provide tools to facilitate
  // checking user-code behavior.
  bool ConfigDebugBeginReturnValueOnce;  // = false          // First-time calls to
                                         // Begin()/BeginChild() will return false. NEEDS TO BE SET
                                         // AT APPLICATION BOOT TIME if you don't want to miss
                                         // windows.
  bool ConfigDebugBeginReturnValueLoop;  // = false          // Some calls to Begin()/BeginChild()
                                         // will return false. Will cycle through window depths then
                                         // repeat. Suggested use: add
                                         // "io.ConfigDebugBeginReturnValue = io.KeyShift" in your
                                         // main loop then occasionally press SHIFT. Windows should
                                         // be flickering while running.

  // Option to deactivate io.AddFocusEvent(false) handling.
  // - May facilitate interactions with a debugger when focus loss leads to clearing inputs data.
  // - Backends may have other side-effects on focus loss, so this will reduce side-effects but not
  // necessary remove all of them.
  bool ConfigDebugIgnoreFocusLoss;  // = false          // Ignore io.AddFocusEvent(false),
                                    // consequently not calling
                                    // io.ClearInputKeys()/io.ClearInputMouse() in input processing.

  // Option to audit .ini data
  bool ConfigDebugIniSettings;  // = false          // Save .ini data with extra comments
                                // (particularly helpful for Docking, but makes saving slower)
  ...
  // Nowadays those would be stored in ImGuiPlatformIO but we are leaving them here for legacy
  // reasons. Optional: Platform/Renderer backend name (informational only! will be displayed in
  ...
  IMGUI_API void AddMousePosEvent(
      float x, float y);  // Queue a mouse position update. Use -FLT_MAX,-FLT_MAX to signify no
                          // mouse (e.g. app not focused and not hovered)
  ...
  IMGUI_API void AddMouseSourceEvent(
      ImGuiMouseSource source);  // Queue a mouse source change (Mouse/TouchScreen/Pen)
  IMGUI_API void AddFocusEvent(
      bool focused);  // Queue a gain/loss of focus for the application (generally based on
                      // OS/platform focus of your window)
  IMGUI_API void AddInputCharacter(unsigned int c);  // Queue a new character input
  IMGUI_API void AddInputCharacterUTF16(
      ImWchar16 c);  // Queue a new character input from a UTF-16 character, it can be a surrogate
  ...
  IMGUI_API void SetKeyEventNativeData(
      ...
      int native_legacy_index =
          -1);  // [Optional] Specify index for legacy <1.87 IsKeyXXX() functions with native
                // indices + specify native keycode, scancode.
  IMGUI_API void SetAppAcceptingEvents(
      bool accepting_events);  // Set master flag for accepting key/mouse/text events (default to
                               // true). Useful if you have native dialog boxes that are
                               // interrupting your application loop/refresh, and you want to
                               // disable events being queued while your app is frozen.
  IMGUI_API void ClearEventsQueue();  // Clear all incoming events.
  IMGUI_API void ClearInputKeys();    // Clear current keyboard/gamepad state + current frame text
                                      // input buffer. Equivalent to releasing all keys/buttons.
  IMGUI_API void ClearInputMouse();   // Clear current mouse state.
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
  IMGUI_API void ClearInputCharacters();  // [Obsoleted in 1.89.8] Clear the current frame text
  ...
  bool
      WantCaptureMouse;  // Set when Dear ImGui will use mouse inputs, in this case do not dispatch
                         // them to your main game/application (either way, always pass on mouse
                         // inputs to imgui). (e.g. unclicked mouse is hovering over an imgui
                         // window, widget is active, mouse was clicked over an imgui window, etc.).
  bool WantCaptureKeyboard;  // Set when Dear ImGui will use keyboard inputs, in this case do not
                             // dispatch them to your main game/application (either way, always pass
                             // keyboard inputs to imgui). (e.g. InputText active, or an imgui
  ...
  bool WantTextInput;  // Mobile/console: when set, you may display an on-screen keyboard. This is
                       // set by Dear ImGui when it wants textual keyboard input to happen (e.g.
                       // when a InputText widget is active).
  bool
      WantSetMousePos;  // MousePos has been altered, backend should reposition mouse on next frame.
                        // Rarely used! Set only when io.ConfigNavMoveSetMousePos is enabled.
  ...
  bool NavActive;   // Keyboard/Gamepad navigation is currently allowed (will handle ImGuiKey_NavXXX
                    // events) = a window is focused and it doesn't use the
                    // ImGuiWindowFlags_NoNavInputs flag.
  bool NavVisible;  // Keyboard/Gamepad navigation highlight is visible and allowed (will handle
                    // ImGuiKey_NavXXX events).
  float Framerate;  // Estimate of application framerate (rolling average over 60 frames, based on
                    // io.DeltaTime), in frame per second. Solely for convenience. Slow applications
                    // may not want to use a moving average or may want to reset underlying buffers
                    // occasionally.
  ...
  ImVec2 MouseDelta;  // Mouse delta. Note that this is zero if either current or previous position
                      // are invalid (-FLT_MAX,-FLT_MAX), so a disappearing/reappearing mouse won't
                      // have a huge delta.
  ...
  // directly, call the AddXXX functions above instead) (reading from those variables is fair game,
  // as they are extremely unlikely to be moving anywhere)
  ...
  bool MouseDown[5];  // Mouse buttons: 0=left, 1=right, 2=middle + extras (ImGuiMouseButton_COUNT
                      ...
                      // us to track if the mouse is being used by your application + available to
  ...
  float MouseWheelH;  // Mouse wheel Horizontal. >0 scrolls Left, <0 scrolls Right. Most users don't
                      // have a mouse with a horizontal wheel, may not be filled by all backends.
  ...
  ImGuiKeyChord
      KeyMods;  // Key mods flags (any of ImGuiMod_Ctrl/ImGuiMod_Shift/ImGuiMod_Alt/ImGuiMod_Super
                // flags, same as io.KeyCtrl/KeyShift/KeyAlt/KeySuper but merged into flags).
                // Read-only, updated by NewFrame()
  ImGuiKeyData KeysData[ImGuiKey_NamedKey_COUNT];  // Key state for all known keys. MUST use 'key -
                                                   // ImGuiKey_NamedKey_BEGIN' as index. Use
                                                   // IsKeyXXX() functions to access this.
  bool WantCaptureMouseUnlessPopupClose;  // Alternative to WantCaptureMouse: (WantCaptureMouse ==
                                          // true && WantCaptureMouseUnlessPopupClose == false) when
                                          // a click over void is expected to close a popup.
  ...
  ImU16 MouseClickedCount[5];  // == 0 (not clicked), == 1 (same as MouseClicked[]), == 2
                               // (double-clicked), == 3 (triple-clicked) etc. when going from !Down
                               // to Down
  ImU16 MouseClickedLastCount[5];  // Count successive number of clicks. Stays valid after mouse
                                   // release. Reset after another click is done.
  bool MouseReleased[5];           // Mouse button went from Down to !Down
  double MouseReleasedTime[5];  // Time of last released (rarely used! but useful to handle delayed
                                // single-click when trying to disambiguate them from double-click).
  bool MouseDownOwned[5];  // Track if button was clicked inside a dear imgui window or over void
                           // blocked by a popup. We don't request mouse capture from the
                           // application if click started outside ImGui bounds.
  bool
      MouseDownOwnedUnlessPopupClose[5];  // Track if button was clicked inside a dear imgui window.
  bool MouseWheelRequestAxisSwap;  // On a non-Mac system, holding SHIFT requests WheelY to perform
                                   // the equivalent of a WheelX event. On a Mac system this is
                                   // already enforced by the system.
  bool MouseCtrlLeftAsRightClick;  // (OSX) Set to true when the current click was a Ctrl+click that
                                   // spawned a simulated right click
  float MouseDownDuration[5];      // Duration the mouse button has been down (0.0f == just clicked)
  float MouseDownDurationPrev[5];  // Previous time the mouse button has been down
  float MouseDragMaxDistanceSqr[5];  // Squared maximum distance of how much mouse has traveled from
                                     // the clicking point (used for moving thresholds)
  float PenPressure;  // Touch/Pen pressure (0.0f to 1.0f, should be >0.0f only when MouseDown[0] ==
                      // true). Helper storage currently unused by Dear ImGui.
  ...
  // Legacy: before 1.87, we required backend to fill io.KeyMap[] (imgui->native map) during
  // initialization and io.KeysDown[] (native indices) every frame. This is still temporarily
  // supported as a legacy feature. However the new preferred scheme is for backend to call
  // io.AddKeyEvent().
  //   Old (<1.87):  ImGui::IsKeyPressed(ImGui::GetIO().KeyMap[ImGuiKey_Space]) --> New (1.87+)
  //   ImGui::IsKeyPressed(ImGuiKey_Space) Old (<1.87):  ImGui::IsKeyPressed(MYPLATFORM_KEY_SPACE)
  //   --> New (1.87+) ImGui::IsKeyPressed(ImGuiKey_Space)
  // Read https://github.com/ocornut/imgui/issues/4921 for details.
  // int       KeyMap[ImGuiKey_COUNT];             // [LEGACY] Input: map of indices into the
  // KeysDown[512] entries array which represent your "native" keyboard state. The first 512 are now
  // unused and should be kept zero. Legacy backend will write into KeyMap[] using ImGuiKey_ indices
  // which are always >512. bool      KeysDown[ImGuiKey_COUNT];           // [LEGACY] Input:
  // Keyboard keys that are pressed (ideally left in the "native" order your engine has access to
  // keyboard keys, so you can use your own defines/enums for keys). This used to be [512] sized. It
  // is now ImGuiKey_COUNT to allow legacy io.KeysDown[GetKeyIndex(...)] to work without an
  // overflow. float     NavInputs[ImGuiNavInput_COUNT];     // [LEGACY] Since 1.88, NavInputs[] was
  // removed. Backends from 1.60 to 1.86 won't build. Feed gamepad inputs via io.AddKeyEvent() and
  // ImGuiKey_GamepadXXX enums. void*     ImeWindowHandle;                    // [Obsoleted in 1.87]
  // Set ImGuiViewport::PlatformHandleRaw instead. Set this to your HWND to get automatic IME cursor
  // positioning.
  ...
  // As this is will affect all users of custom engines/backends, we are providing proper legacy
  // redirection (will obsolete).
...
// details)
// - ImGuiInputTextFlags_CallbackEdit:        Callback on buffer edit. Note that InputText() already
// returns true on edit + you can always use IsItemEdited(). The callback is useful to manipulate
...
struct ImGuiInputTextCallbackData {
  ...
  // - However, during Completion/History/Always callback, Buf always points to our own internal
  // data (it is not the same as your buffer)! Changes to it will be reflected into your own buffer
  // shortly after the callback.
  ...
  char* Buf;     // Text buffer                          // Read-write   // [Resize] Can replace
              // pointer / [Completion,History,Always] Only write to pointed data, don't replace the
              // actual pointer!
  int BufTextLen;  // Text length (in bytes)               // Read-write   //
                   // [Resize,Completion,History,Always] Exclude zero-terminator storage. In C land:
...
// a UI within deep-nested code that runs multiple times every frame. Usage: static
...
struct ImGuiTextBuffer {
  ...
  void resize(int size) {
    if (Buf.Size > size) Buf.Data[size] = 0;
...
struct ImGuiStorage {
  ...
  // - A typical use case where this is convenient for quick hacking (e.g. add storage during a live
  // Edit&Continue session if you can't modify existing struct)
  ...
  // Advanced: for quicker full rebuild of a storage (instead of an incremental one), you may add
  // all your contents and then sort once.
  IMGUI_API void BuildSortByKey();
  // Obsolete: use on your own storage if you know only integer are being stored (open/close all
  // tree nodes)
...
// Flags for ImGuiListClipper (currently not fully exposed in function calls: a future refactor will
// likely add this to ImGuiListClipper::Begin function equivalent)
enum ImGuiListClipperFlags_ {
  ImGuiListClipperFlags_None = 0,
  ImGuiListClipperFlags_NoSetTableRowCounters =
      1 << 0,  // [Internal] Disabled modifying table row counters. Avoid assumption that 1 clipper
               // item == 1 table row.
};

// Helper: Manually clip large list of items.
// If you have lots evenly spaced items and you have random access to the list, you can perform
// coarse clipping based on visibility to only submit items that are in view. The clipper calculates
// the range of visible items and advance the cursor to compensate for the non-visible items we have
// skipped. (Dear ImGui already clip items based on their bounds but: it needs to first layout the
// item to do so, and generally
//  fetching/submitting your own data incurs additional cost. Coarse clipping using ImGuiListClipper
//  allows you to easily scale using lists with tens of thousands of items without a problem)
// Usage:
...
// - The clipper also handles various subtleties related to keyboard/gamepad navigation, wrapping
// etc.
struct ImGuiListClipper {
  ...
  float ItemsHeight;  // [Internal] Height of item after a first step and item submission can
                      // calculate it
  double StartPosY;  // [Internal] Cursor position at the time of Begin() or after table frozen rows
                     // are all processed
  double StartSeekOffsetY;      // [Internal] Account for frozen rows in a table and initial loss of
                                // precision in very large windows.
  ...
  // won't be advanced in the final step, and you can call SeekCursorForItem() manually if you need)
  ...
  IMGUI_API bool Step();  // Call until it returns false. The DisplayStart/DisplayEnd fields will be
                          // set and you can process/draw those items.

  // Call IncludeItemByIndex() or IncludeItemsByIndex() *BEFORE* first call to Step() if you need a
  // range of items to not be clipped, regardless of their visibility. (Due to alignment / padding
  // of certain items it is possible that an extra item may be included on either end of the display
  // range).
  ...
  // IncludeItemsByIndex(item_begin, item_end); } // [renamed in 1.89.9] inline void
  // ForceDisplayRangeByIndices(int item_begin, int item_end) { IncludeItemsByIndex(item_begin,
  // item_end); } // [renamed in 1.89.6] inline ImGuiListClipper(int items_count, float items_height
  // = -1.0f) { memset(this, 0, sizeof(*this)); ItemsCount = -1; Begin(items_count, items_height); }
  // // [removed in 1.79]
#endif
};
...
// - This is in order to allow user enabling implicit cast operators between ImVec2/ImVec4 and their
// own types (using IM_VEC2_CLASS_EXTRA in imconfig.h)
// - Add '#define IMGUI_DEFINE_MATH_OPERATORS' before including this file (or in imconfig.h) to
// access courtesy maths operators for ImVec2 and ImVec4.
// - We intentionally provide ImVec2*float but not float*ImVec2: this is rare enough and we want to
// reduce the surface for possible user mistake.
...
// - User can declare their own format by #defining the 5 _SHIFT/_MASK macros in their imconfig
// file.
// - Any setting other than the default will need custom backend support. The only standard backend
// that supports anything else than the default is DirectX9.
...
// ImGuiSelectionRequestType, ImGuiSelectionRequest, ImGuiMultiSelectIO, ImGuiSelectionBasicStorage)
//-----------------------------------------------------------------------------

// Multi-selection system
// Documentation at: https://github.com/ocornut/imgui/wiki/Multi-Select
// - Refer to 'Demo->Widgets->Selection State & Multi-Select' for demos using this.
// - This system implements standard multi-selection idioms (CTRL+Mouse/Keyboard,
// SHIFT+Mouse/Keyboard, etc)
//   with support for clipper (skipping non-visible items), box-select and many other details.
// - Selectable(), Checkbox() are supported but custom widgets may use it as well.
// - TreeNode() is technically supported but... using this correctly is more complicated: you need
// some sort of linear/random access to your tree,
//   which is suited to advanced trees setups also implementing filters and clipper. We will work
//   toward simplifying and demoing it.
// - In the spirit of Dear ImGui design, your code owns actual selection data.
//   This is designed to allow all kinds of selection storage you may use in your application e.g.
//   set/map/hash.
...
// Usage:
// - Identify submitted items with SetNextItemSelectionUserData(), most likely using an index into
// your current data-set.
// - Store and maintain actual selection data using persistent object identifiers.
// - Usage flow:
//     BEGIN - (1) Call BeginMultiSelect() and retrieve the ImGuiMultiSelectIO* result.
//           - (2) Honor request list (SetAll/SetRange requests) by updating your selection data.
//           Same code as Step 6.
//           - (3) [If using clipper] You need to make sure RangeSrcItem is always submitted.
//           Calculate its index and pass to clipper.IncludeItemByIndex(). If storing indices in
//           ImGuiSelectionUserData, a simple clipper.IncludeItemByIndex(ms_io->RangeSrcItem) call
//           will work.
//     LOOP  - (4) Submit your items with SetNextItemSelectionUserData() + Selectable()/TreeNode()
//     calls. END   - (5) Call EndMultiSelect() and retrieve the ImGuiMultiSelectIO* result.
//           - (6) Honor request list (SetAll/SetRange requests) by updating your selection data.
//           Same code as Step 2.
//     If you submit all items (no clipper), Step 2 and 3 are optional and will be handled by each
//     item themselves. It is fine to always honor those steps.
...
// ImGuiMultiSelectIO.
// - Most applications will store an object INDEX, hence the chosen name and type. Storing an index
// is natural, because
//   SetRange requests will give you two end-points and you will need to iterate/interpolate between
//   them to update your selection.
// - However it is perfectly possible to store a POINTER or another IDENTIFIER inside
// ImGuiSelectionUserData.
//   Our system never assume that you identify items by indices, it never attempts to interpolate
//   between two values.
// - If you enable ImGuiMultiSelectFlags_NoRangeSelect then it is guaranteed that you will never
// have to interpolate
//   between two ImGuiSelectionUserData, which may be a convenient way to use part of the feature
//   with less code work.
// - As most users will want to store an index, for convenience and to reduce confusion we use ImS64
// instead of void*,
//   being syntactically easier to downcast. Feel free to reinterpret_cast and store a pointer
//   inside.
...
enum ImGuiMultiSelectFlags_ {
  ...
  ImGuiMultiSelectFlags_SingleSelect =
      1 << 0,  // Disable selecting more than one item. This is available to allow single-selection
               // code to share same code/logic if desired. It essentially disables the main purpose
               // of BeginMultiSelect() tho!
  ImGuiMultiSelectFlags_NoSelectAll = 1 << 1,  // Disable CTRL+A shortcut to select all.
  ImGuiMultiSelectFlags_NoRangeSelect =
      1 << 2,  // Disable Shift+selection mouse/keyboard support (useful for unordered 2D
               // selection). With BoxSelect is also ensure contiguous SetRange requests are not
               // combined into one. This allows not handling interpolation in SetRange requests.
  ImGuiMultiSelectFlags_NoAutoSelect =
      1 << 3,  // Disable selecting items when navigating (useful for e.g. supporting range-select
               // in a list of checkboxes).
  ImGuiMultiSelectFlags_NoAutoClear =
      1 << 4,  // Disable clearing selection when navigating or selecting another one (generally
               // used with ImGuiMultiSelectFlags_NoAutoSelect. useful for e.g. supporting
  ...
  ImGuiMultiSelectFlags_BoxSelect1d =
      1 << 6,  // Enable box-selection with same width and same x pos items (e.g. full row
               // Selectable()). Box-selection works better with little bit of spacing between items
               // hit-box in order to be able to aim at empty space.
  ImGuiMultiSelectFlags_BoxSelect2d =
      1 << 7,  // Enable box-selection with varying width or varying x pos items support (e.g.
               // different width labels, or 2D layout/grid). This is slower: alters clipping logic
               // so that e.g. horizontal movements will update selection of normally clipped items.
  ...
  ImGuiMultiSelectFlags_ScopeRect =
      1 << 12,  // Scope for _BoxSelect and _ClearOnClickVoid is rectangle encompassing
                // BeginMultiSelect()/EndMultiSelect(). Use if BeginMultiSelect() is called multiple
                // times in same window.
  ...
  ImGuiMultiSelectFlags_SelectOnClickRelease =
      1 << 14,  // Apply selection on mouse release when clicking an unselected item. Allow dragging
                // an unselected item without altering selection.
  // ImGuiMultiSelectFlags_RangeSelect2d       = 1 << 15,  // Shift+Selection uses 2d geometry
  // instead of linear sequence, so possible to use Shift+up/down to select vertically in grid.
  // Analogous to what BoxSelect does.
  ImGuiMultiSelectFlags_NavWrapX =
      1 << 16,  // [Temporary] Enable navigation wrapping on X axis. Provided as a convenience
                // because we don't have a design for the general Nav API for this yet. When the
                // more general feature be public we may obsolete this flag in favor of new one.
};

// Main IO structure returned by BeginMultiSelect()/EndMultiSelect().
// This mainly contains a list of selection requests.
// - Use 'Demo->Tools->Debug Log->Selection' to see requests as they happen.
// - Some fields are only useful if your list is dynamic and allows deletion (getting post-deletion
// focus/state right is shown in the demo)
// - Below: who reads/writes each fields? 'r'=read, 'w'=write, 'ms'=multi-select code,
...
struct ImGuiMultiSelectIO {
  ...
  ImGuiSelectionUserData
      RangeSrcItem;  //  ms:w  app:r     /                // (If using clipper) Begin: Source item
                     //  (often the first selected item) must never be clipped: use
                     //  clipper.IncludeItemByIndex() to ensure it is submitted.
  ImGuiSelectionUserData
      NavIdItem;  //  ms:w, app:r     /                // (If using deletion) Last known
                  //  SetNextItemSelectionUserData() value for NavId (if part of submitted items).
  bool NavIdSelected;  //  ms:w, app:r     /        app:r   // (If using deletion) Last known
                       //  selection state for NavId (if part of submitted items).
  bool RangeSrcReset;  //        app:w     /  ms:r          // (If using deletion) Set before
                       //        EndMultiSelect() to reset ResetSrcItem (e.g. if deleted selection).
  int ItemsCount;      //  ms:w, app:r     /        app:r   // 'int items_count' parameter to
                   //  BeginMultiSelect() is copied here for convenience, allowing simpler calls to
                   //  your ApplyRequests handler. Not used internally.
};

// Selection request type
enum ImGuiSelectionRequestType {
  ImGuiSelectionRequestType_None = 0,
  ImGuiSelectionRequestType_SetAll,  // Request app to clear selection (if Selected==false) or
                                     // select all items (if Selected==true). We cannot set
                                     // RangeFirstItem/RangeLastItem as its contents is entirely up
                                     // to user (not necessarily an index)
  ImGuiSelectionRequestType_SetRange,  // Request app to select/unselect
                                       // [RangeFirstItem..RangeLastItem] items (inclusive) based on
                                       // value of Selected. Only EndMultiSelect() request this, app
                                       // code can read after BeginMultiSelect() and it will always
                                       // be false.
};
...
struct ImGuiSelectionRequest {
  //------------------------------------------// BeginMultiSelect / EndMultiSelect
  ImGuiSelectionRequestType Type;  //  ms:w, app:r     /  ms:w, app:r   // Request type. You'll most
                                   //  often receive 1 Clear + 1 SetRange with a single-item range.
  bool Selected;  //  ms:w, app:r     /  ms:w, app:r   // Parameter for SetAll/SetRange requests
                  //  (true = select, false = unselect)
  ImS8 RangeDirection;  //                  /  ms:w  app:r   // Parameter for SetRange request: +1
                        //                  when RangeFirstItem comes before RangeLastItem, -1
                        //                  otherwise. Useful if you want to preserve selection
                        //                  order on a backward Shift+Click.
  ImGuiSelectionUserData
      RangeFirstItem;  //                  /  ms:w, app:r   // Parameter for SetRange request (this
                       //                  is generally == RangeSrcItem when shift selecting from
                       //                  top to bottom).
  ImGuiSelectionUserData
      RangeLastItem;  //                  /  ms:w, app:r   // Parameter for SetRange request (this
                      //                  is generally == RangeSrcItem when shift selecting from
                      //                  bottom to top). Inclusive!
};

// Optional helper to store multi-selection state + apply multi-selection requests.
// - Used by our demos and provided as a convenience to easily implement basic multi-selection.
// - Iterate selection with 'void* it = NULL; ImGuiID id; while (selection.GetNextSelectedItem(&it,
// &id)) { ... }'
//   Or you can check 'if (Contains(id)) { ... }' for each possible object if their number is not
//   too high to iterate.
// - USING THIS IS NOT MANDATORY. This is only a helper and not a required API.
// To store a multi-selection, in your application you could:
// - Use this helper as a convenience. We use our simple key->value ImGuiStorage as a
// std::set<ImGuiID> replacement.
// - Use your own external storage: e.g. std::set<MyObjectId>, std::vector<MyObjectId>, interval
// trees, intrusively stored selection etc. In ImGuiSelectionBasicStorage we:
// - always use indices in the multi-selection API (passed to SetNextItemSelectionUserData(),
// retrieved in ImGuiMultiSelectIO)
// - use the AdapterIndexToStorageId() indirection layer to abstract how persistent selection data
// is derived from an index.
// - use decently optimized logic to allow queries and insertion of very large selection sets.
// - do not preserve selection order.
// Many combinations are possible depending on how you prefer to store your items and how you prefer
// to store your selection. Large applications are likely to eventually want to get rid of this
// indirection layer and do their own thing. See https://github.com/ocornut/imgui/wiki/Multi-Select
// for details and pseudo-code using this helper.
struct ImGuiSelectionBasicStorage {
  // Members
  int Size;            //          // Number of selected items, maintained by this helper.
  bool PreserveOrder;  // = false  // GetNextSelectedItem() will return ordered selection (currently
                       // implemented by two additional sorts of selection. Could be improved)
  void* UserData;      // = NULL   // User data for use by adapter function        // e.g.
                       // selection.UserData = (void*)my_items;
  ImGuiID (*AdapterIndexToStorageId)(
      ImGuiSelectionBasicStorage* self,
      int idx);  // e.g. selection.AdapterIndexToStorageId = [](ImGuiSelectionBasicStorage* self,
                 // int idx) { return ((MyItems**)self->UserData)[idx]->ID; };
  int _SelectionOrder;  // [Internal] Increasing counter to store selection order
  ImGuiStorage
      _Storage;  // [Internal] Selection set. Think of this as similar to e.g. std::set<ImGuiID>.
                 // Prefer not accessing directly: iterate with GetNextSelectedItem().

  // Methods
  IMGUI_API ImGuiSelectionBasicStorage();
  IMGUI_API void ApplyRequests(
      ImGuiMultiSelectIO*
          ms_io);  // Apply selection requests coming from BeginMultiSelect() and EndMultiSelect()
  ...
  IMGUI_API void SetItemSelected(ImGuiID id,
                                 bool selected);  // Add/remove an item from selection (generally
                                                  // done by ApplyRequests() function)
  IMGUI_API bool GetNextSelectedItem(
      void** opaque_it,
      ImGuiID* out_id);  // Iterate selection with 'void* it = NULL; ImGuiID id; while
...
// Convenient if you want to quickly wire multi-select API on e.g. an array of bool or items storing
...
struct ImGuiSelectionExternalStorage {
  ...
  void (*AdapterSetItemSelected)(
      ...
      bool selected);  // e.g. AdapterSetItemSelected = [](ImGuiSelectionExternalStorage* self, int
                       // idx, bool selected) { ((MyItems**)self->UserData)[idx]->Selected =
                       // selected; }

  // Methods
  IMGUI_API ImGuiSelectionExternalStorage();
  IMGUI_API void ApplyRequests(ImGuiMultiSelectIO* ms_io);  // Apply selection requests by using
                                                            // AdapterSetItemSelected() calls
};
...
// - To use 16-bit indices + allow large meshes: backend need to set 'io.BackendFlags |=
// ImGuiBackendFlags_RendererHasVtxOffset' and handle ImDrawCmd::VtxOffset (recommended).
// - To use 32-bit indices: override with '#define ImDrawIdx unsigned int' in your imconfig.h file.
#ifndef ImDrawIdx
typedef unsigned short
    ImDrawIdx;  // Default: 16-bit (for maximum compatibility with renderer backends)
#endif
...
// imconfig.h) + update rendering backend accordingly.
#ifndef ImDrawCallback
typedef void (*ImDrawCallback)(const ImDrawList* parent_list, const ImDrawCmd* cmd);
#endif

// Special Draw callback value to request renderer backend to reset the graphics/render state.
// The renderer backend needs to handle this special value, otherwise it will crash trying to call a
// function at this address. This is useful, for example, if you submitted callbacks which you know
// have altered the render state and you want it to be restored. Render state is not reset by
// default because they are many perfectly useful way of altering render state (e.g. changing
...
// - The ClipRect/TexRef/VtxOffset fields must be contiguous as we memcmp() them together (this is
// asserted for).
struct ImDrawCmd {
  ImVec4 ClipRect;  // 4*4  // Clipping rectangle (x1, y1, x2, y2). Subtract ImDrawData->DisplayPos
                    // to get clipping rectangle in "viewport" coordinates
  ImTextureRef TexRef;     // 16   // Reference to a font/texture atlas (where backend called
                           // ImTextureData::SetTexID()) or to a user-provided texture ID (via e.g.
                           // ImGui::Image() calls). Both will lead to a ImTextureID value.
  unsigned int VtxOffset;  // 4    // Start offset in vertex buffer.
                           // ImGuiBackendFlags_RendererHasVtxOffset: always 0, otherwise may be >0
                           // to support meshes larger than 64K vertices with 16-bit indices.
  unsigned int IdxOffset;  // 4    // Start offset in index buffer.
  unsigned int ElemCount;  // 4    // Number of indices (multiple of 3) to be rendered as triangles.
  ...
  void* UserCallbackData;       // 4-8  // Callback user data (when UserCallback != NULL). If called
                                // AddCallback() with size == 0, this is a copy of the AddCallback()
                           // argument. If called AddCallback() with size > 0, this is pointing to a
                           // buffer where data is stored.
  int UserCallbackDataSize;    // 4 // Size of callback user data when using storage, otherwise 0.
  int UserCallbackDataOffset;  // 4 // [Internal] Offset of callback user data when using storage,
                               // otherwise -1.

  ImDrawCmd() { memset(this, 0, sizeof(*this)); }  // Also ensure our padding fields are zeroed

  // Since 1.83: returns ImTextureID associated with this draw call. Warning: DO NOT assume this is
  // always same as 'TextureId' (we will change this function for an upcoming feature) Since 1.92:
  // removed ImDrawCmd::TextureId field, the getter function must be used!
  inline ImTextureID GetTexID()
      const;  // == (TexRef._TexData ? TexRef._TexData->TexID : TexRef._TexID)
};
...
// This is because ImVec2/ImU32 are likely not declared at the time you'd want to set your type up.
...
// into out of order. This is used by the Columns/Tables API, so items of each column can be batched
...
enum ImDrawFlags_ {
  ...
  ImDrawFlags_RoundCornersTopLeft =
      1 << 4,  // AddRect(), AddRectFilled(), PathRect(): enable rounding top-left corner only (when
               // rounding > 0.0f, we default to all corners). Was 0x01.
...
// Flags for ImDrawList instance. Those are set automatically by ImGui:: functions from ImGuiIO
...
enum ImDrawListFlags_ {
  ...
  ImDrawListFlags_AntiAliasedLines =
      1
      << 0,  // Enable anti-aliased lines/borders (*2 the number of triangles for 1.0f wide line or
             // lines thin enough to be drawn using textures, otherwise *3 the number of triangles)
  ImDrawListFlags_AntiAliasedLinesUseTex =
      1 << 1,  // Enable anti-aliased lines/borders using textures when possible. Require backend to
...
// == GetMainViewport()->Pos+Size (generally io.DisplaySize). You are totally free to apply whatever
// transformation matrix you want to the data (depending on the use of the transformation you may
// want to apply it to ClipRect as well!) Important: Primitives are always added to the list and not
// culled (culling is done at higher-level by ImGui:: functions), if you use this API a lot consider
...
struct ImDrawList {
  ...
  ImDrawListSharedData*
      _Data;  // Pointer to shared draw data (you can use ImGui::GetDrawListSharedData() to get the
  ...
  // (advanced: you may create and use your own ImDrawListSharedData so you can use ImDrawList
  // without ImGui, but that's more involved)
  ...
  // Primitives
  // - Filled shapes must always use clockwise winding order. The anti-aliasing fringe depends on
  // it. Counter-clockwise shapes will have "inward" anti-aliasing.
  ...
  // (preferred).
  //   In older versions (until Dear ImGui 1.77) the AddCircle functions defaulted to num_segments
  //   == 12. In future versions we will use textures to provide cheaper and higher-quality circles.
  //   Use AddNgon() and AddNgonFilled() functions if you need to guarantee a specific number of
  //   sides.
  ...
  IMGUI_API void AddQuad(const ImVec2& p1, const ImVec2& p2, const ImVec2& p3, const ImVec2& p4,
                         ImU32 col, float thickness = 1.0f);
  IMGUI_API void AddQuadFilled(const ImVec2& p1, const ImVec2& p2, const ImVec2& p3,
                               const ImVec2& p4, ImU32 col);
  IMGUI_API void AddTriangle(const ImVec2& p1, const ImVec2& p2, const ImVec2& p3, ImU32 col,
                             float thickness = 1.0f);
  IMGUI_API void AddTriangleFilled(const ImVec2& p1, const ImVec2& p2, const ImVec2& p3, ImU32 col);
  IMGUI_API void AddCircle(const ImVec2& center, float radius, ImU32 col, int num_segments = 0,
                           float thickness = 1.0f);
  IMGUI_API void AddCircleFilled(const ImVec2& center, float radius, ImU32 col,
                                 int num_segments = 0);
  IMGUI_API void AddNgon(const ImVec2& center, float radius, ImU32 col, int num_segments,
                         float thickness = 1.0f);
  IMGUI_API void AddNgonFilled(const ImVec2& center, float radius, ImU32 col, int num_segments);
  IMGUI_API void AddEllipse(const ImVec2& center, const ImVec2& radius, ImU32 col, float rot = 0.0f,
                            int num_segments = 0, float thickness = 1.0f);
  IMGUI_API void AddEllipseFilled(const ImVec2& center, const ImVec2& radius, ImU32 col,
                                  float rot = 0.0f, int num_segments = 0);
  IMGUI_API void AddText(const ImVec2& pos, ImU32 col, const char* text_begin,
                         const char* text_end = NULL);
  IMGUI_API void AddText(ImFont* font, float font_size, const ImVec2& pos, ImU32 col,
                         const char* text_begin, const char* text_end = NULL,
                         float wrap_width = 0.0f, const ImVec4* cpu_fine_clip_rect = NULL);
  IMGUI_API void AddBezierCubic(const ImVec2& p1, const ImVec2& p2, const ImVec2& p3,
                                const ImVec2& p4, ImU32 col, float thickness,
                                int num_segments = 0);  // Cubic Bezier (4 control points)
  IMGUI_API void AddBezierQuadratic(const ImVec2& p1, const ImVec2& p2, const ImVec2& p3, ImU32 col,
                                    float thickness,
                                    int num_segments = 0);  // Quadratic Bezier (3 control points)

  // General polygon
  // - Only simple polygons are supported by filling functions (no self-intersections, no holes).
  // - Concave polygon fill is more expensive than convex one: it has O(N^2) complexity. Provided as
  // a convenience for the user but not used by the main library.
  IMGUI_API void AddPolyline(const ImVec2* points, int num_points, ImU32 col, ImDrawFlags flags,
                             float thickness);
  ...
  IMGUI_API void AddImage(ImTextureRef tex_ref, const ImVec2& p_min, const ImVec2& p_max,
                          const ImVec2& uv_min = ImVec2(0, 0), const ImVec2& uv_max = ImVec2(1, 1),
                          ImU32 col = IM_COL32_WHITE);
  IMGUI_API void AddImageQuad(ImTextureRef tex_ref, const ImVec2& p1, const ImVec2& p2,
  ...
  IMGUI_API void AddImageRounded(ImTextureRef tex_ref, const ImVec2& p_min, const ImVec2& p_max,
  ...
  // depends on it. Counter-clockwise shapes will have "inward" anti-aliasing.
  //   so e.g. 'PathArcTo(center, radius, PI * -0.5f, PI)' is ok, whereas 'PathArcTo(center, radius,
  //   PI, PI * -0.5f)' won't have correct anti-aliasing when followed by PathFillConvex().
  ...
  IMGUI_API void PathArcTo(const ImVec2& center, float radius, float a_min, float a_max,
                           int num_segments = 0);
  ...
  IMGUI_API void PathEllipticalArcTo(const ImVec2& center, const ImVec2& radius, float rot,
                                     float a_min, float a_max, int num_segments = 0);  // Ellipse
  ...
  // - May be used to alter render state (change sampler, blending, current shader). May be used to
  // emit custom rendering commands (difficult to do correctly, but possible).
  // - Use special ImDrawCallback_ResetRenderState callback to instruct backend to reset its render
  ...
  // - IMPORTANT: please be mindful of the different level of indirection between using size==0
  // (copying argument) and using size>0 (copying pointed data into a buffer).
  //   - If userdata_size == 0: we copy/store the 'userdata' argument as-is. It will be available
  //   unmodified in ImDrawCmd::UserCallbackData during render.
  //   - If userdata_size > 0,  we copy/store 'userdata_size' bytes pointed to by 'userdata'. We
  //   store them in a buffer stored inside the drawlist. ImDrawCmd::UserCallbackData will point
  //   inside that buffer so you have to retrieve data from there. Your callback may need to use
  //   ImDrawCmd::UserCallbackDataSize if you expect dynamically-sized data.
  //   - Support for userdata_size > 0 was added in v1.91.4, October 2024. So earlier code always
  //   only allowed to copy/store a simple void*.
  ...
  IMGUI_API ImDrawList* CloneOutput()
      const;  // Create a clone of the CmdBuffer/IdxBuffer/VtxBuffer. For multi-threaded rendering,
              // consider using `imgui_threaded_rendering` from
              // https://github.com/ocornut/imgui_club instead.
  ...
  IMGUI_API void PrimRectUV(const ImVec2& a, const ImVec2& b, const ImVec2& uv_a,
                            const ImVec2& uv_b, ImU32 col);
  IMGUI_API void PrimQuadUV(const ImVec2& a, const ImVec2& b, const ImVec2& c, const ImVec2& d,
...
#endif
  // inline  void  AddEllipse(const ImVec2& center, float radius_x, float radius_y, ImU32 col, float
  // rot = 0.0f, int num_segments = 0, float thickness = 1.0f) { AddEllipse(center, ImVec2(radius_x,
  // radius_y), col, rot, num_segments, thickness); } // OBSOLETED in 1.90.5 (Mar 2024) inline  void
  // AddEllipseFilled(const ImVec2& center, float radius_x, float radius_y, ImU32 col, float rot =
  // 0.0f, int num_segments = 0) { AddEllipseFilled(center, ImVec2(radius_x, radius_y), col, rot,
  // num_segments); }                        // OBSOLETED in 1.90.5 (Mar 2024) inline  void
  // PathEllipticalArcTo(const ImVec2& center, float radius_x, float radius_y, float rot, float
  // a_min, float a_max, int num_segments = 0) { PathEllipticalArcTo(center, ImVec2(radius_x,
  // radius_y), rot, a_min, a_max, num_segments); } // OBSOLETED in 1.90.5 (Mar 2024) inline  void
  // AddBezierCurve(const ImVec2& p1, const ImVec2& p2, const ImVec2& p3, const ImVec2& p4, ImU32
  // col, float thickness, int num_segments = 0) { AddBezierCubic(p1, p2, p3, p4, col, thickness,
  ...
  IMGUI_API void _PathArcToFastEx(const ImVec2& center, float radius, int a_min_sample,
                                  int a_max_sample, int a_step);
  IMGUI_API void _PathArcToN(const ImVec2& center, float radius, float a_min, float a_max,
                             int num_segments);
};
...
struct ImDrawData {
  ...
  int CmdListsCount;  // == CmdLists.Size. (OBSOLETE: exists for legacy reasons). Number of
                      // ImDrawList* to render.
  ...
  ImVec2 DisplayPos;  // Top-left position of the viewport to render (== top-left of the orthogonal
                      // projection matrix to use) (== GetMainViewport()->Pos for the main viewport,
                      // == (0.0) in most single-viewport applications)
  ImVec2 DisplaySize;  // Size of the viewport to render (== GetMainViewport()->Size for the main
                       // viewport, == io.DisplaySize in most single-viewport applications)
  ImVec2 FramebufferScale;  // Amount of pixels for each unit of DisplaySize. Copied from
                            // viewport->FramebufferScale (== io.DisplayFramebufferScale for main
                            // viewport). Generally (1,1) on normal display, (2,2) on OSX with
                            // Retina display.
  ImGuiViewport* OwnerViewport;  // Viewport carrying the ImDrawData instance, might be of use to
                                 // the renderer (generally not).
  ImVector<ImTextureData*>*
      Textures;  // List of textures to update. Most of the times the list is shared by all
                 // ImDrawData, has only 1 texture and it doesn't need any update. This almost
                 // always points to ImGui::GetPlatformIO().Textures[]. May be overriden or set to
                 // NULL if you want to manually update textures.

  // Functions
  ImDrawData() { Clear(); }
  IMGUI_API void Clear();
  IMGUI_API void AddDrawList(
      ImDrawList* draw_list);  // Helper to add an external draw list into an existing ImDrawData.
  IMGUI_API void
...
// [SECTION] Texture API (ImTextureFormat, ImTextureStatus, ImTextureRect, ImTextureData)
//-----------------------------------------------------------------------------
// In principle, the only data types that user/application code should care about are 'ImTextureRef'
// and 'ImTextureID'. They are defined above in this header file. Read their description to the
// difference between ImTextureRef and ImTextureID. FOR ALL OTHER ImTextureXXXX TYPES: ONLY CORE
// LIBRARY AND RENDERER BACKENDS NEED TO KNOW AND CARE ABOUT THEM.
//-----------------------------------------------------------------------------

#undef Status  // X11 headers are leaking this.

// We intentionally support a limited amount of texture formats to limit burden on CPU-side code and
// extension. Most standard backends only support RGBA32 but we provide a single channel option for
// low-resource/embedded systems.
enum ImTextureFormat {
  ImTextureFormat_RGBA32,  // 4 components per pixel, each is unsigned 8-bit. Total size = TexWidth
                           // * TexHeight * 4
  ImTextureFormat_Alpha8,  // 1 component per pixel, each is unsigned 8-bit. Total size = TexWidth *
                           // TexHeight
};
...
enum ImTextureStatus {
  ...
  ImTextureStatus_WantUpdates,  // Requesting backend to update specific blocks of pixels (write to
                                // texture portions which have never been used before). Set status
                                // OK when done.
  ImTextureStatus_WantDestroy,  // Requesting backend to destroy the texture. Set status to
                                // Destroyed when done.
};

// Coordinates of a rectangle within a texture.
// When a texture is in ImTextureStatus_WantUpdates state, we provide a list of individual
// rectangles to copy to the graphics system. You may use ImTextureData::Updates[] for the list, or
// ImTextureData::UpdateBox for a single bounding box.
...
// This is only useful for (1) core library and (2) backends. End-user/applications do not need to
// care about this. Renderer Backends will create a GPU-side version of this. Why does we store two
// identifiers: TexID and BackendUserData?
// - ImTextureID    TexID           = lower-level identifier stored in ImDrawCmd. ImDrawCmd can
// refer to textures not created by the backend, and for which there's no ImTextureData.
// - void*          BackendUserData = higher-level opaque storage for backend own book-keeping. Some
// backends may have enough with TexID and not need both. In columns below: who reads/writes each
// fields? 'r'=read, 'w'=write, 'core'=main library, 'backend'=renderer backend
struct ImTextureData {
  //------------------------------------------ core / backend
  //---------------------------------------
  int UniqueID;  // w    -   // [DEBUG] Sequential index to facilitate identifying a texture when
                 // debugging/printing. Unique per atlas.
  ImTextureStatus Status;  // rw   rw  // ImTextureStatus_OK/_WantCreate/_WantUpdates/_WantDestroy.
                           // Always use SetStatus() to modify!
  void* BackendUserData;   // -    rw  // Convenience storage for backend. Some backends may have
                           // enough with TexID.
  ImTextureID TexID;  // r    w   // Backend-specific texture identifier. Always use SetTexID() to
                      // modify! The identifier will stored in ImDrawCmd::GetTexID() and passed to
                      // backend's RenderDrawData function.
  ...
  unsigned char* Pixels;   // w    r   // Pointer to buffer holding 'Width*Height' pixels and
  ...
  int UnusedFrames;  // w    r   // In order to facilitate handling Status==WantDestroy in some
                     // backend: this is a count successive frames where the texture was not used.
                     // Always >0 when Status==WantDestroy.
  unsigned short
      RefCount;  // w    r   // Number of contexts using this texture. Used during backend shutdown.
  bool UseColors;  // w    r   // Tell whether our texture data is known to use colors (rather than
                   // just white + alpha).
  bool WantDestroyNextFrame;  // rw   -   // [Internal] Queued to set ImTextureStatus_WantDestroy
                              // next frame. May still be used in the current frame.
  ...
  // - Call SetTexID() and SetStatus() after honoring texture requests. Never modify TexID and
  // Status directly!
  // - A backend may decide to destroy a texture that we did not request to destroy, which is fine
  // (e.g. freeing resources), but we immediately set the texture back in _WantCreate mode.
...
struct ImFontConfig {
  ...
  bool MergeMode;  // false    // Merge into previous ImFont, so you can combine multiple inputs
                   // font into one ImFont (e.g. ASCII font + icons + Japanese glyphs). You may want
                   // to use GlyphOffset.y when merge font of different heights.
  bool PixelSnapH;   // false    // Align every glyph AdvanceX to pixel boundaries. Useful e.g. if
  ...
  ImS8 OversampleH;  // 0 (2)    // Rasterize at higher quality for sub-pixel positioning. 0 == auto
                     // == 1 or 2 depending on size. Note the difference between 2 and 3 is minimal.
                     // You can reduce this to 1 for large glyphs save memory. Read
                     // https://github.com/nothings/stb/blob/master/tests/oversample/README.md for
                     // details.
  ImS8 OversampleV;  // 0 (1)    // Rasterize at higher quality for sub-pixel positioning. 0 == auto
                     // == 1. This is not really useful as we don't use sub-pixel positions on the Y
                     // axis.
  ImWchar EllipsisChar;  // 0        // Explicitly specify Unicode codepoint of ellipsis character.
  ...
  const ImWchar*
      GlyphRanges;  // NULL     // *LEGACY* THE ARRAY DATA NEEDS TO PERSIST AS LONG AS THE FONT IS
                    // ALIVE. Pointer to a user-provided list of Unicode range (2 value per range,
                    // values are inclusive, zero-terminated list).
  const ImWchar*
      GlyphExcludeRanges;  // NULL     // Pointer to a small user-provided list of Unicode ranges (2
                           // value per range, values are inclusive, zero-terminated list). This is
                           // very close to GlyphRanges[] but designed to exclude ranges from a font
                           // source, when merging fonts with overlapping glyphs. Use "Input Glyphs
                           // Overlap Detection Tool" to find about your overlapping ranges.
  // ImVec2        GlyphExtraSpacing;      // 0, 0     // (REMOVED AT IT SEEMS LARGELY OBSOLETE.
  // PLEASE REPORT IF YOU WERE USING THIS). Extra spacing (in pixels) between glyphs when rendered:
  // essentially add to glyph->AdvanceX. Only X axis is supported for now.
  ImVec2 GlyphOffset;  // 0, 0     // Offset (in pixels) all glyphs from this font input. Absolute
                       // value for default size, other sizes will scale this value.
  float GlyphMinAdvanceX;  // 0        // Minimum AdvanceX for glyphs, set Min to align font icons,
                           ...
                           // default size, other sizes will scale this value.
  float GlyphMaxAdvanceX;  // FLT_MAX  // Maximum AdvanceX for glyphs
  float GlyphExtraAdvanceX;  // 0        // Extra spacing (in pixels) between glyphs. Please contact
                             // us if you are using this. // FIXME-NEWATLAS: Intentionally unscaled
  ImU32 FontNo;              // 0        // Index of font within TTF/OTF file
  unsigned int FontLoaderFlags;  // 0        // Settings for custom font builder. THIS IS BUILDER
                                 // IMPLEMENTATION DEPENDENT. Leave as zero if unsure.
  // unsigned int  FontBuilderFlags;       // --       // [Renamed in 1.92] Ue FontLoaderFlags.
  float RasterizerMultiply;  // 1.0f     // Linearly brighten (>1.0f) or darken (<1.0f) font output.
                             // Brightening small fonts may be a good workaround to make them more
                             // readable. This is a silly thing we may remove in the future.
  float RasterizerDensity;   // 1.0f     // [LEGACY: this only makes sense when
                             // ImGuiBackendFlags_RendererHasTextures is not supported] DPI scale
                             // multiplier for rasterization. Not altering other font metrics: makes
                             // it easy to swap between e.g. a 100% and a 400% fonts for a zooming
                            // display, or handle Retina screen. IMPORTANT: If you change this it is
                            // expected that you increase/decrease font scale roughly to the inverse
                            // of this, otherwise quality may look lowered.

  // [Internal]
  ImFontFlags Flags;  // Font flags (don't use just yet, will be exposed in upcoming 1.92.X updates)
  ImFont*
      DstFont;  // Target font (as we merging fonts, multiple ImFontConfig may target the same font)
  const ImFontLoader* FontLoader;  // Custom font backend for this source (default source is the one
                                   // stored in ImFontAtlas)
...
// (Note: some language parsers may fail to convert the bitfield members, in this case maybe drop
...
struct ImFontGlyph {
  ...
  float U0, V0, U1, V1;  // Texture coordinates for the current value of ImFontAtlas->TexRef. Cached
                         // equivalent of calling GetCustomRect() with PackId.
...
// Those values may not be cached/stored as they are only valid for the current value of
// atlas->TexRef (this is in theory derived from ImTextureRect but we use separate structures for
// reasons)
...
enum ImFontAtlasFlags_ {
  ...
  ImFontAtlasFlags_NoBakedLines =
      1
      << 2,  // Don't build thick line textures into the atlas (save a little texture memory, allow
             // support for point/nearest filtering). The AntiAliasedLinesUseTex features uses them,
             // otherwise they will be rendered using polygons (more expensive for CPU/GPU).
};
...
//  - If you don't call any AddFont*** functions, the default font embedded in the code will be
//  loaded for you.
// It is the rendering backend responsibility to upload texture into your graphics API:
//  - ImGui_ImplXXXX_RenderDrawData() functions generally iterate platform_io->Textures[] to
//  create/update/destroy each ImTextureData instance.
//  - Backend then set ImTextureData's TexID and BackendUserData.
//  - Texture id are passed back to you during rendering to identify the texture. Read FAQ entry
//  about ImTextureID/ImTextureRef for more details.
// Legacy path:
...
// - This is an old API and it is currently awkward for those and various other reasons! We will
...
struct ImFontAtlas {
  ...
  IMGUI_API ImFont* AddFontFromFileTTF(const char* filename, float size_pixels = 0.0f,
                                       const ImFontConfig* font_cfg = NULL,
                                       const ImWchar* glyph_ranges = NULL);
  IMGUI_API ImFont* AddFontFromMemoryTTF(
      void* font_data, int font_data_size, float size_pixels = 0.0f,
      const ImFontConfig* font_cfg = NULL,
  ...
  IMGUI_API ImFont* AddFontFromMemoryCompressedBase85TTF(
      const char* compressed_font_data_base85, float size_pixels = 0.0f,
      const ImFontConfig* font_cfg = NULL,
      const ImWchar* glyph_ranges =
  ...
  IMGUI_API void ClearInputData();  // [OBSOLETE] Clear input data (all ImFontConfig structures
                                    // including sizes, TTF data, glyph ranges, etc.) = all the data
                                    // used to build the texture and fonts.
  IMGUI_API void ClearFonts();  // [OBSOLETE] Clear input+output font data (same as ClearInputData()
                                // + glyphs storage, UV coordinates).
  IMGUI_API void ClearTexData();  // [OBSOLETE] Clear CPU-side copy of the texture data. Saves RAM
                                  // once the texture has been copied to graphics memory.
  ...
  // engine). Then store your texture handle with SetTexID().
  // - The pitch is always = Width * BytesPerPixels (1 or 4)
  // - Building in RGBA32 format is provided for convenience and compatibility, but note that unless
  // you manually manipulate or copy color data into
  //   the texture (e.g. when using the AddCustomRect*** api), then the RGB pixels emitted will
  //   always be white (~75% of memory/bandwidth waste).
  ...
  //   - In backend: replace calls to ImFontAtlas::SetTexID() with calls to
  //   ImTextureData::SetTexID() after honoring texture creation.
  ...
  }  // Bit ambiguous: used to detect when user didn't build texture but effectively we should check
     // TexID != 0 except that would be backend dependent..
#endif
  ...
  // zero-terminated list) NB: Make sure that your string are UTF-8 and NOT in your local code page.
  // Read https://github.com/ocornut/imgui/blob/master/docs/FONTS.md/#about-utf-8-encoding for
  // details. NB: Consider using ImFontGlyphRangesBuilder to build glyph ranges from textual data.
  ...
  // - Since 1.92.X, packing is done immediately in the function call (previously packing was done
  // during the Build call)
  // - You can render your pixels into the texture right after calling the AddCustomRect()
  // functions.
  // - VERY IMPORTANT:
  //   - Texture may be created/resized at any time when calling ImGui or ImFontAtlas functions.
  //   - IT WILL INVALIDATE RECTANGLE DATA SUCH AS UV COORDINATES. Always use latest values from
  //   GetCustomRect().
  //   - UV coordinates are associated to the current texture identifier aka 'atlas->TexRef'. Both
  //   TexRef and UV coordinates are typically changed at the same time.
  // - If you render colored output into your custom rectangles: set 'atlas->TexPixelsUseColors =
  // true' as this may help some backends decide of preferred texture format.
  // - Read docs/FONTS.md for more details about using colorful icons.
  // - Note: this API may be reworked further in order to facilitate supporting e.g. multi-monitor,
  ...
  IMGUI_API ImFontAtlasRectId
  AddCustomRect(int width, int height,
                ImFontAtlasRect* out_r =
                    NULL);  // Register a rectangle. Return -1 (ImFontAtlasRectId_Invalid) on error.
  IMGUI_API void RemoveCustomRect(
      ImFontAtlasRectId id);  // Unregister a rectangle. Existing pixels will stay in texture until
                              // resized / garbage collected.
  IMGUI_API bool GetCustomRect(ImFontAtlasRectId id, ImFontAtlasRect* out_r)
      const;  // Get rectangle coordinates for current texture. Valid immediately, never store this
              // (read above)!
  ...
  ImTextureFormat TexDesiredFormat;  // Desired texture format (default to ImTextureFormat_RGBA32
                                     // but may be changed to ImTextureFormat_Alpha8).
  int TexGlyphPadding;  // FIXME: Should be called "TexPackPadding". Padding between glyphs within
                        // texture in pixels. Defaults to 1. If your rendering method doesn't rely
                        // on bilinear filtering you may set this to 0 (will also need to set
  ...
  // Output
  // - Because textures are dynamically created/resized, the current texture identifier may changed
  // at *ANY TIME* during the frame.
  // - This should not affect you as you can always use the latest value. But note that any
  // precomputed UV coordinates are only valid for the current TexRef.
  ...
  union {
    ImTextureRef TexRef;
    ImTextureRef TexID;
  };  // Latest texture identifier == TexData->GetTexRef(). // RENAMED TexID to TexRef in 1.92.x
#endif
  ImTextureData* TexData;  // Latest texture.

  // [Internal]
  ImVector<ImTextureData*> TexList;  // Texture list (most often TexList.Size == 1). TexData is
                                     // always == TexList.back(). DO NOT USE DIRECTLY, USE
                                     // GetDrawData().Textures[]/GetPlatformIO().Textures[] instead!
  bool Locked;  // Marked as locked during ImGui::NewFrame()..EndFrame() scope if TexUpdates are not
                // supported. Any attempt to modify the atlas will assert.
  bool RendererHasTextures;  // Copy of (BackendFlags & ImGuiBackendFlags_RendererHasTextures) from
                             // supporting context.
  bool TexIsBuilt;  // Set when texture was built matching current font input. Mostly useful for
                    // legacy IsBuilt() call.
  bool TexPixelsUseColors;  // Tell whether our texture data is known to use colors (rather than
                            ...
                            // conversion process.
  ImVec2 TexUvScale;       // = (1.0f/TexData->TexWidth, 1.0f/TexData->TexHeight). May change as new
                           // texture gets created.
  ImVec2 TexUvWhitePixel;  // Texture coordinates to a white pixel. May change as new texture gets
                           // created.
  ImVector<ImFont*>
  ...
  ImFontAtlasBuilder* Builder;  // Opaque interface to our data that doesn't need to be public and
                                // may be discarded when rebuilding.
  const ImFontLoader* FontLoader;  // Font loader opaque interface (default to use FreeType when
                                   // IMGUI_ENABLE_FREETYPE is defined, otherwise default to use
                                   // stb_truetype). Use SetFontLoader() to change this at runtime.
  const char*
      FontLoaderName;    // Font loader name (for display e.g. in About box) == FontLoader->Name
  void* FontLoaderData;  // Font backend opaque storage
  unsigned int FontLoaderFlags;  // Shared flags (for all fonts) for font loader. THIS IS BUILD
                                 // IMPLEMENTATION DEPENDENT (e.g. Per-font override is also
                                 // available in ImFontConfig).
  int RefCount;                  // Number of contexts using this atlas
  ImGuiContext*
      OwnerContext;  // Context which own the atlas will be in charge of updating and destroying it.

  // [Obsolete]
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
  // Legacy: You can request your rectangles to be mapped as font glyph (given a font + Unicode
  // point), so you can render e.g. custom colorful icons and use them as regular glyphs. --> Prefer
  // using a custom ImFontLoader.
  ...
  inline void CalcCustomRectUV(const ImFontAtlasRect* r, ImVec2* out_uv_min,
  ...
  IMGUI_API ImFontAtlasRectId AddCustomRectFontGlyph(
      ...
      const ImVec2& offset =
          ImVec2(0, 0));  // OBSOLETED in 1.92.X: Use custom ImFontLoader in ImFontConfig
  IMGUI_API ImFontAtlasRectId AddCustomRectFontGlyphForSize(
      ImFont* font, float font_size, ImWchar codepoint, int w, int h, float advance_x,
      const ImVec2& offset = ImVec2(0, 0));  // ADDED AND OBSOLETED in 1.92.X
#endif
  // unsigned int                      FontBuilderFlags;        // OBSOLETED in 1.92.X: Renamed to
  // FontLoaderFlags. int                               TexDesiredWidth;         // OBSOLETED
  // in 1.92.X: Force texture width before calling Build(). Must be a power-of-two. If have many
  // glyphs your graphics API have texture size restrictions you may want to increase texture width
  // to decrease height. typedef ImFontAtlasRect           ImFontAtlasCustomRect;   // OBSOLETED
  // in 1.92.X typedef ImFontAtlasCustomRect     CustomRect;              // OBSOLETED in 1.72+
...
struct ImFontBaked {
  ...
  ImVector<float> IndexAdvanceX;  // 12-16 // out // Sparse. Glyphs->AdvanceX in a directly
                                  // indexable way (cache-friendly for CalcTextSize functions which
                                  // only this info, and are often bottleneck in large UI).
  float FallbackAdvanceX;         // 4     // out // FindGlyph(FallbackChar)->AdvanceX
  float Size;  // 4     // in  // Height of characters/line, set during loading (doesn't change
               // after loading)
  ...
  float Ascent, Descent;  // 4+4   // out // Ascent: distance from top to bottom of e.g. 'A'
                          // [0..FontSize] (unscaled)
  unsigned int MetricsTotalSurface : 26;  // 3  // out // Total surface in pixels to get an idea of
                                          // the font rasterization/texture cost (not exact, we
  ...
  unsigned int
      LoadNoRenderOnLayout : 1;  // 0  //     // Enable a two-steps mode where CalcTextSize() calls
                                 ...
                                 // be rendered, otherwise it is slower because we'd do one query on
                                 // the first CalcTextSize and one query on the first Draw.
  ...
  IMGUI_API ImFontGlyph* FindGlyph(
      ImWchar c);  // Return U+FFFD glyph if requested glyph doesn't exists.
...
// Font flags
// (in future versions as we redesign font loading API, this will become more important and better
// documented. for now please consider this as internal/advanced use)
enum ImFontFlags_ {
  ImFontFlags_None = 0,
  ImFontFlags_NoLoadError =
      1 << 1,  // Disable throwing an error/assert when calling AddFontXXX() with missing file/data.
               // Calling code is expected to check AddFontXXX() return value.
  ImFontFlags_NoLoadGlyphs = 1 << 2,  // [Internal] Disable loading new glyphs.
  ImFontFlags_LockBakedSizes =
      1 << 3,  // [Internal] Disable loading new baked sizes, disable garbage collecting current
               // ones. e.g. if you want to lock a font to a single size. Important: if you use this
               // to preload given sizes, consider the possibility of multiple font density used on
               // Retina display.
};

// Font runtime data and rendering
// - ImFontAtlas automatically loads a default embedded font for you if you didn't load one
// manually.
// - Since 1.92.X a font may be rendered as any size! Therefore a font doesn't have one specific
// size.
// - Use 'font->GetFontBaked(size)' to retrieve the ImFontBaked* corresponding to a given size.
// - If you used g.Font + g.FontSize (which is frequent from the ImGui layer), you can use
// g.FontBaked as a shortcut, as g.FontBaked == g.Font->GetFontBaked(g.FontSize).
struct ImFont {
  // [Internal] Members: Hot ~12-20 bytes
  ImFontBaked*
      LastBaked;  // 4-8   // Cache last bound baked. NEVER USE DIRECTLY. Use GetFontBaked().
  ImFontAtlas* ContainerAtlas;  // 4-8   // What we have been loaded into.
  ImFontFlags Flags;            // 4     // Font flags.
  float
      CurrentRasterizerDensity;  // Current rasterizer density. This is a varying state of the font.

  // [Internal] Members: Cold ~24-52 bytes
  // Conceptually Sources[] is the list of font sources merged to create this font.
  ImGuiID FontId;    // Unique identifier for the font
  float LegacySize;  // 4     // in  // Font size passed to AddFont(). Use for old code calling
                     // PushFont() expecting to use that size. (use ImGui::GetFontBaked() to get
                     // font baked at current bound size).
  ImVector<ImFontConfig*>
      Sources;  // 16    // in  // List of sources. Pointers within ContainerAtlas->Sources[]
  ImWchar EllipsisChar;  // 2-4   // out // Character used for ellipsis rendering ('...').
  ImWchar FallbackChar;  // 2-4   // out // Character used if a glyph isn't found (U+FFFD, '?')
  ImU8 Used8kPagesMap[(IM_UNICODE_CODEPOINT_MAX + 1) / 8192 /
                      8];  // 1 bytes if ImWchar=ImWchar16, 16 bytes if ImWchar==ImWchar32. Store
                           // 1-bit for each block of 4K codepoints that has one active glyph. This
                           // is mainly used to facilitate iterations across all used codepoints.
  bool EllipsisAutoBake;   // 1     //     // Mark when the "..." glyph needs to be generated.
  ImGuiStorage
      RemapPairs;  // 16    //     // Remapping pairs when using AddRemapChar(), otherwise empty.
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
  float Scale;  // 4     // in  // Legacy base font scale (~1.0f), multiplied by the per-window font
  ...
  IMGUI_API ImFontBaked* GetFontBaked(
      float font_size, float density = -1.0f);  // Get or create baked data for given size
  IMGUI_API ImVec2 CalcTextSizeA(float size, float max_width, float wrap_width,
                                 const char* text_begin, const char* text_end = NULL,
                                 const char** out_remaining = NULL);
  IMGUI_API const char* CalcWordWrapPosition(float size, const char* text, const char* text_end,
                                             float wrap_width);
  IMGUI_API void RenderChar(ImDrawList* draw_list, float size, const ImVec2& pos, ImU32 col,
                            ImWchar c, const ImVec4* cpu_fine_clip = NULL);
  IMGUI_API void RenderText(ImDrawList* draw_list, float size, const ImVec2& pos, ImU32 col,
                            const ImVec4& clip_rect, const char* text_begin, const char* text_end,
                            float wrap_width = 0.0f, ImDrawTextFlags flags = 0);
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
  inline const char* CalcWordWrapPositionA(float scale, const char* text, const char* text_end,
                                           float wrap_width) {
  ...
  IMGUI_API void AddRemapChar(
      ImWchar from_codepoint,
      ImWchar to_codepoint);  // Makes 'from_codepoint' character points to 'to_codepoint' glyph.
...
// Using an indirection to avoid patching ImDrawCmd after a SetTexID() call (but this could be an
// alternative solution too)
inline ImTextureID ImDrawCmd::GetTexID() const {
  // If you are getting this assert: A renderer backend with support for
  // ImGuiBackendFlags_RendererHasTextures (1.92) must iterate and handle ImTextureData requests
  // stored in ImDrawData::Textures[].
  ImTextureID tex_id =
      TexRef._TexData ? TexRef._TexData->TexID : TexRef._TexID;  // == TexRef.GetTexID() above.
  if (TexRef._TexData != NULL)
    IM_ASSERT(
        tex_id != ImTextureID_Invalid &&
        "ImDrawCmd is referring to ImTextureData that wasn't uploaded to graphics system. Backend "
        "must call ImTextureData::SetTexID() after handling ImTextureStatus_WantCreate request!");
  return tex_id;
}
...
enum ImGuiViewportFlags_ {
  ...
  ImGuiViewportFlags_OwnedByApp = 1 << 2,  // Platform Window: Is created/managed by the application
                                           // (rather than a dear imgui backend)
};
...
struct ImGuiViewport {
  ...
  ImVec2 Pos;   // Main Area: Position of the viewport (Dear ImGui coordinates are the same as OS
                // desktop/native coordinates)
  ImVec2 Size;  // Main Area: Size of the viewport.
  ImVec2
      FramebufferScale;  // Density of the viewport for Retina display (always 1,1 on Windows, may
                         // be 2,2 etc on macOS/iOS). This will affect font rasterizer density.
  ImVec2 WorkPos;   // Work Area: Position of the viewport minus task bars, menus bars, status bars
                    // (>= Pos)
  ImVec2 WorkSize;  // Work Area: Size of the viewport minus task bars, menu bars, status bars (<=
                    // Size)

  // Platform/Backend Dependent Data
  void* PlatformHandle;     // void* to hold higher-level, platform window handle (e.g. HWND,
                            // GLFWWindow*, SDL_Window*)
  void* PlatformHandleRaw;  // void* to hold lower-level, platform-native window handle (under Win32
                            // this is expected to be a HWND, unused for other platforms)
...
struct ImGuiPlatformIO {
  ...
  // (default to use native Win32 clipboard on Windows, otherwise uses a private clipboard. Override
  // to access OS clipboard on other architectures)
  ...
  // Windows)
  void (*Platform_SetImeDataFn)(ImGuiContext* ctx, ImGuiViewport* viewport,
                                ImGuiPlatformImeData* data);
  void* Platform_ImeUserData;
  // void      (*SetPlatformImeDataFn)(ImGuiViewport* viewport, ImGuiPlatformImeData* data); //
  // [Renamed to platform_io.PlatformSetImeDataFn in 1.91.1]

  // Optional: Platform locale
  // [Experimental] Configure decimal point e.g. '.' or ',' useful for some languages (e.g. German),
  ...
  // Optional: Maximum texture size supported by renderer (used to adjust how we size textures). 0
  // if not known.
  int Renderer_TextureMaxWidth;
  int Renderer_TextureMaxHeight;

  // Written by some backends during ImGui_ImplXXXX_RenderDrawData() call to point backend_specific
  // ImGui_ImplXXXX_RenderState* structure.
  ...
  // The ImGui_ImplXXXX_RenderDrawData() function of each backend generally access this via
  // ImDrawData::Textures which points to this. The array is available here mostly because backends
  // will want to destroy textures on shutdown.
  ImVector<ImTextureData*>
      Textures;  // List of textures used by Dear ImGui (most often 1) + contents of external
  ...
  void ClearPlatformHandlers();  // Clear all Platform_XXX fields. Typically called on Platform
                                 // Backend shutdown.
  void ClearRendererHandlers();  // Clear all Renderer_XXX fields. Typically called on Renderer
                                 // Backend shutdown.
};

// (Optional) Support for IME (Input Method Editor) via the platform_io.Platform_SetImeDataFn()
...
// Please keep your copy of dear imgui up to date! Occasionally set '#define
...
IMGUI_API void SetWindowFontScale(
    float scale);  // Set font scale factor for current window. Prefer using PushFont(NULL,
                   // style.FontSizeBase * factor) or use style.FontScaleMain to scale all windows.
// OBSOLETED in 1.91.9 (from February 2025)
IMGUI_API void Image(
    ImTextureRef tex_ref, const ImVec2& image_size, const ImVec2& uv0, const ImVec2& uv1,
    const ImVec4& tint_col,
    const ImVec4& border_col);  // <-- 'border_col' was removed in favor of ImGuiCol_ImageBorder. If
                                // you use 'tint_col', use ImageWithBg() instead.
...
IMGUI_API ImVec2
GetContentRegionMax();  // Content boundaries max (e.g. window boundaries including scrolling, or
                        // current column boundaries). You should never need this. Always use
                        // GetCursorScreenPos() and GetContentRegionAvail()!
IMGUI_API ImVec2
GetWindowContentRegionMin();  // Content boundaries min for the window (roughly (0,0)-Scroll), in
                              // window-local coordinates. You should never need this. Always use
                              // GetCursorScreenPos() and GetContentRegionAvail()!
IMGUI_API ImVec2
GetWindowContentRegionMax();  // Content boundaries max for the window (roughly (0,0)+Size-Scroll),
                              // in window-local coordinates. You should never need this. Always use
                              // GetCursorScreenPos() and GetContentRegionAvail()!
// OBSOLETED in 1.90.0 (from September 2023)
inline bool BeginChildFrame(ImGuiID id, const ImVec2& size, ImGuiWindowFlags window_flags = 0) {
  return BeginChild(id, size, ImGuiChildFlags_FrameStyle, window_flags);
}
inline void EndChildFrame() { EndChild(); }
// inline bool       BeginChild(const char* str_id, const ImVec2& size_arg, bool borders,
// ImGuiWindowFlags window_flags){ return BeginChild(str_id, size_arg, borders ?
// ImGuiChildFlags_Borders : ImGuiChildFlags_None, window_flags); } // Unnecessary as true ==
// ImGuiChildFlags_Borders inline bool       BeginChild(ImGuiID id, const ImVec2& size_arg, bool
// borders, ImGuiWindowFlags window_flags)        { return BeginChild(id, size_arg, borders ?
// ImGuiChildFlags_Borders : ImGuiChildFlags_None, window_flags);     } // Unnecessary as true ==
// ImGuiChildFlags_Borders
...
// Some of the older obsolete names along with their replacement (commented out so they are not
// reported in IDE)
//-- OBSOLETED in 1.89.4 (from March 2023)
// static inline void  PushAllowKeyboardFocus(bool tab_stop)                                       {
// PushItemFlag(ImGuiItemFlags_NoTabStop, !tab_stop); } static inline void  PopAllowKeyboardFocus()
// { PopItemFlag(); }
//-- OBSOLETED in 1.89 (from August 2022)
// IMGUI_API bool      ImageButton(ImTextureID user_texture_id, const ImVec2& size, const ImVec2&
// uv0 = ImVec2(0, 0), const ImVec2& uv1 = ImVec2(1, 1), int frame_padding = -1, const ImVec4&
// bg_col = ImVec4(0, 0, 0, 0), const ImVec4& tint_col = ImVec4(1, 1, 1, 1)); // --> Use new
// ImageButton() signature (explicit item id, regular FramePadding). Refer to code in 1.91 if you
// want to grab a copy of this version.
//-- OBSOLETED in 1.88 (from May 2022)
// static inline void  CaptureKeyboardFromApp(bool want_capture_keyboard = true)                   {
// SetNextFrameWantCaptureKeyboard(want_capture_keyboard); } // Renamed as name was misleading +
// removed default value. static inline void  CaptureMouseFromApp(bool want_capture_mouse = true) {
// SetNextFrameWantCaptureMouse(want_capture_mouse); }       // Renamed as name was misleading +
// removed default value.
//-- OBSOLETED in 1.87 (from February 2022, more formally obsoleted April 2024)
// IMGUI_API ImGuiKey  GetKeyIndex(ImGuiKey key);                                                  {
// IM_ASSERT(key >= ImGuiKey_NamedKey_BEGIN && key < ImGuiKey_NamedKey_END); const ImGuiKeyData*
// key_data = GetKeyData(key); return (ImGuiKey)(key_data - g.IO.KeysData); } // Map ImGuiKey_*
// values into legacy native key index. == io.KeyMap[key]. When using a 1.87+ backend using
// io.AddKeyEvent(), calling GetKeyIndex() with ANY ImGuiKey_XXXX values will return the same value!
// static inline ImGuiKey GetKeyIndex(ImGuiKey key)                                                {
// IM_ASSERT(key >= ImGuiKey_NamedKey_BEGIN && key < ImGuiKey_NamedKey_END); return key; }
//-- OBSOLETED in 1.86 (from November 2021)
// IMGUI_API void      CalcListClipping(int items_count, float items_height, int*
// out_items_display_start, int* out_items_display_end); // Code removed, see 1.90 for last version
// of the code. Calculate range of visible items for large list of evenly sized items. Prefer using
// ImGuiListClipper.
...
// return BeginListBox(label, size); } static inline bool  ListBoxHeader(const char* label, int
// items_count, int height_in_items = -1) { float height = GetTextLineHeightWithSpacing() *
// ((height_in_items < 0 ? ImMin(items_count, 7) : height_in_items) + 0.25f) +
// GetStyle().FramePadding.y * 2.0f; return BeginListBox(label, ImVec2(0.0f, height)); } // Helper
// to calculate size from items_count and height_in_items static inline void  ListBoxFooter() {
// EndListBox(); }
//-- OBSOLETED in 1.79 (from August 2020)
// static inline void  OpenPopupContextItem(const char* str_id = NULL, ImGuiMouseButton mb = 1)    {
// OpenPopupOnItemClick(str_id, mb); } // Bool return value removed. Use IsWindowAppearing() in
// BeginPopup() instead. Renamed in 1.77, renamed back in 1.79. Sorry!
//-- OBSOLETED in 1.78 (from June 2020): Old drag/sliders functions that took a 'float power > 1.0f'
//argument instead of ImGuiSliderFlags_Logarithmic. See github.com/ocornut/imgui/issues/3361 for
//details. IMGUI_API bool      DragScalar(const char* label, ImGuiDataType data_type, void* p_data,
// float v_speed, const void* p_min, const void* p_max, const char* format, float power = 1.0f) //
// OBSOLETED in 1.78 (from June 2020) IMGUI_API bool      DragScalarN(const char* label,
// ImGuiDataType data_type, void* p_data, int components, float v_speed, const void* p_min, const
// void* p_max, const char* format, float power = 1.0f);                                          //
// OBSOLETED in 1.78 (from June 2020) IMGUI_API bool      SliderScalar(const char* label,
// ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format,
// float power = 1.0f);                                                                        //
// OBSOLETED in 1.78 (from June 2020) IMGUI_API bool      SliderScalarN(const char* label,
// ImGuiDataType data_type, void* p_data, int components, const void* p_min, const void* p_max,
// const char* format, float power = 1.0f);                                                       //
// OBSOLETED in 1.78 (from June 2020) static inline bool  DragFloat(const char* label, float* v,
// float v_speed, float v_min, float v_max, const char* format, float power = 1.0f)    { return
// DragScalar(label, ImGuiDataType_Float, v, v_speed, &v_min, &v_max, format, power); }     //
// OBSOLETED in 1.78 (from June 2020) static inline bool  DragFloat2(const char* label, float v[2],
// float v_speed, float v_min, float v_max, const char* format, float power = 1.0f) { return
// DragScalarN(label, ImGuiDataType_Float, v, 2, v_speed, &v_min, &v_max, format, power); } //
// OBSOLETED in 1.78 (from June 2020) static inline bool  DragFloat3(const char* label, float v[3],
// float v_speed, float v_min, float v_max, const char* format, float power = 1.0f) { return
// DragScalarN(label, ImGuiDataType_Float, v, 3, v_speed, &v_min, &v_max, format, power); } //
// OBSOLETED in 1.78 (from June 2020) static inline bool  DragFloat4(const char* label, float v[4],
// float v_speed, float v_min, float v_max, const char* format, float power = 1.0f) { return
// DragScalarN(label, ImGuiDataType_Float, v, 4, v_speed, &v_min, &v_max, format, power); } //
// OBSOLETED in 1.78 (from June 2020) static inline bool  SliderFloat(const char* label, float* v,
// float v_min, float v_max, const char* format, float power = 1.0f)                 { return
// SliderScalar(label, ImGuiDataType_Float, v, &v_min, &v_max, format, power); }            //
// OBSOLETED in 1.78 (from June 2020) static inline bool  SliderFloat2(const char* label, float
// v[2], float v_min, float v_max, const char* format, float power = 1.0f)              { return
// SliderScalarN(label, ImGuiDataType_Float, v, 2, &v_min, &v_max, format, power); }        //
// OBSOLETED in 1.78 (from June 2020) static inline bool  SliderFloat3(const char* label, float
// v[3], float v_min, float v_max, const char* format, float power = 1.0f)              { return
// SliderScalarN(label, ImGuiDataType_Float, v, 3, &v_min, &v_max, format, power); }        //
// OBSOLETED in 1.78 (from June 2020) static inline bool  SliderFloat4(const char* label, float
// v[4], float v_min, float v_max, const char* format, float power = 1.0f)              { return
// SliderScalarN(label, ImGuiDataType_Float, v, 4, &v_min, &v_max, format, power); }        //
// OBSOLETED in 1.78 (from June 2020)
//-- OBSOLETED in 1.77 and before
// static inline bool  BeginPopupContextWindow(const char* str_id, ImGuiMouseButton mb, bool
// over_items) { return BeginPopupContextWindow(str_id, mb | (over_items ? 0 :
// ImGuiPopupFlags_NoOpenOverItems)); } // OBSOLETED in 1.77 (from June 2020) static inline void
// TreeAdvanceToLabelPos()               { SetCursorPosX(GetCursorPosX() +
// GetTreeNodeToLabelSpacing()); }   // OBSOLETED in 1.72 (from July 2019) static inline void
// SetNextTreeNodeOpen(bool open, ImGuiCond cond = 0) { SetNextItemOpen(open, cond); } // OBSOLETED
// in 1.71 (from June 2019) static inline float GetContentRegionAvailWidth()          { return
// GetContentRegionAvail().x; }                               // OBSOLETED in 1.70 (from May 2019)
// static inline ImDrawList* GetOverlayDrawList()            { return GetForegroundDrawList(); } //
// OBSOLETED in 1.69 (from Mar 2019) static inline void  SetScrollHere(float ratio = 0.5f)     {
// SetScrollHereY(ratio); }                                          // OBSOLETED in 1.66 (from Nov
// 2018) static inline bool  IsItemDeactivatedAfterChange()        { return
// IsItemDeactivatedAfterEdit(); }                            // OBSOLETED in 1.63 (from Aug 2018)
//-- OBSOLETED in 1.60 and before
// static inline bool  IsAnyWindowFocused()                  { return
// IsWindowFocused(ImGuiFocusedFlags_AnyWindow); }            // OBSOLETED in 1.60 (from Apr 2018)
// static inline bool  IsAnyWindowHovered()                  { return
// IsWindowHovered(ImGuiHoveredFlags_AnyWindow); }            // OBSOLETED in 1.60 (between Dec 2017
// and Apr 2018) static inline void  ShowTestWindow()                      { return
// ShowDemoWindow(); }                                        // OBSOLETED in 1.53 (between Oct 2017
// and Dec 2017) static inline bool  IsRootWindowFocused()                 { return
// IsWindowFocused(ImGuiFocusedFlags_RootWindow); }           // OBSOLETED in 1.53 (between Oct 2017
// and Dec 2017) static inline bool  IsRootWindowOrAnyChildFocused()       { return
// IsWindowFocused(ImGuiFocusedFlags_RootAndChildWindows); }  // OBSOLETED in 1.53 (between Oct 2017
// and Dec 2017) static inline void  SetNextWindowContentWidth(float w)    {
// SetNextWindowContentSize(ImVec2(w, 0.0f)); }                      // OBSOLETED in 1.53 (between
// Oct 2017 and Dec 2017) static inline float GetItemsLineHeightWithSpacing()       { return
// GetFrameHeightWithSpacing(); }                             // OBSOLETED in 1.53 (between Oct 2017
// and Dec 2017) IMGUI_API bool      Begin(char* name, bool* p_open, ImVec2 size_first_use, float
// bg_alpha = -1.0f, ImGuiWindowFlags flags=0); // OBSOLETED in 1.52 (between Aug 2017 and Oct
// 2017): Equivalent of using SetNextWindowSize(size, ImGuiCond_FirstUseEver) and
// SetNextWindowBgAlpha(). static inline bool  IsRootWindowOrAnyChildHovered()       { return
// IsWindowHovered(ImGuiHoveredFlags_RootAndChildWindows); }  // OBSOLETED in 1.52 (between Aug 2017
// and Oct 2017) static inline void  AlignFirstTextHeightToWidgets()       {
// AlignTextToFramePadding(); }                                      // OBSOLETED in 1.52 (between
// Aug 2017 and Oct 2017) static inline void  SetNextWindowPosCenter(ImGuiCond c=0) {
// SetNextWindowPos(GetMainViewport()->GetCenter(), c, ImVec2(0.5f,0.5f)); } // OBSOLETED in 1.52
// (between Aug 2017 and Oct 2017) static inline bool  IsItemHoveredRect()                   {
// return IsItemHovered(ImGuiHoveredFlags_RectOnly); }               // OBSOLETED in 1.51 (between
// Jun 2017 and Aug 2017) static inline bool  IsPosHoveringAnyWindow(const ImVec2&) { IM_ASSERT(0);
// return false; }                                     // OBSOLETED in 1.51 (between Jun 2017 and
// Aug 2017): This was misleading and partly broken. You probably want to use the
// io.WantCaptureMouse flag instead. static inline bool  IsMouseHoveringAnyWindow()            {
// return IsWindowHovered(ImGuiHoveredFlags_AnyWindow); }            // OBSOLETED in 1.51 (between
// Jun 2017 and Aug 2017) static inline bool  IsMouseHoveringWindow()               { return
// IsWindowHovered(ImGuiHoveredFlags_AllowWhenBlockedByPopup |
// ImGuiHoveredFlags_AllowWhenBlockedByActiveItem); }       // OBSOLETED in 1.51 (between Jun 2017
// and Aug 2017)
//-- OBSOLETED in 1.50 and before
// static inline bool  CollapsingHeader(char* label, const char* str_id, bool framed = true, bool
// default_open = false) { return CollapsingHeader(label, (default_open ? (1 << 5) : 0)); } //
// OBSOLETED in 1.49 static inline ImFont*GetWindowFont()                      { return GetFont(); }
// // OBSOLETED in 1.48 static inline float GetWindowFontSize()                   { return
// GetFontSize(); }                                           // OBSOLETED in 1.48 static inline
// void  SetScrollPosHere()                    { SetScrollHere(); } // OBSOLETED in 1.42
}  // namespace ImGui
...
// 'font->Glyphs.back()->Colored' after calling AddCustomRectFontGlyph() We could make ImTextureRect
// an union to use old names, but 1) this would be confusing 2) the fix is easy 3)
// ImFontAtlasCustomRect was always a rather esoteric api.
...
/*struct ImFontAtlasCustomRect
{
    ...
    unsigned int    GlyphID:31;     // Input    // [Internal] For custom font glyphs only (ID <
0x110000) unsigned int    GlyphColored:1; // Input    // [Internal] For custom font glyphs only:
glyph is colored, removed tinting. float           GlyphAdvanceX;  // Input    // [Internal] For
custom font glyphs only: glyph xadvance ImVec2          GlyphOffset;    // Input    // [Internal]
For custom font glyphs only: glyph display offset ImFont*         Font;           // Input    //
[Internal] For custom font glyphs only: target font ImFontAtlasCustomRect()         { X = Y =
0xFFFF; Width = Height = 0; GlyphID = 0; GlyphColored = 0; GlyphAdvanceX = 0.0f; GlyphOffset =
...
//PathRect() typedef ImDrawFlags ImDrawCornerFlags; enum ImDrawCornerFlags_
//{
//    ImDrawCornerFlags_None      = ImDrawFlags_RoundCornersNone,         // Was == 0 prior to 1.82,
//    this is now == ImDrawFlags_RoundCornersNone which is != 0 and not implicit
//    ImDrawCornerFlags_TopLeft   = ImDrawFlags_RoundCornersTopLeft,      // Was == 0x01 (1 << 0)
//    prior to 1.82. Order matches ImDrawFlags_NoRoundCorner* flag (we exploit this internally).
//    ImDrawCornerFlags_TopRight  = ImDrawFlags_RoundCornersTopRight,     // Was == 0x02 (1 << 1)
//    prior to 1.82. ImDrawCornerFlags_BotLeft   = ImDrawFlags_RoundCornersBottomLeft,   // Was ==
//    0x04 (1 << 2) prior to 1.82. ImDrawCornerFlags_BotRight  =
//    ImDrawFlags_RoundCornersBottomRight,  // Was == 0x08 (1 << 3) prior to 1.82.
//    ImDrawCornerFlags_All       = ImDrawFlags_RoundCornersAll,          // Was == 0x0F prior
//    to 1.82 ImDrawCornerFlags_Top       = ImDrawCornerFlags_TopLeft | ImDrawCornerFlags_TopRight,
//    ImDrawCornerFlags_Bot       = ImDrawCornerFlags_BotLeft | ImDrawCornerFlags_BotRight,
//    ImDrawCornerFlags_Left      = ImDrawCornerFlags_TopLeft | ImDrawCornerFlags_BotLeft,
//    ImDrawCornerFlags_Right     = ImDrawCornerFlags_TopRight | ImDrawCornerFlags_BotRight,
//};

// RENAMED and MERGED both ImGuiKey_ModXXX and ImGuiModFlags_XXX into ImGuiMod_XXX (from September
// 2022) RENAMED ImGuiKeyModFlags -> ImGuiModFlags in 1.88 (from April 2022). Exceptionally
// commented out ahead of obscolescence schedule to reduce confusion and because they were not meant
// to be used in the first place.
// typedef ImGuiKeyChord ImGuiModFlags;      // == int. We generally use ImGuiKeyChord to mean "a
// ImGuiKey or-ed with any number of ImGuiMod_XXX value", so you may store mods in there. enum
// ImGuiModFlags_ { ImGuiModFlags_None = 0, ImGuiModFlags_Ctrl = ImGuiMod_Ctrl, ImGuiModFlags_Shift
// = ImGuiMod_Shift, ImGuiModFlags_Alt = ImGuiMod_Alt, ImGuiModFlags_Super = ImGuiMod_Super };
// typedef ImGuiKeyChord ImGuiKeyModFlags; // == int
// enum ImGuiKeyModFlags_ { ImGuiKeyModFlags_None = 0, ImGuiKeyModFlags_Ctrl = ImGuiMod_Ctrl,
// ImGuiKeyModFlags_Shift = ImGuiMod_Shift, ImGuiKeyModFlags_Alt = ImGuiMod_Alt,
...
// May be convenient for some users to only explicitly include vanilla imgui.h and have extra stuff
// included.

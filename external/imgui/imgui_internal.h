...
// You may use this file to debug, understand or extend Dear ImGui features but we don't provide any
...
#endif
#endif
// Emscripten has partial SSE 4.2 support where _mm_crc32_u32 is not available. See
// https://emscripten.org/docs/porting/simd.html#id11 and #8213
...
#pragma clang diagnostic ignored "-Wdeprecated-enum-enum-conversion"  // warning: bitwise operation
                                                                      // between different
                                                                      // enumeration types
                                                                      // ('XXXFlags_' and
                                                                      // 'XXXFlagsPrivate_') is
...
#pragma GCC diagnostic ignored "-Wdeprecated-enum-enum-conversion"  // warning: bitwise operation
                                                                    // between different enumeration
                                                                    // types ('XXXFlags_' and
                                                                    // 'XXXFlagsPrivate_') is
                                                                    // deprecated
#endif

// In 1.89.4, we moved the implementation of "courtesy maths operators" from imgui_internal.h in
// imgui.h As they are frequently requested, we do not want to encourage to many people using
// imgui_internal.h
...
// You can compile with both by defining both IMGUI_ENABLE_FREETYPE and IMGUI_ENABLE_STB_TRUETYPE
// together.
...
// Utilities
// (other types which are not forwarded declared are: ImBitArray<>, ImSpan<>, ImSpanAllocator<>,
// ImStableVector<>, ImPool<>, ImChunkStream<>)
...
// ImGui
struct ImGuiBoxSelectState;  // Box-selection state (currently used by multi-selection, could
                             // potentially be used by others)
struct ImGuiColorMod;        // Stacked color modifier, backup of modified data so we can restore it
struct ImGuiContext;         // Main Dear ImGui context
struct ImGuiContextHook;     // Hook for extensions like ImGuiTestEngine
struct ImGuiDataTypeInfo;    // Type information associated to a ImGuiDataType enum
struct ImGuiDeactivatedItemData;  // Data for IsItemDeactivated()/IsItemDeactivatedAfterEdit()
                                  // function.
struct ImGuiErrorRecoveryState;   // Storage of stack sizes for error handling and recovery
struct ImGuiGroupData;            // Stacked storage data for BeginGroup()/EndGroup()
struct ImGuiInputTextState;       // Internal state of the currently focused/edited text input box
struct ImGuiInputTextDeactivateData;  // Short term storage to backup text of a deactivating
                                      // InputText() while another is stealing active id
...
struct ImGuiStyleVarInfo;  // Style variable information (e.g. to access style variables from an
                           // enum)
...
struct ImGuiTableTempData;  // Temporary storage for one table (one per table in the stack), shared
                            // between tables.
...
struct ImGuiWindowTempData;  // Temporary storage for one window (that's the data which in theory we
                             // could ditch at the end of the frame, in practice we currently keep
                             // it for each window)
struct ImGuiWindowSettings;  // Storage for a window .ini settings (we keep one of those even if the
                             // actual window wasn't instanced during this session)

// Enumerations
// Use your programming IDE "Go to definition" facility on the names of the center columns to find
// the actual flags/enum lists.
enum ImGuiLocKey : int;       // -> enum ImGuiLocKey              // Enum: a localization entry for
                              // translation.
typedef int ImGuiLayoutType;  // -> enum ImGuiLayoutType_         // Enum: Horizontal or vertical

// Flags
typedef int ImDrawTextFlags;     // -> enum ImDrawTextFlags_         // Flags: for
                                 // ImTextCalcWordWrapPositionEx()
typedef int ImGuiActivateFlags;  // -> enum ImGuiActivateFlags_      // Flags: for navigation/focus
                                 // function (will be for ActivateItem() later)
typedef int ImGuiDebugLogFlags;  // -> enum ImGuiDebugLogFlags_      // Flags: for
                                 // ShowDebugLogWindow(), g.DebugLogFlags
typedef int ImGuiFocusRequestFlags;  // -> enum ImGuiFocusRequestFlags_  // Flags: for FocusWindow()
typedef int ImGuiItemStatusFlags;    // -> enum ImGuiItemStatusFlags_    // Flags: for
                                     // g.LastItemData.StatusFlags
typedef int ImGuiOldColumnFlags;  // -> enum ImGuiOldColumnFlags_     // Flags: for BeginColumns()
typedef int ImGuiLogFlags;        // -> enum ImGuiLogFlags_           // Flags: for LogBegin() text
                                  // capturing function
typedef int
    ImGuiNavRenderCursorFlags;  // -> enum ImGuiNavRenderCursorFlags_//Flags: for RenderNavCursor()
typedef int
...
typedef int ImGuiScrollFlags;  // -> enum ImGuiScrollFlags_        // Flags: for ScrollToItem() and
                               // navigation requests
typedef int ImGuiSeparatorFlags;  // -> enum ImGuiSeparatorFlags_     // Flags: for SeparatorEx()
typedef int ImGuiTextFlags;       // -> enum ImGuiTextFlags_          // Flags: for TextEx()
typedef int ImGuiTooltipFlags;    // -> enum ImGuiTooltipFlags_       // Flags: for BeginTooltipEx()
typedef int ImGuiTypingSelectFlags;   // -> enum ImGuiTypingSelectFlags_  // Flags: for
                                      // GetTypingSelectRequest()
typedef int ImGuiWindowRefreshFlags;  // -> enum ImGuiWindowRefreshFlags_ // Flags: for
...
// (since IMGUI_VERSION_NUM >= 18729: IMGUI_DEBUG_LOG was reworked into IMGUI_DEBUG_PRINTF (and
// removed framecount from it). If you were using a #define IMGUI_DEBUG_LOG please rename)
...
// Debug Logging for ShowDebugLogWindow(). This is designed for relatively rare events so please
// don't spam.
...
#define IMGUI_DEBUG_LOG_ACTIVEID(...)                                                     \
  do {                                                                                    \
    if (g.DebugLogFlags & ImGuiDebugLogFlags_EventActiveId) IMGUI_DEBUG_LOG(__VA_ARGS__); \
  } while (0)
#define IMGUI_DEBUG_LOG_FOCUS(...)                                                     \
  do {                                                                                 \
    if (g.DebugLogFlags & ImGuiDebugLogFlags_EventFocus) IMGUI_DEBUG_LOG(__VA_ARGS__); \
  } while (0)
#define IMGUI_DEBUG_LOG_POPUP(...)                                                     \
  do {                                                                                 \
    if (g.DebugLogFlags & ImGuiDebugLogFlags_EventPopup) IMGUI_DEBUG_LOG(__VA_ARGS__); \
  } while (0)
#define IMGUI_DEBUG_LOG_NAV(...)                                                     \
  do {                                                                               \
    if (g.DebugLogFlags & ImGuiDebugLogFlags_EventNav) IMGUI_DEBUG_LOG(__VA_ARGS__); \
  } while (0)
#define IMGUI_DEBUG_LOG_SELECTION(...)                                                     \
  do {                                                                                     \
    if (g.DebugLogFlags & ImGuiDebugLogFlags_EventSelection) IMGUI_DEBUG_LOG(__VA_ARGS__); \
  } while (0)
#define IMGUI_DEBUG_LOG_CLIPPER(...)                                                     \
  do {                                                                                   \
    if (g.DebugLogFlags & ImGuiDebugLogFlags_EventClipper) IMGUI_DEBUG_LOG(__VA_ARGS__); \
  } while (0)
#define IMGUI_DEBUG_LOG_IO(...)                                                     \
  do {                                                                              \
    if (g.DebugLogFlags & ImGuiDebugLogFlags_EventIO) IMGUI_DEBUG_LOG(__VA_ARGS__); \
  } while (0)
#define IMGUI_DEBUG_LOG_FONT(...)                                                             \
  do {                                                                                        \
    ImGuiContext* g2 = GImGui;                                                                \
    if (g2 && g2->DebugLogFlags & ImGuiDebugLogFlags_EventFont) IMGUI_DEBUG_LOG(__VA_ARGS__); \
  } while (0)  // Called from ImFontAtlas function which may operate without a context.
#define IMGUI_DEBUG_LOG_INPUTROUTING(...)                                                     \
  do {                                                                                        \
    if (g.DebugLogFlags & ImGuiDebugLogFlags_EventInputRouting) IMGUI_DEBUG_LOG(__VA_ARGS__); \
  } while (0)
...
#define IM_NEWLINE "\n"
#endif
#ifndef IM_TABSIZE  // Until we move this to runtime and/or add proper tab support, at least allow
                    // users to compile-time override
#define IM_TABSIZE (4)
#endif
#define IM_MEMALIGN(_OFF, _ALIGN) \
  (((_OFF) + ((_ALIGN) - 1)) &    \
   ~((_ALIGN) -                   \
     1))  // Memory align e.g. IM_ALIGN(0,4)=0, IM_ALIGN(1,4)=4, IM_ALIGN(4,4)=4, IM_ALIGN(5,4)=8
...
#define IM_STRINGIFY(_X) \
  IM_STRINGIFY_HELPER(_X)  // Preprocessor idiom to stringify e.g. an integer.
...
// This will call IM_DEBUG_BREAK() which you may redefine yourself. See
// https://github.com/scottt/debugbreak for more reference.
...
#define IM_DEBUG_BREAK() \
  IM_ASSERT(0)  // It is expected that you define IM_DEBUG_BREAK() into something that will break
                // nicely in a debugger!
#endif
...
inline unsigned int ImCountSetBits(unsigned int v) {
...
IMGUI_API const char* ImStrbol(const char* buf_mid_line,
...
inline bool ImCharIsXdigitA(char c) {
...
IMGUI_API int ImFormatStringV(char* buf, size_t buf_size, const char* fmt, va_list args)
    IM_FMTLIST(3);
IMGUI_API void ImFormatStringToTempBuffer(const char** out_buf, const char** out_buf_end,
                                          const char* fmt, ...) IM_FMTARGS(3);
IMGUI_API void ImFormatStringToTempBufferV(const char** out_buf, const char** out_buf_end,
                                           const char* fmt, va_list args) IM_FMTLIST(3);
IMGUI_API const char* ImParseFormatFindStart(const char* format);
IMGUI_API const char* ImParseFormatFindEnd(const char* format);
IMGUI_API const char* ImParseFormatTrimDecorations(const char* format, char* buf, size_t buf_size);
IMGUI_API void ImParseFormatSanitizeForPrinting(const char* fmt_in, char* fmt_out,
                                                size_t fmt_out_size);
IMGUI_API const char* ImParseFormatSanitizeForScanning(const char* fmt_in, char* fmt_out,
                                                       size_t fmt_out_size);
...
IMGUI_API int ImTextStrFromUtf8(
    ImWchar* out_buf, int out_buf_size, const char* in_text, const char* in_text_end,
...
IMGUI_API const char* ImTextFindPreviousUtf8Codepoint(
    const char* in_text_start, const char* in_text_curr);  // return previous UTF-8 code-point.
IMGUI_API int ImTextCountLines(
    const char* in_text,
    const char* in_text_end);  // return number of lines taken by text. trailing carriage return
                               // doesn't count as an extra line.

// Helpers: High-level text functions (DO NOT USE!!! THIS IS A MINIMAL SUBSET OF LARGER UPCOMING
// CHANGES)
enum ImDrawTextFlags_ {
  ImDrawTextFlags_None = 0,
  ImDrawTextFlags_CpuFineClip =
      1 << 0,  // Must be == 1/true for legacy with 'bool cpu_fine_clip' arg to RenderText()
  ImDrawTextFlags_WrapKeepBlanks = 1 << 1,
  ImDrawTextFlags_StopOnNewLine = 1 << 2,
};
IMGUI_API ImVec2 ImFontCalcTextSizeEx(ImFont* font, float size, float max_width, float wrap_width,
...
IMGUI_API const char* ImFontCalcWordWrapPositionEx(ImFont* font, float size, const char* text,
...
#else
#define IMGUI_DISABLE_TTY_FUNCTIONS  // Can't use stdout, fflush if we are not using default file
                                     // functions
#endif
...
#endif
// - ImMin/ImMax/ImClamp/ImLerp/ImSwap are used by widgets which support variety of types:
...
// Helpers: Geometry
IMGUI_API ImVec2 ImBezierCubicCalc(const ImVec2& p1, const ImVec2& p2, const ImVec2& p3,
                                   const ImVec2& p4, float t);
IMGUI_API ImVec2 ImBezierCubicClosestPoint(
...
IMGUI_API ImVec2 ImBezierQuadraticCalc(const ImVec2& p1, const ImVec2& p2, const ImVec2& p3,
                                       float t);
IMGUI_API ImVec2 ImLineClosestPoint(const ImVec2& a, const ImVec2& b, const ImVec2& p);
IMGUI_API bool ImTriangleContainsPoint(const ImVec2& a, const ImVec2& b, const ImVec2& c,
                                       const ImVec2& p);
IMGUI_API ImVec2 ImTriangleClosestPoint(const ImVec2& a, const ImVec2& b, const ImVec2& c,
                                        const ImVec2& p);
IMGUI_API void ImTriangleBarycentricCoords(const ImVec2& a, const ImVec2& b, const ImVec2& c,
                                           const ImVec2& p, float& out_u, float& out_v,
                                           float& out_w);
...
#define IM_BITARRAY_CLEARBIT(_ARRAY, _N) \
  ((_ARRAY[(_N) >> 5] &=                 \
    ~((ImU32)1 << ((_N) & 31))))  // Macro version of ImBitArrayClearBit(): ensure args have
...
// Helper: ImStableVector<>
// Allocating chunks of BLOCK_SIZE items. Objects pointers are never invalidated when growing, only
// by clear(). Important: does not destruct anything! Implemented only the minimum set of functions
// we need for it.
...
struct ImStableVector {
  ...
  inline void reserve(int new_cap) {
    ...
    for (int n = old_count; n < new_count; n++) Blocks[n] = (T*)IM_ALLOC(sizeof(T) * BLOCK_SIZE);
    Capacity = new_cap;
  }
  inline T& operator[](int i) {
...
struct ImPool {
  ...
  // To iterate a ImPool: for (int n = 0; n < pool.GetMapSize(); n++) if (T* t =
  // pool.TryGetMapData(n)) { ... } Can be avoided if you know .Remove() has never been called on
  // the pool, or AliveCount == GetMapSize()
  ...
  }  // It is the map we need iterate to find valid items, since we don't have "alive" storage
     // anywhere
...
// Helper: ImGuiTextIndex
// Maintain a line index for a text buffer. This is a strong candidate to be moved into the public
// API.
struct ImGuiTextIndex {
  ImVector<int> Offsets;
  int EndOffset = 0;  // Because we don't own text buffer we need to maintain EndOffset (may bake in
                      // LineOffsets?)
...
// Helper: ImGuiStorage
IMGUI_API ImGuiStoragePair* ImLowerBound(ImGuiStoragePair* in_begin, ImGuiStoragePair* in_end,
                                         ImGuiID key);
...
// ImDrawList: Helper function to calculate a circle's segment count given its radius and a "maximum
// error" value. Estimation of number of circle segment based on error is derived using method
// described in https://stackoverflow.com/a/2244088/15194693 Number of segments (N) is calculated
// using equation:
...
// You may want to create your own instance of you try to ImDrawList completely without ImGui. In
...
struct IMGUI_API ImDrawListSharedData {
  ...
  ImGuiContext*
      Context;  // [OPTIONAL] Link to Dear ImGui context. 99% of ImDrawList/ImFontAtlas can function
                // without an ImGui context, but this facilitate handling one legacy edge case.
...
struct ImDrawDataBuilder {
  ImVector<ImDrawList*>* Layers[2];  // Pointers to global layers for: regular, tooltip. LayersP[0]
                                     // is owned by DrawData.
...
// - input: PushItemFlag() manipulates g.CurrentItemFlags, g.NextItemData.ItemFlags, ItemAdd() calls
// may add extra flags too.
// - output: stored in g.LastItemData.ItemFlags
enum ImGuiItemFlagsPrivate_ {
  // Controlled by user
  ImGuiItemFlags_Disabled =
      1 << 10,  // false     // Disable interactions (DOES NOT affect visuals. DO NOT mix direct use
                // of this with BeginDisabled(). See BeginDisabled()/EndDisabled() for full disable
                // feature, and github #211).
  ImGuiItemFlags_ReadOnly = 1 << 11,  // false     // [ALPHA] Allow hovering interactions but
                                      // underlying value is not changed.
  ImGuiItemFlags_MixedValue =
      1 << 12,  // false     // [BETA] Represent a mixed/indeterminate value, generally
  ...
  ImGuiItemFlags_AllowOverlap = 1 << 14,  // false     // Allow being overlapped by another widget.
                                          // Not-hovered to Hovered transition deferred by a frame.
  ImGuiItemFlags_NoNavDisableMouseHover =
      1 << 15,  // false     // Nav keyboard/gamepad mode doesn't disable hover highlight (behave as
                // if NavHighlightItemUnderNav==false).
  ImGuiItemFlags_NoMarkEdited = 1 << 16,  // false     // Skip calling MarkItemEdited()
  ImGuiItemFlags_NoFocus =
      1 << 17,  // false     // [EXPERIMENTAL: Not very well specced] Clicking doesn't take focus.
                // Automatically sets ImGuiButtonFlags_NoFocus + ImGuiButtonFlags_NoNavFocus in
                // ButtonBehavior().

  // Controlled by widget code
  ImGuiItemFlags_Inputable =
      1 << 20,  // false     // [WIP] Auto-activate input mode when tab focused. Currently only used
                // and supported by a few items before it becomes a generic feature.
  ...
  ImGuiItemFlags_Default_ =
      ImGuiItemFlags_AutoClosePopups,  // Please don't change, use PushItemFlag() instead.

  // Obsolete
  // ImGuiItemFlags_SelectableDontClosePopup = !ImGuiItemFlags_AutoClosePopups, // Can't have a
  // redirect as we inverted the behavior
};
...
enum ImGuiItemStatusFlags_ {
  ...
  ImGuiItemStatusFlags_ToggledSelection =
      1
      << 3,  // Set when Selectable(), TreeNode() reports toggling a selection. We can't report
             // "Selected", only state changes, in order to easily handle clipping with less issues.
  ImGuiItemStatusFlags_ToggledOpen =
      1 << 4,  // Set when TreeNode() reports toggling their open state.
  ImGuiItemStatusFlags_HasDeactivated = 1 << 5,  // Set if the widget/group is able to provide data
  ...
  ImGuiItemStatusFlags_Visible =
      1
      << 8,  // [WIP] Set when item is overlapping the current clipping rectangle (Used internally.
             // Please don't use yet: API/system will change as we refactor Itemadd()).
  ImGuiItemStatusFlags_HasClipRect = 1 << 9,  // g.LastItemData.ClipRect is valid.
  ImGuiItemStatusFlags_HasShortcut =
      1 << 10,  // g.LastItemData.Shortcut valid. Set by SetNextItemShortcut() -> ItemAdd().
// ImGuiItemStatusFlags_FocusedByTabbing = 1 << 8,   // Removed IN 1.90.1 (Dec 2023). The trigger is
// part of g.NavActivateId. See commit 54c1bdeceb.
...
// Additional status + semantic for ImGuiTestEngine
  ...
  ImGuiItemStatusFlags_Inputable =
...
enum ImGuiHoveredFlagsPrivate_ {
  ...
  ImGuiHoveredFlags_AllowedMaskForIsWindowHovered =
      ...
      ImGuiHoveredFlags_AllowWhenBlockedByActiveItem | ImGuiHoveredFlags_ForTooltip |
      ImGuiHoveredFlags_Stationary,
  ImGuiHoveredFlags_AllowedMaskForIsItemHovered =
      ImGuiHoveredFlags_AllowWhenBlockedByPopup | ImGuiHoveredFlags_AllowWhenBlockedByActiveItem |
      ImGuiHoveredFlags_AllowWhenOverlapped | ImGuiHoveredFlags_AllowWhenDisabled |
      ImGuiHoveredFlags_NoNavOverride | ImGuiHoveredFlags_ForTooltip |
...
enum ImGuiInputTextFlagsPrivate_ {
  ...
  ImGuiInputTextFlags_MergedItem = 1
                                   << 27,  // For internal use by TempInputText(), will skip calling
                                           // ItemAdd(). Require bounding-box to strictly match.
  ImGuiInputTextFlags_LocalizeDecimalPoint =
      1 << 28,  // For internal use by InputScalar() and TempInputScalar()
};
...
enum ImGuiButtonFlagsPrivate_ {
  ...
  // ImGuiButtonFlags_Repeat               = 1 << 10,  // hold to repeat -> use
  // ImGuiItemFlags_ButtonRepeat instead.
  ImGuiButtonFlags_FlattenChildren =
      1 << 11,  // allow interactions even if a child window is overlapping
  ImGuiButtonFlags_AllowOverlap =
      1
      << 12,  // require previous frame HoveredId to either match id or be null before being usable.
  // ImGuiButtonFlags_DontClosePopups      = 1 << 13,  // disable automatically closing parent popup
  // on press ImGuiButtonFlags_Disabled             = 1 << 14,  // disable interactions -> use
  // BeginDisabled() or ImGuiItemFlags_Disabled
  ImGuiButtonFlags_AlignTextBaseLine =
      1 << 15,  // vertically align button to match text baseline - ButtonEx() only // FIXME: Should
                // be removed and handled by SmallButton(), not possible currently because of
                // DC.CursorPosPrevLine
  ImGuiButtonFlags_NoKeyModsAllowed = 1
  ...
  ImGuiButtonFlags_NoNavFocus =
      1 << 18,  // don't override navigation focus when activated (FIXME: this is essentially used
                // every time an item uses ImGuiItemFlags_NoNav, but because legacy specs don't
                // requires LastItemData to be set ButtonBehavior(), we can't poll
  ...
  ImGuiButtonFlags_NoSetKeyOwner =
      1 << 20,  // don't set key/input owner on the initial click (note: mouse buttons are keys!
                // often, the key in question will be ImGuiKey_MouseLeft!)
  ImGuiButtonFlags_NoTestKeyOwner =
      1 << 21,  // don't test key/input owner when polling the key (note: mouse buttons are keys!
                // often, the key in question will be ImGuiKey_MouseLeft!)
...
enum ImGuiSelectableFlagsPrivate_ {
  ...
  ImGuiSelectableFlags_SpanAvailWidth =
      1 << 24,  // Span all avail width even if we declared less for layout purpose. FIXME: We may
  ...
  ImGuiSelectableFlags_NoSetKeyOwner =
      1 << 27,  // Don't set key/input owner on the initial click (note: mouse buttons are keys!
                // often, the key in question will be ImGuiKey_MouseLeft!)
};
...
enum ImGuiTreeNodeFlagsPrivate_ {
  ImGuiTreeNodeFlags_NoNavFocus =
      1 << 27,  // Don't claim nav focus when interacting with this item (#8551)
  ...
  ImGuiTreeNodeFlags_DrawLinesMask_ = ImGuiTreeNodeFlags_DrawLinesNone |
...
// Flags for FocusWindow(). This is not called ImGuiFocusFlags to avoid confusion with public-facing
// ImGuiFocusedFlags.
// FIXME: Once we finishing replacing more uses of
// GetTopMostPopupModal()+IsWindowWithinBeginStackOf() and FindBlockingModal() with this, we may
// want to change the flag to be opt-out instead of opt-in.
...
enum ImGuiLogFlags_ {
  ...
  ImGuiLogFlags_OutputMask_ = ImGuiLogFlags_OutputTTY | ImGuiLogFlags_OutputFile |
                              ImGuiLogFlags_OutputBuffer | ImGuiLogFlags_OutputClipboard,
};
...
struct IMGUI_API ImGuiInputTextState {
  ...
  ImGuiInputTextFlags Flags;  // copy of InputText() flags. may be used to check if e.g.
                              // ImGuiInputTextFlags_Password is set.
  ImGuiID ID;                 // widget id owning the text state
  int TextLen;                // UTF-8 length of the string in TextA (in bytes)
  const char* TextSrc;        // == TextA.Data unless read-only, in which case == buf passed to
                        // InputText(). Field only set and valid _inside_ the call InputText() call.
  ImVector<char> TextA;  // main UTF8 buffer. TextA.Size is a buffer size! Should always be >=
                         // buf_size passed by user (and of course >= CurLenA + 1).
  ImVector<char> TextToRevertTo;  // value to revert to when pressing Escape = backup of end-user
                                  // buffer at the time of focus (in UTF-8, unaltered)
  ...
  ImVec2 Scroll;     // horizontal offset (managed manually) + vertical scrolling (pulled from child
                     // window's own Scroll.y)
  ...
  bool CursorFollow;   // set when we want scrolling to follow the current cursor position (not
                       // always!)
  bool CursorCenterY;  // set when we want scrolling to be centered over the cursor position (while
                       // resizing a word-wrapping field)
  bool SelectedAllMouseLock;  // after a double-click to select all, we ignore further mouse drags
                              // to update selection
  bool Edited;                // edited this frame
  bool WantReloadUserBuf;     // force a reload of user buf so it may be modified externally. may be
                              // automatic in future version.
  ImS8 LastMoveDirectionLR;   // ImGuiDir_Left or ImGuiDir_Right. track last movement direction so
                              // when cursor cross over a word-wrapping boundaries we can display it
                              // on either line depending on last move.s
  ...
  // If you modify underlying user-passed const char* while active you need to call this (InputText
  // V2 may lift this)
  //   strcpy(my_buf, "hello");
  //   if (ImGuiInputTextState* state = ImGui::GetInputTextState(id)) // id may be
  //   ImGui::GetItemID() is last item
...
enum ImGuiWindowRefreshFlags_ {
  ...
  ImGuiWindowRefreshFlags_TryToAvoidRefresh =
      1 << 0,  // [EXPERIMENTAL] Try to keep existing contents, USER MUST NOT HONOR BEGIN()
               // RETURNING FALSE AND NOT APPEND.
...
struct ImGuiNextWindowData {
  ...
  ImVec2 MenuBarOffsetMinVal;  // (Always on) This is not exposed publicly, so we don't clear it and
...
struct ImGuiNextItemData {
  ...
  // Members below are NOT cleared by ItemAdd() meaning they are still valid during e.g.
  // NavProcessItem(). Always rely on HasFlags.
  ImGuiID FocusScopeId;  // Set by SetNextItemSelectionUserData()
  ImGuiSelectionUserData
      SelectionUserData;   // Set by SetNextItemSelectionUserData() (note that NULL/0 is a valid
                           // value, we use -1 == ImGuiSelectionUserData_Invalid to mark invalid
                           // values)
...
struct ImGuiLastItemData {
  ...
  // Rarely used fields are not explicitly cleared, only valid when the corresponding
  // ImGuiItemStatusFlags are set.
  ImRect DisplayRect;  // Display rectangle. ONLY VALID IF (StatusFlags &
                       // ImGuiItemStatusFlags_HasDisplayRect) is set.
  ImRect ClipRect;  // Clip rectangle at the time of submitting item. ONLY VALID IF (StatusFlags &
                    // ImGuiItemStatusFlags_HasClipRect) is set..
  ImGuiKeyChord Shortcut;  // Shortcut at the time of submitting item. ONLY VALID IF (StatusFlags &
                           // ImGuiItemStatusFlags_HasShortcut) is set..
...
struct ImGuiPopupData {
  ...
  ImGuiWindow*
      RestoreNavWindow;  // Set on OpenPopup(), a NavWindow that will be restored on popup close
  int ParentNavLayer;    // Resolved on BeginPopup(). Actually a ImGuiNavLayer type (declared down
                         // below), initialized to -1 which is not part of an enum, but serves
                         // well-enough as "not any of layers" value
...
#define ImGuiKey_NavGamepadActivate \
  (g.IO.ConfigNavSwapGamepadButtons ? ImGuiKey_GamepadFaceRight : ImGuiKey_GamepadFaceDown)
#define ImGuiKey_NavGamepadCancel \
  (g.IO.ConfigNavSwapGamepadButtons ? ImGuiKey_GamepadFaceDown : ImGuiKey_GamepadFaceRight)
...
enum ImGuiInputSource : int {
  ...
  ImGuiInputSource_Mouse,  // Note: may be Mouse or TouchScreen or Pen. See io.MouseSource to
                           // distinguish them.
...
// FIXME: Structures in the union below need to be declared as anonymous unions appears to be an
// extension? Using ImVec2() would fail on Clang 'union member 'MousePos' has a non-trivial default
// constructor'
...
struct ImGuiInputEvent {
  ...
  ImU32 EventId;  // Unique, sequential increasing integer to identify an event (if you need to
                  // correlate them to other data).
  union {
...
// Input function taking an 'ImGuiID owner_id' argument defaults to (ImGuiKeyOwner_Any == 0) aka
// don't test ownership, which matches legacy behavior.
...
// #define ImGuiKeyOwner_None ImGuiKeyOwner_NoOwner  // We previously called this
// 'ImGuiKeyOwner_None' but it was inconsistent with our pattern that _None values == 0 and quite
// dangerous. Also using _NoOwner makes the IsKeyPressed() calls more explicit.
...
struct ImGuiKeyRoutingData {
  ...
  ImU16 Mods;  // Technically we'd only need 4-bits but for simplify we store ImGuiMod_ values which
               // need 16-bits.
...
// Routing table: maintain a desired owner for each possible key-chord (key + mods), and setup owner
// in NewFrame() when mods are matching. Stored in main context (1 instance)
struct ImGuiKeyRoutingTable {
  ...
  ImVector<ImGuiKeyRoutingData>
      EntriesNext;  // Double-buffer to avoid reallocation (could use a shared buffer)

  ImGuiKeyRoutingTable() { Clear(); }
  void Clear() {
    for (int n = 0; n < IM_ARRAYSIZE(Index); n++) Index[n] = -1;
    Entries.clear();
    EntriesNext.clear();
  }
};
...
struct ImGuiKeyOwnerData {
  ...
  bool LockUntilRelease;  // Reading this key requires explicit owner id (until key is released).
                          // Set by ImGuiInputFlags_LockUntilRelease. When this is true
...
// Flags for extended versions of IsKeyPressed(), IsMouseClicked(), Shortcut(), SetKeyOwner(),
// SetItemKeyOwner() Don't mistake with ImGuiInputTextFlags! (which is for ImGui::InputText()
// function)
enum ImGuiInputFlagsPrivate_ {
  ...
  ImGuiInputFlags_RepeatRateNavTweak =
      1 << 3,  // Repeat rate: Faster
               ...
               // - In theory ImGuiInputFlags_RepeatUntilOtherKeyPress may be a desirable default,
               // but it would break too many behavior so everything is opt-in.
  ImGuiInputFlags_RepeatUntilRelease =
      1 << 4,  // Stop repeating when released (default for all functions except Shortcut). This
               // only exists to allow overriding Shortcut() default behavior.
  ...
  ImGuiInputFlags_RepeatUntilKeyModsChangeFromNone =
      1 << 6,  // Stop repeating when released OR if keyboard mods are leaving the None state.
               // Allows going from Mod+Key to Key by releasing Mod.
  ImGuiInputFlags_RepeatUntilOtherKeyPress = 1
                                             << 7,  // Stop repeating when released OR if any other
                                                    // keyboard key is pressed during the repeat

  // Flags for SetKeyOwner(), SetItemKeyOwner()
  // - Locking key away from non-input aware code. Locking is useful to make input-owner-aware code
  // steal keys from non-input-owner-aware code. If all code is input-owner-aware locking would
  // never be necessary.
  ImGuiInputFlags_LockThisFrame =
      1 << 20,  // Further accesses to key data will require EXPLICIT owner ID (ImGuiKeyOwner_Any/0
                // will NOT accepted for polling). Cleared at end of frame.
  ImGuiInputFlags_LockUntilRelease =
      1 << 21,  // Further accesses to key data will require EXPLICIT owner ID (ImGuiKeyOwner_Any/0
                // will NOT accepted for polling). Cleared when the key is released or at end of
                // each frame if key is released.
  ...
  ImGuiInputFlags_SupportedByShortcut = ImGuiInputFlags_RepeatMask_ |
                                        ImGuiInputFlags_RouteTypeMask_ |
                                        ImGuiInputFlags_RouteOptionsMask_,
  ImGuiInputFlags_SupportedBySetNextItemShortcut =
      ImGuiInputFlags_RepeatMask_ | ImGuiInputFlags_RouteTypeMask_ |
...
enum ImGuiActivateFlags_ {
  ...
  ImGuiActivateFlags_PreferInput = 1 << 0,  // Favor activation that requires keyboard text input
                                            // (e.g. for Slider/Drag). Default for Enter key.
  ImGuiActivateFlags_PreferTweak =
      1 << 1,  // Favor activation for tweaking with arrows or gamepad (e.g. for Slider/Drag).
               // Default for Space key and if keyboard is not used.
  ImGuiActivateFlags_TryToPreserveState = 1
                                          << 2,  // Request widget to preserve state if it can (e.g.
                                                 // InputText will try to preserve cursor/selection)
  ImGuiActivateFlags_FromTabbing =
      1 << 3,  // Activation requested by a tabbing request (ImGuiNavMoveFlags_IsTabbing)
  ...
  ImGuiActivateFlags_FromFocusApi =
      1 << 5,  // Activation requested by an api request (ImGuiNavMoveFlags_FocusApi)
};
...
enum ImGuiScrollFlags_ {
  ...
  ImGuiScrollFlags_KeepVisibleEdgeX =
      1 << 0,  // If item is not visible: scroll as little as possible on X axis to bring item back
               // into view [default for X axis]
  ImGuiScrollFlags_KeepVisibleEdgeY =
      1 << 1,  // If item is not visible: scroll as little as possible on Y axis to bring item back
               // into view [default for Y axis for windows that are already visible]
  ...
  ImGuiScrollFlags_NoScrollParent =
      1 << 6,  // Disable forwarding scrolling to parent window if required to keep item/rect
               // visible (only scroll window the function was applied to).
...
enum ImGuiNavRenderCursorFlags_ {
  ...
  ImGuiNavRenderCursorFlags_Compact =
      1 << 1,  // Compact highlight, no padding/distance from focused item
  ImGuiNavRenderCursorFlags_AlwaysDraw =
      1 << 2,  // Draw rectangular highlight if (g.NavId == id) even when g.NavCursorVisible ==
               // false, aka even when using the mouse.
...
// ImGuiNavHighlightFlags_TypeThin       = ImGuiNavRenderCursorFlags_Compact,    // Renamed
// in 1.90.2
#endif
};
...
enum ImGuiNavMoveFlags_ {
  ...
  ImGuiNavMoveFlags_WrapMask_ = ImGuiNavMoveFlags_LoopX | ImGuiNavMoveFlags_LoopY |
  ...
  ImGuiNavMoveFlags_AlsoScoreVisibleSet =
      1 << 5,  // Store alternate result in NavMoveResultLocalVisible that only comprise elements
               // that are already fully visible (used by PageUp/PageDown)
  ImGuiNavMoveFlags_ScrollToEdgeY =
      1 << 6,  // Force scrolling to min/max (used by Home/End) // FIXME-NAV: Aim to remove or
               // reword, probably unnecessary
  ImGuiNavMoveFlags_Forwarded = 1 << 7,
  ImGuiNavMoveFlags_DebugNoResult = 1 << 8,  // Dummy scoring for debug purpose, don't apply result
  ImGuiNavMoveFlags_FocusApi =
      1 << 9,  // Requests from focus API can land/focus/activate items even if they are marked with
               // _NoTabStop (see NavProcessItemForTabbingRequest() for details)
  ...
  ImGuiNavMoveFlags_NoClearActiveId =
      1 << 15,  // (Experimental) Do not clear active id when applying move result
};
...
struct ImGuiNavItemData {
  ImGuiWindow* Window;       // Init,Move    // Best candidate window
  ...
  ImGuiSelectionUserData
      SelectionUserData;  // I+Mov    // Best candidate SetNextItemSelectionUserData() value. Valid
                          // if (ItemFlags & ImGuiItemFlags_HasSelectionUserData)
...
enum ImGuiTypingSelectFlags_ {
  ...
  ImGuiTypingSelectFlags_AllowBackspace =
      1 << 0,  // Backspace to delete character inputs. If using: ensure GetTypingSelectRequest() is
               // not called more than once per frame (filter by e.g. focus state)
  ImGuiTypingSelectFlags_AllowSingleCharMode =
      1 << 1,  // Allow "single char" search mode which is activated when pressing the same
...
struct IMGUI_API ImGuiTypingSelectRequest {
  ...
  int SearchBufferLen;
  const char* SearchBuffer;  // Search buffer contents (use full string. unless SingleCharMode is
                             // set, in which case use SingleCharSize).
  bool SelectRequest;        // Set when buffer was modified this frame, requesting a selection.
  bool SingleCharMode;  // Notify when buffer contains same character repeated, to implement special
                        // mode. In this situation it preferred to not display any on-screen search
                        // indication.
  ImS8 SingleCharSize;  // Length in bytes of first letter codepoint (1 for ascii, 2-4 for UTF-8).
                        // If (SearchBufferLen==RepeatCharSize) only 1 letter has been input.
};
...
struct IMGUI_API ImGuiTypingSelectState {
  ...
  char
      SearchBuffer[64];  // Search buffer: no need to make dynamic as this search is very transient.
  ImGuiID FocusScope;
  int LastRequestFrame = 0;
  float LastRequestTime = 0.0f;
  bool SingleCharModeLock = false;  // After a certain single char repeat count we lock into
...
enum ImGuiOldColumnFlags_ {
  ...
  ImGuiOldColumnFlags_GrowParentContentsSize =
      1 << 4,  // Restore pre-1.51 behavior of extending the parent window contents size but
...
struct ImGuiBoxSelectState {
  ...
  bool UnclipMode;           // (Temp/Transient, here in hot area). Set/cleared by the
                             // BeginMultiSelect()/EndMultiSelect() owning active box-select.
  ImRect UnclipRect;         // Rectangle where ItemAdd() clipping may be temporarily disabled. Need
                             // support by multi-select supporting widgets.
  ImRect BoxSelectRectPrev;  // Selection rectangle in absolute coordinates (derived every frame
                             // from BoxSelectStartPosRel and MousePos)
...
struct IMGUI_API ImGuiMultiSelectTempData {
  ImGuiMultiSelectIO
      IO;  // MUST BE FIRST FIELD. Requests are set and returned by
           // BeginMultiSelect()/EndMultiSelect() + written to by user during the loop.
  ...
  bool IsFocused;  // Set if currently focusing the selection scope (any item of the selection). May
                   // be used if you have custom shortcut associated to selection.
  bool IsKeyboardSetRange;  // Set by BeginMultiSelect() when using Shift+Navigation. Because
                            // scrolling may be affected we can't afford a frame of lag with
                            // Shift+Navigation.
  bool NavIdPassedBy;
...
struct IMGUI_API ImGuiMultiSelectState {
  ...
  int LastSelectionSize;  // Set by BeginMultiSelect() based on optional info provided by user. May
                          // be -1 if unknown.
  ImS8 RangeSelected;     // -1 (don't have) or true/false
  ImS8 NavIdSelected;     // -1 (don't have) or true/false
  ImGuiSelectionUserData RangeSrcItem;  //
  ImGuiSelectionUserData
      NavIdItem;  // SetNextItemSelectionUserData() value for NavId (if part of submitted items)
...
struct ImGuiViewportP : public ImGuiViewport {
  int BgFgDrawListsLastFrame[2];  // Last frame number the background (0) and foreground (1) draw
                                  // lists were used
  ImDrawList* BgFgDrawLists[2];  // Convenience background (0) and foreground (1) draw lists. We use
                                 // them to draw software mouser cursor when io.MouseDrawCursor is
                                 // set and to draw most debug overlays.
  ...
  ImVec2 WorkInsetMin;  // Work Area inset locked for the frame. GetWorkRect() always fits within
                        // GetMainRect().
  ImVec2 WorkInsetMax;  // "
  ImVec2 BuildWorkInsetMin;  // Work Area inset accumulator for current frame, to become next
                             // frame's WorkInset
  ...
  // Helpers to retrieve ImRect (we don't need to store BuildWorkRect as every access tend to change
  // it, hence the code asymmetry)
...
// Because we never destroy or rename ImGuiWindowSettings, we can store the names in a separate
// buffer easily. (this is designed to be stored in a ImChunkStream buffer, with the variable-length
// Name following our structure)
...
// This is experimental and not officially supported, it'll probably fall short of features, if/when
// it does we may backtrack.
...
// - The message will always be a string literal, in order to increase likelihood of being display
// by an assert handler.
// - See 'Demo->Configuration->Error Handling' and ImGuiIO definitions for details on error
// handling.
...
// The error callback is currently not public, as it is expected that only advanced users will rely
// on it.
typedef void (*ImGuiErrorCallback)(ImGuiContext* ctx, void* user_data,
                                   const char* msg);  // Function signature for g.ErrorCallback
...
enum ImGuiDebugLogFlags_ {
  ...
  ImGuiDebugLogFlags_EventMask_ =
      ImGuiDebugLogFlags_EventError | ImGuiDebugLogFlags_EventActiveId |
...
struct ImGuiIDStackTool {
  ...
  bool QueryHookActive;  // Used to disambiguate the case where DebugHookIdInfoId == 0 which is
                         // valid.
...
struct ImGuiContext {
  ...
  ImGuiStyle Style;
  ImVector<ImFontAtlas*> FontAtlases;  // List of font atlases used by the context (generally only
                                       // contains g.IO.Fonts aka the main font atlas)
  ImFont* Font;                        // Currently bound font. (== FontStack.back().Font)
  ImFontBaked*
      FontBaked;  // Currently bound font at currently bound size. (== Font->GetFontBaked(FontSize))
  float FontSize;  // Currently bound font size == line height (== FontSizeBase + externals scales
                   // applied in the UpdateCurrentFontSize() function).
  float FontSizeBase;    // Font size before scaling == style.FontSizeBase == value passed to
                         // PushFont() when specified.
  float FontBakedScale;  // == FontBaked->Size / FontSize. Scale factor over baked size. Rarely used
                         // nowadays, very often == 1.0f.
  ...
  char
      ContextName[16];  // Storage for a context name (to facilitate debugging multi-context setups)

  // Inputs
  ImVector<ImGuiInputEvent>
      InputEventsQueue;  // Input events which will be trickled/written into IO structure.
  ImVector<ImGuiInputEvent>
      InputEventsTrail;  // Past input events processed in NewFrame(). This is to allow
  ...
  ImVector<ImGuiWindow*> WindowsTempSortBuffer;  // Temporary buffer used in EndFrame() to reorder
                                                 // windows so parents are kept before their child
  ImVector<ImGuiWindowStackData> CurrentWindowStack;
  ImGuiStorage WindowsById;         // Map window's ImGuiID to ImGuiWindow*
  int WindowsActiveCount;           // Number of unique windows submitted by frame
  float WindowsBorderHoverPadding;  // Padding around resizable windows for which hovering on counts
                                    // as hovering the window == ImMax(style.TouchExtraPadding,
                                    // style.WindowBorderHoverPadding). This isn't so multi-dpi
                                    // friendly.
  ImGuiID DebugBreakInWindow;       // Set to break in Begin() call.
  ImGuiWindow* CurrentWindow;       // Window being drawn into
  ImGuiWindow* HoveredWindow;  // Window the mouse is hovering. Will typically catch mouse inputs.
  ImGuiWindow* HoveredWindowUnderMovingWindow;  // Hovered window ignoring MovingWindow. Only set if
                                                // MovingWindow is set.
  ImGuiWindow*
      HoveredWindowBeforeClear;  // Window the mouse is hovering. Filled even with _NoMouse. This is
                                 // currently useful for multi-context compositors.
  ImGuiWindow* MovingWindow;     // Track the window we clicked on (in order to preserve focus). The
                              // actual window that is moved is generally MovingWindow->RootWindow.
  ImGuiWindow*
  ...
  ImGuiID DebugHookIdInfoId;  // Will call core hooks: DebugHookIdInfo() from GetID functions, used
                              // by ID Stack Tool [next HoveredId/ActiveId to not pull in an extra
                              // cache-line]
  ImGuiID HoveredId;          // Hovered widget, filled during the frame
  ImGuiID HoveredIdPreviousFrame;
  int HoveredIdPreviousFrameItemCount;  // Count numbers of items using the same ID as last frame's
                                        // hovered id
  float HoveredIdTimer;                 // Measure contiguous hovering time
  float HoveredIdNotActiveTimer;  // Measure contiguous hovering time where the item has not been
                                  // active
  bool HoveredIdAllowOverlap;
  bool HoveredIdIsDisabled;  // At least one widget passed the rect test, but has been discarded by
                             // disabled flag or popup inhibit. May be true even if HoveredId == 0.
  bool ItemUnclipByLog;  // Disable ItemAdd() clipping, essentially a memory-locality friendly copy
                         // of LogEnabled
  ...
  ImGuiID
      ActiveIdDisabledId;  // When clicking a disabled item we set ActiveId=window->MoveId to avoid
                           // interference with widget code. Actual item ID is stored here.
  ...
  ImGuiInputSource ActiveIdSource;  // Activating source: ImGuiInputSource_Mouse OR
                                    // ImGuiInputSource_Keyboard OR ImGuiInputSource_Gamepad
  ...
  // - Input query can then read input by specifying ImGuiKeyOwner_Any (== 0), ImGuiKeyOwner_NoOwner
  // (== -1) or a custom ID.
  // - Routing is requested ahead of time for a given chord (Key + Mods) and granted in NewFrame().
  double LastKeyModsChangeTime;  // Record the last time key mods changed (affect repeat delay when
                                 // using shortcut logic)
  double LastKeyModsChangeFromNoneTime;  // Record the last time key mods changed away from being 0
                                         // (affect repeat delay when using shortcut logic)
  double LastKeyboardKeyPressTime;  // Record the last time a keyboard key (ignore mouse/gamepad
                                    // ones) was pressed.
  ImBitArrayForNamedKeys
      KeysMayBeCharInput;  // Lookup to tell if a key can emit char input, see
                           // IsKeyChordPotentiallyCharInput(). sizeof() = 20 bytes
  ImGuiKeyOwnerData KeysOwnerData[ImGuiKey_NamedKey_COUNT];
  ImGuiKeyRoutingTable KeysRoutingTable;
  ImU32 ActiveIdUsingNavDirMask;  // Active widget will want to read those nav move requests (e.g.
                                  // can activate a button and move away from it)
  bool ActiveIdUsingAllKeyboardKeys;  // Active widget will want to read all keyboard keys inputs.
                                      // (this is a shortcut for not taking ownership of 100+ keys,
                                      // frequently used by drag operations)
  ImGuiKeyChord
      DebugBreakInShortcutRouting;  // Set to break in SetShortcutRouting()/Shortcut() calls.
  // ImU32                 ActiveIdUsingNavInputMask;          // [OBSOLETE] Since
  // (IMGUI_VERSION_NUM >= 18804) : 'g.ActiveIdUsingNavInputMask |= (1 << ImGuiNavInput_Cancel);'
  // becomes --> 'SetKeyOwner(ImGuiKey_Escape, g.ActiveId) and/or
  // SetKeyOwner(ImGuiKey_NavGamepadCancel, g.ActiveId);'

  // Next window/item data
  ImGuiID CurrentFocusScopeId;  // Value for currently appending items == g.FocusScopeStack.back().
                                // Not to be mistaken with g.NavFocusScopeId.
  ImGuiItemFlags
      CurrentItemFlags;   // Value for currently appending items == g.ItemFlagsStack.back()
  ImGuiID DebugLocateId;  // Storage for DebugLocateItemOnHover() feature: this is read by ItemAdd()
                          // so we keep it in a hot/cached location
  ...
  ImVector<ImGuiFocusScopeData>
      FocusScopeStack;  // Stack for PushFocusScope()/PopFocusScope() - inherited by BeginChild(),
                        // pushed into by Begin()
  ...
  // Viewports
  ImVector<ImGuiViewportP*> Viewports;  // Active viewports (Size==1 in 'master' branch). Each
                                        // viewports hold their copy of ImDrawData.

  // Keyboard/Gamepad Navigation
  bool NavCursorVisible;  // Nav focus cursor/rectangle is visible? We hide it after a mouse click.
                          // We show it after a nav move.
  bool NavHighlightItemUnderNav;  // Disable mouse hovering highlight. Highlight navigation focused
                                  // item instead of mouse hovered item.
  // bool                  NavDisableHighlight;                // Old name for !g.NavCursorVisible
  // before 1.91.4 (2024/10/18). OPPOSITE VALUE (g.NavDisableHighlight == !g.NavCursorVisible) bool
  // NavDisableMouseHover;               // Old name for g.NavHighlightItemUnderNav before 1.91.1
  // (2024/10/18) this was called When user starts using keyboard/gamepad, we hide mouse hovering
  // highlight until mouse is touched again.
  bool NavMousePosDirty;    // When set we will update mouse position if io.ConfigNavMoveSetMousePos
                            // is set (not enabled by default)
  bool NavIdIsAlive;        // Nav widget has been seen this frame ~~ NavRectRel is valid
  ImGuiID NavId;            // Focused item for navigation
  ImGuiWindow* NavWindow;   // Focused window for navigation. Could be called 'FocusedWindow'
  ImGuiID NavFocusScopeId;  // Focused focus scope (e.g. selection code often wants to "clear other
                            // items" when landing on an item of the same scope)
  ImGuiNavLayer NavLayer;   // Focused layer (main scrolling layer, or menu/title bar layer)
  ImGuiID NavActivateId;    // ~~ (g.ActiveId == 0) && (IsKeyPressed(ImGuiKey_Space) ||
                          // IsKeyDown(ImGuiKey_Enter) || IsKeyPressed(ImGuiKey_NavGamepadActivate))
                          // ? NavId : 0, also set when calling ActivateItemByID()
  ImGuiID NavActivateDownId;  // ~~ IsKeyDown(ImGuiKey_Space) || IsKeyDown(ImGuiKey_Enter) ||
                              // IsKeyDown(ImGuiKey_NavGamepadActivate) ? NavId : 0
  ImGuiID
      NavActivatePressedId;  // ~~ IsKeyPressed(ImGuiKey_Space) || IsKeyPressed(ImGuiKey_Enter) ||
                             // IsKeyPressed(ImGuiKey_NavGamepadActivate) ? NavId : 0 (no repeat)
  ImGuiActivateFlags NavActivateFlags;
  ImVector<ImGuiFocusScopeData>
      NavFocusRoute;  // Reversed copy focus scope stack for NavId (should contains
                      // NavFocusScopeId). This essentially follow the
                      // window->ParentWindowForFocusRoute chain.
  ImGuiID NavHighlightActivatedId;
  float NavHighlightActivatedTimer;
  ImGuiID NavNextActivateId;  // Set by ActivateItemByID(), queued until next frame.
  ImGuiActivateFlags NavNextActivateFlags;
  ImGuiInputSource NavInputSource;  // Keyboard or Gamepad mode? THIS CAN ONLY BE
                                    // ImGuiInputSource_Keyboard or ImGuiInputSource_Gamepad
  ImGuiSelectionUserData
      NavLastValidSelectionUserData;  // Last valid data passed to SetNextItemSelectionUser(), or
                                      // -1. For current window. Not reset when focusing an item
                                      // that doesn't have selection data.
  ImS8 NavCursorHideFrames;
  // ImGuiID               NavActivateInputId;                 // Removed in 1.89.4 (July 2023).
  // This is now part of g.NavActivateId and sets g.NavActivateFlags |=
  // ImGuiActivateFlags_PreferInput. See commit c9a53aa74, issue #5606.

  // Navigation: Init & Move Requests
  bool NavAnyRequest;   // ~~ NavMoveRequest || NavInitRequest this is to perform early out in
                        // ItemAdd()
  bool NavInitRequest;  // Init request for appearing window to select first item
  bool NavInitRequestFromMove;
  ImGuiNavItemData NavInitResult;  // Init request result (first item of the window, or one for
                                   // which SetItemDefaultFocus() was called)
  ...
  ImRect NavScoringNoClipRect;  // Some nav operations (such as PageUp/PageDown) enforce a region
                                // which clipper will attempt to always keep submitted
  int NavScoringDebugCount;     // Metrics for debugging
  int NavTabbingDir;            // Generally -1 or +1, 0 when tabbing without a nav id
  int NavTabbingCounter;        // >0 when counting items for tabbing
  ImGuiNavItemData NavMoveResultLocal;  // Best move request candidate within NavWindow
  ImGuiNavItemData
      NavMoveResultLocalVisible;  // Best move request candidate within NavWindow that are mostly
                                  // visible (when using ImGuiNavMoveFlags_AlsoScoreVisibleSet flag)
  ImGuiNavItemData NavMoveResultOther;  // Best move request candidate within NavWindow's flattened
                                        // hierarchy (when using ImGuiWindowFlags_NavFlattened flag)
  ImGuiNavItemData NavTabbingResultFirst;  // First tabbing request candidate within NavWindow and
                                           // flattened hierarchy

  // Navigation: record of last move request
  ImGuiID NavJustMovedFromFocusScopeId;  // Just navigated from this focus scope id (result of a
                                         // successfully MoveRequest).
  ImGuiID NavJustMovedToId;  // Just navigated to this id (result of a successfully MoveRequest).
  ImGuiID NavJustMovedToFocusScopeId;  // Just navigated to this focus scope id (result of a
                                       // successfully MoveRequest).
  ImGuiKeyChord NavJustMovedToKeyMods;
  bool NavJustMovedToIsTabbing;  // Copy of ImGuiNavMoveFlags_IsTabbing. Maybe we should store whole
                                 // flags.
  bool NavJustMovedToHasSelectionData;  // Copy of move result's ItemFlags &
                                        // ImGuiItemFlags_HasSelectionUserData). Maybe we should
                                        // just store ImGuiNavItemData.

  // Navigation: Windowing (CTRL+TAB for list, or Menu button + keys or directional pads to
  // move/resize)
  bool
      ConfigNavWindowingWithGamepad;  // = true. Enable CTRL+TAB by holding ImGuiKey_GamepadFaceLeft
                                      // (== ImGuiKey_NavGamepadMenu). When false, the button may
                                      // still be used to toggle Menu layer.
  ImGuiKeyChord
      ConfigNavWindowingKeyNext;  // = ImGuiMod_Ctrl | ImGuiKey_Tab (or ImGuiMod_Super |
                                  // ImGuiKey_Tab on OS X). For reconfiguration (see #4828)
  ImGuiKeyChord
      ConfigNavWindowingKeyPrev;  // = ImGuiMod_Ctrl | ImGuiMod_Shift | ImGuiKey_Tab (or
                                  // ImGuiMod_Super | ImGuiMod_Shift | ImGuiKey_Tab on OS X)
  ImGuiWindow*
      NavWindowingTarget;  // Target window when doing CTRL+Tab (or Pad Menu + FocusPrev/Next), this
                           // window is temporarily displayed top-most!
  ImGuiWindow* NavWindowingTargetAnim;  // Record of last valid NavWindowingTarget until DimBgRatio
                                        // and NavWindowingHighlightAlpha becomes 0.0f, so the
                                        // fade-out can stay on it.
  ImGuiWindow* NavWindowingListWindow;  // Internal window actually listing the CTRL+Tab contents
  float NavWindowingTimer;
  float NavWindowingHighlightAlpha;
  ImGuiInputSource NavWindowingInputSource;
  bool NavWindowingToggleLayer;    // Set while Alt or GamepadMenu is held, may be cleared by other
                                   // operations, and processed when releasing the key.
  ImGuiKey NavWindowingToggleKey;  // Keyboard/gamepad key used when toggling to menu layer.
  ImVec2 NavWindowingAccumDeltaPos;
  ImVec2 NavWindowingAccumDeltaSize;

  // Render
  float DimBgRatio;  // 0.0..1.0 animation when fading in a dimming background (for modal window and
                     // CTRL+TAB list)
  ...
  ImRect DragDropTargetRect;  // Store rectangle of current target candidate (we favor small targets
                              // when overlapping)
  ...
  ImGuiID DragDropHoldJustPressedId;  // Set when holding a payload just made ButtonBehavior()
                                      // return a press.
  ...
  int TablesTempDataStacked;  // Temporary table data size (because we leave previous instances
                              // undestructed, we generally don't use TablesTempData.Size)
  ImVector<ImGuiTableTempData> TablesTempData;  // Temporary table data (buffers reused/shared
                                                // across instances, support nesting)
  ...
  int MultiSelectTempDataStacked;  // Temporary multi-select data size (because we leave previous
                                   // instances undestructed, we generally don't use
                                   // MultiSelectTempData.Size)
  ...
  float HoverItemDelayClearTimer;         // Currently used by IsItemHovered(): grace time before
                                          // g.TooltipHoverTimer gets cleared.
  ImGuiID HoverItemUnlockedStationaryId;  // Mouse has once been stationary on this item. Only reset
                                          // after departing the item.
  ImGuiID HoverWindowUnlockedStationaryId;  // Mouse has once been stationary on this window. Only
                                            // reset after departing the window.
  ...
  ImGuiID ColorEditCurrentID;            // Set temporarily while inside of the parent-most
                                         // ColorEdit4/ColorPicker4 (because they call each others).
  ImGuiID ColorEditSavedID;              // ID we are saving/restoring HS for
  float ColorEditSavedHue;  // Backup of last Hue associated to LastColor, so we can restore Hue in
                            // lossy RGB<>HSV round trips
  float ColorEditSavedSat;  // Backup of last Saturation associated to LastColor, so we can restore
                            // Saturation in lossy RGB<>HSV round trips
  ...
  float
      ScrollbarClickDeltaToGrabCenter;  // When scrolling to mouse location: distance between mouse
                                        // and center of grab box, normalized in parent space.
  ...
  // Platform support
  ImGuiPlatformImeData PlatformImeData;  // Data updated by current frame. Will be applied at end of
                                         // the frame. For some backends, this is required to have
                                         // WantVisible=true in order to receive text message.
  ImGuiPlatformImeData PlatformImeDataPrev;  // Previous frame data. When changed we call the
                                             // platform_io.Platform_SetImeDataFn() handler.

  // Extensions
  // FIXME: We could provide an API to register one slot in an array held in ImGuiContext?
  ImVector<ImTextureData*>
      UserTextures;  // List of textures created/managed by user or third-party extension.
                     // Automatically appended into platform_io.Textures[].
  ...
  const char* LogNextPrefix;  // See comment in LogSetNextTextDecoration(): doesn't copy underlying
                              // data, use carefully!
  ...
  // Debug Tools
  // (some of the highly frequently used data are interleaved in other structures above:
  // DebugBreakXXX fields, DebugHookIdInfo, DebugLocateId etc.)
  ...
  ImU8 DebugLocateFrames;  // For DebugLocateItemOnHover(). This is used together with DebugLocateId
                           // which is in a hot/cached spot above.
  ...
  // Misc
  float FramerateSecPerFrame[60];  // Calculate estimate of framerate for user over the last 60
                                   // frames..
  ...
  int WantTextInputNextFrame;  // Copied in EndFrame() from g.PlatformImeData.WantTextInput. Needs
                               // to be set for some backends (SDL3) to emit character inputs.
...
// ImGuiDrawContext, hence the DC variable name in ImGuiWindow. (That's theory, in practice the
// delimitation between ImGuiWindow and ImGuiWindowTempData is quite tenuous and could be
// reconsidered..) (This doesn't need a constructor because we zero-clear it as part of ImGuiWindow
// and all frame-temporary data are setup on Begin)
struct IMGUI_API ImGuiWindowTempData {
  // Layout
  ImVec2 CursorPos;  // Current emitting position, in absolute coordinates.
  ImVec2 CursorPosPrevLine;
  ImVec2 CursorStartPos;  // Initial position after Begin(), generally ~ window position +
                          // WindowPadding.
  ImVec2 CursorMaxPos;  // Used to implicitly calculate ContentSize at the beginning of next frame,
                        // for scrolling range and auto-resize. Always growing during the frame.
  ImVec2 IdealMaxPos;   // Used to implicitly calculate ContentSizeIdeal at the beginning of next
  ...
  ImVec1 GroupOffset;
  ImVec2 CursorStartPosLossyness;  // Record the loss of precision of CursorStartPos due to really
  ...
  short
      NavLayersActiveMaskNext;  // Which layers have been written to (accumulator for current frame)
  bool NavIsScrollPushableX;    // Set when current work location may be scrolled horizontally when
                              // moving left / right. This is generally always true UNLESS within a
                              // column.
  ...
  ImU32 TreeHasStackDataDepthMask;  // Store whether given depth has ImGuiTreeNodeStackData data.
                                    // Could be turned into a ImU64 if necessary.
  ImU32 TreeRecordsClippedNodesY2Mask;  // Store whether we should keep recording Y2. Cleared when
                                        // passing clip max. Equivalent TreeHasStackDataDepthMask
                                        // value should always be set.
  ImVector<ImGuiWindow*> ChildWindows;
  ImGuiStorage* StateStorage;       // Current persistent per-window storage (store e.g. tree node
                                    // open/close state)
  ...
  float ItemWidth;  // Current item width (>0.0: width in pixels, <0.0: align xx pixels to the right
                    // of window).
  float TextWrapPos;  // Current text wrap pos.
  ImVector<float>
      ItemWidthStack;  // Store item widths to restore (attention: .back() is not == ItemWidth)
  ImVector<float> TextWrapPosStack;  // Store text wrap pos to restore (attention: .back() is not ==
                                     // TextWrapPos)
};
...
struct IMGUI_API ImGuiWindow {
  ...
  ImGuiViewportP* Viewport;    // Always set in Begin(). Inactive windows may have a NULL value here
                               // if their viewport was discarded.
  ImVec2 Pos;                  // Position (always rounded-up to nearest pixel)
  ImVec2 Size;                 // Current size (==SizeFull or collapsed title bar size)
  ImVec2 SizeFull;             // Size when non collapsed
  ImVec2 ContentSize;  // Size of contents/scrollable client area (calculated from the extents reach
                       // of the cursor) from previous frame. Does not include window decoration or
                       // window padding.
  ImVec2 ContentSizeIdeal;
  ImVec2 ContentSizeExplicit;  // Size of contents/scrollable client area explicitly request by the
                               // user via SetNextWindowContentSize().
  ...
  float TitleBarHeight,
      MenuBarHeight;  // Note that those used to be function before 2024/05/28. If you have old code
                      // calling TitleBarHeight() you can change it to TitleBarHeight.
  float DecoOuterSizeX1,
      DecoOuterSizeY1;  // Left/Up offsets. Sum of non-scrolling outer decorations (X1 generally ==
                        // 0.0f. Y1 generally = TitleBarHeight + MenuBarHeight). Locked during
                        // Begin().
  float DecoOuterSizeX2, DecoOuterSizeY2;  // Right/Down offsets (X2 generally == ScrollbarSize.x,
                                           // Y2 == ScrollbarSizes.y).
  float DecoInnerSizeX1,
      DecoInnerSizeY1;  // Applied AFTER/OVER InnerRect. Specialized for Tables as they use
  ...
  ImVec2 Scroll;
  ImVec2 ScrollMax;
  ImVec2 ScrollTarget;  // target scroll position. stored as cursor position with scrolling canceled
                        // out, so the highest point is always 0.0f. (FLT_MAX for no change)
  ImVec2 ScrollTargetCenterRatio;  // 0.0f = scroll so that target position is at top, 0.5f = scroll
                                   // so that target position is centered
  ImVec2 ScrollTargetEdgeSnapDist;  // 0.0f = no snapping, >0.0f snapping threshold
  ImVec2 ScrollbarSizes;  // Size taken by each scrollbars on their smaller axis. Pay attention!
                          // ScrollbarSizes.x == width of the vertical scrollbar, ScrollbarSizes.y =
                          // height of the horizontal scrollbar.
  bool ScrollbarX, ScrollbarY;             // Are scrollbars visible?
  bool ScrollbarXStabilizeEnabled;         // Was ScrollbarX previously auto-stabilized?
  ImU8 ScrollbarXStabilizeToggledHistory;  // Used to stabilize scrollbar visibility in case of
                                           // feedback loops
  ...
  signed char
      ResizeBorderHovered;  // Current border being hovered for resize (-1: none, otherwise 0-3)
  ...
  short BeginOrderWithinParent;   // Begin() order within immediate parent window, if we are a child
                                  // window. Otherwise 0.
  short BeginOrderWithinContext;  // Begin() order within entire imgui context. This is mostly used
                                  // for debugging submission order related issues.
  ...
  ImGuiCond
      SetWindowSizeAllowFlags : 8;  // store acceptable condition flags for SetNextWindowSize() use.
  ImGuiCond SetWindowCollapsedAllowFlags : 8;  // store acceptable condition flags for
                                               // SetNextWindowCollapsed() use.
  ImVec2 SetWindowPosVal;  // store window position when using a non-zero Pivot (position set needs
                           // to be processed when we know the window size)
  ...
  ImVector<ImGuiID> IDStack;  // ID stack. ID are hashes seeded with the value at the top of the
  ...
  ImRect
      ParentWorkRect;  // Backup of WorkRect before entering a container such as columns/tables.
                       // Used by e.g. SpanAllColumns functions to easily access. Stacked containers
                       // are responsible for maintaining this. // FIXME-WORKRECT: Could be a stack?
  ImRect ClipRect;  // Current clipping/scissoring rectangle, evolve as we are using PushClipRect(),
                    // etc. == DrawList->clip_rect_stack.back().
  ...
  ImVec2ih HitTestHoleSize;  // Define an optional rectangular hole where mouse will pass-through
                             // the window.
  ...
  float FontRefSize;   // This is a copy of window->CalcFontSize() at the time of Begin(), trying to
                       // phase out CalcFontSize() especially as it may be called on non-current
                       // window.
  ...
  ImGuiWindow* ParentWindow;  // If we are a child _or_ popup _or_ docked window, this is pointing
                              // to our parent. Otherwise NULL.
  ImGuiWindow* ParentWindowInBeginStack;
  ImGuiWindow* RootWindow;  // Point to ourself or first ancestor that is not a child window.
                            // Doesn't cross through popups/dock nodes.
  ImGuiWindow* RootWindowPopupTree;  // Point to ourself or first ancestor that is not a child
                                     // window. Cross through popups parent<>child.
  ImGuiWindow*
  ...
  ImGuiWindow* RootWindowForNav;       // Point to ourself or first ancestor which doesn't have the
                                       // NavFlattened flag.
  ImGuiWindow*
      ParentWindowForFocusRoute;  // Set to manual link a window to its logical parent so that
                                  // Shortcut() chain are honoerd (e.g. Tool linked to Document)
  ...
  ImVec2
      NavPreferredScoringPosRel[ImGuiNavLayer_COUNT];  // Preferred X/Y position updated when moving
                                                       // on a given axis, reset to FLT_MAX.
 ...
 public:
  ...
  // [Obsolete] ImGuiWindow::CalcFontSize() was removed in 1.92.x because error-prone/misleading.
  // You can use window->FontRefSize for a copy of g.FontSize at the time of the last Begin() call
  // for this window.
  // float     CalcFontSize() const    { ImGuiContext& g = *Ctx; return g.FontSizeBase *
  // FontWindowScale * FontWindowScaleParents;
};
...
enum ImGuiTabItemFlagsPrivate_ {
  ...
  // ImGuiTabItemFlags_Unsorted                = 1 << 23,  // [Docking] Trailing tabs with the
  // _Unsorted flag will be sorted based on the DockOrder of their Window.
};
...
struct ImGuiTabItem {
  ...
  int LastFrameSelected;  // This allows us to infer an ordered list of the last activated tabs with
                          // little maintenance
  float Offset;           // Position relative to beginning of tab
  float Width;            // Width currently displayed
  float ContentWidth;    // Width of label + padding, stored during BeginTabItem() call (misnamed as
                         // "Content" would normally imply width of label only)
  float RequestedWidth;  // Width optionally requested by caller, -1.0f is unused
  ImS32 NameOffset;      // When Window==NULL, offset to name within parent ImGuiTabBar::TabsNames
  ImS16 BeginOrder;      // BeginTabItem() order, used to re-order tabs after toggling
                         // ImGuiTabBarFlags_Reorderable
  ImS16 IndexDuringLayout;  // Index only used during TabBarLayout(). Tabs gets reordered so
                            // 'Tabs[n].IndexDuringLayout == n' but may mismatch during additions.
...
struct IMGUI_API ImGuiTabBar {
  ...
  ImGuiID VisibleTabId;  // Can occasionally be != SelectedTabId (e.g. when previewing contents for
                         // CTRL+TAB preview)
  int CurrFrameVisible;
  int PrevFrameVisible;
  ImRect BarRect;
  float BarRectPrevWidth;  // Backup of previous width. When width change we enforce keep horizontal
                           // scroll on focused tab.
...
#define IM_COL32_DISABLE \
  IM_COL32(0, 0, 0, 1)  // Special sentinel code which cannot be used as a regular color.
#define IMGUI_TABLE_MAX_COLUMNS \
  512  // Arbitrary "safety" maximum, may be lifted in the future if needed. Must fit in
...
struct ImGuiTableColumn {
  ImGuiTableColumnFlags Flags;  // Flags after some patching (not directly same as provided by
                                // user). See ImGuiTableColumnFlags_
  float WidthGiven;  // Final/actual width visible == (MaxX - MinX), locked in TableUpdateLayout().
                     // May be > WidthRequest to honor minimum width, may be < WidthRequest to honor
  ...
  ImGuiTableColumnIdx DisplayOrder;  // Index within Table's IndexToDisplayOrder[] (column may be
                                     // reordered by users)
  ImGuiTableColumnIdx
      IndexWithinEnabledSet;  // Index within enabled/visible set (<= IndexToDisplayOrder)
  ImGuiTableColumnIdx PrevEnabledColumn;  // Index of prev enabled/visible column within Columns[],
                                          // -1 if first enabled/visible column
  ImGuiTableColumnIdx NextEnabledColumn;  // Index of next enabled/visible column within Columns[],
                                          // -1 if last enabled/visible column
  ImGuiTableColumnIdx SortOrder;  // Index of this column within sort specs, -1 if not sorting on
                                  // this column, 0 for single-sort, may be >0 on multi-sort
  ImGuiTableDrawChannelIdx DrawChannelCurrent;   // Index within DrawSplitter.Channels[]
  ImGuiTableDrawChannelIdx DrawChannelFrozen;    // Draw channels for frozen rows (often headers)
  ImGuiTableDrawChannelIdx DrawChannelUnfrozen;  // Draw channels for unfrozen rows
  bool IsEnabled;      // IsUserEnabled && (Flags & ImGuiTableColumnFlags_Disabled) == 0
  bool IsUserEnabled;  // Is the column not marked Hidden by the user? (unrelated to being off view,
                       // e.g. clipped by scrolling).
  bool IsUserEnabledNextFrame;
  bool IsVisibleX;  // Is actually in view (e.g. overlapping the host window clipping rectangle, not
                    // scrolled).
  bool IsVisibleY;
  ...
  ImU8 SortDirectionsAvailList;  // Ordered list of available sort directions (2-bits each, total
                                 // 8-bits)
...
// Per-instance data that needs preserving across frames (seemingly most others do not need to be
// preserved aside from debug needs. Does that means they could be moved to ImGuiTableTempData?)
// sizeof() ~ 24 bytes
struct ImGuiTableInstanceData {
  ImGuiID TableInstanceID;
  float LastOuterHeight;  // Outer height from last frame
  float
      LastTopHeadersRowHeight;  // Height of first consecutive header rows from last frame (FIXME:
                                // this is used assuming consecutive headers are in same frozen set)
...
struct IMGUI_API ImGuiTable {
  ...
  ImBitArrayPtr
      EnabledMaskByIndex;  // Column Index -> IsEnabled map (== not hidden by user/api) in a format
                           // adequate for iterating column without touching cold data
  ImBitArrayPtr VisibleMaskByIndex;  // Column Index -> IsVisibleX|IsVisibleY map (== not hidden by
                                     // user/api && not hidden by scrolling/cliprect)
  ImGuiTableFlags SettingsLoadedFlags;  // Which data were loaded from the .ini file (e.g. when
  ...
  int CurrentColumn;
  ImS16
      InstanceCurrent;  // Count of BeginTable() calls with same ID in the same frame (generally 0).
                        // This is a little bit similar to BeginCount for a window, but multiple
                        // tables with the same ID are multiple tables, they are just synced.
  ImS16 InstanceInteracted;  // Mark which instance (generally 0) of the same ID is being interacted
                             // with
  float RowPosY1;
  float RowPosY2;
  ...
  int RowBgColorCounter;  // Counter for alternating background colors (can be fast-forwarded by e.g
                          // clipper), not same as CurrentRow because header rows typically don't
                          // increase this.
  ...
  float ResizeLockMinContentsX2;  // Lock minimum contents width while resizing down in order to not
                                  // create feedback loops. But we allow growing the table.
  ...
  ImRect OuterRect;  // Note: for non-scrolling table, OuterRect.Max.y is often FLT_MAX until
  ...
  ImRect InnerRect;  // InnerRect but without decoration. As with OuterRect, for non-scrolling
                     // tables, InnerRect.Max.y is "
  ImRect WorkRect;
  ImRect InnerClipRect;
  ImRect BgClipRect;  // We use this to cpu-clip cell background color fill, evolve during the frame
                      // as we cross frozen rows boundaries
  ...
  ImDrawListSplitter*
      DrawSplitter;  // Shortcut to TempData->DrawSplitter while in table. Isolate draw commands per
                     // columns to avoid switching clip rect constantly
  ImGuiTableInstanceData InstanceDataFirst;
  ImVector<ImGuiTableInstanceData>
      InstanceDataExtra;  // FIXME-OPT: Using a small-vector pattern would be good.
  ...
  ImGuiTableColumnIdx
      ColumnsEnabledFixedCount;  // Number of enabled columns using fixed width (<= ColumnsCount)
  ...
  ImGuiTableColumnIdx ContextPopupColumn;  // Column right-clicked on, of -1 if opening context menu
                                           // from a neutral/empty spot
  ImGuiTableColumnIdx FreezeRowsRequest;   // Requested frozen rows count
  ImGuiTableColumnIdx FreezeRowsCount;     // Actual frozen row count (== FreezeRowsRequest, or == 0
                                           // when no scrolling offset)
  ImGuiTableColumnIdx FreezeColumnsRequest;  // Requested frozen columns count
  ImGuiTableColumnIdx FreezeColumnsCount;  // Actual frozen columns count (== FreezeColumnsRequest,
                                           // or == 0 when no scrolling offset)
  ImGuiTableColumnIdx RowCellDataCurrent;  // Index of current RowCellData[] entry in current row
  ImGuiTableDrawChannelIdx DummyDrawChannel;  // Redirect non-visible columns here.
  ImGuiTableDrawChannelIdx
      Bg2DrawChannelCurrent;  // For Selectable() and other widgets drawing across columns after the
                              // freezing line. Index within DrawSplitter.Channels[]
  ...
  bool IsContextPopupOpen;  // Set when default context menu is open (also see: ContextPopupColumn,
                            // InstanceInteracted).
  bool DisableDefaultContextMenu;  // Disable default context menu. You may submit your own using
  ...
  bool HasScrollbarYCurr;  // Whether ANY instance of this table had a vertical scrollbar during the
                           // current frame.
  bool HasScrollbarYPrev;  // Whether ANY instance of this table had a vertical scrollbar during the
                           // previous.
...
struct IMGUI_API ImGuiTableTempData {
  ...
  ImRect
      HostBackupParentWorkRect;  // Backup of InnerWindow->ParentWorkRect at the end of BeginTable()
  ImVec2
...
// ImGuiTableColumnSettings, etc.)
struct ImGuiTableSettings {
  ImGuiID ID;  // Set to 0 to invalidate/delete the setting
  ImGuiTableFlags
      SaveFlags;  // Indicate data we want to save using the Resizable/Reorderable/Sortable/Hideable
...
// Fonts, drawing
IMGUI_API void RegisterUserTexture(
    ImTextureData* tex);  // Register external texture. EXPERIMENTAL: DO NOT USE YET.
...
IMGUI_API void SetCurrentFont(ImFont* font, float font_size_before_scaling,
...
}  // This seemingly unnecessary wrapper simplifies compatibility between the 'master' and 'docking'
   // branches.
IMGUI_API ImDrawList* GetBackgroundDrawList(
    ImGuiViewport* viewport);  // get background draw list for the given viewport. this draw list
...
IMGUI_API ImDrawList* GetForegroundDrawList(
    ImGuiViewport* viewport);  // get foreground draw list for the given viewport. this draw list
                               // will be the last rendered one. Useful to quickly draw shapes/text
                               // over dear imgui contents.
IMGUI_API void AddDrawListToDrawDataEx(ImDrawData* draw_data, ImVector<ImDrawList*>* out_list,
                                       ImDrawList* draw_list);

// Init
IMGUI_API void Initialize();
IMGUI_API void Shutdown();  // Since 1.60 this is a _private_ function. You can call
...
IMGUI_API void FindHoveredWindowEx(const ImVec2& pos, bool find_first_and_in_any_viewport,
...
IMGUI_API ImVec2 ScrollToRectEx(ImGuiWindow* window, const ImRect& rect,
...
IMGUI_API void MarkItemEdited(ImGuiID id);  // Mark data associated to given item as "edited", used
                                            // by IsItemDeactivatedAfterEdit() function.
IMGUI_API void PushOverrideID(ImGuiID id);  // Push given value as-is at the top of the ID stack
...
IMGUI_API void ShrinkWidths(ImGuiShrinkWidthItem* items, int count, float width_excess,
                            float width_min);
IMGUI_API void CalcClipRectVisibleItemsY(const ImRect& clip_rect, const ImVec2& pos,
...
// Logging/Capture
IMGUI_API void LogBegin(ImGuiLogFlags flags,
...
// Childs
IMGUI_API bool BeginChildEx(const char* name, ImGuiID id, const ImVec2& size_arg,
...
IMGUI_API ImVec2 FindBestWindowPosForPopupEx(const ImVec2& ref_pos, const ImVec2& size,
...
// Menus
IMGUI_API bool BeginViewportSideBar(const char* name, ImGuiViewport* viewport, ImGuiDir dir,
                                    float size, ImGuiWindowFlags window_flags);
IMGUI_API bool BeginMenuEx(const char* label, const char* icon, bool enabled = true);
IMGUI_API bool MenuItemEx(const char* label, const char* icon, const char* shortcut = NULL,
...
IMGUI_API void NavMoveRequestSubmit(ImGuiDir move_dir, ImGuiDir clip_dir,
...
IMGUI_API void NavMoveRequestResolveWithPastTreeNode(ImGuiNavItemData* result,
...
IMGUI_API void SetNavID(ImGuiID id, ImGuiNavLayer nav_layer, ImGuiID focus_scope_id,
                        const ImRect& rect_rel);
IMGUI_API void SetNavFocusScope(ImGuiID focus_scope_id);

// Focus/Activation
// This should be part of a larger set of API: FocusItem(offset = -1), FocusItemByID(id),
// ActivateItem(offset = -1), ActivateItemByID(id) etc. which are much harder to design and
// implement than expected. I have a couple of private branches on this matter but it's not simple.
// For now implementing the easy ones.
IMGUI_API void FocusItem();  // Focus last item (no selection/activation).
IMGUI_API void ActivateItemByID(
    ImGuiID id);  // Activate an item by ID (button, checkbox, tree node etc.). Activation is queued
                  // and processed on the next frame when the item is encountered again. Was called
                  // 'ActivateItem()' before 1.89.7.
...
IMGUI_API ImVec2 GetKeyMagnitude2d(ImGuiKey key_left, ImGuiKey key_right, ImGuiKey key_up,
                                   ImGuiKey key_down);
...
// - Ownership is most often claimed as a result of reacting to a press/down event (but occasionally
// may be claimed ahead).
// - Input queries can then read input by specifying ImGuiKeyOwner_Any (== 0), ImGuiKeyOwner_NoOwner
// (== -1) or a custom ID.
// - Legacy input queries (without specifying an owner or _Any or _None) are equivalent to using
// ImGuiKeyOwner_Any (== 0).
// - Input ownership is automatically released on the frame after a key is released. Therefore:
//   - for ownership registration happening as a result of a down/press event, the SetKeyOwner()
//   call may be done once (common case).
//   - for ownership registration happening ahead of a down/press event, the SetKeyOwner() call
//   needs to be made every frame (happens if e.g. claiming ownership on hover).
// - SetItemKeyOwner() is a shortcut for common simple case. A custom widget will probably want to
// call SetKeyOwner() multiple times directly based on its interaction state.
// - This is marked experimental because not all widgets are fully honoring the Set/Test idioms. We
// will need to move forward step by step.
//   Please open a GitHub Issue to submit your usage scenario or if there's a use case you need
//   solved.
IMGUI_API ImGuiID GetKeyOwner(ImGuiKey key);
IMGUI_API void SetKeyOwner(ImGuiKey key, ImGuiID owner_id, ImGuiInputFlags flags = 0);
IMGUI_API void SetKeyOwnersForKeyChord(ImGuiKeyChord key, ImGuiID owner_id,
                                       ImGuiInputFlags flags = 0);
IMGUI_API void SetItemKeyOwner(
    ImGuiKey key,
    ImGuiInputFlags
        flags);  // Set key owner to last item if it is hovered or active. Equivalent to 'if
                 // (IsItemHovered() || IsItemActive()) { SetKeyOwner(key, GetItemID());'.
IMGUI_API bool TestKeyOwner(
    ImGuiKey key,
...
// - Important: legacy IsKeyPressed(ImGuiKey, bool repeat=true) _DEFAULTS_ to repeat, new
// IsKeyPressed() requires _EXPLICIT_ ImGuiInputFlags_Repeat flag.
// - Expected to be later promoted to public API, the prototypes are designed to replace existing
// ones (since owner_id can default to Any == 0)
// - Specifying a value for 'ImGuiID owner' will test that EITHER the key is NOT owned (UNLESS
// locked), EITHER the key is owned by 'owner'.
//   Legacy functions use ImGuiKeyOwner_Any meaning that they typically ignore ownership, unless a
//   call to SetKeyOwner() explicitly used ImGuiInputFlags_LockThisFrame or
//   ImGuiInputFlags_LockUntilRelease.
// - Binding generators may want to ignore those for now, or suffix them with Ex() until we decide
// if this gets moved into public API.
IMGUI_API bool IsKeyDown(ImGuiKey key, ImGuiID owner_id);
IMGUI_API bool IsKeyPressed(
    ImGuiKey key, ImGuiInputFlags flags,
    ImGuiID owner_id = 0);  // Important: when transitioning from old to new IsKeyPressed(): old API
                            // has "bool repeat = true", so would default to repeat. New API
                            // requiress explicit ImGuiInputFlags_Repeat.
IMGUI_API bool IsKeyReleased(ImGuiKey key, ImGuiID owner_id);
IMGUI_API bool IsKeyChordPressed(ImGuiKeyChord key_chord, ImGuiInputFlags flags,
                                 ImGuiID owner_id = 0);
...
// - When a policy (except for ImGuiInputFlags_RouteAlways *) is set, Shortcut() will register
...
//   and bypassing route registration and check)
// - When using one of the routing option:
//   - The default route is ImGuiInputFlags_RouteFocused (accept inputs if window is in focus stack.
//   Deep-most focused window takes inputs. ActiveId takes inputs over deep-most focused window.)
//   - Routes are requested given a chord (key + modifiers) and a routing policy.
//   - Routes are resolved during NewFrame(): if keyboard modifiers are matching current ones:
//   SetKeyOwner() is called + route is granted for the frame.
//   - Each route may be granted to a single owner. When multiple requests are made we have policies
//   to select the winning route (e.g. deep most window).
//   - Multiple read sites may use the same owner id can all access the granted route.
//   - When owner_id is 0 we use the current Focus Scope ID as a owner ID in order to identify our
//   location.
// - You can chain two unrelated windows in the focus stack using
// SetWindowParentWindowForFocusRoute()
//   e.g. if you have a tool window associated to a document, and you want document shortcuts to run
//   when the tool is focused.
...
// - Selection patterns generally need to react (e.g. clear a selection) when landing on one item of
// the set.
//   So in order to identify a set multiple lists in same window may each need a focus scope.
//   If you imagine an hypothetical BeginSelectionGroup()/EndSelectionGroup() api, it would likely
//   call PushFocusScope()/EndFocusScope()
// - Shortcut routing also use focus scope as a default location identifier if an owner is not
// provided. We don't use the ID Stack for this as it is common to want them separate.
...
// Typing-Select API
// (provide Windows Explorer style "select items by typing partial name" + "cycle through items by
// typing same letter" feature) (this is currently not documented nor used by main library, but
// should work. See "widgets_typingselect" in imgui_test_suite for usage code. Please let us know if
// you use this!)
IMGUI_API ImGuiTypingSelectRequest* GetTypingSelectRequest(
    ImGuiTypingSelectFlags flags = ImGuiTypingSelectFlags_None);
...
// Box-Select API
IMGUI_API bool BeginBoxSelect(const ImRect& scope_rect, ImGuiWindow* window, ImGuiID box_select_id,
...
IMGUI_API void MultiSelectAddSetRange(ImGuiMultiSelectTempData* ms, bool selected, int range_dir,
                                      ImGuiSelectionUserData first_item,
                                      ImGuiSelectionUserData last_item);
inline ImGuiBoxSelectState* GetBoxSelectState(ImGuiID id) {
  ImGuiContext& g = *GImGui;
  return (id != 0 && g.BoxSelectState.ID == id && g.BoxSelectState.IsActive) ? &g.BoxSelectState
                                                                             : NULL;
}
...
IMGUI_API void TableSetColumnSortDirection(int column_n, ImGuiSortDirection sort_direction,
                                           bool append_to_sort_specs);
IMGUI_API int
TableGetHoveredRow();  // Retrieve *PREVIOUS FRAME* hovered row. This difference with
...
IMGUI_API bool BeginTableEx(const char* name, ImGuiID id, int columns_count,
                            ImGuiTableFlags flags = 0, const ImVec2& outer_size = ImVec2(0, 0),
...
IMGUI_API void TableDrawDefaultContextMenu(ImGuiTable* table,
...
IMGUI_API void TabBarQueueReorderFromMousePos(ImGuiTabBar* tab_bar, ImGuiTabItem* tab,
                                              ImVec2 mouse_pos);
...
IMGUI_API void TabItemBackground(ImDrawList* draw_list, const ImRect& bb, ImGuiTabItemFlags flags,
                                 ImU32 col);
IMGUI_API void TabItemLabelAndCloseButton(ImDrawList* draw_list, const ImRect& bb,
...
IMGUI_API void RenderTextWrapped(ImVec2 pos, const char* text, const char* text_end,
                                 float wrap_width);
IMGUI_API void RenderTextClipped(const ImVec2& pos_min, const ImVec2& pos_max, const char* text,
...
IMGUI_API void RenderTextEllipsis(ImDrawList* draw_list, const ImVec2& pos_min,
                                  const ImVec2& pos_max, float ellipsis_max_x, const char* text,
                                  const char* text_end, const ImVec2* text_size_if_known);
IMGUI_API void RenderFrame(ImVec2 p_min, ImVec2 p_max, ImU32 fill_col, bool borders = true,
                           float rounding = 0.0f);
IMGUI_API void RenderFrameBorder(ImVec2 p_min, ImVec2 p_max, float rounding = 0.0f);
IMGUI_API void RenderColorRectWithAlphaCheckerboard(ImDrawList* draw_list, ImVec2 p_min,
                                                    ImVec2 p_max, ImU32 fill_col, float grid_step,
                                                    ImVec2 grid_off, float rounding = 0.0f,
                                                    ImDrawFlags flags = 0);
IMGUI_API void RenderNavCursor(
...
IMGUI_API void RenderMouseCursor(ImVec2 pos, float scale, ImGuiMouseCursor mouse_cursor,
                                 ImU32 col_fill, ImU32 col_border, ImU32 col_shadow);

// Render helpers (those functions don't access any ImGui state!)
IMGUI_API void RenderArrow(ImDrawList* draw_list, ImVec2 pos, ImU32 col, ImGuiDir dir,
                           float scale = 1.0f);
IMGUI_API void RenderBullet(ImDrawList* draw_list, ImVec2 pos, ImU32 col);
IMGUI_API void RenderCheckMark(ImDrawList* draw_list, ImVec2 pos, ImU32 col, float sz);
IMGUI_API void RenderArrowPointingAt(ImDrawList* draw_list, ImVec2 pos, ImVec2 half_sz,
                                     ImGuiDir direction, ImU32 col);
IMGUI_API void RenderRectFilledRangeH(ImDrawList* draw_list, const ImRect& rect, ImU32 col,
                                      float x_start_norm, float x_end_norm, float rounding);
IMGUI_API void RenderRectFilledWithHole(ImDrawList* draw_list, const ImRect& outer,
...
IMGUI_API void TextAligned(float align_x, float size_x, const char* fmt,
                           ...);  // FIXME-WIP: Works but API is likely to be reworked. This is
                                  // designed for 1 item on the line. (#7024)
IMGUI_API void TextAlignedV(float align_x, float size_x, const char* fmt, va_list args);

// Widgets
IMGUI_API bool ButtonEx(const char* label, const ImVec2& size_arg = ImVec2(0, 0),
                        ImGuiButtonFlags flags = 0);
IMGUI_API bool ArrowButtonEx(const char* str_id, ImGuiDir dir, ImVec2 size_arg,
                             ImGuiButtonFlags flags = 0);
IMGUI_API bool ImageButtonEx(ImGuiID id, ImTextureRef tex_ref, const ImVec2& image_size,
                             const ImVec2& uv0, const ImVec2& uv1, const ImVec4& bg_col,
                             const ImVec4& tint_col, ImGuiButtonFlags flags = 0);
IMGUI_API void SeparatorEx(ImGuiSeparatorFlags flags, float thickness = 1.0f);
IMGUI_API void SeparatorTextEx(ImGuiID id, const char* label, const char* label_end,
                               float extra_width);
...
IMGUI_API bool ScrollbarEx(const ImRect& bb, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v,
                           ImS64 avail_v, ImS64 contents_v, ImDrawFlags draw_rounding_flags = 0);
...
IMGUI_API bool ButtonBehavior(const ImRect& bb, ImGuiID id, bool* out_hovered, bool* out_held,
                              ImGuiButtonFlags flags = 0);
IMGUI_API bool DragBehavior(ImGuiID id, ImGuiDataType data_type, void* p_v, float v_speed,
                            const void* p_min, const void* p_max, const char* format,
                            ImGuiSliderFlags flags);
IMGUI_API bool SliderBehavior(const ImRect& bb, ImGuiID id, ImGuiDataType data_type, void* p_v,
                              const void* p_min, const void* p_max, const char* format,
                              ImGuiSliderFlags flags, ImRect* out_grab_bb);
IMGUI_API bool SplitterBehavior(const ImRect& bb, ImGuiID id, ImGuiAxis axis, float* size1,
                                float* size2, float min_size1, float min_size2,
                                float hover_extend = 0.0f, float hover_visibility_delay = 0.0f,
                                ImU32 bg_col = 0);
...
IMGUI_API bool TreeNodeUpdateNextOpen(
    ImGuiID storage_id,
    ImGuiTreeNodeFlags flags);  // Return open state. Consume previous SetNextItemOpen() data, if
                                // any. May return true when logging.
...
IMGUI_API float ScaleRatioFromValueT(ImGuiDataType data_type, T v, T v_min, T v_max,
                                     bool is_logarithmic, float logarithmic_zero_epsilon,
                                     float zero_deadzone_size);
template <typename T, typename SIGNED_T, typename FLOAT_T>
IMGUI_API T ScaleValueFromRatioT(ImGuiDataType data_type, float t, T v_min, T v_max,
                                 bool is_logarithmic, float logarithmic_zero_epsilon,
                                 float zero_deadzone_size);
template <typename T, typename SIGNED_T, typename FLOAT_T>
IMGUI_API bool DragBehaviorT(ImGuiDataType data_type, T* v, float v_speed, T v_min, T v_max,
                             const char* format, ImGuiSliderFlags flags);
template <typename T, typename SIGNED_T, typename FLOAT_T>
IMGUI_API bool SliderBehaviorT(const ImRect& bb, ImGuiID id, ImGuiDataType data_type, T* v, T v_min,
                               T v_max, const char* format, ImGuiSliderFlags flags,
                               ImRect* out_grab_bb);
template <typename T>
...
IMGUI_API int DataTypeFormatString(char* buf, int buf_size, ImGuiDataType data_type,
                                   const void* p_data, const char* format);
IMGUI_API void DataTypeApplyOp(ImGuiDataType data_type, int op, void* output, const void* arg_1,
                               const void* arg_2);
IMGUI_API bool DataTypeApplyFromText(const char* buf, ImGuiDataType data_type, void* p_data,
                                     const char* format, void* p_data_when_empty = NULL);
IMGUI_API int DataTypeCompare(ImGuiDataType data_type, const void* arg_1, const void* arg_2);
IMGUI_API bool DataTypeClamp(ImGuiDataType data_type, void* p_data, const void* p_min,
                             const void* p_max);
...
IMGUI_API bool TempInputScalar(const ImRect& bb, ImGuiID id, const char* label,
                               ...
                               const void* p_clamp_min = NULL, const void* p_clamp_max = NULL);
...
}  // This may be useful to apply workaround that a based on distinguish whenever an item is active
...
IMGUI_API void DebugHookIdInfo(ImGuiID id, ImGuiDataType data_type, const void* data_id,
                               const void* data_id_end);
IMGUI_API void DebugNodeColumns(ImGuiOldColumns* columns);
IMGUI_API void DebugNodeDrawList(ImGuiWindow* window, ImGuiViewportP* viewport,
...
IMGUI_API void DebugNodeTexture(
    ImTextureData* tex, int int_id,
    const ImFontAtlasRect* highlight_rect =
        NULL);  // ID used to facilitate persisting the "current" texture.
...
IMGUI_API void DebugRenderViewportThumbnail(ImDrawList* draw_list, ImGuiViewportP* viewport,
                                            const ImRect& bb);

// Obsolete functions
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
// inline void   SetItemUsingMouseWheel()                                            {
// SetItemKeyOwner(ImGuiKey_MouseWheelY); }      // Changed in 1.89 inline bool
// TreeNodeBehaviorIsOpen(ImGuiID id, ImGuiTreeNodeFlags flags = 0)    { return
// TreeNodeUpdateNextOpen(id, flags); }   // Renamed in 1.89 inline bool   IsKeyPressedMap(ImGuiKey
// key, bool repeat = true)                   { IM_ASSERT(IsNamedKey(key)); return IsKeyPressed(key,
// repeat); } // Removed in 1.87: Mapping from named key is always identity!

// Refactored focus/nav/tabbing system in 1.82 and 1.84. If you have old/custom copy-and-pasted
// widgets which used FocusableItemRegister():
//  (Old) IMGUI_VERSION_NUM  < 18209: using 'ItemAdd(....)'                              and 'bool
//  tab_focused = FocusableItemRegister(...)' (Old) IMGUI_VERSION_NUM >= 18209: using 'ItemAdd(...,
...
//  ImGuiItemFlags_Inputable)'     and 'bool tab_focused = (g.NavActivateId == id &&
//  (g.NavActivateFlags & ImGuiActivateFlags_PreferInput))'
// inline bool   FocusableItemRegister(ImGuiWindow* window, ImGuiID id)              // -> pass
// ImGuiItemAddFlags_Inputable flag to ItemAdd() inline void   FocusableItemUnregister(ImGuiWindow*
// window)                        // -> unnecessary: TempInputText() uses
...
struct ImFontLoader {
  ...
  bool (*FontBakedInit)(ImFontAtlas* atlas, ImFontConfig* src, ImFontBaked* baked,
                        void* loader_data_for_baked_src);
  void (*FontBakedDestroy)(ImFontAtlas* atlas, ImFontConfig* src, ImFontBaked* baked,
                           void* loader_data_for_baked_src);
  bool (*FontBakedLoadGlyph)(ImFontAtlas* atlas, ImFontConfig* src, ImFontBaked* baked,
                             void* loader_data_for_baked_src, ImWchar codepoint,
                             ImFontGlyph* out_glyph, float* out_advance_x);

  // Size of backend data, Per Baked * Per Source. Buffers are managed by core to avoid excessive
  // allocations.
...
#endif
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
typedef ImFontLoader
    ImFontBuilderIO;  // [renamed/changed in 1.92] The types are not actually compatible but we
...
inline bool operator==(const ImTextureRef& lhs, const ImTextureRef& rhs) {
...
#define ImFontAtlasRectId_IndexMask_ \
  (0x0007FFFF)  // 20-bits signed: index to access builder->RectsIndex[].
#define ImFontAtlasRectId_GenerationMask_ \
  (0x3FF00000)  // 10-bits: entry generation, so each ID is unique and get can safely detected old
                // identifiers.
...
inline unsigned int ImFontAtlasRectId_GetGeneration(ImFontAtlasRectId id) {
  return (unsigned int)(id & ImFontAtlasRectId_GenerationMask_) >>
         ImFontAtlasRectId_GenerationShift_;
}
inline ImFontAtlasRectId ImFontAtlasRectId_Make(int index_idx, int gen_idx) {
  IM_ASSERT(index_idx >= 0 && index_idx <= ImFontAtlasRectId_IndexMask_ &&
            gen_idx <= (ImFontAtlasRectId_GenerationMask_ >> ImFontAtlasRectId_GenerationShift_));
...
// We handle this with an indirection. While Rects[] may be in theory shuffled, compacted etc.,
// RectsIndex[] cannot it is keyed by ImFontAtlasRectId. RectsIndex[] is used both as an index into
// Rects[] and an index into itself. This is basically a free-list. See
// ImFontAtlasBuildAllocRectIndexEntry() code. Having this also makes it easier to e.g. sort
// rectangles during repack.
struct ImFontAtlasRectEntry {
  int TargetIndex : 20;  // When Used: ImFontAtlasRectId -> into Rects[]. When unused: index to next
                         // unused RectsIndex[] slot to consume free-list.
...
// We avoid dragging imstb_rectpack.h into public header (partly because binding generators are
...
struct ImFontAtlasBuilder {
  stbrp_context_opaque
      PackContext;  // Actually 'stbrp_context' but we don't want to define this in the header file.
  ...
  int RectsPackedSurface;  // Number of packed pixels. Used when compacting to heuristically find
                           // the ideal texture size.
  ...
  bool PreloadedAllGlyphsRanges;  // Set when missing ImGuiBackendFlags_RendererHasTextures features
                                  // forces atlas to preload everything.
  ...
  ImFontAtlasRectId PackIdMouseCursors;  // White pixel + mouse cursors. Also happen to be fallback
                                         // in case of packing failure.
...
IMGUI_API void ImFontAtlasBuildRenderBitmapFromString(ImFontAtlas* atlas, int x, int y, int w,
                                                      int h, const char* in_str,
                                                      char in_marker_char);
...
IMGUI_API void ImFontAtlasBuildSetupFontSpecialGlyphs(ImFontAtlas* atlas, ImFont* font,
                                                      ImFontConfig* src);
IMGUI_API void ImFontAtlasBuildLegacyPreloadAllGlyphRanges(ImFontAtlas* atlas);  // Legacy
IMGUI_API void ImFontAtlasBuildGetOversampleFactors(ImFontConfig* src, ImFontBaked* baked,
...
IMGUI_API bool ImFontAtlasFontInitOutput(
    ImFontAtlas* atlas, ImFont* font);  // Using FontDestroyOutput/FontInitOutput sequence useful
                                        // notably if font loader params have changed
...
IMGUI_API ImFontBaked* ImFontAtlasBakedGetClosestMatch(ImFontAtlas* atlas, ImFont* font,
                                                       float font_size,
                                                       float font_rasterizer_density);
IMGUI_API ImFontBaked* ImFontAtlasBakedAdd(ImFontAtlas* atlas, ImFont* font, float font_size,
...
IMGUI_API ImFontGlyph* ImFontAtlasBakedAddFontGlyph(ImFontAtlas* atlas, ImFontBaked* baked,
                                                    ImFontConfig* src, const ImFontGlyph* in_glyph);
IMGUI_API void ImFontAtlasBakedAddFontGlyphAdvancedX(ImFontAtlas* atlas, ImFontBaked* baked,
                                                     ImFontConfig* src, ImWchar codepoint,
                                                     float advance_x);
IMGUI_API void ImFontAtlasBakedDiscardFontGlyph(ImFontAtlas* atlas, ImFont* font,
                                                ImFontBaked* baked, ImFontGlyph* glyph);
IMGUI_API void ImFontAtlasBakedSetFontGlyphBitmap(ImFontAtlas* atlas, ImFontBaked* baked,
                                                  ImFontConfig* src, ImFontGlyph* glyph,
                                                  ImTextureRect* r, const unsigned char* src_pixels,
...
IMGUI_API void ImFontAtlasUpdateDrawListsTextures(ImFontAtlas* atlas, ImTextureRef old_tex,
                                                  ImTextureRef new_tex);
IMGUI_API void ImFontAtlasUpdateDrawListsSharedData(ImFontAtlas* atlas);

IMGUI_API void ImFontAtlasTextureBlockConvert(const unsigned char* src_pixels,
                                              ImTextureFormat src_fmt, int src_pitch,
...
IMGUI_API void ImFontAtlasTextureBlockPostProcessMultiply(ImFontAtlasPostProcessData* data,
                                                          float multiply_factor);
IMGUI_API void ImFontAtlasTextureBlockFill(ImTextureData* dst_tex, int dst_x, int dst_y, int w,
                                           int h, ImU32 col);
IMGUI_API void ImFontAtlasTextureBlockCopy(ImTextureData* src_tex, int src_x, int src_y,
                                           ImTextureData* dst_tex, int dst_x, int dst_y, int w,
                                           int h);
IMGUI_API void ImFontAtlasTextureBlockQueueUpload(ImFontAtlas* atlas, ImTextureData* tex, int x,
                                                  int y, int w, int h);
...
IMGUI_API bool ImFontAtlasGetMouseCursorTexData(ImFontAtlas* atlas, ImGuiMouseCursor cursor_type,
...
extern void ImGuiTestEngineHook_ItemAdd(
    ImGuiContext* ctx, ImGuiID id, const ImRect& bb,
    const ImGuiLastItemData* item_data);  // item_data may be NULL
extern void ImGuiTestEngineHook_ItemInfo(ImGuiContext* ctx, ImGuiID id, const char* label,

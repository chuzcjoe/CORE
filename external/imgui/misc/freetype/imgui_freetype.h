...
enum ImGuiFreeTypeLoaderFlags_ {
  ...
  ImGuiFreeTypeLoaderFlags_LightHinting =
      1
      << 3,  // A lighter hinting algorithm for gray-level modes. Many generated glyphs are fuzzier
             // but better resemble their original shape. This is achieved by snapping glyphs to the
             // pixel grid only vertically (Y-axis), as is done by Microsoft's ClearType and Adobe's
             // proprietary font renderer. This preserves inter-glyph spacing in horizontal text.
...
// - you can manually assign this builder with
// 'atlas->SetFontLoader(ImGuiFreeType::GetFontLoader())'
// - prefer deep-copying this into your own ImFontLoader instance if you use hot-reloading that
// messes up static data.
...
// Display UI to edit ImFontAtlas::FontLoaderFlags (shared) or ImFontConfig::FontLoaderFlags (single
// source)
IMGUI_API bool DebugEditFontLoaderFlags(ImGuiFreeTypeLoaderFlags* p_font_loader_flags);

// Obsolete names (will be removed)
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
// IMGUI_API const ImFontBuilderIO* GetBuilderForFreeType(); // Renamed/changed in 1.92. Change
// 'io.Fonts->FontBuilderIO = ImGuiFreeType::GetBuilderForFreeType()' to
// 'io.Fonts->SetFontLoader(ImGuiFreeType::GetFontLoader())' if you need runtime selection. static
// inline bool BuildFontAtlas(ImFontAtlas* atlas, unsigned int flags = 0) { atlas->FontBuilderIO =
// GetBuilderForFreeType(); atlas->FontLoaderFlags = flags; return atlas->Build(); } // Prefer using

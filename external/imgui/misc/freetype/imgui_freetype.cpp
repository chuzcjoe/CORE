...
//  2025/06/11: refactored for the new ImFontLoader architecture, and
//  ImGuiBackendFlags_RendererHasTextures support. 2024/10/17: added plutosvg support for SVG Fonts
//  (seems faster/better than lunasvg). Enable by using '#define IMGUI_ENABLE_FREETYPE_PLUTOSVG'.
//  (#7927) 2023/11/13: added support for ImFontConfig::RasterizationDensity field for scaling
//  render density without scaling metrics. 2023/08/01: added support for SVG fonts, enable by using
//  '#define IMGUI_ENABLE_FREETYPE_LUNASVG'. (#6591) 2023/01/04: fixed a packing issue which in some
//  occurrences would prevent large amount of glyphs from being packed correctly. 2021/08/23: fixed
//  crash when FT_Render_Glyph() fails to render a glyph and returns nullptr. 2021/03/05: added
//  ImGuiFreeTypeBuilderFlags_Bitmap to load bitmap glyphs. 2021/03/02: set
//  'atlas->TexPixelsUseColors = true' to help some backends with deciding of a preferred texture
//  format. 2021/01/28: added support for color-layered glyphs via
//  ImGuiFreeTypeBuilderFlags_LoadColor (require Freetype 2.10+). 2021/01/26: simplified integration
//  by using '#define IMGUI_ENABLE_FREETYPE'. renamed ImGuiFreeType::XXX flags to
//  ImGuiFreeTypeBuilderFlags_XXX for consistency with other API. removed
//  ImGuiFreeType::BuildFontAtlas(). 2020/06/04: fix for rare case where FT_Get_Char_Index() succeed
//  but FT_Load_Glyph() fails. 2019/02/09: added RasterizerFlags::Monochrome flag to disable font
//  anti-aliasing (combine with ::MonoHinting for best results!) 2019/01/15: added support for imgui
//  allocators + added FreeType only override function SetAllocatorFunctions(). 2019/01/10:
//  re-factored to match big update in STB builder. fixed texture height waste. fixed redundant
//  glyphs when merging. support for glyph padding. 2018/06/08: added support for
//  ImFontConfig::GlyphMinAdvanceX, GlyphMaxAdvanceX. 2018/02/04: moved to main imgui repository
//  (away from http://www.github.com/ocornut/imgui_club) 2018/01/22: fix for addition of
//  ImFontAtlas::TexUvscale member. 2017/10/22: minor inconsequential change to match change in
//  master (removed an unnecessary statement). 2017/09/26: fixes for imgui internal changes.
//  2017/08/26: cleanup, optimizations, support for ImFontConfig::RasterizerFlags,
//  ImFontConfig::RasterizerMultiply. 2017/08/16: imported from
//  https://github.com/Vuhdo/imgui_freetype into http://www.github.com/ocornut/imgui_club, updated
//  for latest changes in ImFontAtlas, minor tweaks.

// About Gamma Correct Blending:
// - FreeType assumes blending in linear space rather than gamma space.
// - See https://www.freetype.org/freetype2/docs/reference/ft2-base_interface.html#FT_Render_Glyph
// - For correct results you need to be using sRGB and convert to linear space in the pixel shader
// output.
// - The default dear imgui styles will be impacted by this change (alpha values will need
// tweaking).

// FIXME: cfg.OversampleH, OversampleV are not supported, but generally not necessary with this
// rasterizer because Hinting makes everything look better.
...
#ifndef __clang__
#pragma GCC diagnostic ignored "-Wsubobject-linkage"  // warning: 'xxxx' has a field 'xxxx' whose
...
static void* (*GImGuiFreeTypeAllocFunc)(size_t size,
...
#define FT_CEIL(X) \
  (((X + 63) & -64) / 64)  // From SDL_ttf: Handy routines for converting from fixed point
...
struct ImGui_ImplFreeType_FontSrcData {
  // Initialize from an external data buffer. Doesn't copy data, and you must ensure it stays valid
  // up to this object lifetime.
  bool InitFont(FT_Library ft_library, ImFontConfig* src,
                ImGuiFreeTypeLoaderFlags extra_user_flags);
  void CloseFont();
...
// Stored in ImFontBaked::FontLoaderDatas: pointer to SourcesCount instances of this. ALLOCATED BY
// CORE.
...
bool ImGui_ImplFreeType_FontSrcData::InitFont(FT_Library ft_library, ImFontConfig* src,
                                              ImGuiFreeTypeLoaderFlags extra_font_loader_flags) {
  FT_Error error = FT_New_Memory_Face(ft_library, (uint8_t*)src->FontData,
                                      (FT_Long)src->FontDataSize, (FT_Long)src->FontNo, &FtFace);
  ...
  if (UserFlags & ImGuiFreeTypeLoaderFlags_NoAutoHint) LoadFlags |= FT_LOAD_NO_AUTOHINT;
  if (UserFlags & ImGuiFreeTypeLoaderFlags_ForceAutoHint) LoadFlags |= FT_LOAD_FORCE_AUTOHINT;
  ...
  if (UserFlags & ImGuiFreeTypeLoaderFlags_LoadColor) LoadFlags |= FT_LOAD_COLOR;
...
static const FT_Glyph_Metrics* ImGui_ImplFreeType_LoadGlyph(
    ImGui_ImplFreeType_FontSrcData* src_data, uint32_t codepoint) {
  ...
  // You can use FreeType 2.10, or the patched version of 2.11.0 in VcPkg, or probably any upcoming
  // FreeType version.
  ...
  IM_ASSERT(slot->format != FT_GLYPH_FORMAT_SVG &&
            ...
            "IMGUI_ENABLE_FREETYPE_LUNASVG in imconfig.h and install required libraries in order "
            "to use this font");
#endif
  IM_ASSERT(slot->format == FT_GLYPH_FORMAT_OUTLINE || slot->format == FT_GLYPH_FORMAT_BITMAP);
  ...
  if (src_data->UserFlags & ImGuiFreeTypeLoaderFlags_Bold) FT_GlyphSlot_Embolden(slot);
...
static void ImGui_ImplFreeType_BlitGlyph(const FT_Bitmap* ft_bitmap, uint32_t* dst,
                                         uint32_t dst_pitch) {
  ...
  switch (ft_bitmap->pixel_mode) {
    case FT_PIXEL_MODE_GRAY:  // Grayscale image, 1 byte per pixel.
    {
      for (uint32_t y = 0; y < h; y++, src += src_pitch, dst += dst_pitch)
        for (uint32_t x = 0; x < w; x++) dst[x] = IM_COL32(255, 255, 255, src[x]);
      break;
    }
...
static void ImGui_ImplFreeType_FontSrcDestroy(ImFontAtlas* atlas, ImFontConfig* src) {
  IM_UNUSED(atlas);
  ImGui_ImplFreeType_FontSrcData* bd_font_data =
      (ImGui_ImplFreeType_FontSrcData*)src->FontLoaderData;
  IM_DELETE(bd_font_data);
  src->FontLoaderData = nullptr;
}

static bool ImGui_ImplFreeType_FontBakedInit(ImFontAtlas* atlas, ImFontConfig* src,
                                             ImFontBaked* baked, void* loader_data_for_baked_src) {
  IM_UNUSED(atlas);
  ...
  // Vuhdo 2017: "I'm not sure how to deal with font sizes properly. As far as I understand,
  // currently ImGui assumes that the 'pixel_height' is a maximum height of an any given glyph, i.e.
  // it's the sum of font's ascender and descender. Seems strange to me. FT_Set_Pixel_Sizes()
  // doesn't seem to get us the same result." (FT_Set_Pixel_Sizes() essentially calls
  // FT_Request_Size() with FT_SIZE_REQUEST_TYPE_NOMINAL)
  const float rasterizer_density = src->RasterizerDensity * baked->RasterizerDensity;
  FT_Size_RequestRec req;
  req.type = (bd_font_data->UserFlags & ImGuiFreeTypeLoaderFlags_Bitmap)
  ...
  if (src->MergeMode == false) {
    ...
    // LineSpacing     = (float)FT_CEIL(metrics.height) * scale;         // The baseline-to-baseline
    // distance. Note that it usually is larger than the sum of the ascender and descender taken as
    // absolute values. There is also no guarantee that no glyphs extend above or below subsequent
    // baselines when using this distance. Think of it as a value the designer of the font finds
    // appropriate. LineGap         = (float)FT_CEIL(metrics.height - metrics.ascender +
    // metrics.descender) * scale; // The spacing in pixels between one row's descent and the next
    // row's ascent. MaxAdvanceWidth = (float)FT_CEIL(metrics.max_advance) * scale;    // This field
    // gives the maximum horizontal cursor advance for all glyphs in the font.
  }
  return true;
}

static void ImGui_ImplFreeType_FontBakedDestroy(ImFontAtlas* atlas, ImFontConfig* src,
                                                ImFontBaked* baked,
...
static bool ImGui_ImplFreeType_FontBakedLoadGlyph(ImFontAtlas* atlas, ImFontConfig* src,
                                                  ImFontBaked* baked,
                                                  void* loader_data_for_baked_src,
  ...
  if (error != 0 || ft_bitmap == nullptr) return false;
  ...
  if (is_visible) {
    ...
    ImFontAtlasBakedSetFontGlyphBitmap(atlas, baked, src, out_glyph, r,
                                       (const unsigned char*)temp_buffer, ImTextureFormat_RGBA32,
                                       w * 4);
  }
...
static bool ImGui_ImplFreetype_FontSrcContainsGlyph(ImFontAtlas* atlas, ImFontConfig* src,
                                                    ImWchar codepoint) {
  IM_UNUSED(atlas);
  ImGui_ImplFreeType_FontSrcData* bd_font_data =
      (ImGui_ImplFreeType_FontSrcData*)src->FontLoaderData;
...
bool ImGuiFreeType::DebugEditFontLoaderFlags(unsigned int* p_font_loader_flags) {
  bool edited = false;
  edited |=
      ImGui::CheckboxFlags("NoHinting", p_font_loader_flags, ImGuiFreeTypeLoaderFlags_NoHinting);
  edited |=
      ImGui::CheckboxFlags("NoAutoHint", p_font_loader_flags, ImGuiFreeTypeLoaderFlags_NoAutoHint);
  edited |= ImGui::CheckboxFlags("ForceAutoHint", p_font_loader_flags,
  ...
  edited |= ImGui::CheckboxFlags("MonoHinting", p_font_loader_flags,
  ...
  edited |=
      ImGui::CheckboxFlags("Monochrome", p_font_loader_flags, ImGuiFreeTypeLoaderFlags_Monochrome);
  edited |=
      ImGui::CheckboxFlags("LoadColor", p_font_loader_flags, ImGuiFreeTypeLoaderFlags_LoadColor);
...
// https://gitlab.freedesktop.org/freetype/freetype-demos/-/blob/master/src/rsvg-port.c The original
// code from the demo is licensed under CeCILL-C Free Software License Agreement
...
static FT_Error ImGuiLunasvgPortRender(FT_GlyphSlot slot, FT_Pointer* _state) {
  ...
  // If there was an error while loading the svg in ImGuiLunasvgPortPresetSlot(), the renderer hook
...
#else
  state->svg->setMatrix(
      state->svg->matrix().identity());       // Reset the svg matrix to the default value
  state->svg->render(bitmap, state->matrix);  // state->matrix is already scaled and translated
#endif
  state->err = FT_Err_Ok;
  return state->err;
...
static FT_Error ImGuiLunasvgPortPresetSlot(FT_GlyphSlot slot, FT_Bool cache, FT_Pointer* _state) {
  ...
  // This function is called twice, once in the FT_Load_Glyph() and another right before
  // ImGuiLunasvgPortRender(). If it's the latter, don't do anything because it's // already done in
  // the former.
  if (cache) return state->err;

  state->svg = lunasvg::Document::loadFromData((const char*)document->svg_document,
  ...
  // Compute all the bearings and set them correctly. The outline is scaled already, we just need to
  ...
  slot->metrics.width = FT_Pos(
      IM_ROUND(metrics_width * 64.0));  // Using IM_ROUND() assume width and height are positive

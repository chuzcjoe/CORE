...
// - Base85:   ~5 bytes of source code for 4 bytes of input data. 5 bytes stored in binary
// (suggested by @mmalex).
// - As int:  ~11 bytes of source code for 4 bytes of input data. 4 bytes stored in binary.
// Endianness dependent, need swapping on big-endian CPU.
// - As char: ~12 bytes of source code for 4 bytes of input data. 4 bytes stored in binary. Not
// endianness dependent. Load compressed TTF fonts with
...
int main(int argc, char **argv) {
  if (argc < 3) {
    ...
    printf(
        " -u32    = ~11 bytes of source per 4 bytes of data. 4 bytes in binary. Need endianness "
        "swapping on big-endian.\n");
    printf(
        " -base85 =  ~5 bytes of source per 4 bytes of data. 5 bytes in binary. Need decoder.\n");
    return 0;
  }
  ...
  while (argn < (argc - 2) && argv[argn][0] == '-') {
    if (strcmp(argv[argn], "-u8") == 0) {
      source_encoding = SourceEncoding_U8;
      argn++;
    } else if (strcmp(argv[argn], "-u32") == 0) {
      source_encoding = SourceEncoding_U32;
      argn++;
    } else if (strcmp(argv[argn], "-base85") == 0) {
      source_encoding = SourceEncoding_Base85;
      argn++;
  ...
  bool ret = binary_to_compressed_c(argv[argn], argv[argn + 1], source_encoding, use_compression,
                                    use_static);
  if (!ret) fprintf(stderr, "Error opening or reading file: '%s'\n", argv[argn]);
  return ret ? 0 : 1;
}
...
bool binary_to_compressed_c(const char *filename, const char *symbol,
                            SourceEncoding source_encoding, bool use_compression, bool use_static) {
  // Read file
  ...
  int compressed_sz =
      use_compression ? stb_compress((stb_uchar *)compressed, (stb_uchar *)data, data_sz) : data_sz;
  if (use_compression) memset(compressed + compressed_sz, 0, maxlen - compressed_sz);
  ...
  if (source_encoding == SourceEncoding_Base85) {
    ...
    for (int src_i = 0; src_i < compressed_sz; src_i += 4) {
      // This is made a little more complicated by the fact that ??X sequences are interpreted as
      // trigraphs by old C/C++ compilers. So we need to escape pairs of ??.
      ...
      if ((src_i % 112) == 112 - 4) fprintf(out, "\"\n    \"");
    }
    fprintf(out, "\";\n\n");
    ...
    for (int i = 0; i < compressed_sz; i++) {
      ...
      if (column == 0) fprintf(out, "\n    ");
      column += fprintf(out, "%d,", d);
      if (column >= 180) column = 0;
    }
    fprintf(out, "\n};\n\n");
  ...
  // Cleanup
  delete[] data;
  if (use_compression) delete[] compressed;
  return true;
}

...
#pragma clang diagnostic ignored "-Wunused-macros"  // warning: macro is not used // we define
                                                    // snprintf/vsnprintf on Windows so they are
...
// For InputTextEx()
static bool InputTextFilterCharacter(ImGuiContext* ctx, unsigned int* p_char,
                                     ImGuiInputTextFlags flags, ImGuiInputTextCallback callback,
                                     void* user_data, bool input_source_is_clipboard = false);
static ImVec2 InputTextCalcTextSize(ImGuiContext* ctx, const char* text_begin,
                                    const char* text_end_display, const char* text_end,
                                    const char** out_remaining = NULL, ImVec2* out_offset = NULL,
...
void ImGui::TextEx(const char* text, const char* text_end, ImGuiTextFlags flags) {
  ...
  if (text_end == NULL) text_end = text + ImStrlen(text);  // FIXME-OPT
  ...
  if (text_end - text <= 2000 || wrap_enabled) {
    ...
    // - From this point we will only compute the width of lines that are visible. Optimization only
    // available when word-wrapping is disabled.
...
void ImGui::TextWrappedV(const char* fmt, va_list args) {
  ImGuiContext& g = *GImGui;
  const bool need_backup = (g.CurrentWindow->DC.TextWrapPos <
...
// FIXME-WIP: Works but API is likely to be reworked. This is designed for 1 item on the line.
// (#7024)
void ImGui::TextAlignedV(float align_x, float size_x, const char* fmt, va_list args) {
  ...
  if (align_x > 0.0f && text_size.x < size_x) pos.x += ImTrunc((size_x - text_size.x) * align_x);
  ...
  window->DC.CursorMaxPos.x = backup_max_pos.x;  // Cancel out extending content size because
                                                 // right-aligned text would otherwise mess it up.

  if (size_x < text_size.x &&
      IsItemHovered(ImGuiHoveredFlags_NoNavOverride | ImGuiHoveredFlags_AllowWhenDisabled |
...
void ImGui::LabelTextV(const char* label, const char* fmt, va_list args) {
  ...
  const ImRect total_bb(
      pos, pos + ImVec2(w + (label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f),
  ...
  if (label_size.x > 0.0f)
    RenderText(
        ImVec2(value_bb.Max.x + style.ItemInnerSpacing.x, value_bb.Min.y + style.FramePadding.y),
        label);
}
...
// - FIXME: For refactor we could output flags, incl mouse hovered vs nav keyboard vs nav triggered
// etc.
//   And better standardize how widgets use 'GetColor32((held && hovered) ? ... : hovered ? ...)' vs
//   'GetColor32(held ? ... : hovered ? ...);' For mouse feedback we typically prefer the 'held &&
//   hovered' test, but for nav feedback not always. Outputting hovered=true on Activation may be
//   misleading.
// - Since v1.91.2 (Sept 2024) we included io.ConfigDebugHighlightIdConflicts feature.
//   One idiom which was previously valid which will now emit a warning is when using multiple
//   overlaid ButtonBehavior() with same ID and different MouseButton (see #8030). You can fix it
//   by:
//       (1) switching to use a single ButtonBehavior() with multiple _MouseButton flags.
//    or (2) surrounding those calls with PushItemFlag(ImGuiItemFlags_AllowDuplicateId, true); ...
//    PopItemFlag()
bool ImGui::ButtonBehavior(const ImRect& bb, ImGuiID id, bool* out_hovered, bool* out_held,
  ...
  // Note that _both_ ButtonFlags and ItemFlags are valid sources, so copy one into the item_flags
  // and only check that.
  ImGuiItemFlags item_flags =
      (g.LastItemData.ID == id ? g.LastItemData.ItemFlags : g.CurrentItemFlags);
  if (flags & ImGuiButtonFlags_AllowOverlap) item_flags |= ImGuiItemFlags_AllowOverlap;
  if (item_flags & ImGuiItemFlags_NoFocus)
    flags |= ImGuiButtonFlags_NoFocus | ImGuiButtonFlags_NoNavFocus;

  // Default only reacts to left mouse button
  if ((flags & ImGuiButtonFlags_MouseButtonMask_) == 0) flags |= ImGuiButtonFlags_MouseButtonLeft;
  ...
  if (g.LastItemData.ID != id) IMGUI_TEST_ENGINE_ITEM_ADD(id, bb, NULL);
#endif
  ...
  if (hovered) {
    ...
    // - Technically we only need some values in one code path, but since this is gated by hovered
    // test this is fine.
    int mouse_button_clicked = -1;
    int mouse_button_released = -1;
    for (int button = 0; button < 3; button++)
      if (flags & (ImGuiButtonFlags_MouseButtonLeft
                   << button))  // Handle ImGuiButtonFlags_MouseButtonRight and
                                // ImGuiButtonFlags_MouseButtonMiddle here.
      {
        if (IsMouseClicked(button, ImGuiInputFlags_None, test_owner_id) &&
    ...
    if (mods_ok) {
          ...
          if (!(flags & ImGuiButtonFlags_NoNavFocus)) {
            ...
            FocusWindow(
                ...
                ImGuiFocusRequestFlags_RestoreFocusedChild);  // Still need to focus and bring to
                                                              // front, but try to avoid losing
                                                              // NavId when navigating a child
          }
        }
      ...
      if (flags & ImGuiButtonFlags_PressedOnRelease) {
        if (mouse_button_released != -1) {
          ...
          if (!(flags & ImGuiButtonFlags_NoNavFocus))
            SetFocusID(id, window);  // FIXME: Lack of FocusWindow() call here is inconsistent with
  ...
  // We report navigated and navigation-activated items as hovered but we don't set g.HoveredId to
  ...
  if (g.NavActivateDownId == id) {
    ...
    if (!nav_activated_by_inputs && (item_flags & ImGuiItemFlags_ButtonRepeat)) {
      ...
      nav_activated_by_inputs =
          CalcTypematicRepeatAmount(t1 - g.IO.DeltaTime, t1, g.IO.KeyRepeatDelay,
    ...
    if (nav_activated_by_code || nav_activated_by_inputs) {
      ...
      if (!(flags & ImGuiButtonFlags_NoNavFocus) &&
        ...
        SetFocusID(id, window);
      if (g.NavActivateFlags & ImGuiActivateFlags_FromShortcut) g.ActiveIdFromShortcut = true;
    }
  }
  ...
  if (g.ActiveId == id) {
    if (g.ActiveIdSource == ImGuiInputSource_Mouse) {
      ...
      if (mouse_button == -1) {
        // Fallback for the rare situation were g.ActiveId was set programmatically or from another
        // widget (e.g. #6304).
        ClearActiveID();
    ...
    } else if (g.ActiveIdSource == ImGuiInputSource_Keyboard ||
               g.ActiveIdSource == ImGuiInputSource_Gamepad) {
      // When activated using Nav, we hold on the ActiveID until activation button is released
      if (g.NavActivateDownId == id)
        held = true;  // hovered == true not true as we are already likely hovered on direct
                      // activation.
      else
        ClearActiveID();
    }
...
bool ImGui::CloseButton(ImGuiID id, const ImVec2& pos) {
  ...
  // Tweak 1: Shrink hit-testing area if button covers an abnormally large proportion of the visible
  // region. That's in order to facilitate moving the window away. (#3825) This may better be
  // applied as a general hit-rect reduction mechanism for all widgets to ensure the area to move
  // window is always accessible?
  ...
  // Alt,Right,Activate sequence can always close a window. (this isn't the common behavior of
  // buttons, but it doesn't affect the user because navigation tends to keep items visible in
  // scrolling layer).
  ...
  if (hovered) window->DrawList->AddRectFilled(bb.Min, bb.Max, bg_col);
  ...
  window->DrawList->AddLine(cross_center + ImVec2(+cross_extent, +cross_extent),
                            cross_center + ImVec2(-cross_extent, -cross_extent), cross_col,
                            cross_thickness);
  window->DrawList->AddLine(cross_center + ImVec2(+cross_extent, -cross_extent),
                            cross_center + ImVec2(-cross_extent, +cross_extent), cross_col,
                            cross_thickness);
...
bool ImGui::CollapseButton(ImGuiID id, const ImVec2& pos) {
  ...
  ImU32 bg_col = GetColorU32((held && hovered) ? ImGuiCol_ButtonActive
                             : hovered         ? ImGuiCol_ButtonHovered
                                               : ImGuiCol_Button);
  ImU32 text_col = GetColorU32(ImGuiCol_Text);
  if (hovered || held) window->DrawList->AddRectFilled(bb.Min, bb.Max, bg_col);
  RenderNavCursor(bb, id, ImGuiNavRenderCursorFlags_Compact);
...
ImRect ImGui::GetWindowScrollbarRect(ImGuiWindow* window, ImGuiAxis axis) {
  ...
  if (axis == ImGuiAxis_X)
    ...
    return ImRect(ImMax(outer_rect.Min.x, outer_rect.Max.x - border_size - scrollbar_size),
                  inner_rect.Min.y + border_top, outer_rect.Max.x - border_size,
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v,
                        ImS64 size_visible_v, ImS64 size_contents_v,
                        ImDrawFlags draw_rounding_flags) {
  ImGuiContext& g = *GImGui;
  ImGuiWindow* window = g.CurrentWindow;
  if (window->SkipItems) return false;
  ...
  ImRect bb = bb_frame;
  float padding =
      IM_TRUNC(ImMin(style.ScrollbarPadding, ImMin(bb_frame_width, bb_frame_height) * 0.5f));
  bb.Expand(-padding);
  ...
  // total scrollable amount) But we maintain a minimum size in pixel to allow for the user to still
  // aim inside.
  ...
  const float grab_h_pixels =
      ImClamp(scrollbar_size_v * ((float)size_visible_v / (float)win_size_v), grab_h_minsize,
              scrollbar_size_v);
  ...
  if (held && allow_interaction && grab_h_norm < 1.0f) {
    ...
    if (g.ActiveIdIsJustActivated) {
      // On initial click when held_dir == 0 (clicked over grab): calculate the distance between
      // mouse and the center of the grab
      ...
      g.ScrollbarClickDeltaToGrabCenter = (held_dir == 0 && !g.IO.KeyShift)
    ...
    if (g.ScrollbarSeekMode == 0) {
      ...
      *p_scroll_v = (ImS64)(scroll_v_norm * scroll_max);
      ...
      // Page by page
      if (IsMouseClicked(ImGuiMouseButton_Left, ImGuiInputFlags_Repeat) &&
          held_dir == g.ScrollbarSeekMode) {
        float page_dir = (g.ScrollbarSeekMode > 0.0f) ? +1.0f : -1.0f;
        *p_scroll_v =
            ImClamp(*p_scroll_v + (ImS64)(page_dir * size_visible_v), (ImS64)0, scroll_max);
      }
    }
...
void ImGui::ImageWithBg(ImTextureRef tex_ref, const ImVec2& image_size, const ImVec2& uv0,
  ...
  window->DrawList->AddImage(tex_ref, bb.Min + padding, bb.Max - padding, uv0, uv1,
                             GetColorU32(tint_col));
}

void ImGui::Image(ImTextureRef tex_ref, const ImVec2& image_size, const ImVec2& uv0,
                  const ImVec2& uv1) {
...
void ImGui::Image(ImTextureRef tex_ref, const ImVec2& image_size, const ImVec2& uv0,
  ...
  PushStyleVar(ImGuiStyleVar_ImageBorderSize,
               (border_col.w > 0.0f) ? ImMax(1.0f, g.Style.ImageBorderSize)
                                     : 0.0f);  // Preserve legacy behavior where border is always
                                               // visible when border_col's Alpha is >0.0f
...
bool ImGui::ImageButtonEx(ImGuiID id, ImTextureRef tex_ref, const ImVec2& image_size,
...
// - ImageButton() adds style.FramePadding*2.0f to provided size. This is in order to facilitate
...
// - old ImageButton() used ImTextureID as item id (created issue with multiple buttons with same
...
/*
bool ImGui::ImageButton(ImTextureID user_texture_id, const ImVec2& size, const ImVec2& uv0, const
ImVec2& uv1, int frame_padding, const ImVec4& bg_col, const ImVec4& tint_col)
{
    // Default to using texture ID as ID. User can still push string/integer prefixes.
    PushID((ImTextureID)(intptr_t)user_texture_id);
    if (frame_padding >= 0)
        PushStyleVar(ImGuiStyleVar_FramePadding, ImVec2((float)frame_padding,
(float)frame_padding)); bool ret = ImageButton("", user_texture_id, size, uv0, uv1, bg_col,
tint_col); if (frame_padding >= 0) PopStyleVar(); PopID(); return ret;
}
*/
...
bool ImGui::RadioButton(const char* label, bool active) {
  ...
  if (style.FrameBorderSize > 0.0f) {
    ...
    window->DrawList->AddCircle(center, radius, GetColorU32(ImGuiCol_Border), num_segment,
...
void ImGui::ProgressBar(float fraction, const ImVec2& size_arg, const char* overlay) {
  ...
  // The value must be animated along with time, so e.g. passing '-1.0f * ImGui::GetTime()' as
  // fraction works.
  ...
  RenderRectFilledRangeH(window->DrawList, bb, GetColorU32(ImGuiCol_PlotHistogram), fill_n0,
                         fill_n1, style.FrameRounding);
  ...
  if (!is_indeterminate || overlay != NULL) {
    ...
    if (overlay_size.x > 0.0f) {
      ...
      RenderTextClipped(
          ImVec2(ImClamp(text_x, bb.Min.x, bb.Max.x - overlay_size.x - style.ItemInnerSpacing.x),
                 bb.Min.y),
...
bool ImGui::TextLink(const char* label) {
  ...
  {
    // FIXME-STYLE: Read comments above. This widget is NOT written in the same style as some
    // earlier widgets, as we are currently experimenting/planning a different styling system.
    float h, s, v;
...
void ImGui::NewLine() {
  ...
  if (window->DC.CurrLineSize.y > 0.0f)  // In the event that we are on a line with items that is
...
// FIXME: Surprisingly, this seemingly trivial widget is a victim of many different legacy/tricky
// layout issues. Note how thickness == 1.0f is handled specifically as not moving CursorPos by
// 'thickness', but other values are.
void ImGui::SeparatorEx(ImGuiSeparatorFlags flags, float thickness) {
  ...
  if (flags & ImGuiSeparatorFlags_Vertical) {
    ...
    const ImRect bb(ImVec2(window->DC.CursorPos.x, y1),
                    ImVec2(window->DC.CursorPos.x + thickness, y2));
    ...
    // We currently don't need to provide the same feature for tables because tables naturally have
    // border features.
    ...
    // FIXME: This prevents ->CursorMaxPos based bounding box evaluation from working (e.g.
    // TableEndCell)
    const float thickness_for_layout =
        (thickness == 1.0f) ? 0.0f
                            : thickness;  // FIXME: See 1.70/1.71 Separator() change: makes legacy
                                          // 1-px separator not affect layout yet. Should change.
...
void ImGui::Separator() {
  ...
  // FIXME: We cannot g.Style.SeparatorTextBorderSize for thickness as it relates to SeparatorText()
  // which is a decorated separator, not defaulting to 1.0f.
  ...
  if (window->DC.CurrentColumns) flags |= ImGuiSeparatorFlags_SpanAllColumns;
...
void ImGui::SeparatorTextEx(ImGuiID id, const char* label, const char* label_end, float extra_w) {
  ...
  const float text_baseline_y = ImTrunc(
      (bb.GetHeight() - label_size.y) * style.SeparatorTextAlign.y +
  ...
  if (label_size.x > 0.0f) {
    ...
    RenderTextEllipsis(window->DrawList, label_pos,
                       ImVec2(bb.Max.x, bb.Max.y + style.ItemSpacing.y), bb.Max.x, label, label_end,
                       &label_size);
  } else {
    if (g.LogEnabled) LogText("---");
...
void ImGui::SeparatorText(const char* label) {
  ...
  // The SeparatorText() vs SeparatorTextEx() distinction is designed to be considerate that we may
  // want:
  // - allow separator-text to be draggable items (would require a stable ID + a noticeable
  // highlight)
  // - this high-level entry point to allow formatting? (which in turns may require ID separate from
  // formatted string)
...
bool ImGui::SplitterBehavior(const ImRect& bb, ImGuiID id, ImGuiAxis axis, float* size1,
  ...
  if (!ItemAdd(bb, id, NULL, ImGuiItemFlags_NoNav)) return false;
  ...
  if (hovered)
    g.LastItemData.StatusFlags |=
        ImGuiItemStatusFlags_HoveredRect;  // for IsItemHovered(), because bb_interact is larger
                                           // than bb
...
void ImGui::ShrinkWidths(ImGuiShrinkWidthItem* items, int count, float width_excess,
                         float width_min) {
  if (count == 1) {
    if (items[0].Width >= 0.0f) items[0].Width = ImMax(items[0].Width - width_excess, width_min);
    return;
  }
  ImQsort(items, (size_t)count, sizeof(ImGuiShrinkWidthItem),
          ShrinkWidthItemComparer);  // Sort largest first, smallest last.
  ...
  while (width_excess > 0.001f && count_same_width < count) {
    ...
    for (int item_n = 0; item_n < count_same_width; item_n++) {
      float width_to_remove_for_this_item =
          ImMin(base_width_to_remove_per_item, items[item_n].Width - width_min);
  ...
  // Ensure that e.g. the right-most tab of a shrunk tab-bar always reaches exactly at the same
  // distance from the right-most edge of the tab bar separator.
  width_excess = 0.0f;
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags) {
  ...
  if (flags & ImGuiComboFlags_WidthFitPreview)
    IM_ASSERT((flags &
               (ImGuiComboFlags_NoPreview | (ImGuiComboFlags)ImGuiComboFlags_CustomPreview)) == 0);
  ...
  if (!(flags & ImGuiComboFlags_NoPreview))
    window->DrawList->AddRectFilled(
        bb.Min, ImVec2(value_x2, bb.Max.y), frame_col, style.FrameRounding,
  ...
  if (!(flags & ImGuiComboFlags_NoArrowButton)) {
    ...
    ImU32 text_col = GetColorU32(ImGuiCol_Text);
    window->DrawList->AddRectFilled(
        ImVec2(value_x2, bb.Min.y), bb.Max, bg_col, style.FrameRounding,
...
bool ImGui::BeginComboPopup(ImGuiID popup_id, const ImRect& bb, ImGuiComboFlags flags) {
  ...
  ImFormatString(name, IM_ARRAYSIZE(name), "##Combo_%02d",
                 g.BeginComboDepth);  // Recycle windows based on depth

  // Set position given a custom constraint (peak into expected window size so we can position it)
  // FIXME: This might be easier to express with an hypothetical SetNextWindowPosConstraints()
  // function?
  // FIXME: This might be moved to Begin() or at least around the same spot where Tooltips and other
  // Popups are calling FindBestWindowPosForPopupEx()?
    ...
    if (popup_window->WasActive) {
      ...
      popup_window->AutoPosLastDirection =
          (flags & ImGuiComboFlags_PopupAlignLeft)
              ? ImGuiDir_Left
              : ImGuiDir_Down;  // Left = "Below, Toward Left", Down = "Below, Toward Right
                                // (default)"
      ImRect r_outer = GetPopupAllowedExtentRect(popup_window);
      ImVec2 pos = FindBestWindowPosForPopupEx(bb.GetBL(), size_expected,
                                               &popup_window->AutoPosLastDirection, r_outer, bb,
                                               ImGuiPopupPositionPolicy_ComboBox);
      SetNextWindowPos(pos);
    }

  // We don't use BeginPopupEx() solely because we have a custom name string, which we could make an
  // argument to BeginPopupEx()
  ...
  if (!ret) {
    EndPopup();
    if (!g.IO.ConfigDebugBeginReturnValueOnce &&
        !g.IO.ConfigDebugBeginReturnValueLoop)  // Begin may only return false with those debug
                                                // tools activated.
...
// Call directly after the BeginCombo/EndCombo block. The preview is designed to only host
// non-interactive elements (Experimental, see GitHub issues: #1658, #4168)
bool ImGui::BeginComboPreview() {
  ...
  IM_ASSERT(
      ...
      g.LastItemData.Rect.Min.y ==
          preview_data->PreviewRect.Min.y);  // Didn't call after BeginCombo/EndCombo block or
...
void ImGui::EndComboPreview() {
  ...
  // FIXME: Using CursorMaxPos approximation instead of correct AABB which we will store in
  // ImDrawCmd in the future
...
bool ImGui::Combo(const char* label, int* current_item,
                  const char* (*getter)(void* user_data, int idx), void* user_data, int items_count,
...
static const char* ImGuiGetNameFromIndexOldToNewCallback(void* user_data, int idx) {
  ImGuiGetNameFromIndexOldToNewCallbackData* data =
      (ImGuiGetNameFromIndexOldToNewCallbackData*)user_data;
  const char* s = NULL;
...
bool ImGui::ListBox(const char* label, int* current_item,
                    bool (*old_getter)(void*, int, const char**), void* user_data, int items_count,
                    int height_in_items) {
  ImGuiGetNameFromIndexOldToNewCallbackData old_to_new_data = {user_data, old_getter};
  return ListBox(label, current_item, ImGuiGetNameFromIndexOldToNewCallback, &old_to_new_data,
...
bool ImGui::Combo(const char* label, int* current_item,
                  ...
                  int popup_max_height_in_items) {
  ImGuiGetNameFromIndexOldToNewCallbackData old_to_new_data = {user_data, old_getter};
  return Combo(label, current_item, ImGuiGetNameFromIndexOldToNewCallback, &old_to_new_data,
...
void ImGui::DataTypeApplyOp(ImGuiDataType data_type, int op, void* output, const void* arg1,
  ...
  switch (data_type) {
    ...
    case ImGuiDataType_S16:
      if (op == '+') {
        *(ImS16*)output =
            ImAddClampOverflow(*(const ImS16*)arg1, *(const ImS16*)arg2, IM_S16_MIN, IM_S16_MAX);
      }
      if (op == '-') {
        *(ImS16*)output =
            ImSubClampOverflow(*(const ImS16*)arg1, *(const ImS16*)arg2, IM_S16_MIN, IM_S16_MAX);
      }
      return;
    case ImGuiDataType_U16:
      if (op == '+') {
        *(ImU16*)output =
            ImAddClampOverflow(*(const ImU16*)arg1, *(const ImU16*)arg2, IM_U16_MIN, IM_U16_MAX);
      }
      if (op == '-') {
        *(ImU16*)output =
            ImSubClampOverflow(*(const ImU16*)arg1, *(const ImU16*)arg2, IM_U16_MIN, IM_U16_MAX);
      }
      return;
    case ImGuiDataType_S32:
      if (op == '+') {
        *(ImS32*)output =
            ImAddClampOverflow(*(const ImS32*)arg1, *(const ImS32*)arg2, IM_S32_MIN, IM_S32_MAX);
      }
      if (op == '-') {
        *(ImS32*)output =
            ImSubClampOverflow(*(const ImS32*)arg1, *(const ImS32*)arg2, IM_S32_MIN, IM_S32_MAX);
      }
      return;
    case ImGuiDataType_U32:
      if (op == '+') {
        *(ImU32*)output =
            ImAddClampOverflow(*(const ImU32*)arg1, *(const ImU32*)arg2, IM_U32_MIN, IM_U32_MAX);
      }
      if (op == '-') {
        *(ImU32*)output =
            ImSubClampOverflow(*(const ImU32*)arg1, *(const ImU32*)arg2, IM_U32_MIN, IM_U32_MAX);
      }
      return;
    case ImGuiDataType_S64:
      if (op == '+') {
        *(ImS64*)output =
            ImAddClampOverflow(*(const ImS64*)arg1, *(const ImS64*)arg2, IM_S64_MIN, IM_S64_MAX);
      }
      if (op == '-') {
        *(ImS64*)output =
            ImSubClampOverflow(*(const ImS64*)arg1, *(const ImS64*)arg2, IM_S64_MIN, IM_S64_MAX);
      }
      return;
    case ImGuiDataType_U64:
      if (op == '+') {
        *(ImU64*)output =
            ImAddClampOverflow(*(const ImU64*)arg1, *(const ImU64*)arg2, IM_U64_MIN, IM_U64_MAX);
      }
      if (op == '-') {
        *(ImU64*)output =
            ImSubClampOverflow(*(const ImU64*)arg1, *(const ImU64*)arg2, IM_U64_MIN, IM_U64_MAX);
      }
      return;
...
// mess..
bool ImGui::DataTypeApplyFromText(const char* buf, ImGuiDataType data_type, void* p_data,
                                  const char* format, void* p_data_when_empty) {
  ...
  // Sanitize format
  // - For float/double we have to ignore format with precision (e.g. "%.2f") because sscanf doesn't
  // take them in, so force them into %f and %lf
  // - In theory could treat empty format as using default, but this would only cover rare/bizarre
  // case of using InputScalar() + integer + format string without %.
  char format_sanitized[32];
  if (data_type == ImGuiDataType_Float || data_type == ImGuiDataType_Double)
    format = type_info->ScanFmt;
  else
    format =
        ImParseFormatSanitizeForScanning(format, format_sanitized, IM_ARRAYSIZE(format_sanitized));

  // Small types need a 32-bit buffer to receive the result from scanf()
  int v32 = 0;
  if (sscanf(buf, format, type_info->Size >= 4 ? p_data : &v32) < 1) return false;
  if (type_info->Size < 4) {
    if (data_type == ImGuiDataType_S8)
...
bool ImGui::DragBehaviorT(ImGuiDataType data_type, TYPE* v, float v_speed, const TYPE v_min,
  ...
  const bool is_bounded =
      (v_min < v_max) ||
      ((v_min == v_max) && (v_min != 0.0f || (flags & ImGuiSliderFlags_ClampZeroRange)));
  ...
  if (*v != v_cur && is_bounded) {
    if (is_wrapped) {
      ...
      if (v_cur < v_min || (v_cur > *v && adjust_delta < 0.0f && !is_floating_point)) v_cur = v_min;
      if (v_cur > v_max || (v_cur < *v && adjust_delta > 0.0f && !is_floating_point)) v_cur = v_max;
    }
  }
...
bool ImGui::DragBehavior(ImGuiID id, ImGuiDataType data_type, void* p_v, float v_speed,
  ...
  IM_ASSERT((flags == 1 || (flags & ImGuiSliderFlags_InvalidMask_) == 0) &&
            "Invalid ImGuiSliderFlags flags! Has the legacy 'float power' argument been mistakenly "
            "cast to flags? Call function with ImGuiSliderFlags_Logarithmic flags instead.");

  ImGuiContext& g = *GImGui;
  if (g.ActiveId == id) {
    // Those are the things we can do easily outside the DragBehaviorT<> template, saves code
    // generation.
...
bool ImGui::DragScalar(const char* label, ImGuiDataType data_type, void* p_data, float v_speed,
  ...
  if (!temp_input_is_active) {
    ...
    // Store initial value (not used by main lib but available as a convenience but some mods e.g.
    // to revert)
  ...
  if (temp_input_is_active) {
    ...
    if ((flags & ImGuiSliderFlags_ClampOnInput) && (p_min != NULL || p_max != NULL)) {
      ...
      if (p_min == NULL || p_max == NULL || clamp_range_dir < 0)
        ...
        clamp_enabled = DataTypeIsZero(data_type, p_min)
  ...
  if (is_logarithmic) {
    ...
    else if ((v_min * v_max) < 0.0f)  // Range crosses zero, so split into two portions
    {
      ...
      if (v == 0.0f)
        ...
        result =
            zero_point_snap_R + ((float)(ImLog((FLOATTYPE)v_clamped / logarithmic_zero_epsilon) /
      ...
      result =
          (float)(ImLog((FLOATTYPE)v_clamped / v_min_fudged) / ImLog(v_max_fudged / v_min_fudged));
...
TYPE ImGui::ScaleValueFromRatioT(ImGuiDataType data_type, float t, TYPE v_min, TYPE v_max,
  ...
  // "mathematically correct" but non-intuitive behaviors like a fully-left slider not actually
  // reaching the minimum value. Also generally simpler.
  ...
  if (is_logarithmic) {
    ...
    if ((v_min * v_max) < 0.0f)  // Range crosses zero, so we have to do this in two parts
    {
      ...
      if (t_with_flip >= zero_point_snap_L && t_with_flip <= zero_point_snap_R)
        ...
        result =
            (TYPE)(logarithmic_zero_epsilon * ImPow(v_max_fudged / logarithmic_zero_epsilon,
...
bool ImGui::SliderBehaviorT(const ImRect& bb, ImGuiID id, ImGuiDataType data_type, TYPE* v,
  ...
  const float v_range_f =
      (float)(v_min < v_max
                  ? v_max - v_min
                  : v_min - v_max);  // We don't need high precision for what we do with it.
      ...
      if (!g.IO.MouseDown[0]) {
        ...
        if (g.ActiveIdIsJustActivated) {
          ...
          const bool clicked_around_grab =
              ...
              (mouse_abs_pos <=
               grab_pos + grab_sz * 0.5f + 1.0f);  // No harm being extra generous here.
...
bool ImGui::SliderBehavior(const ImRect& bb, ImGuiID id, ImGuiDataType data_type, void* p_v,
  ...
  IM_ASSERT((flags == 1 || (flags & ImGuiSliderFlags_InvalidMask_) == 0) &&
            "Invalid ImGuiSliderFlags flags! Has the legacy 'float power' argument been mistakenly "
            "cast to flags? Call function with ImGuiSliderFlags_Logarithmic flags instead.");
  IM_ASSERT((flags & ImGuiSliderFlags_WrapAround) ==
            0);  // Not supported by SliderXXX(), only by DragXXX()

  switch (data_type) {
    case ImGuiDataType_S8: {
      ImS32 v32 = (ImS32) * (ImS8*)p_v;
      bool r =
          SliderBehaviorT<ImS32, ImS32, float>(bb, id, ImGuiDataType_S32, &v32, *(const ImS8*)p_min,
    ...
    case ImGuiDataType_U8: {
      ImU32 v32 = (ImU32) * (ImU8*)p_v;
      bool r =
          SliderBehaviorT<ImU32, ImS32, float>(bb, id, ImGuiDataType_U32, &v32, *(const ImU8*)p_min,
...
bool ImGui::SliderAngle(const char* label, float* v_rad, float v_degrees_min, float v_degrees_max,
  ...
  if (value_changed) *v_rad = v_deg * (2 * IM_PI) / 360.0f;
  return value_changed;
}
...
bool ImGui::VSliderScalar(const char* label, const ImVec2& size, ImGuiDataType data_type,
  ...
  if (clicked || g.NavActivateId == id) {
    if (clicked) SetKeyOwner(ImGuiKey_MouseLeft, id);
    SetActiveID(id, window);
    SetFocusID(id, window);
    FocusWindow(window);
...
const char* ImParseFormatFindEnd(const char* fmt) {
  // Printf/scanf types modifiers: I/L/h/j/l/t/w/z. Other uppercase letters qualify as types aka end
  // of the format.
  if (fmt[0] != '%') return fmt;
...
void ImParseFormatSanitizeForPrinting(const char* fmt_in, char* fmt_out, size_t fmt_out_size) {
  ...
  IM_ASSERT((size_t)(fmt_end - fmt_in + 1) <
...
// - For scanning we need to remove all width and precision fields and flags "%+3.7f" -> "%f". BUT
// don't strip types like "%I64d" which includes digits. ! "%07I64d" -> "%I64d"
const char* ImParseFormatSanitizeForScanning(const char* fmt_in, char* fmt_out,
                                             size_t fmt_out_size) {
  const char* fmt_end = ImParseFormatFindEnd(fmt_in);
...
// FIXME: Among other things, setting ImGuiItemFlags_AllowDuplicateId in LastItemData is currently
// correct but the expected relationship between TempInputXXX functions and LastItemData is a little
// fishy.
bool ImGui::TempInputText(const ImRect& bb, ImGuiID id, const char* label, char* buf, int buf_size,
  ...
  bool value_changed =
      InputTextEx(label, NULL, buf, buf_size, bb.GetSize(), flags | ImGuiInputTextFlags_MergedItem);
  if (init) {
...
bool ImGui::TempInputScalar(const ImRect& bb, ImGuiID id, const char* label,
  ...
  g.LastItemData.ItemFlags |=
      ImGuiItemFlags_NoMarkEdited;  // Because TempInputText() uses ImGuiInputTextFlags_MergedItem
                                    // it doesn't submit a new item, so we poke LastItemData.
  bool value_changed = false;
...
bool ImGui::InputScalar(const char* label, ImGuiDataType data_type, void* p_data,
  ...
  IM_ASSERT((flags & ImGuiInputTextFlags_EnterReturnsTrue) ==
            0);  // Not supported by InputScalar(). Please open an issue if you this would be useful
                 // to you. Otherwise use IsItemDeactivatedAfterEdit()!
  ...
  if (p_step == NULL) {
    if (InputText(label, buf, IM_ARRAYSIZE(buf), flags))
      value_changed = DataTypeApplyFromText(
          buf, data_type, p_data, format,
          (flags & ImGuiInputTextFlags_ParseEmptyRefVal) ? p_data_default : NULL);
  } else {
...
bool ImGui::InputTextWithHint(const char* label, const char* hint, char* buf, size_t buf_size,
                              ...
                              void* user_data) {
  IM_ASSERT(
      !(flags & ImGuiInputTextFlags_Multiline));  // call InputTextMultiline() or  InputTextEx()
                                                  // manually if you need multi-line + hint.
  return InputTextEx(label, hint, buf, (int)buf_size, ImVec2(0, 0), flags, callback, user_data);
}

static ImVec2 InputTextCalcTextSize(ImGuiContext* ctx, const char* text_begin,
                                    const char* text_end_display, const char* text_end,
                                    const char** out_remaining, ImVec2* out_offset,
  ...
  return ImFontCalcTextSizeEx(g.Font, g.FontSize, FLT_MAX, obj->WrapWidth, text_begin,
                              text_end_display, text_end, out_remaining, out_offset, flags);
}

// Wrapper for stb_textedit.h to edit text (our wrapper is for: statically sized buffer,
// single-line, wchar characters. InputText converts between UTF-8 and wchar) With our UTF-8 use of
// stb_textedit:
// - STB_TEXTEDIT_GETCHAR is nothing more than a a "GETBYTE". It's only used to compare to ascii or
// to copy blocks of text so we are fine.
// - One exception is the STB_TEXTEDIT_IS_SPACE feature which would expect a full char in order to
// handle full-width space such as 0x3000 (see ImCharIsBlankW).
...
static char STB_TEXTEDIT_GETCHAR(const ImGuiInputTextState* obj, int idx) {
  IM_ASSERT(idx >= 0 && idx <= obj->TextLen);
...
static void STB_TEXTEDIT_LAYOUTROW(StbTexteditRow* r, ImGuiInputTextState* obj,
                                   int line_start_idx) {
  const char* text = obj->TextSrc;
  const char* text_remaining = NULL;
  const ImVec2 size = InputTextCalcTextSize(
      obj->Ctx, text + line_start_idx, text + obj->TextLen, text + obj->TextLen, &text_remaining,
      NULL, ImDrawTextFlags_StopOnNewLine | ImDrawTextFlags_WrapKeepBlanks);
  r->x0 = 0.0f;
  r->x1 = size.x;
...
static bool ImCharIsSeparatorW(unsigned int c) {
  static const unsigned int separator_list[] = {
      ',', 0x3001, '.',  0x3002, ';', 0xFF1B, '(',    0xFF08, ')',  0xFF09,
      '{', 0xFF5B, '}',  0xFF5D, '[', 0x300C, ']',    0x300D, '|',  0xFF5C,
      '!', 0xFF01, '\\', 0xFFE5, '/', 0x30FB, 0xFF0F, '\n',   '\r',
  };
...
// Reimplementation of stb_textedit_move_line_start()/stb_textedit_move_line_end() which supports
// word-wrapping.
static int STB_TEXTEDIT_MOVELINESTART_IMPL(ImGuiInputTextState* obj,
                                           ImStb::STB_TexteditState* state, int cursor) {
  if (state->single_line) return 0;
  ...
  if (obj->WrapWidth > 0.0f) {
    ...
    while (p >= p_bol) {
      const char* p_eol = ImFontCalcWordWrapPositionEx(
          g.Font, g.FontSize, p, text_end, obj->WrapWidth, ImDrawTextFlags_WrapKeepBlanks);
      if (p == p_cursor)  // If we are already on a visible beginning-of-line, return real
                          // beginning-of-line (would be same as regular handler below)
        return (int)(p_bol - obj->TextSrc);
      if (p_eol == p_cursor && obj->TextA[cursor] != '\n' &&
          obj->LastMoveDirectionLR == ImGuiDir_Left)
        return (int)(p_bol - obj->TextSrc);
      if (p_eol >= p_cursor) return (int)(p - obj->TextSrc);
      p = (*p_eol == '\n') ? p_eol + 1 : p_eol;
    }
  }
  ...
  while (cursor > 0) {
    ...
    if (STB_TEXTEDIT_GETCHAR(obj, prev_cursor) == STB_TEXTEDIT_NEWLINE) break;
    cursor = prev_cursor;
  }
  return cursor;
}

static int STB_TEXTEDIT_MOVELINEEND_IMPL(ImGuiInputTextState* obj, ImStb::STB_TexteditState* state,
                                         int cursor) {
  int n = STB_TEXTEDIT_STRINGLEN(obj);
  if (state->single_line) return n;
  ...
  if (obj->WrapWidth > 0.0f) {
    ...
    while (p < text_end) {
      ...
      if (p_eol == p_cursor &&
          obj->LastMoveDirectionLR != ImGuiDir_Left)  // If we are already on a visible end-of-line,
                                                      // switch to regular handle
        break;
      if (p_eol > p_cursor) return cursor;
      p = (*p_eol == '\n') ? p_eol + 1 : p_eol;
    }
  }
...
static bool STB_TEXTEDIT_INSERTCHARS(ImGuiInputTextState* obj, int pos, const char* new_text,
                                     int new_text_len) {
  ...
  if (pos != text_len) memmove(text + pos + new_text_len, text + pos, (size_t)(text_len - pos));
...
// We added an extra indirection where 'Stb' is heap-allocated, in order facilitate the work of
// bindings generators.
...
ImGuiInputTextState::~ImGuiInputTextState() { IM_DELETE(Stb); }
...
void ImGuiInputTextState::OnKeyPressed(int key) {
  ...
  if (key_u == STB_TEXTEDIT_K_LEFT || key_u == STB_TEXTEDIT_K_LINESTART ||
      key_u == STB_TEXTEDIT_K_TEXTSTART || key_u == STB_TEXTEDIT_K_BACKSPACE ||
      key_u == STB_TEXTEDIT_K_WORDLEFT)
    LastMoveDirectionLR = ImGuiDir_Left;
  else if (key_u == STB_TEXTEDIT_K_RIGHT || key_u == STB_TEXTEDIT_K_LINEEND ||
           key_u == STB_TEXTEDIT_K_TEXTEND || key_u == STB_TEXTEDIT_K_DELETE ||
           key_u == STB_TEXTEDIT_K_WORDRIGHT)
...
void ImGuiInputTextState::OnCharPressed(unsigned int c) {
  ...
  // The changes we had to make to stb_textedit_key made it very much UTF-8 specific which is not
  // too great.
  char utf8[5];
...
// Those functions are not inlined in imgui_internal.h, allowing us to hide ImStbTexteditState from
// that header.
...
float ImGuiInputTextState::GetPreferredOffsetX() const {
  return Stb->has_preferred_x ? Stb->preferred_x : -1;
}
...
void ImGuiInputTextCallbackData::InsertChars(int pos, const char* new_text,
  ...
  // Accept null ranges
  if (new_text == new_text_end) return;
...
static bool InputTextFilterCharacter(ImGuiContext* ctx, unsigned int* p_char,
                                     ImGuiInputTextFlags flags, ImGuiInputTextCallback callback,
                                     void* user_data, bool input_source_is_clipboard) {
  ...
  if (c < 0x20) {
    ...
    pass |= (c == '\n') && (flags & ImGuiInputTextFlags_Multiline) !=
                               0;  // Note that an Enter KEY will emit \r and be ignored (we poll
                                   // for KEY in InputText() code)
    ...
    apply_named_filters =
        false;  // Override named filters below so newline and tabs can still be inserted.
  }
  ...
  if (apply_named_filters &&
    ...
    // affect the output/input of printf/scanf to use e.g. ',' instead of '.'. The standard mandate
    // that programs starts in the "C" locale where the decimal point is '.'. We don't really intend
    // to provide widespread support for it, but out of empathy for people stuck with using odd API,
    // we support the bare minimum aka overriding the decimal point. Change the default
    // decimal_point with:
    //   ImGui::GetPlatformIO()->Platform_LocaleDecimalPoint = *localeconv()->decimal_point;
    // Users of non-default decimal point (in particular ',') may be affected by word-selection
    // logic (is_word_boundary_from_right/is_word_boundary_from_left) functions.
    ImGuiContext& g = *ctx;
    ...
    if (flags & (ImGuiInputTextFlags_CharsDecimal | ImGuiInputTextFlags_CharsScientific |
                 (ImGuiInputTextFlags)ImGuiInputTextFlags_LocalizeDecimalPoint))
      if (c == '.' || c == ',') c = c_decimal_point;

    // Full-width -> half-width conversion for numeric fields:
    // https://en.wikipedia.org/wiki/Halfwidth_and_Fullwidth_Forms_(Unicode_block) While this is
    // mostly convenient, this has the side-effect for uninformed users accidentally inputting
    // full-width characters that they may scratch their head as to why it works in numerical fields
    // vs in generic text fields it would require support in the font.
    if (flags & (ImGuiInputTextFlags_CharsDecimal | ImGuiInputTextFlags_CharsScientific |
...
// Note that this doesn't directly alter state->TextA, state->TextLen. They are expected to be made
// valid separately.
// FIXME: Ideally we should transition toward (1) making InsertChars()/DeleteChars() update
// undo-stack (2) discourage (and keep reconcile) or obsolete (and remove reconcile) accessing
// buffer directly.
static void InputTextReconcileUndoState(ImGuiInputTextState* state, const char* old_buf,
                                        int old_length, const char* new_buf, int new_length) {
  const int shorter_length = ImMin(old_length, new_length);
  int first_diff;
  for (first_diff = 0; first_diff < shorter_length; first_diff++)
    if (old_buf[first_diff] != new_buf[first_diff]) break;
  ...
  for (; old_last_diff >= first_diff && new_last_diff >= first_diff;
    ...
    if (old_buf[old_last_diff] != new_buf[new_last_diff]) break;
  ...
  if (insert_len > 0 || delete_len > 0)
    if (IMSTB_TEXTEDIT_CHARTYPE* p =
      ...
      for (int i = 0; i < delete_len; i++) p[i] = old_buf[first_diff + i];
}

// As InputText() retain textual data and we currently provide a path for user to not retain it (via
// local variables) we need some form of hook to reapply data back to user buffer on deactivation
// frame. (#4714) It would be more desirable that we discourage users from taking advantage of the
// "user not retaining data" trick, but that more likely be attractive when we do have _NoLiveEdit
// flag available.
...
// 'max_output_buffer_size' happens to be a meaningful optimization to avoid writing the full
// line_index when not necessarily needed (e.g. very large buffer, scrolled up, inactive)
static int InputTextLineIndexBuild(ImGuiInputTextFlags flags, ImGuiTextIndex* line_index,
                                   const char* buf, const char* buf_end, float wrap_width,
                                   int max_output_buffer_size, const char** out_buf_end) {
  ImGuiContext& g = *GImGui;
  int size = 0;
  const char* s;
  if (flags & ImGuiInputTextFlags_WordWrap) {
    for (s = buf; s < buf_end; s = (*s == '\n') ? s + 1 : s) {
      if (size++ <= max_output_buffer_size) line_index->Offsets.push_back((int)(s - buf));
      s = ImFontCalcWordWrapPositionEx(g.Font, g.FontSize, s, buf_end, wrap_width,
  ...
  if (out_buf_end != NULL) *out_buf_end = buf_end = s;
  if (size == 0) {
...
static ImVec2 InputTextLineIndexGetPosOffset(ImGuiContext& g, ImGuiInputTextState* state,
  ...
  if (it > it_begin)
    if (it == it_end || *it != cursor_n ||
        (state != NULL && state->WrapWidth > 0.0f && state->LastMoveDirectionLR == ImGuiDir_Right &&
         cursor_ptr[-1] != '\n' && cursor_ptr[-1] != 0))
      it--;
  ...
  ImVec2 offset;
  offset.x = InputTextCalcTextSize(&g, line_start, cursor_ptr, buf_end, NULL, NULL,
                                   ImDrawTextFlags_WrapKeepBlanks)
                 .x;
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size,
  ...
  IM_ASSERT(!(
      ...
      (flags & ImGuiInputTextFlags_Multiline)));  // Multiline does not not work with left-trimming
  IM_ASSERT((flags & ImGuiInputTextFlags_WordWrap) == 0 ||
            (flags & ImGuiInputTextFlags_Password) ==
                0);  // WordWrap does not work with Password mode.
  IM_ASSERT((flags & ImGuiInputTextFlags_WordWrap) == 0 ||
            (flags & ImGuiInputTextFlags_Multiline) !=
                0);  // WordWrap does not work in single-line mode.
  ...
  if (is_multiline)  // Open group before calling GetID() because groups tracks id created within
                     // their scope (including the scrollbar)
    BeginGroup();
  const ImGuiID id = window->GetID(label);
  const ImVec2 label_size = CalcTextSize(label, NULL, true);
  ...
  const ImVec2 total_size =
      ImVec2(frame_size.x + (label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f),
             frame_size.y);
  ...
  if (is_multiline) {
    ...
    // Prevent NavActivation from explicit Tabbing when our widget accepts Tab inputs: this allows
    // cycling through widgets without stopping.
    if (g.NavActivateId == id && (g.NavActivateFlags & ImGuiActivateFlags_FromTabbing) &&
        !(g.NavActivateFlags & ImGuiActivateFlags_FromFocusApi) &&
    ...
    bool child_visible = BeginChildEx(label, id, frame_bb.GetSize(), ImGuiChildFlags_Borders,
  ...
  // Ensure mouse cursor is set even after switching to keyboard/gamepad mode. May generalize
  // further? (#6417)
  bool hovered =
      ItemHoverable(frame_bb, id, g.LastItemData.ItemFlags | ImGuiItemFlags_NoNavDisableMouseHover);
  ...
  if (g.LastItemData.ItemFlags & ImGuiItemFlags_ReadOnly) flags |= ImGuiInputTextFlags_ReadOnly;
  ...
  if (is_resizable)
    IM_ASSERT(
        callback !=
        NULL);  // Must provide a callback if you set the ImGuiInputTextFlags_CallbackResize flag!

  // Word-wrapping: enforcing a fixed width not altered by vertical scrollbar makes things easier,
  // notably to track cursor reliably and avoid one-frame glitches. Instead of using
  // ImGuiWindowFlags_AlwaysVerticalScrollbar we account for that space if the scrollbar is not
  // visible.
  const bool is_wordwrap = (flags & ImGuiInputTextFlags_WordWrap) != 0;
  float wrap_width = 0.0f;
  if (is_wordwrap)
    wrap_width = ImMax(1.0f, GetContentRegionAvail().x +
                                 (draw_window->ScrollbarY ? 0.0f : -g.Style.ScrollbarSize));
  ...
  const bool init_changed_specs =
      (state != NULL &&
       state->Stb->single_line != !is_multiline);  // state != NULL means its our state.
  ...
  if (init_reload_from_user_buf) {
    ...
    // Backup state of deactivating item so they'll have a chance to do a write to output buffer on
    // the same frame they report IsItemDeactivatedAfterEdit (#4714)
    InputTextDeactivateHook(state->ID);

    // Take a copy of the initial buffer value.
    // From the moment we focused we are normally ignoring the content of 'buf' (unless we are in
    // read-only mode)
    const int buf_len = (int)ImStrlen(buf);
    IM_ASSERT(((buf_len + 1 <= buf_size) || (buf_len == 0 && buf_size == 0)) &&
              "Is your input buffer properly zero-terminated?");
    state->TextToRevertTo.resize(buf_len + 1);  // UTF-8. we use +1 to make sure that .Data is
                                                // always pointing to at least an empty string.
    memcpy(state->TextToRevertTo.Data, buf, buf_len + 1);

    // Preserve cursor position and undo/redo stack if we come back to same widget
    // FIXME: Since we reworked this on 2022/06, may want to differentiate recycle_cursor vs
    // recycle_undostate?
    bool recycle_state = (state->ID == id && !init_changed_specs);
    if (recycle_state &&
        (state->TextLen != buf_len ||
         (state->TextA.Data == NULL || strncmp(state->TextA.Data, buf, buf_len) != 0)))
      recycle_state = false;
    ...
    if (flags & ImGuiInputTextFlags_ElideLeft)
      state->Scroll.x +=
          ImMax(0.0f, CalcTextSize(buf).x - frame_size.x + style.FramePadding.x * 2.0f);
    ...
    if (!is_multiline) {
      if (flags & ImGuiInputTextFlags_AutoSelectAll) select_all = true;
  ...
  if (g.ActiveId == id) {
    ...
    // FIXME: The reason we don't use Shortcut() is we would need a routing flag to specify multiple
    // mods, or to all mods combination into individual shortcuts.
    const ImGuiKey always_owned_keys[] = {
        ImGuiKey_LeftArrow, ImGuiKey_RightArrow, ImGuiKey_Enter, ImGuiKey_KeypadEnter,
        ImGuiKey_Delete,    ImGuiKey_Backspace,  ImGuiKey_Home,  ImGuiKey_End};
    for (ImGuiKey key : always_owned_keys) SetKeyOwner(key, id);
    if (user_clicked) SetKeyOwner(ImGuiKey_MouseLeft, id);
    g.ActiveIdUsingNavDirMask |= (1 << ImGuiDir_Left) | (1 << ImGuiDir_Right);
    ...
    // FIXME: May be a problem to always steal Alt on OSX, would ideally still allow an
    // uninterrupted Alt down-up to toggle menu
    if (is_osx) SetKeyOwner(ImGuiMod_Alt, id);

    // Expose scroll in a manner that is agnostic to us using a child window
    if (is_multiline && state != NULL) state->Scroll.y = draw_window->Scroll.y;

    // Read-only mode always ever read from source buffer. Refresh TextLen when active.
    if (is_readonly && state != NULL) state->TextLen = (int)ImStrlen(buf);
    // if (is_readonly && state != NULL)
    //     state->TextA.clear(); // Uncomment to facilitate debugging, but we otherwise prefer to
    //     keep/amortize th allocation.
  }
  if (state != NULL) state->TextSrc = is_readonly ? buf : state->TextA.Data;
  ...
  if (is_password && !is_displaying_hint) PushPasswordFont();
  ...
  if (g.ActiveId == id) {
    ...
    if (select_all) {
      ...
      if ((multiclick_count % 2) == 0) {
        ...
        // We always use the "Mac" word advance for double-click select vs CTRL+Right which use the
        // platform dependent variant:
        // FIXME: There are likely many ways to improve this behavior, but there's no "right"
        // behavior (depends on use-case, software, OS)
        const bool is_bol = (state->Stb->cursor == 0) ||
                            ImStb::STB_TEXTEDIT_GETCHAR(state, state->Stb->cursor - 1) == '\n';
    ...
    // We expect backends to emit a Tab key but some also emit a Tab character which we ignore
    // (#2467, #1336) (For Tab and Enter: Win32/SFML/Allegro are sending both keys and chars, GLFW
    // and SDL are only sending keys. For Space they all send all threes)
  ...
  if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id) {
    ...
    // Using Shortcut() with ImGuiInputFlags_RouteFocused (default policy) to allow routing
    // operations for other code (e.g. calling window trying to use CTRL+A and CTRL+B: former would
    // be handled by InputText) Otherwise we could simply assume that we own the keys as we are
    // active.
    ...
    const bool is_copy = (Shortcut(ImGuiMod_Ctrl | ImGuiKey_C, 0, id) ||
    ...
    const bool is_paste = (Shortcut(ImGuiMod_Ctrl | ImGuiKey_V, f_repeat, id) ||
                          ...
                          !is_readonly;
    const bool is_undo =
        (Shortcut(ImGuiMod_Ctrl | ImGuiKey_Z, f_repeat, id)) && !is_readonly && is_undoable;
    ...
    // We allow validate/cancel with Nav source (gamepad) to makes it easier to undo an accidental
    // NavInput press with no keyboard wired, but otherwise it isn't very useful.
    ...
    const bool is_enter_pressed =
        IsKeyPressed(ImGuiKey_Enter, true) || IsKeyPressed(ImGuiKey_KeypadEnter, true);
    ...
    const bool is_cancel =
        Shortcut(ImGuiKey_Escape, f_repeat, id) ||
        (nav_gamepad_active && Shortcut(ImGuiKey_NavGamepadCancel, f_repeat, id));

    // FIXME: Should use more Shortcut() and reduce IsKeyPressed()+SetKeyOwner(), but requires
    // modifiers combination to be taken account of.
    // FIXME-OSX: Missing support for Alt(option)+Right/Left = go to end of line, or next line if
    // already in end of line.
    if (IsKeyPressed(ImGuiKey_LeftArrow)) {
      ...
      if (!state->HasSelection()) {
        // OSX doesn't seem to have Super+Delete to delete until end-of-line, so we don't emulate
        // that (as opposed to Super+Backspace)
      ...
      if (g.PlatformIO.Platform_SetClipboardTextFn != NULL) {
        // SetClipboardText() only takes null terminated strings + state->TextSrc may point to
        // read-only user buffer, so we need to make a copy.
        const int ib =
            state->HasSelection() ? ImMin(state->Stb->select_start, state->Stb->select_end) : 0;
  ...
  if (g.ActiveId == id) {
    ...
    if (revert_edit && !is_readonly) {
      if (flags & ImGuiInputTextFlags_EscapeClearsAll) {
        ...
        // current buffer contents. Push records into the undo stack so we can CTRL+Z the revert
        // operation itself
        value_changed = true;
        stb_textedit_replace(state, state->Stb, state->TextToRevertTo.Data,
                             state->TextToRevertTo.Size - 1);
      }
    }

    // FIXME-OPT: We always reapply the live buffer back to the input buffer before clearing
    // ActiveId, even though strictly speaking it wasn't modified on this frame. Should mark dirty
    // state from the stb_textedit callbacks. If we do that, need to ensure that as special case,
    // 'validated == true' also writes back. This also allows the user to use InputText() without
    // maintaining any user-side storage. (please note that if you use this property along
    ...
    const bool apply_edit_back_to_user_buffer =
        true;  // !revert_edit || (validated && (flags & ImGuiInputTextFlags_EnterReturnsTrue) !=
               // 0);
    if (apply_edit_back_to_user_buffer) {
      ...
      if ((flags & (ImGuiInputTextFlags_CallbackCompletion | ImGuiInputTextFlags_CallbackHistory |
        ...
        // The reason we specify the usage semantic (Completion/History) is that Completion needs to
        // disable keyboard TABBING at the moment.
        ImGuiInputTextFlags event_flag = 0;
        ImGuiKey event_key = ImGuiKey_None;
        if ((flags & ImGuiInputTextFlags_CallbackCompletion) != 0 &&
            Shortcut(ImGuiKey_Tab, 0, id)) {
        ...
        if (event_flag) {
          ...
          // FIXME-OPT: Undo stack reconcile needs a backup of the data until we rework API, see
          // #7925
          ...
          callback_buf =
              is_readonly
                  ? buf
                  : state->TextA.Data;  // Pointer may have been invalidated by a resize callback
          ...
          if (buf_dirty) {
            ...
            InputTextReconcileUndoState(state, state->CallbackTextBackup.Data,
                                        ...
                                        callback_data.BufTextLen);
            state->TextLen = callback_data.BufTextLen;  // Assume correct length and valid UTF-8
  ...
  if (g.InputTextDeactivatedState.ID == id) {
    if (g.ActiveId != id && IsItemDeactivatedAfterEdit() && !is_readonly &&
      ...
      // apply_new_text_length, apply_new_text);
    }
  ...
  // application of the value) Otherwise request text input ahead for next frame.
  ...
  if (is_multiline) clip_rect.ClipWith(draw_window->ClipRect);
  ...
  // At this point we need to handle the possibility that a callback could have modified the
  // underlying buffer (#8368)
  const bool new_is_displaying_hint =
      (hint != NULL && (buf_display_from_state ? state->TextA.Data : buf)[0] == 0);
  if (new_is_displaying_hint != is_displaying_hint) {
    if (is_password && !is_displaying_hint) PopPasswordFont();
  ...
  if (is_displaying_hint) {
    ...
    if (render_cursor || render_selection || g.ActiveId == id)
      ...
      buf_display_end = NULL;  // Inactive multi-line: end of buffer will be output by
                               // InputTextLineIndexBuild() special strchr() path.
    else
  ...
  int line_count = 1;
  if (is_multiline)
    line_count = InputTextLineIndexBuild(
        flags, line_index, buf_display, buf_display_end, wrap_width,
        (render_cursor && state && state->CursorFollow) ? INT_MAX : line_visible_n1 + 1,
        buf_display_end ? NULL : &buf_display_end);
  ...
  // GetForegroundDrawList()->AddRect(draw_pos + ImVec2(0, line_visible_n0 * g.FontSize), draw_pos +
  // ImVec2(frame_size.x, line_visible_n1 * g.FontSize), IM_COL32(255, 0, 0, 255));

  // Calculate blinking cursor position
  const ImVec2 cursor_offset =
      render_cursor && state ? InputTextLineIndexGetPosOffset(g, state, line_index, buf_display,
  ...
  if (render_cursor || render_selection) {
    ...
    if (new_scroll_y != scroll_y) {
      ...
      CalcClipRectVisibleItemsY(clip_rect, draw_pos, g.FontSize, &line_visible_n0,
                                &line_visible_n1);
      line_visible_n1 = ImMin(line_visible_n1, line_count);
    }
    ...
    if (render_selection) {
      const ImU32 bg_color = GetColorU32(
      ...
      const float bg_offy_up =
          is_multiline ? 0.0f : -1.0f;  // FIXME: those offsets should be part of the style? they
                                        // don't play so well with multi-line selection.
      const float bg_offy_dn = is_multiline ? 0.0f : 2.0f;
      const float bg_eol_width = IM_TRUNC(g.FontBaked->GetCharAdvance((ImWchar)' ') *
                                          0.50f);  // So we can see selected empty lines

      const char* text_selected_begin =
          buf_display + ImMin(state->Stb->select_start, state->Stb->select_end);
      const char* text_selected_end =
          buf_display + ImMax(state->Stb->select_start, state->Stb->select_end);
  ...
  if (g.ActiveId != id && flags & ImGuiInputTextFlags_ElideLeft)
    draw_pos.x = ImMin(draw_pos.x,
                       frame_bb.Max.x - CalcTextSize(buf_display, NULL).x - style.FramePadding.x);
  // draw_scroll.x = state->Scroll.x; // Preserve scroll when inactive?

  // Render text
  if ((is_multiline || (buf_display_end - buf_display) < buf_display_max_length) &&
      (text_col & IM_COL32_A_MASK) && (line_visible_n0 < line_visible_n1))
  ...
  if (render_cursor) {
    ...
    // Notify OS of text input position for advanced IME (-1 x offset so that Windows IME can cover
    // our cursor. Bit of an extra nicety.) This is required for some backends (SDL3) to start
    // emitting character/text inputs. As per #6341, make sure we don't set that on the deactivating
    // frame.
    if (!is_readonly && g.ActiveId == id) {
      ImGuiPlatformImeData* ime_data = &g.PlatformImeData;  // (this is a public struct, passed to
  ...
  if (is_multiline) {
    // For focus requests to work on our multiline we need to ensure our child ItemAdd() call
    // specifies the ImGuiItemFlags_Inputable (see #4761, #7870)...
    ...
    // ...and then we need to undo the group overriding last item data, which gets a bit messy as
    // EndGroup() tries to forward scrollbar being active...
...
void ImGui::DebugNodeInputTextState(ImGuiInputTextState* state) {
  ...
  Text("TextLen: %d, Cursor: %d%s, Selection: %d..%d", state->TextLen, stb_state->cursor,
       (state->Flags & ImGuiInputTextFlags_WordWrap)
           ? (state->LastMoveDirectionLR == ImGuiDir_Left ? " (L)" : " (R)")
           : "",
  ...
  Text("undo_point: %d, redo_point: %d, undo_char_point: %d, redo_char_point: %d",
       undo_state->undo_point, undo_state->redo_point, undo_state->undo_char_point,
                 ...
                 ImGuiChildFlags_Borders | ImGuiChildFlags_ResizeY))  // Visualize undo state
  {
    ...
    for (int n = 0; n < IMSTB_TEXTEDIT_UNDOSTATECOUNT; n++) {
      ...
      Text("%c [%02d] where %03d, insert %03d, delete %03d, char_storage %03d \"%.*s\"",
           undo_rec_type, n, undo_rec->where, undo_rec->insert_length, undo_rec->delete_length,
           undo_rec->char_storage, buf_preview_len, buf_preview_str);
...
// ColorEdit supports RGB and HSV inputs. In case of RGB input resulting color may have undefined
// hue and/or saturation. Since widget displays both RGB and HSV values we must preserve hue and
// saturation to prevent these values resetting.
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags) {
  ...
  flags |=
      (g.ColorEditOptions & ~(ImGuiColorEditFlags_DisplayMask_ | ImGuiColorEditFlags_DataTypeMask_ |
                              ImGuiColorEditFlags_PickerMask_ | ImGuiColorEditFlags_InputMask_));
  IM_ASSERT(
      ImIsPowerOfTwo(flags & ImGuiColorEditFlags_DisplayMask_));  // Check that only 1 is selected
  IM_ASSERT(
      ImIsPowerOfTwo(flags & ImGuiColorEditFlags_InputMask_));  // Check that only 1 is selected

  const bool alpha = (flags & ImGuiColorEditFlags_NoAlpha) == 0;
  const bool hdr = (flags & ImGuiColorEditFlags_HDR) != 0;
  const int components = alpha ? 4 : 3;
  const float w_button =
      (flags & ImGuiColorEditFlags_NoSmallPreview) ? 0.0f : (square_sz + style.ItemInnerSpacing.x);
  ...
  if ((flags & (ImGuiColorEditFlags_DisplayRGB | ImGuiColorEditFlags_DisplayHSV)) != 0 &&
    ...
    for (int n = 0; n < components; n++) {
      ...
      // FIXME: When ImGuiColorEditFlags_HDR flag is passed HS values snap in weird ways when SV
      // values go below 0.
  ...
  if (label != label_display_end && !(flags & ImGuiColorEditFlags_NoLabel)) {
    ...
    // ImGuiDir_Left), but we need to use SameLine() to setup baseline correctly. Might want to
    // refactor SameLine() to simplify this.
  ...
  // NB: The flag test is merely an optional micro-optimization, BeginDragDropTarget() does the same
  // test.
  ...
  if (value_changed &&
      g.LastItemData.ID !=
          0)  // In case of ID collision, the second EndGroup() won't catch g.ActiveId
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags,
  ...
  const bool is_readonly =
      ((g.NextItemData.ItemFlags | g.CurrentItemFlags) & ImGuiItemFlags_ReadOnly) != 0;
  g.NextItemData.ClearFlags();
  ...
  IM_ASSERT(
      ImIsPowerOfTwo(flags & ImGuiColorEditFlags_PickerMask_));  // Check that only 1 is selected
  IM_ASSERT(
      ImIsPowerOfTwo(flags & ImGuiColorEditFlags_InputMask_));  // Check that only 1 is selected
  if (!(flags & ImGuiColorEditFlags_NoOptions))
  ...
  if (flags & ImGuiColorEditFlags_PickerHueWheel) {
    ...
    if (IsItemActive() && !is_readonly) {
      ...
      if (ImTriangleContainsPoint(triangle_pa, triangle_pb, triangle_pc,
        ...
        float uu, vv, ww;
        ImTriangleBarycentricCoords(triangle_pa, triangle_pb, triangle_pc, current_off_unrotated,
                                    uu, vv, ww);
    ...
    if (IsItemActive() && !is_readonly) {
      ...
      ColorEditRestoreH(col, &H);  // Greatly reduces hue jitter and reset to 0 when hue == 255 and
                                   // color is rapidly modified using SV square.
      value_changed = value_changed_sv = true;
    }
    if (!(flags & ImGuiColorEditFlags_NoOptions))
      OpenPopupOnItemClick("context", ImGuiPopupFlags_MouseButtonRight);
  ...
  if (!(flags & ImGuiColorEditFlags_NoSidePreview)) {
    ...
    ImGuiColorEditFlags sub_flags_to_forward =
        ImGuiColorEditFlags_InputMask_ | ImGuiColorEditFlags_HDR | ImGuiColorEditFlags_AlphaMask_ |
  ...
  if ((flags & ImGuiColorEditFlags_NoInputs) == 0) {
    ...
    ImGuiColorEditFlags sub_flags_to_forward =
        ImGuiColorEditFlags_DataTypeMask_ | ImGuiColorEditFlags_InputMask_ |
        ImGuiColorEditFlags_HDR | ImGuiColorEditFlags_AlphaMask_ | ImGuiColorEditFlags_NoOptions |
        ImGuiColorEditFlags_NoTooltip | ImGuiColorEditFlags_NoSmallPreview;
    ...
    if (flags & ImGuiColorEditFlags_DisplayRGB || (flags & ImGuiColorEditFlags_DisplayMask_) == 0)
      if (ColorEdit4("##rgb", col, sub_flags | ImGuiColorEditFlags_DisplayRGB)) {
        ...
        // vs. using the InputText or DropTarget. For the later we don't want to run the hue-wrap
        // canceling code. If you are well versed in HSV picker please provide your input! (See
        // #2050)
  ...
  if (flags & ImGuiColorEditFlags_PickerHueWheel) {
    ...
    for (int n = 0; n < 6; n++) {
      ...
      ShadeVertsLinearColorGradientKeepAlpha(draw_list, vert_start_idx, vert_end_idx, gradient_p0,
                                             gradient_p1, col_hues[n], col_hues[n + 1]);
    }
  ...
  draw_list->AddCircleFilled(sv_cursor_pos, sv_cursor_rad, user_col32_striped_of_alpha,
                             sv_cursor_segments);
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags,
  ...
  const float default_size = GetFrameHeight();
  const ImVec2 size(size_arg.x == 0.0f ? default_size : size_arg.x,
  ...
  if ((flags & ImGuiColorEditFlags_NoBorder) == 0) {
    off = -0.75f;  // The border (using Col_FrameBg) tends to look off when color is near-opaque and
                   // rounding is enabled. This offset seemed like a good middle ground to reduce
                   // those artifacts.
    bb_inner.Expand(off);
  }
  if ((flags & ImGuiColorEditFlags_AlphaPreviewHalf) && col_rgb.w < 1.0f) {
    ...
    if ((flags & ImGuiColorEditFlags_AlphaNoBg) == 0)
      ...
      window->DrawList->AddRectFilled(ImVec2(bb_inner.Min.x + grid_step, bb_inner.Min.y),
    ...
    if (col_source.w < 1.0f && (flags & ImGuiColorEditFlags_AlphaNoBg) == 0)
      RenderColorRectWithAlphaCheckerboard(window->DrawList, bb_inner.Min, bb_inner.Max,
                                           GetColorU32(col_source), grid_step, ImVec2(off, off),
                                           rounding);
    else
  ...
  if (!(flags & ImGuiColorEditFlags_NoTooltip) && hovered &&
    ...
    ColorTooltip(desc_id, &col.x,
                 flags & (ImGuiColorEditFlags_InputMask_ | ImGuiColorEditFlags_AlphaMask_));
...
void ImGui::SetColorEditOptions(ImGuiColorEditFlags flags) {
  ...
  IM_ASSERT(
      ImIsPowerOfTwo(flags & ImGuiColorEditFlags_DisplayMask_));  // Check only 1 option is selected
  IM_ASSERT(ImIsPowerOfTwo(flags &
                           ImGuiColorEditFlags_DataTypeMask_));  // Check only 1 option is selected
  IM_ASSERT(
      ImIsPowerOfTwo(flags & ImGuiColorEditFlags_PickerMask_));  // Check only 1 option is selected
  IM_ASSERT(
      ImIsPowerOfTwo(flags & ImGuiColorEditFlags_InputMask_));  // Check only 1 option is selected
...
void ImGui::ColorTooltip(const char* text, const float* col, ImGuiColorEditFlags flags) {
  ...
  ImGuiColorEditFlags flags_to_forward =
      ImGuiColorEditFlags_InputMask_ | ImGuiColorEditFlags_AlphaMask_;
  ColorButton("##preview", cf, (flags & flags_to_forward) | ImGuiColorEditFlags_NoTooltip, sz);
  SameLine();
  if ((flags & ImGuiColorEditFlags_InputRGB) || !(flags & ImGuiColorEditFlags_InputMask_)) {
    if (flags & ImGuiColorEditFlags_NoAlpha)
      Text("#%02X%02X%02X\nR: %d, G: %d, B: %d\n(%.3f, %.3f, %.3f)", cr, cg, cb, cr, cg, cb, col[0],
           col[1], col[2]);
    else
      Text("#%02X%02X%02X%02X\nR:%d, G:%d, B:%d, A:%d\n(%.3f, %.3f, %.3f, %.3f)", cr, cg, cb, ca,
           cr, cg, cb, ca, col[0], col[1], col[2], col[3]);
  } else if (flags & ImGuiColorEditFlags_InputHSV) {
    if (flags & ImGuiColorEditFlags_NoAlpha)
...
bool ImGui::TreeNodeExV(const char* str_id, ImGuiTreeNodeFlags flags, const char* fmt,
                        va_list args) {
  ImGuiWindow* window = GetCurrentWindow();
  if (window->SkipItems) return false;
...
bool ImGui::TreeNodeExV(const void* ptr_id, ImGuiTreeNodeFlags flags, const char* fmt,
                        va_list args) {
  ImGuiWindow* window = GetCurrentWindow();
  if (window->SkipItems) return false;
...
bool ImGui::TreeNodeUpdateNextOpen(ImGuiID storage_id, ImGuiTreeNodeFlags flags) {
  ...
  if (g.NextItemData.HasFlags & ImGuiNextItemDataFlags_HasOpen) {
    if (g.NextItemData.OpenCond & ImGuiCond_Always) {
      ...
      // We treat ImGuiCond_Once and ImGuiCond_FirstUseEver the same because tree node state are not
      // saved persistently.
...
static void TreeNodeStoreStackData(ImGuiTreeNodeFlags flags, float x1) {
  ...
  tree_node_data->DrawLinesX1 =
      draw_lines ? (x1 + g.FontSize * 0.5f + g.Style.FramePadding.x) : +FLT_MAX;
...
// When using public API, currently 'id == storage_id' is always true, but we separate the values to
// facilitate advanced user code doing storage queries outside of UI loop.
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label,
  ...
  if (display_frame) {
    const float outer_extend = IM_TRUNC(
        window->WindowPadding.x * 0.5f);  // Framed header expand a little outside of current limits
  ...
  // and TreePop(). It will become tempting to enable ImGuiTreeNodeFlags_NavLeftJumpsToParent by
  // default or move it to ImGuiStyle.
  bool store_tree_node_stack_data = false;
  if ((flags & ImGuiTreeNodeFlags_DrawLinesMask_) == 0) flags |= g.Style.TreeLinesFlags;
  ...
  if (!is_visible) {
    if ((flags & ImGuiTreeNodeFlags_DrawLinesToNodes) &&
      ...
      parent_data->DrawLinesToNodesY2 = ImMax(
  ...
  const bool is_multi_select = (g.LastItemData.ItemFlags & ImGuiItemFlags_IsMultiSelect) != 0;
  if (is_multi_select)  // We absolutely need to distinguish open vs select so _OpenOnArrow comes by
                        // default
  ...
  if (flags & ImGuiTreeNodeFlags_NoNavFocus) button_flags |= ImGuiButtonFlags_NoNavFocus;
  ...
  // Render
  {
    ...
    if (is_multi_select)
      nav_render_cursor_flags |=
          ImGuiNavRenderCursorFlags_AlwaysDraw;  // Always show the nav rectangle
    if (display_frame) {
      // Framed type
      ...
      if (span_all_columns && !span_all_columns_label) TablePopBackgroundChannel();
      if (flags & ImGuiTreeNodeFlags_Bullet)
        ...
        RenderArrow(
            window->DrawList, ImVec2(text_pos.x - text_offset_x + padding.x, text_pos.y), text_col,
    ...
    if (draw_tree_lines)
      TreeNodeDrawLineToChildNode(
          ImVec2(text_pos.x - text_offset_x + padding.x, text_pos.y + g.FontSize * 0.5f));
    ...
    if (span_all_columns_label) TablePopBackgroundChannel();
  }
...
void ImGui::TreeNodeDrawLineToChildNode(const ImVec2& target_pos) {
  ...
  if (window->DC.TreeDepth == 0 ||
      (window->DC.TreeHasStackDataDepthMask & (1 << (window->DC.TreeDepth - 1))) == 0)
    return;
  ...
  float rounding =
      (g.Style.TreeLinesRounding > 0.0f) ? ImMin(x2 - x1, g.Style.TreeLinesRounding) : 0.0f;
  ...
  if (rounding > 0.0f) {
    ...
    if (x1 < x2) window->DrawList->PathLineTo(ImVec2(x2, y));
    window->DrawList->PathStroke(GetColorU32(ImGuiCol_TreeLines), ImDrawFlags_None,
                                 g.Style.TreeLinesSize);
  } else {
    window->DrawList->AddLine(ImVec2(x1, y), ImVec2(x2, y), GetColorU32(ImGuiCol_TreeLines),
                              g.Style.TreeLinesSize);
  }
}
...
void ImGui::TreeNodeDrawLineToTreePop(const ImGuiTreeNodeStackData* data) {
  ...
  if (data->TreeFlags & ImGuiTreeNodeFlags_DrawLinesFull) {
    ...
    if (g.CurrentTable) y2_full = ImMax(g.CurrentTable->RowPosY2, y2_full);
    y2_full = ImTrunc(y2_full - g.Style.ItemSpacing.y - g.FontSize * 0.5f);
    if (y2 + (g.Style.ItemSpacing.y + g.Style.TreeLinesRounding) <
        y2_full)  // FIXME: threshold to use ToNodes Y2 instead of Full Y2 when close by
                  // ItemSpacing.y
      y2 = y2_full;
  }
  ...
  if (data->DrawLinesTableColumn != -1) TablePushColumnChannel(data->DrawLinesTableColumn);
  window->DrawList->AddLine(ImVec2(x, y1), ImVec2(x, y2), GetColorU32(ImGuiCol_TreeLines),
                            g.Style.TreeLinesSize);
  if (data->DrawLinesTableColumn != -1) TablePopColumnChannel();
}
...
void ImGui::TreePop() {
  ...
  if (window->DC.TreeHasStackDataDepthMask & tree_depth_mask) {
    ...
    // Handle Left arrow to move to parent tree node (when ImGuiTreeNodeFlags_NavLeftJumpsToParent
    // is enabled)
  ...
  IM_ASSERT(window->IDStack.Size >
            1);  // There should always be 1 element in the IDStack (pushed during window creation).
                 // If this triggers you called TreePop/PopID too much.
  PopID();
}
...
bool ImGui::CollapsingHeader(const char* label, bool* p_visible, ImGuiTreeNodeFlags flags) {
  ...
  if (p_visible)
    flags |= ImGuiTreeNodeFlags_AllowOverlap |
             (ImGuiTreeNodeFlags)ImGuiTreeNodeFlags_ClipLabelForTrailingButton;
...
// ImGuiSelectableFlags_AllowOverlap are also frequently used flags.
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags,
  ...
  // FIXME: Not part of layout so not included in clipper calculation, but ItemSize currently
  // doesn't allow offsetting CursorPos.
  ...
  if (!is_visible)
    if (!is_multi_select || !g.BoxSelectState.UnclipMode ||
        !g.BoxSelectState.UnclipRect.Overlaps(
            bb))  // Extra layer of "no logic clip" for box-select support (would be more overhead
                  // to add to ItemAdd)
      return false;
  ...
  // navigation can be resumed with keyboard/gamepad
  ...
  if (is_visible) {
    ...
    if (highlighted || selected) {
      // Between 1.91.0 and 1.91.4 we made selected Selectable use an arbitrary lerp between _Header
      // and _HeaderHovered. Removed that now. (#8106)
  ...
  // Users of BeginMultiSelect()/EndMultiSelect() scope: you may call
  // ImGui::IsItemToggledSelection() to retrieve selection toggle, only useful if you need that
  // state updated (e.g. for rendering purpose) before reaching EndMultiSelect().
  IMGUI_TEST_ENGINE_ITEM_INFO(id, label, g.LastItemData.StatusFlags);
...
// This would typically only be called on the focused window or location you want to grab inputs
// for, e.g.
//   if (ImGui::IsWindowFocused(...))
//       if (ImGuiTypingSelectRequest* req = ImGui::GetTypingSelectRequest())
//           focus_idx = ImGui::TypingSelectFindMatch(req, my_items.size(), [](void*, int n) {
//           return my_items[n]->Name; }, &my_items, -1);
// However the code is written in a way where calling it from multiple locations is safe (e.g. to
// obtain buffer).
ImGuiTypingSelectRequest* ImGui::GetTypingSelectRequest(ImGuiTypingSelectFlags flags) {
  ...
  if (data->SearchBuffer[0] != 0) {
    ...
    clear_buffer |= g.ActiveId != 0 &&
                    g.NavActivateId == 0;  // Allow temporary SPACE activation to not interfere
    clear_buffer |= IsKeyPressed(ImGuiKey_Escape) || IsKeyPressed(ImGuiKey_Enter);
    clear_buffer |=
        IsKeyPressed(ImGuiKey_Backspace) && (flags & ImGuiTypingSelectFlags_AllowBackspace) == 0;
    // if (clear_buffer) { IMGUI_DEBUG_LOG("GetTypingSelectRequest(): Clear SearchBuffer.\n"); }
    if (clear_buffer) data->Clear();
  }
  ...
  // Handle backspace
  if ((flags & ImGuiTypingSelectFlags_AllowBackspace) &&
      IsKeyPressed(ImGuiKey_Backspace, ImGuiInputFlags_Repeat)) {
    char* p = (char*)(void*)ImTextFindPreviousUtf8Codepoint(data->SearchBuffer,
  ...
  if (flags & ImGuiTypingSelectFlags_AllowSingleCharMode) {
    ...
    for (; p < buf_end; p += c0_len)
      if (memcmp(buf_begin, p, (size_t)c0_len) != 0) break;
    ...
    data->SingleCharModeLock |=
        (single_char_count >=
         TYPING_SELECT_SINGLE_CHAR_COUNT_FOR_LOCK);  // From now on we stop search matching to lock
                                                     // to single char mode.
  }
...
static int ImStrimatchlen(const char* s1, const char* s1_end, const char* s2) {
  int match_len = 0;
  while (s1 < s1_end && ImToUpper(*s1++) == ImToUpper(*s2++)) match_len++;
  return match_len;
}
...
//   if your SetNextItemSelectionUserData() values are indices, you can obtain it from
//   ImGuiMultiSelectIO::NavIdItem, otherwise from g.NavLastValidSelectionUserData.
int ImGui::TypingSelectFindMatch(ImGuiTypingSelectRequest* req, int items_count,
                                 ...
                                 int nav_item_idx) {
  if (req == NULL || req->SelectRequest ==
                         false)  // Support NULL parameter so both calls can be done from same spot.
    return -1;
  int idx = -1;
  if (req->SingleCharMode && (req->Flags & ImGuiTypingSelectFlags_AllowSingleCharMode))
    idx = TypingSelectFindNextSingleCharMatch(req, items_count, get_item_name_func, user_data,
                                              nav_item_idx);
  else
...
// Special handling when a single character is repeated: perform search on a single letter and goes
// to next.
int ImGui::TypingSelectFindNextSingleCharMatch(ImGuiTypingSelectRequest* req, int items_count,
  ...
  for (int idx = 0; idx < items_count; idx++) {
    ...
    if (ImStrimatchlen(req->SearchBuffer, req->SearchBuffer + req->SingleCharSize, item_name) <
        req->SingleCharSize)
      continue;
    ...
    if (first_match_idx == -1 &&
        nav_item_idx ==
            -1)  // Return first match immediately if we don't have a nav_item_idx value.
      return idx;
...
int ImGui::TypingSelectFindBestLeadingMatch(ImGuiTypingSelectRequest* req, int items_count,
  ...
  for (int idx = 0; idx < items_count; idx++) {
    const char* item_name = get_item_name_func(user_data, idx);
    const int match_len =
        ImStrimatchlen(req->SearchBuffer, req->SearchBuffer + req->SearchBufferLen, item_name);
...
void ImGui::DebugNodeTypingSelectState(ImGuiTypingSelectState* data) {
  ...
  Text("SingleCharMode = %d, Size = %d, Lock = %d", data->Request.SingleCharMode,
       data->Request.SingleCharSize, data->SingleCharModeLock);
...
//-------------------------------------------------------------------------
// [SECTION] Widgets: Box-Select support
// This has been extracted away from Multi-Select logic in the hope that it could eventually be used
// elsewhere, but hasn't been yet.
//-------------------------------------------------------------------------
...
static void BoxSelectScrollWithMouseDrag(ImGuiBoxSelectState* bs, ImGuiWindow* window,
  ...
  for (int n = 0; n < 2; n++)  // each axis
  {
    ...
    const float speed_multiplier = ImLinearRemapClamp(
        g.FontSize, g.FontSize * 5.0f, 1.0f, 4.0f, ImAbs(dist));  // x1 to x4 depending on distance
    ...
    if (scroll_step_i == 0.0f) continue;
    if (n == 0)
...
bool ImGui::BeginBoxSelect(const ImRect& scope_rect, ImGuiWindow* window, ImGuiID box_select_id,
  ...
  // validate it here and lock geometry.
  bs->UnclipMode = false;
  ...
  if (ms_flags &
      ImGuiMultiSelectFlags_ScopeWindow)  // Box-select scrolling only happens with ScopeWindow
  ...
  if (ms_flags & ImGuiMultiSelectFlags_BoxSelect2d)
    if (bs->BoxSelectRectPrev.Min.x != bs->BoxSelectRectCurr.Min.x ||
      ...
      bs->UnclipMode = true;
      bs->UnclipRect = bs->BoxSelectRectPrev;  // FIXME-OPT: UnclipRect x coordinates could be
                                               // intersection of Prev and Curr rect on X axis.
      bs->UnclipRect.Add(bs->BoxSelectRectCurr);
    }

  // GetForegroundDrawList()->AddRect(bs->UnclipRect.Min, bs->UnclipRect.Max, IM_COL32(255,0,0,200),
  // 0.0f, 0, 3.0f); GetForegroundDrawList()->AddRect(bs->BoxSelectRectPrev.Min,
  // bs->BoxSelectRectPrev.Max, IM_COL32(255,0,0,200), 0.0f, 0, 3.0f);
  // GetForegroundDrawList()->AddRect(bs->BoxSelectRectCurr.Min, bs->BoxSelectRectCurr.Max,
...
void ImGui::EndBoxSelect(const ImRect& scope_rect, ImGuiMultiSelectFlags ms_flags) {
  ...
  bs->EndPosRel = WindowPosAbsToRel(
      window,
      ImClamp(g.IO.MousePos, scope_rect.Min,
              scope_rect.Max));  // Clamp stored position according to current scrolling view
  ImRect box_select_r = bs->BoxSelectRectCurr;
  box_select_r.ClipWith(scope_rect);
  window->DrawList->AddRectFilled(
      box_select_r.Min, box_select_r.Max,
  ...
  // Scroll
  const bool enable_scroll = (ms_flags & ImGuiMultiSelectFlags_ScopeWindow) &&
                             (ms_flags & ImGuiMultiSelectFlags_BoxSelectNoScroll) == 0;
  if (enable_scroll) {
    ...
    if (!scroll_r.Contains(g.IO.MousePos)) BoxSelectScrollWithMouseDrag(bs, window, scroll_r);
  }
}
  ...
  for (const ImGuiSelectionRequest& req : io->Requests) {
    if (req.Type == ImGuiSelectionRequestType_SetAll)
      IMGUI_DEBUG_LOG_SELECTION("[selection] %s: Request: SetAll %d (= %s)\n", function,
                                req.Selected, req.Selected ? "SelectAll" : "Clear");
    if (req.Type == ImGuiSelectionRequestType_SetRange)
      IMGUI_DEBUG_LOG_SELECTION("[selection] %s: Request: SetRange %" IM_PRId64 "..%" IM_PRId64
...
static ImRect CalcScopeRect(ImGuiMultiSelectTempData* ms, ImGuiWindow* window) {
  ...
  if (ms->Flags & ImGuiMultiSelectFlags_ScopeRect) {
    ...
    // When a table, pull HostClipRect, which allows us to predict ClipRect before first row/layout
    // is performed. (#7970)
    ImRect scope_rect = window->InnerClipRect;
    if (g.CurrentTable != NULL) scope_rect = g.CurrentTable->HostClipRect;

    // Add inner table decoration (#7821) // FIXME: Why not baking in InnerClipRect?
    scope_rect.Min = ImMin(
        scope_rect.Min + ImVec2(window->DecoInnerSizeX1, window->DecoInnerSizeY1), scope_rect.Max);
    return scope_rect;
  }
}

// Return ImGuiMultiSelectIO structure.
// Lifetime: don't hold on ImGuiMultiSelectIO* pointers over multiple frames or past any subsequent
// call to BeginMultiSelect() or EndMultiSelect(). Passing 'selection_size' and 'items_count'
// parameters is currently optional.
// - 'selection_size' is useful to disable some shortcut routing: e.g.
// ImGuiMultiSelectFlags_ClearOnEscape won't claim Escape key when selection_size 0,
//    allowing a first press to clear selection THEN the second press to leave child window and
//    return to parent.
// - 'items_count' is stored in ImGuiMultiSelectIO which makes it a convenient way to pass the
// information to your ApplyRequest() handler (but you may pass it differently).
// - If they are costly for you to compute (e.g. external intrusive selection without maintaining
// size), you may avoid them and pass -1.
...
ImGuiMultiSelectIO* ImGui::BeginMultiSelect(ImGuiMultiSelectFlags flags, int selection_size,
                                            int items_count) {
  ImGuiContext& g = *GImGui;
  ImGuiWindow* window = g.CurrentWindow;
  ...
  if (flags & ImGuiMultiSelectFlags_BoxSelect2d) flags &= ~ImGuiMultiSelectFlags_BoxSelect1d;

  // FIXME: Workaround to the fact we override CursorMaxPos, meaning size measurement are lost.
  // (#8250) They should perhaps be stacked properly?
  if (ImGuiTable* table = g.CurrentTable)
    if (table->CurrentColumn != -1)
      TableEndCell(table);  // This is currently safe to call multiple time. If that properly is
                            // lost we can extract the "save measurement" part of it.
  ...
  if (flags & ImGuiMultiSelectFlags_ScopeWindow)  // Mark parent child window as navigable into,
                                                  // with highlight. Assume user will always submit
                                                  // interactive items.
    window->DC.NavLayersActiveMask |= 1 << ImGuiNavLayer_Main;

  // Use copy of keyboard mods at the time of the request, otherwise we would requires mods to be
  // held for an extra frame.
  ms->KeyMods =
      g.NavJustMovedToId ? (g.NavJustMovedToIsTabbing ? 0 : g.NavJustMovedToKeyMods) : g.IO.KeyMods;
  if (flags & ImGuiMultiSelectFlags_NoRangeSelect) ms->KeyMods &= ~ImGuiMod_Shift;
  ...
  if (g.NavJustMovedToId != 0 && g.NavJustMovedToFocusScopeId == ms->FocusScopeId &&
      g.NavJustMovedToHasSelectionData) {
    if (ms->KeyMods & ImGuiMod_Shift) ms->IsKeyboardSetRange = true;
    if (ms->IsKeyboardSetRange)
      IM_ASSERT(storage->RangeSrcItem !=
                ImGuiSelectionUserData_Invalid);  // Not ready -> could clear?
  ...
  if (ms->IsFocused) {
    ...
    // - Only claim shortcut if selection is not empty, allowing further presses on Escape to e.g.
    // leave current child window.
    // - Box select also handle Escape and needs to pass an id to bypass
    ...
    if (!(flags & ImGuiMultiSelectFlags_SingleSelect) &&
      ...
      if (Shortcut(ImGuiMod_Ctrl | ImGuiKey_A)) request_select_all = true;
  }
...
ImGuiMultiSelectIO* ImGui::EndMultiSelect() {
  ...
  ImGuiWindow* window = g.CurrentWindow;
  IM_ASSERT_USER_ERROR(ms->FocusScopeId == g.CurrentFocusScopeId,
  ...
  if (ms->IsFocused) {
    // We currently don't allow user code to modify RangeSrcItem by writing to BeginIO's version,
    // but that would be an easy change here.
    if (ms->IO.RangeSrcReset ||
        (ms->RangeSrcPassedBy == false &&
         ms->IO.RangeSrcItem !=
             ImGuiSelectionUserData_Invalid))  // Can't read storage->RangeSrcItem here -> we want
                                               // the state at beginning of the scope (see tests for
                                               // easy failure)
    {
  ...
  if (ms->IsEndIO == false) ms->IO.Requests.resize(0);
      ...
      if (!g.BoxSelectState.IsActive && !g.BoxSelectState.IsStarting &&
        ...
        if (ms->Flags & ImGuiMultiSelectFlags_ScopeRect)
          SetNavID(0, ImGuiNavLayer_Main, ms->FocusScopeId,
                   ImRect(g.IO.MousePos, g.IO.MousePos));  // Automatically switch FocusScope for
                                                           // initial click from void to box-select.
      }
    }
  ...
  g.CurrentMultiSelect = (--g.MultiSelectTempDataStacked > 0)
...
void ImGui::SetNextItemSelectionUserData(ImGuiSelectionUserData selection_user_data) {
  ...
  // This designed so widgets can also cheaply set this before calling ItemAdd(), so we are not tied
  // to MultiSelect api.
  ImGuiContext& g = *GImGui;
  ...
  if (ImGuiMultiSelectTempData* ms = g.CurrentMultiSelect) {
    // Auto updating RangeSrcPassedBy for cases were clipper is not used (done before ItemAdd()
    // clipping)
...
void ImGui::MultiSelectItemHeader(ImGuiID id, bool* p_selected, ImGuiButtonFlags* p_button_flags) {
  ...
  if (ms->IsFocused) {
    ...
    // This is only useful if the user hasn't processed them already, and this only works if the
    // user isn't using the clipper. If you are using a clipper you need to process the SetAll
    // request after calling BeginMultiSelect()
    if (ms->LoopRequestSetAll != -1) selected = (ms->LoopRequestSetAll == 1);

    // When using SHIFT+Nav: because it can incur scrolling we cannot afford a frame of lag with the
    // selection highlight (otherwise scrolling would happen before selection) For this to work, we
    // need someone to set 'RangeSrcPassedBy = true' at some point (either clipper either
    // SetNextItemSelectionUserData() function)
    if (ms->IsKeyboardSetRange) {
      IM_ASSERT(id != 0 && (ms->KeyMods & ImGuiMod_Shift) != 0);
      const bool is_range_dst =
          (ms->RangeDstPassedBy == false) &&
          g.NavJustMovedToId == id;  // Assume that g.NavJustMovedToId is not clipped.
      if (is_range_dst) ms->RangeDstPassedBy = true;
      if (is_range_dst &&
          storage->RangeSrcItem == ImGuiSelectionUserData_Invalid)  // If we don't have RangeSrc,
                                                                    // assign RangeSrc = RangeDst
      {
      ...
      if (is_range_src || is_range_dst || ms->RangeSrcPassedBy != ms->RangeDstPassedBy) {
        ...
        selected = (storage->RangeSelected != 0);
                 ...
                 (ms->Flags & ImGuiMultiSelectFlags_NoAutoClear) == 0) {
        // Clear other items
        selected = false;
      }
    }
  ...
  // Enabling this test makes actions using CTRL+SHIFT delay their effect on MouseUp which is
  // annoying, but it allows drag and drop of multiple items.
...
// This is all rather complex, best to run and refer to "widgets_multiselect_xxx" tests in
// imgui_test_suite.
void ImGui::MultiSelectItemFooter(ImGuiID id, bool* p_selected, bool* p_pressed) {
  ...
  if (g.NavJustMovedToId == id) {
    if ((flags & ImGuiMultiSelectFlags_NoAutoSelect) == 0) {
      ...
      if (is_shift)
        ...
        apply_to_range_src =
            true;  // Since if (pressed) {} main block is not running we update this
    }
  }
    ...
    if (ImGuiBoxSelectState* bs = GetBoxSelectState(ms->BoxSelectId)) {
      ...
      if ((rect_overlap_curr && !rect_overlap_prev && !selected) ||
        ...
        if (storage->LastSelectionSize <= 0 && bs->IsStartedSetNavIdOnce) {
          pressed = true;  // First item act as a pressed: code below will emit selection request
                           // and set NavId (whatever we emit here will be overridden anyway)
  ...
  // Right-click handling.
  // FIXME-MULTISELECT: Currently filtered out by ImGuiMultiSelectFlags_NoAutoSelect but maybe
  // should be moved to Selectable(). See https://github.com/ocornut/imgui/pull/5816
  ...
  // Unlike Space, Enter doesn't alter selection (but can still return a press) unless current item
  // is not selected. The later, "unless current item is not select", may become optional? It seems
  // like a better default if Enter doesn't necessarily open something (unlike e.g. Windows
  // explorer). For use case where Enter always open something, we might decide to make this
  // optional?
  ...
  if (pressed && (!enter_pressed || !selected)) {
    // Box-select
    ImGuiInputSource input_source = (g.NavJustMovedToId == id || g.NavActivateId == id)
                                        ? g.NavInputSource
    ...
    if ((flags & ImGuiMultiSelectFlags_NoAutoClear) == 0) {
      ...
      else if ((input_source == ImGuiInputSource_Keyboard ||
                input_source == ImGuiInputSource_Gamepad) &&
               is_shift && !is_ctrl)
        request_clear = true;  // With is_shift==false the RequestClear was done in BeginIO, not
    ...
    if (is_shift && !is_singleselect) {
      ...
      if ((flags & ImGuiMultiSelectFlags_NoAutoSelect) == 0) {
        ...
        range_selected =
            (is_ctrl && storage->RangeSelected != -1) ? (storage->RangeSelected != 0) : true;
      } else {
  ...
  // Update/store the selection state of the Source item (used by CTRL+SHIFT, when Source is
  // unselected we perform a range unselect)
...
void ImGui::MultiSelectAddSetAll(ImGuiMultiSelectTempData* ms, bool selected) {
  ImGuiSelectionRequest req = {ImGuiSelectionRequestType_SetAll, selected, 0,
                               ImGuiSelectionUserData_Invalid, ImGuiSelectionUserData_Invalid};
...
void ImGui::MultiSelectAddSetRange(ImGuiMultiSelectTempData* ms, bool selected, int range_dir,
  ...
  // Merge contiguous spans into same request (unless NoRangeSelect is set which guarantees
  // single-item ranges)
  if (ms->IO.Requests.Size > 0 && first_item == last_item &&
      (ms->Flags & ImGuiMultiSelectFlags_NoRangeSelect) == 0) {
    ImGuiSelectionRequest* prev = &ms->IO.Requests.Data[ms->IO.Requests.Size - 1];
    if (prev->Type == ImGuiSelectionRequestType_SetRange &&
        prev->RangeLastItem == ms->LastSubmittedItem && prev->Selected == selected) {
...
void ImGui::DebugNodeMultiSelectState(ImGuiMultiSelectState* storage) {
  ...
  bool open =
      TreeNode((void*)(intptr_t)storage->ID, "MultiSelect 0x%08X in '%s'%s", storage->ID,
               storage->Window ? storage->Window->Name : "N/A", is_active ? "" : " *Inactive*");
  if (!is_active) {
    PopStyleColor();
  }
  if (!open) return;
  Text("RangeSrcItem = %" IM_PRId64 " (0x%" IM_PRIX64 "), RangeSelected = %d",
       storage->RangeSrcItem, storage->RangeSrcItem, storage->RangeSelected);
  Text("NavIdItem = %" IM_PRId64 " (0x%" IM_PRIX64 "), NavIdSelected = %d", storage->NavIdItem,
       storage->NavIdItem, storage->NavIdSelected);
...
// GetNextSelectedItem() is an abstraction allowing us to change our underlying actual storage
// system without impacting user. (e.g. store unselected vs compact down, compact down on demand,
// use raw ImVector<ImGuiID> instead of ImGuiStorage...)
...
static void ImGuiSelectionBasicStorage_BatchSetItemSelected(ImGuiSelectionBasicStorage* selection,
                                                            ImGuiID id, bool selected,
                                                            int size_before_amends,
                                                            int selection_order) {
  ImGuiStorage* storage = &selection->_Storage;
  ImGuiStoragePair* it =
      ImLowerBound(storage->Data.Data, storage->Data.Data + size_before_amends, id);
  ...
  if (selected && !is_contained)
    storage->Data.push_back(
        ImGuiStoragePair(id, selection_order));  // Push unsorted at end of vector, will be sorted
                                                 // in SelectionMultiAmendsFinish()
  else if (is_contained)
    it->val_i = selected ? selection_order : 0;  // Modify in-place.
  selection->Size += selected ? +1 : -1;
}

static void ImGuiSelectionBasicStorage_BatchFinish(ImGuiSelectionBasicStorage* selection,
                                                   bool selected, int size_before_amends) {
  ImGuiStorage* storage = &selection->_Storage;
...
// - Honoring SetRange requests requires that you can iterate/interpolate between RangeFirstItem and
// RangeLastItem.
//   - In this demo we often submit indices to SetNextItemSelectionUserData() + store the same
//   indices in persistent selection.
//   - Your code may do differently. If you store pointers or objects ID in ImGuiSelectionUserData
//   you may need to perform
//     a lookup in order to have some way to iterate/interpolate between two items.
// - A full-featured application is likely to allow search/filtering which is likely to lead to
// using indices
//   and constructing a view index <> object id/ptr data structure anyway.
// WHEN YOUR APPLICATION SETTLES ON A CHOICE, YOU WILL PROBABLY PREFER TO GET RID OF THIS
// UNNECESSARY 'ImGuiSelectionBasicStorage' INDIRECTION LOGIC. Notice that with the simplest adapter
// (using indices everywhere), all functions return their parameters. The most simple implementation
// (using indices everywhere) would look like:
//   for (ImGuiSelectionRequest& req : ms_io->Requests)
//   {
//      if (req.Type == ImGuiSelectionRequestType_SetAll)    { Clear(); if (req.Selected) { for (int
//      n = 0; n < items_count; n++) { SetItemSelected(n, true); } } if (req.Type ==
//      ImGuiSelectionRequestType_SetRange)  { for (int n = (int)ms_io->RangeFirstItem; n <=
//      (int)ms_io->RangeLastItem; n++) { SetItemSelected(n, ms_io->Selected); } }
//   }
void ImGuiSelectionBasicStorage::ApplyRequests(ImGuiMultiSelectIO* ms_io) {
  ...
  // It makes sense when using ImGuiSelectionBasicStorage to simply pass your items count to
  // BeginMultiSelect(). Other scheme may handle SetAll differently.
  ...
  // - Optimized select can append unsorted, then sort in a second pass. Optimized unselect can
  // clear in-place then compact in a second pass.
  // - A more optimal version wouldn't even use ImGuiStorage but directly a ImVector<ImGuiID> to
  // reduce bandwidth, but this is a reasonable trade off to reuse code.
  // - There are many ways this could be better optimized. The worse case scenario being: using
  // BoxSelect2d in a grid, box-select scrolling down while wiggling
  ...
  for (ImGuiSelectionRequest& req : ms_io->Requests) {
    if (req.Type == ImGuiSelectionRequestType_SetAll) {
      ...
      // ms_io->Requests.index_from_ptr(&req), ms_io->Requests.Size, selection_changes,
      // req.Selected);
      if (selection_changes == 1 || (selection_changes < Size / 100)) {
        // Multiple sorted insertion + copy likely to be faster.
        // Technically we could do a single copy with a little more work (sort sequential SetRange
        // requests)
        for (int idx = (int)req.RangeFirstItem; idx <= (int)req.RangeLastItem; idx++)
          SetItemSelected(GetStorageIdFromIndex(idx), req.Selected);
      } else {
        // Append insertion + single sort likely be faster.
        // Use req.RangeDirection to set order field so that shift+clicking from 1 to 5 is different
        // than shift+clicking from 5 to 1
        const int size_before_amends = _Storage.Data.Size;
        int selection_order =
            _SelectionOrder + ((req.RangeDirection < 0) ? selection_changes - 1 : 0);
        ...
        if (req.Selected) _SelectionOrder += selection_changes;
        ImGuiSelectionBasicStorage_BatchFinish(this, req.Selected, size_before_amends);
      }
    }
  }
...
// Apply requests coming from BeginMultiSelect() and EndMultiSelect().
// We also pull 'ms_io->ItemsCount' as passed for BeginMultiSelect() for consistency with
// ImGuiSelectionBasicStorage This makes no assumption about underlying storage.
...
// ImGuiChildFlags_FrameStyle flag for stylistic changes + displaying a label. This handle some
// subtleties with capturing info from the label. If you don't need a label you can pretty much
// directly use ImGui::BeginChild() with ImGuiChildFlags_FrameStyle. Tip: To have a list filling the
...
bool ImGui::BeginListBox(const char* label, const ImVec2& size_arg) {
  ...
  // scrollbar.
  ImVec2 size =
      ImTrunc(CalcItemSize(size_arg, CalcItemWidth(),
  ...
  // FIXME-OPT: We could omit the BeginGroup() if label_size.x == 0.0f but would need to omit the
  // EndGroup() as well.
  BeginGroup();
  if (label_size.x > 0.0f) {
    ImVec2 label_pos =
...
bool ImGui::ListBox(const char* label, int* current_item,
  ...
  ImVec2 size(0.0f, ImTrunc(GetTextLineHeightWithSpacing() * height_in_items_f +
...
int ImGui::PlotEx(ImGuiPlotType plot_type, const char* label,
  ...
  ItemSize(total_bb, style.FramePadding.y);
  if (!ItemAdd(total_bb, id, &frame_bb, ImGuiItemFlags_NoNav)) return -1;
  bool hovered;
...
void ImGui::PlotLines(const char* label, float (*values_getter)(void* data, int idx), void* data,
  ...
  PlotEx(ImGuiPlotType_Lines, label, values_getter, data, values_count, values_offset, overlay_text,
...
void ImGuiMenuColumns::Update(float spacing, bool window_reappearing) {
  if (window_reappearing) memset(Widths, 0, sizeof(Widths));
...
void ImGuiMenuColumns::CalcNextTotalWidth(bool update_offsets) {
  ...
  for (int i = 0; i < IM_ARRAYSIZE(Widths); i++) {
    ...
    if (want_spacing && width > 0) offset += Spacing;
...
bool ImGui::BeginMenuBar() {
  ...
  ImRect clip_rect(
      IM_ROUND(bar_rect.Min.x + window->WindowBorderSize * 0.5f),
      IM_ROUND(bar_rect.Min.y + border_top),
      IM_ROUND(ImMax(bar_rect.Min.x, bar_rect.Max.x - ImMax(window->WindowRounding,
                                                            window->WindowBorderSize * 0.5f))),
...
void ImGui::EndMenuBar() {
  ...
  if (NavMoveRequestButNoResultYet() &&
    ...
    if (nav_earliest_child->ParentWindow == window &&
      ...
      // situation. We could remove it by scoring in advance for multiple window (probably not worth
      // bothering)
      const ImGuiNavLayer layer = ImGuiNavLayer_Menu;
      IM_ASSERT(window->DC.NavLayersActiveMaskNext &
      ...
      if (g.NavCursorVisible) {
        g.NavCursorVisible = false;  // Hide nav cursor for the current frame so we don't see the
                                     // intermediary selection. Will be set again
  ...
  // FIXME: Extremely confusing, cleanup by (a) working on WorkRect stack system (b) not using a
  // Group confusingly here.
  ...
  EndGroup();  // Restore position on layer 0 // FIXME: Misleading to use a group for that
               // backup/restore
  window->DC.LayoutType = ImGuiLayoutType_Vertical;
  window->DC.IsSameLine = false;
...
bool ImGui::BeginViewportSideBar(const char* name, ImGuiViewport* viewport_p, ImGuiDir dir,
  ...
  if (bar_window == NULL || bar_window->BeginCount == 0) {
    ...
    if (dir == ImGuiDir_Right || dir == ImGuiDir_Down) pos[axis] = avail_rect.Max[axis] - axis_size;
...
bool ImGui::BeginMainMenuBar() {
  ...
  // Temporarily disable _NoSavedSettings, in the off-chance that tables or child windows submitted
  // within the menu-bar may want to use settings. (#8356)
...
void ImGui::EndMainMenuBar() {
  ...
  if (!g.CurrentWindow->DC.MenuBarAppending) {
    IM_ASSERT_USER_ERROR(
        0, "Calling EndMainMenuBar() not from a menu-bar!");  // Not technically testing that it is
                                                              // the main menu bar
    return;
  }
...
static bool IsRootOfOpenMenuSet() {
  ...
  // multiple menu sets from each others (e.g. inside menu bar vs loose menu items) based on parent
  // ID. This would however prevent the use of e.g. PushID() user code submitting menus. Previously
  // this worked between popup and a first child menu because the first child menu always had the
  // _ChildWindow flag, making hovering on parent popup possible while first child menu was focused
  // - but this was generally a bug with other side effects. Instead we don't treat Popup
  ...
  // check to allow hovering between root window/popup and first child menu. In the end, lack of ID
  // check made it so we could no longer differentiate between separate menu sets. To compensate for
  // that, we at least check parent window nav layer. This fixes the most common case of menu
  // opening on hover when moving between window content and menu bar. Multiple different menu sets
  // in same nav layer would still open on hover, but that should be a lesser problem, because if
  // such menus are close in proximity in window content then it won't feel weird and if they are
  // far apart it likely won't be a problem anyone runs into.
...
bool ImGui::BeginMenuEx(const char* label, const char* icon, bool enabled) {
  ...
  // menu would steal focus and not allow hovering on parent menu) The first menu in a hierarchy
  // isn't so hovering doesn't get across (otherwise e.g. resizing borders with
  // ImGuiButtonFlags_FlattenChildren would react), but top-most BeginMenu() will bypass that
  // limitation.
  ...
  if (g.MenusIdSubmittedThisFrame.contains(id)) {
    if (menu_is_open)
      menu_is_open = BeginPopupMenuEx(
          id, label, window_flags);  // menu_is_open can be 'false' when the popup is completely
  ...
  // Odd hack to allow hovering across menus of a same menu-set (otherwise we wouldn't be able to
  // hover parent without always being a Child window) This is only done for items for the menu set
  // and not the full parent window.
  ...
  if (window->DC.LayoutType == ImGuiLayoutType_Horizontal) {
    ...
    PopStyleVar();
    window->DC.CursorPos.x += IM_TRUNC(
        style.ItemSpacing.x *
        (-1.0f +
    ...
    float min_w = window->DC.MenuColumns.DeclColumns(icon_w, label_size.x, 0.0f,
                                                     checkmark_w);  // Feedback to next frame
    ...
    LogSetNextTextDecoration("", ">");
    RenderText(text_pos, label);
    if (icon_w > 0.0f) RenderText(pos + ImVec2(offsets->OffsetIcon, 0.0f), icon);
    RenderArrow(window->DrawList,
                                 ...
                                 // (ImGuiWindowFlags_Popup|ImGuiWindowFlags_ChildMenu))
  {
    ...
    ImGuiWindow* child_menu_window =
        (child_popup && child_popup->Window && child_popup->Window->ParentWindow == window)
            ? child_popup->Window
            : NULL;
    if (g.HoveredWindow == window && child_menu_window != NULL) {
      ...
      const float pad_farmost_h = ImClamp(ImFabs(ta.x - tb.x) * 0.30f, ref_unit * 0.5f,
                                          ref_unit * 2.5f);  // Add a bit of extra slack.
      ta.x += child_dir * -0.5f;
      tb.x += child_dir * ref_unit;
      tc.x += child_dir * ref_unit;
      tb.y = ta.y + ImMax((tb.y - pad_farmost_h) - ta.y,
                          -ref_unit * 8.0f);  // Triangle has maximum height to limit the slope and
                                              // the bias toward large sub-menus
      tc.y = ta.y + ImMin((tc.y + pad_farmost_h) - ta.y, +ref_unit * 8.0f);
      moving_toward_child_menu = ImTriangleContainsPoint(ta, tb, tc, g.IO.MousePos);
      // GetForegroundDrawList()->AddTriangleFilled(ta, tb, tc, moving_toward_child_menu ?
      // IM_COL32(0,128,0,128) : IM_COL32(128,0,0,128)); // [DEBUG]
    }

    // The 'HovereWindow == window' check creates an inconsistency (e.g. moving away from menu
    // slowly tends to hit same window, whereas moving away fast does not) But we also need to not
    // close the top-menu menu when moving over void. Perhaps we should extend the triangle check to
    // a larger polygon. (Remember to test this on BeginPopup("A")->BeginMenu("B") sequence which
    // behaves slightly differently as B isn't a Child of A and hovering isn't shared.)
  ...
  if (want_open && !menu_is_open && g.OpenPopupStack.Size > g.BeginPopupStack.Size) {
    // Don't reopen/recycle same menu level in the same frame if it is a different menu ID, first
    // close the other menu and yield for a frame.
    OpenPopup(label);
  } else if (want_open) {
    menu_is_open = true;
    OpenPopup(label, ImGuiPopupFlags_NoReopen);  // | (want_open_nav_init ?
                                                 // ImGuiPopupFlags_NoReopenAlwaysNavInit : 0));
  }

  if (menu_is_open) {
    ImGuiLastItemData last_item_in_parent = g.LastItemData;
    SetNextWindowPos(popup_pos,
                     ImGuiCond_Always);  // Note: misleading: the value will serve as reference for
                                         // FindBestWindowPosForPopup(), not actual pos.
    PushStyleVar(ImGuiStyleVar_ChildRounding,
                 style.PopupRounding);  // First level will use _PopupRounding, subsequent will use
                                        // _ChildRounding
    menu_is_open =
        BeginPopupMenuEx(id, label, window_flags);  // menu_is_open may be 'false' when the popup is
    ...
    if (menu_is_open) {
      ...
      // (This fixes using IsItemClicked() and IsItemHovered(), but IsItemHovered() also relies on
      // its support for ImGuiItemFlags_NoWindowHoverableCheck)
...
bool ImGui::BeginMenu(const char* label, bool enabled) { return BeginMenuEx(label, NULL, enabled); }
...
bool ImGui::MenuItemEx(const char* label, const char* icon, const char* shortcut, bool selected,
                       bool enabled) {
  ImGuiWindow* window = GetCurrentWindow();
  if (window->SkipItems) return false;
  ...
  const ImGuiMenuColumns* offsets = &window->DC.MenuColumns;
  if (window->DC.LayoutType == ImGuiLayoutType_Horizontal) {
    // Mimic the exact layout spacing of BeginMenu() to allow MenuItem() inside a menu bar, which is
    // a little misleading but may be useful Note that in this situation: we don't render the
    ...
    PopStyleVar();
    if (g.LastItemData.StatusFlags & ImGuiItemStatusFlags_Visible) RenderText(text_pos, label);
    window->DC.CursorPos.x += IM_TRUNC(
        style.ItemSpacing.x *
        (-1.0f +
         0.5f));  // -1 spacing to compensate the spacing added when Selectable() did a SameLine().
...
bool ImGui::BeginTabBarEx(ImGuiTabBar* tab_bar, const ImRect& tab_bar_bb, ImGuiTabBarFlags flags) {
  ...
  // Ensure correct ordering when toggling ImGuiTabBarFlags_Reorderable flag, or when a new tab was
  // added while being not reorderable
  ...
  // Draw separator
  // (it would be misleading to draw this in EndTabBar() suggesting that it may be drawn over tabs,
  // as tab bar are appendable)
  const ImU32 col = GetColorU32((flags & ImGuiTabBarFlags_IsFocused) ? ImGuiCol_TabSelected
...
static void ImGui::TabBarLayout(ImGuiTabBar* tab_bar) {
  ...
  const float shrink_min_width =
      (tab_bar->Flags & ImGuiTabBarFlags_FittingPolicyMixed) ? g.Style.TabMinWidthShrink : 1.0f;
  ...
  for (int tab_n = 0; tab_n < tab_bar->Tabs.Size; tab_n++) {
    ...
    // order we occasionally insert new tabs that don't have a width yet, and we cannot wait for the
    // next BeginTabItem() call. We cannot compute this width within TabBarAddTab() because font
    // size depends on the active window.
    const char* tab_name = TabBarGetTabName(tab_bar, tab);
    const bool has_close_button_or_unsaved_marker =
        (tab->Flags & ImGuiTabItemFlags_NoCloseButton) == 0 ||
        (tab->Flags & ImGuiTabItemFlags_UnsavedDocument);
    ...
    section->WidthAfterShrinkMinWidth +=
        ImMin(tab->ContentWidth, shrink_min_width) +
  ...
  for (int section_n = 0; section_n < 3; section_n++) {
    ...
    width_all_tabs_after_min_width_shrink +=
        sections[section_n].WidthAfterShrinkMinWidth + sections[section_n].Spacing;
  }

  // Horizontal scrolling buttons
  // Important: note that TabBarScrollButtons() will alter BarRect.Max.x.
  const bool can_scroll = (tab_bar->Flags & ImGuiTabBarFlags_FittingPolicyScroll) ||
                          (tab_bar->Flags & ImGuiTabBarFlags_FittingPolicyMixed);
  const float width_all_tabs_to_use_for_scroll =
      (tab_bar->Flags & ImGuiTabBarFlags_FittingPolicyScroll)
          ? tab_bar->WidthAllTabs
          : width_all_tabs_after_min_width_shrink;
  tab_bar->ScrollButtonEnabled =
      ((width_all_tabs_to_use_for_scroll > tab_bar->BarRect.GetWidth() && tab_bar->Tabs.Size > 1) &&
       !(tab_bar->Flags & ImGuiTabBarFlags_NoTabListScrollingButtons) && can_scroll);
  ...
  if (scroll_to_tab_id == 0 && scroll_to_selected_tab) scroll_to_tab_id = tab_bar->SelectedTabId;
  ...
  const bool can_shrink = (tab_bar->Flags & ImGuiTabBarFlags_FittingPolicyShrink) ||
                          (tab_bar->Flags & ImGuiTabBarFlags_FittingPolicyMixed);
  ...
  const bool tab_bar_appearing = (tab_bar->PrevFrameVisible + 1 < g.FrameCount);
  if (found_selected_tab_id == false && !tab_bar_appearing) tab_bar->SelectedTabId = 0;
  ...
  else if (tab_bar->ScrollButtonEnabled &&
...
ImGuiTabItem* ImGui::TabBarFindTabByOrder(ImGuiTabBar* tab_bar, int order) {
  if (order < 0 || order >= tab_bar->Tabs.Size) return NULL;
  return &tab_bar->Tabs[order];
}

ImGuiTabItem* ImGui::TabBarGetCurrentTab(ImGuiTabBar* tab_bar) {
  if (tab_bar->LastTabItemIdx < 0 || tab_bar->LastTabItemIdx >= tab_bar->Tabs.Size) return NULL;
...
// The *TabId fields are already set by the docking system _before_ the actual TabItem was created,
...
void ImGui::TabBarCloseTab(ImGuiTabBar* tab_bar, ImGuiTabItem* tab) {
  ...
  if ((tab->Flags & (ImGuiTabItemFlags_UnsavedDocument | ImGuiTabItemFlags_NoAssumedClosure)) ==
      0) {
    // This will remove a frame of lag for selecting another tab on closure.
    // However we don't run it in the case where the 'Unsaved' flag is set, so user gets a chance to
    // fully undo the closure
    tab->WantClose = true;
    if (tab_bar->VisibleTabId == tab->ID) {
    ...
    // Actually select before expecting closure attempt (on an UnsavedDocument tab user is expect to
    // e.g. show a popup)
...
void ImGui::TabBarQueueFocus(ImGuiTabBar* tab_bar, const char* tab_name) {
  IM_ASSERT((tab_bar->Flags & ImGuiTabBarFlags_DockNode) ==
            0);  // Only supported for manual/explicit tab bars
...
void ImGui::TabBarQueueReorderFromMousePos(ImGuiTabBar* tab_bar, ImGuiTabItem* src_tab,
                                           ImVec2 mouse_pos) {
  ImGuiContext& g = *GImGui;
  IM_ASSERT(tab_bar->ReorderRequestTabId == 0);
...
bool ImGui::TabBarProcessReorder(ImGuiTabBar* tab_bar) {
  ...
  // Reordered tabs must share the same section
  // (Note: TabBarQueueReorderFromMousePos() also has a similar test but since we allow direct calls
  // to TabBarQueueReorder() we do it here too)
...
bool ImGui::TabItemButton(const char* label, ImGuiTabItemFlags flags) {
  ...
  return TabItemEx(tab_bar, label, NULL,
                   flags | ImGuiTabItemFlags_Button | ImGuiTabItemFlags_NoReorder, NULL);
}
...
void ImGui::TabItemSpacing(const char* str_id, ImGuiTabItemFlags flags, float width) {
  ...
  TabItemEx(
      tab_bar, str_id, NULL,
      flags | ImGuiTabItemFlags_Button | ImGuiTabItemFlags_NoReorder | ImGuiTabItemFlags_Invisible,
      NULL);
}
...
bool ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open,
  ...
  // We make a call to ItemAdd() so that attempts to use a contextual popup menu with an implicit ID
  // won't use an older ID.
  IMGUI_TEST_ENGINE_ITEM_INFO(id, label, g.LastItemData.StatusFlags);
  if (p_open && !*p_open) {
  ...
  ImVec2 size =
      TabItemCalcSize(label, (p_open != NULL) || (flags & ImGuiTabItemFlags_UnsavedDocument));
  tab->RequestedWidth = -1.0f;
  if (g.NextItemData.HasFlags & ImGuiNextItemDataFlags_HasWidth)
    size.x = tab->RequestedWidth = g.NextItemData.Width;
  if (tab_is_new) tab->Width = ImMax(1.0f, size.x);
  ...
  const bool tab_just_unsaved = (flags & ImGuiTabItemFlags_UnsavedDocument) &&
                                !(tab->Flags & ImGuiTabItemFlags_UnsavedDocument);
  ...
  // On the very first frame of a tab bar we let first tab contents be visible to minimize appearing
  // glitches
  ...
  if (is_central_section)
    window->DC.CursorPos =
        tab_bar->BarRect.Min + ImVec2(IM_TRUNC(tab->Offset - tab_bar->ScrollingAnim), 0.0f);
  else
  ...
  // Render tab shape
  const bool is_visible = (g.LastItemData.StatusFlags & ImGuiItemStatusFlags_Visible) &&
                          !(flags & ImGuiTabItemFlags_Invisible);
  if (is_visible) {
    ...
    if (tab_contents_visible && (tab_bar->Flags & ImGuiTabBarFlags_DrawSelectedOverline) &&
      ...
      ImU32 overline_col = GetColorU32(tab_bar_focused ? ImGuiCol_TabSelectedOverline
    ...
    TabItemLabelAndCloseButton(
        ...
        tab_bar->FramePadding, label, id, close_button_id, tab_contents_visible, &just_closed,
        &text_clipped);
    ...
    // Tooltip
    // (Won't work over the close button because ItemOverlap systems messes up with HoveredIdTimer->
    // seems ok) (We test IsItemHovered() to discard e.g. when another item is active or drag and
    // drop over the tab bar, which g.HoveredId ignores)
...
void ImGui::TabItemBackground(ImDrawList* draw_list, const ImRect& bb, ImGuiTabItemFlags flags,
  ...
  const float rounding = ImMax(
      0.0f, ImMin((flags & ImGuiTabItemFlags_Button) ? g.Style.FrameRounding : g.Style.TabRounding,
                  width * 0.5f - 1.0f));
  ...
  if (g.Style.TabBorderSize > 0.0f) {
    ...
    draw_list->PathArcToFast(ImVec2(bb.Min.x + rounding + 0.5f, y1 + rounding + 0.5f), rounding, 6,
                             9);
    draw_list->PathArcToFast(ImVec2(bb.Max.x - rounding - 0.5f, y1 + rounding + 0.5f), rounding, 9,
                             12);
...
void ImGui::TabItemLabelAndCloseButton(ImDrawList* draw_list, const ImRect& bb,
  ...
  ImRect text_ellipsis_clip_bb(bb.Min.x + frame_padding.x, bb.Min.y + frame_padding.y,
  ...
  if (out_text_clipped) {
    *out_text_clipped = (text_ellipsis_clip_bb.Min.x + label_size.x) > text_ellipsis_clip_bb.Max.x;
    // draw_list->AddCircle(text_ellipsis_clip_bb.Min, 3.0f, *out_text_clipped ? IM_COL32(255, 0, 0,
    // 255) : IM_COL32(0, 255, 0, 255));
  }

  const float button_sz = g.FontSize;
  const ImVec2 button_pos(ImMax(bb.Min.x, bb.Max.x - frame_padding.x - button_sz),
                          bb.Min.y + frame_padding.y);

  // Close Button & Unsaved Marker
  // We are relying on a subtle and confusing distinction between 'hovered' and 'g.HoveredId' which
  // happens because we are using ImGuiButtonFlags_AllowOverlapMode + SetItemAllowOverlap()
    ...
    if (is_contents_visible)
      close_button_visible =
          (g.Style.TabCloseButtonMinWidthSelected < 0.0f)
              ? true
              : (is_hovered &&
                 bb.GetWidth() >= ImMax(button_sz, g.Style.TabCloseButtonMinWidthSelected));
    else
      close_button_visible =
          (g.Style.TabCloseButtonMinWidthUnselected < 0.0f)
              ? true
              : (is_hovered &&
                 bb.GetWidth() >= ImMax(button_sz, g.Style.TabCloseButtonMinWidthUnselected));
  }
  ...
  if (unsaved_marker_visible) {
    ...
    if (is_hovered && !(flags & ImGuiTabItemFlags_NoCloseWithMiddleMouseButton) &&
        IsMouseClicked(2))
      close_button_pressed = true;
  }

  // This is all rather complicated
  // (the main idea is that because the close button only appears on hover, we don't want it to
  // alter the ellipsis position)
  ...
  if (close_button_visible || unsaved_marker_visible) {
    const bool visible_without_hover =
        ...
        (is_contents_visible ? g.Style.TabCloseButtonMinWidthSelected
                             : g.Style.TabCloseButtonMinWidthUnselected) < 0.0f;

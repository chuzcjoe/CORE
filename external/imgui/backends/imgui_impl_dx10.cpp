...
//  [X] Renderer: User texture binding. Use 'ID3D10ShaderResourceView*' as texture identifier. Read
...
//  2025-06-11: DirectX10: Added support for ImGuiBackendFlags_RendererHasTextures, for dynamic font
//  atlas. 2025-05-07: DirectX10: Honor draw_data->FramebufferScale to allow for custom backends and
//  experiment using it (consistently with other renderer backends, even though in normal condition
//  it is not set under Windows). 2025-01-06: DirectX10: Expose selected render state in
//  ImGui_ImplDX10_RenderState, which you can access in 'void* platform_io.Renderer_RenderState'
//  during draw callbacks. 2024-10-07: DirectX10: Changed default texture sampler to Clamp instead
//  of Repeat/Wrap. 2022-10-11: Using 'nullptr' instead of 'NULL' as per our switch to C++11.
//  2021-06-29: Reorganized backend to pull data from a single structure to facilitate usage with
//  multiple-contexts (all g_XXXX access changed to bd->XXXX). 2021-05-19: DirectX10: Replaced
//  direct access to ImDrawCmd::TextureId with a call to ImDrawCmd::GetTexID(). (will become a
//  requirement) 2021-02-18: DirectX10: Change blending equation to preserve alpha in output buffer.
//  2019-07-21: DirectX10: Backup, clear and restore Geometry Shader is any is bound when calling
//  ImGui_ImplDX10_RenderDrawData(). 2019-05-29: DirectX10: Added support for large mesh (64K+
//  vertices), enable ImGuiBackendFlags_RendererHasVtxOffset flag. 2019-04-30: DirectX10: Added
//  support for special ImDrawCallback_ResetRenderState callback to reset render state. 2018-12-03:
//  Misc: Added #pragma comment statement to automatically link with d3dcompiler.lib when using
//  D3DCompile(). 2018-11-30: Misc: Setting up io.BackendRendererName so it can be displayed in the
//  About Window. 2018-07-13: DirectX10: Fixed unreleased resources in Init and Shutdown functions.
//  2018-06-08: Misc: Extracted imgui_impl_dx10.cpp/.h away from the old combined DX10+Win32
//  example. 2018-06-08: DirectX10: Use draw_data->DisplayPos and draw_data->DisplaySize to setup
//  projection matrix and clipping rectangle. 2018-04-09: Misc: Fixed erroneous call to
//  io.Fonts->ClearInputData() + ClearTexData() that was left in DX10 example but removed in 1.47
//  (Nov 2015) on other backends. 2018-02-16: Misc: Obsoleted the io.RenderDrawListsFn callback and
//  exposed ImGui_ImplDX10_RenderDrawData() in the .h file so you can call it yourself. 2018-02-06:
...
static ImGui_ImplDX10_Data* ImGui_ImplDX10_GetBackendData() {
  return ImGui::GetCurrentContext() ? (ImGui_ImplDX10_Data*)ImGui::GetIO().BackendRendererUserData
                                    : nullptr;
}
...
static void ImGui_ImplDX10_SetupRenderState(ImDrawData* draw_data, ID3D10Device* device) {
  ...
  device->IASetIndexBuffer(bd->pIB,
...
void ImGui_ImplDX10_RenderDrawData(ImDrawData* draw_data) {
  ...
  if (draw_data->Textures != nullptr)
    for (ImTextureData* tex : *draw_data->Textures)
      if (tex->Status != ImTextureStatus_OK) ImGui_ImplDX10_UpdateTexture(tex);
  ...
  if (!bd->pVB || bd->VertexBufferSize < draw_data->TotalVtxCount) {
    ...
    desc.MiscFlags = 0;
    if (device->CreateBuffer(&desc, nullptr, &bd->pVB) < 0) return;
  }
  ...
  if (!bd->pIB || bd->IndexBufferSize < draw_data->TotalIdxCount) {
    ...
    desc.CPUAccessFlags = D3D10_CPU_ACCESS_WRITE;
    if (device->CreateBuffer(&desc, nullptr, &bd->pIB) < 0) return;
  }
  ...
  for (const ImDrawList* draw_list : draw_data->CmdLists) {
    for (int cmd_i = 0; cmd_i < draw_list->CmdBuffer.Size; cmd_i++) {
      ...
      if (pcmd->UserCallback != nullptr) {
        ...
        const D3D10_RECT r = {(LONG)clip_min.x, (LONG)clip_min.y, (LONG)clip_max.x,
                              (LONG)clip_max.y};
  ...
  device->PSSetShader(old.PS);
  if (old.PS) old.PS->Release();
  device->VSSetShader(old.VS);
  if (old.VS) old.VS->Release();
  device->GSSetShader(old.GS);
...
void ImGui_ImplDX10_UpdateTexture(ImTextureData* tex) {
  ...
  if (tex->Status == ImTextureStatus_WantCreate) {
    ...
    bd->pd3dDevice->CreateShaderResourceView(backend_tex->pTexture, &srv_desc,
    ...
    for (ImTextureRect& r : tex->Updates) {
      D3D10_BOX box = {(UINT)r.x,         (UINT)r.y,         (UINT)0,
                       (UINT)(r.x + r.w), (UINT)(r.y + r.h), (UINT)1};
      bd->pd3dDevice->UpdateSubresource(backend_tex->pTexture, 0, &box, tex->GetPixelsAt(r.x, r.y),
                                        (UINT)tex->GetPitch(), 0);
    }
    tex->SetStatus(ImTextureStatus_OK);
  }
...
bool ImGui_ImplDX10_CreateDeviceObjects() {
  ...
  // given version of d3dcompiler_XX.dll (see D3DCOMPILER_DLL_A) If you would like to use this DX10
...
void ImGui_ImplDX10_InvalidateDeviceObjects() {
  ...
  for (ImTextureData* tex : ImGui::GetPlatformIO().Textures)
    if (tex->RefCount == 1) ImGui_ImplDX10_DestroyTexture(tex);
...
void ImGui_ImplDX10_NewFrame() {
  ...
  IM_ASSERT(bd != nullptr &&
            "Context or backend not initialized! Did you call ImGui_ImplDX10_Init()?");

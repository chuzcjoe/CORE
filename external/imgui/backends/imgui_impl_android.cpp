...
//  AKEYCODE_* values are obsolete since 1.87 and not supported since 1.91.5] [X] Platform: Mouse
//  support. Can discriminate Mouse/TouchScreen/Pen.
// Missing features or Issues:
//  [ ] Platform: Clipboard support.
//  [ ] Platform: Gamepad support.
//  [ ] Platform: Mouse cursor shape and visibility (ImGuiBackendFlags_HasMouseCursors). Disable
//  with 'io.ConfigFlags |= ImGuiConfigFlags_NoMouseCursorChange'. FIXME: Check if this is even
//  possible with Android.
// Important:
//  - Consider using SDL or GLFW backend on Android, which will be more full-featured than this.
//  - FIXME: On-screen keyboard currently needs to be enabled by the application (see examples/ and
//  issue #3446)
//  - FIXME: Unicode character inputs needs to be passed by Dear ImGui by the application (see
//  examples/ and issue #3446)
...
//  (1.87+). 2022-01-10: Inputs: calling new io.AddKeyEvent(), io.AddKeyModsEvent() +
//  io.SetKeyEventNativeData() API (1.87+). Support for full ImGuiKey range. 2021-03-04: Initial
//  version.
...
int32_t ImGui_ImplAndroid_HandleInputEvent(const AInputEvent* input_event) {
  ...
  switch (event_type) {
    case AINPUT_EVENT_TYPE_KEY: {
      ...
      switch (event_action) {
        // FIXME: AKEY_EVENT_ACTION_DOWN and AKEY_EVENT_ACTION_UP occur at once as soon as a touch
        // pointer goes up from a key. We use a simple key event queue/ and process one event per
        // key per frame in ImGui_ImplAndroid_NewFrame()...or consider using IO queue, if suitable:
    ...
    case AINPUT_EVENT_TYPE_MOTION: {
      ...
      switch (event_action) {
        ...
        case AMOTION_EVENT_ACTION_UP: {
          ...
          // AMOTION_EVENT_ACTION_DOWN/_UP, but we have to process them separately to identify the
          // actual button pressed. This is done below via
          // AMOTION_EVENT_ACTION_BUTTON_PRESS/_RELEASE. Here, we only process "FINGER" input (and
        ...
        case AMOTION_EVENT_ACTION_HOVER_MOVE:  // Hovering: Tool moves while NOT pressed (such as a
                                               // physical mouse)
        ...
        case AMOTION_EVENT_ACTION_SCROLL:
          io.AddMouseWheelEvent(AMotionEvent_getAxisValue(input_event, AMOTION_EVENT_AXIS_HSCROLL,
                                                          event_pointer_index),

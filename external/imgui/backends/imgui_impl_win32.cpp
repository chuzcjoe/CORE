// dear imgui: Platform Backend for Windows (standard windows API for 32-bits AND 64-bits
// applications) This needs to be used along with a Renderer (e.g. DirectX11, OpenGL3, Vulkan..)

// Implemented features:
//  [X] Platform: Clipboard support (for Win32 this is actually part of core dear imgui)
//  [X] Platform: Mouse support. Can discriminate Mouse/TouchScreen/Pen.
//  [X] Platform: Keyboard support. Since 1.87 we are using the io.AddKeyEvent() function. Pass
//  ImGuiKey values to all key functions e.g. ImGui::IsKeyPressed(ImGuiKey_Space). [Legacy VK_*
//  values are obsolete since 1.87 and not supported since 1.91.5] [X] Platform: Gamepad support.
//  [X] Platform: Mouse cursor shape and visibility (ImGuiBackendFlags_HasMouseCursors). Disable
//  with 'io.ConfigFlags |= ImGuiConfigFlags_NoMouseCursorChange'.
...
// #define IMGUI_IMPL_WIN32_DISABLE_GAMEPAD              // Disable gamepad support. This was
// meaningful before <1.81 but we now load XInput dynamically so the option is now less relevant.

// CHANGELOG
// (minor and older changes stripped away, please see git history for details)
//  2025-09-23: Inputs: Minor optimization not submitting gamepad input if packet number has not
//  changed. 2025-09-18: Call platform_io.ClearPlatformHandlers() on shutdown. 2025-04-30: Inputs:
//  Fixed an issue where externally losing mouse capture (due to e.g. focus loss) would fail to
//  claim it again the next subsequent click. (#8594) 2025-03-10: When dealing with OEM keys, use
//  scancodes instead of translated keycodes to choose ImGuiKey values. (#7136, #7201, #7206, #7306,
//  #7670, #7672, #8468) 2025-02-18: Added ImGuiMouseCursor_Wait and ImGuiMouseCursor_Progress mouse
//  cursor support. 2024-07-08: Inputs: Fixed ImGuiMod_Super being mapped to VK_APPS instead of
//  VK_LWIN||VK_RWIN. (#7768) 2023-10-05: Inputs: Added support for extra ImGuiKey values: F13 to
//  F24 function keys, app back/forward keys. 2023-09-25: Inputs: Synthesize key-down event on
//  key-up for VK_SNAPSHOT / ImGuiKey_PrintScreen as Windows doesn't emit it (same behavior as
//  GLFW/SDL). 2023-09-07: Inputs: Added support for keyboard codepage conversion for when
//  application is compiled in MBCS mode and using a non-Unicode window. 2023-04-19: Added
//  ImGui_ImplWin32_InitForOpenGL() to facilitate combining raw Win32/Winapi with OpenGL. (#3218)
//  2023-04-04: Inputs: Added support for io.AddMouseSourceEvent() to discriminate
//  ImGuiMouseSource_Mouse/ImGuiMouseSource_TouchScreen/ImGuiMouseSource_Pen. (#2702) 2023-02-15:
//  Inputs: Use WM_NCMOUSEMOVE / WM_NCMOUSELEAVE to track mouse position over non-client area (e.g.
//  OS decorations) when app is not focused. (#6045, #6162) 2023-02-02: Inputs: Flipping
//  WM_MOUSEHWHEEL (horizontal mouse-wheel) value to match other backends and offer consistent
//  horizontal scrolling direction. (#4019, #6096, #1463) 2022-10-11: Using 'nullptr' instead of
//  'NULL' as per our switch to C++11. 2022-09-28: Inputs: Convert WM_CHAR values with
//  MultiByteToWideChar() when window class was registered as MBCS (not Unicode). 2022-09-26:
//  Inputs: Renamed ImGuiKey_ModXXX introduced in 1.87 to ImGuiMod_XXX (old names still supported).
//  2022-01-26: Inputs: replaced short-lived io.AddKeyModsEvent() (added two weeks ago) with
//  io.AddKeyEvent() using ImGuiKey_ModXXX flags. Sorry for the confusion. 2021-01-20: Inputs:
//  calling new io.AddKeyAnalogEvent() for gamepad support, instead of writing directly to
//  io.NavInputs[]. 2022-01-17: Inputs: calling new io.AddMousePosEvent(), io.AddMouseButtonEvent(),
//  io.AddMouseWheelEvent() API (1.87+). 2022-01-17: Inputs: always update key mods next and before
//  a key event (not in NewFrame) to fix input queue with very low framerates. 2022-01-12: Inputs:
//  Update mouse inputs using WM_MOUSEMOVE/WM_MOUSELEAVE + fallback to provide it when focused but
//  not hovered/captured. More standard and will allow us to pass it to future input queue API.
//  2022-01-12: Inputs: Maintain our own copy of MouseButtonsDown mask instead of using
...
//  full ImGuiKey range. 2021-12-16: Inputs: Fill
//  VK_LCONTROL/VK_RCONTROL/VK_LSHIFT/VK_RSHIFT/VK_LMENU/VK_RMENU for completeness. 2021-08-17:
//  Calling io.AddFocusEvent() on WM_SETFOCUS/WM_KILLFOCUS messages. 2021-08-02: Inputs: Fixed
//  keyboard modifiers being reported when host window doesn't have focus. 2021-07-29: Inputs:
//  MousePos is correctly reported when the host platform window is hovered but not focused (using
//  TrackMouseEvent() to receive WM_MOUSELEAVE events). 2021-06-29: Reorganized backend to pull data
//  from a single structure to facilitate usage with multiple-contexts (all g_XXXX access changed to
//  bd->XXXX). 2021-06-08: Fixed ImGui_ImplWin32_EnableDpiAwareness() and
//  ImGui_ImplWin32_GetDpiScaleForMonitor() to handle Windows 8.1/10 features without a manifest
//  (per-monitor DPI, and properly calls SetProcessDpiAwareness() on 8.1). 2021-03-23: Inputs:
//  Clearing keyboard down array when losing focus (WM_KILLFOCUS). 2021-02-18: Added
//  ImGui_ImplWin32_EnableAlphaCompositing(). Non Visual Studio users will need to link with
//  dwmapi.lib (MinGW/gcc: use -ldwmapi). 2021-02-17: Fixed ImGui_ImplWin32_EnableDpiAwareness()
//  attempting to get SetProcessDpiAwareness from shcore.dll on Windows 8 whereas it is only
//  supported on Windows 8.1. 2021-01-25: Inputs: Dynamically loading XInput DLL. 2020-12-04: Misc:
//  Fixed setting of io.DisplaySize to invalid/uninitialized data when after hwnd has been closed.
...
//  the client area when dragging. 2016-11-12: Inputs: Only call Win32 ::SetCursor(nullptr) when
...
static ImGui_ImplWin32_Data* ImGui_ImplWin32_GetBackendData() {
  return ImGui::GetCurrentContext() ? (ImGui_ImplWin32_Data*)ImGui::GetIO().BackendPlatformUserData
                                    : nullptr;
}
...
static void ImGui_ImplWin32_AddKeyEvent(ImGuiIO& io, ImGuiKey key, bool down, int native_keycode,
...
static void ImGui_ImplWin32_ProcessKeyEventsWorkarounds(ImGuiIO& io) {
  // Left & right Shift keys: when both are pressed together, Windows tend to not generate the
  // WM_KEYUP event for the first released one.
  ...
  // Sometimes WM_KEYUP for Win key is not passed down to the app (e.g. for Win+V on some setups,
  // according to GLFW).
...
static void ImGui_ImplWin32_UpdateMouseData(ImGuiIO& io) {
  ...
  if (is_app_focused) {
    ...
    // (Optional) Fallback to provide mouse position when focused (WM_MOUSEMOVE already provides
    // this when hovered or captured) This also fills a short gap when clicking non-client area:
    // WM_NCMOUSELEAVE -> modal OS move -> gap -> WM_NCMOUSEMOVE
...
static void ImGui_ImplWin32_UpdateGamepads(ImGuiIO& io) {
  ...
  if (!bd->HasGamepad || bd->XInputGetState == nullptr ||
    ...
    return;
  io.BackendFlags |= ImGuiBackendFlags_HasGamepad;
  if (bd->XInputPacketNumber != 0 && bd->XInputPacketNumber == xinput_state.dwPacketNumber) return;
...
#define MAP_ANALOG(KEY_NO, VALUE, V0, V1)                      \
  ...
  MAP_ANALOG(ImGuiKey_GamepadLStickLeft, gamepad.sThumbLX, -XINPUT_GAMEPAD_LEFT_THUMB_DEADZONE,
             -32768);
  MAP_ANALOG(ImGuiKey_GamepadLStickRight, gamepad.sThumbLX, +XINPUT_GAMEPAD_LEFT_THUMB_DEADZONE,
             +32767);
  MAP_ANALOG(ImGuiKey_GamepadLStickUp, gamepad.sThumbLY, +XINPUT_GAMEPAD_LEFT_THUMB_DEADZONE,
             +32767);
  MAP_ANALOG(ImGuiKey_GamepadLStickDown, gamepad.sThumbLY, -XINPUT_GAMEPAD_LEFT_THUMB_DEADZONE,
             -32768);
  MAP_ANALOG(ImGuiKey_GamepadRStickLeft, gamepad.sThumbRX, -XINPUT_GAMEPAD_LEFT_THUMB_DEADZONE,
             -32768);
  MAP_ANALOG(ImGuiKey_GamepadRStickRight, gamepad.sThumbRX, +XINPUT_GAMEPAD_LEFT_THUMB_DEADZONE,
             +32767);
  MAP_ANALOG(ImGuiKey_GamepadRStickUp, gamepad.sThumbRY, +XINPUT_GAMEPAD_LEFT_THUMB_DEADZONE,
             +32767);
  MAP_ANALOG(ImGuiKey_GamepadRStickDown, gamepad.sThumbRY, -XINPUT_GAMEPAD_LEFT_THUMB_DEADZONE,
             -32768);
#undef MAP_BUTTON
#undef MAP_ANALOG
...
void ImGui_ImplWin32_NewFrame() {
  ...
  IM_ASSERT(bd != nullptr &&
            "Context or backend not initialized? Did you call ImGui_ImplWin32_Init()?");
  ImGuiIO& io = ImGui::GetIO();
...
ImGuiKey ImGui_ImplWin32_KeyEventToImGuiKey(WPARAM wParam, LPARAM lParam) {
  ...
  if ((wParam == VK_RETURN) && (HIWORD(lParam) & KF_EXTENDED)) return ImGuiKey_KeypadEnter;
  ...
  switch (scancode) {
    case 41:
      return ImGuiKey_GraveAccent;  // VK_OEM_8 in EN-UK, VK_OEM_3 in EN-US, VK_OEM_7 in FR,
                                    // VK_OEM_5 in DE, etc.
    case 12:
...
// Prefer to call this at the top of the message handler to avoid the possibility of other Win32
// calls interfering with this.
...
// Call from your application's message handler. Keep calling your message handler unless this
// function returns TRUE. When implementing your own backend, you can read the io.WantCaptureMouse,
...
// inputs to Dear ImGui, and hide them from your application based on those two flags. PS: We treat
// DBLCLK messages as regular mouse down messages, so this code will work on windows classes that
...
ImGui_ImplWin32_WndProcHandlerEx(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam,
...
IMGUI_IMPL_API LRESULT ImGui_ImplWin32_WndProcHandler(HWND hwnd, UINT msg, WPARAM wParam,
                                                      LPARAM lParam) {
  // Most backends don't have silent checks like this one, but we need it because WndProc are called
  // early in CreateWindow(). We silently allow both context or just only backend data to be
  // nullptr.
  if (ImGui::GetCurrentContext() == nullptr) return 0;
  return ImGui_ImplWin32_WndProcHandlerEx(hwnd, msg, wParam, lParam, ImGui::GetIO());
}

// This version is in theory thread-safe in the sense that no path should access
// ImGui::GetCurrentContext().
IMGUI_IMPL_API LRESULT ImGui_ImplWin32_WndProcHandlerEx(HWND hwnd, UINT msg, WPARAM wParam,
  ...
  switch (msg) {
    ...
    case WM_NCMOUSEMOVE: {
      ...
      if (msg == WM_NCMOUSEMOVE &&
          ::ScreenToClient(hwnd, &mouse_pos) ==
              FALSE)  // WM_NCMOUSEMOVE are provided in absolute coordinates.
        return 0;
    ...
    case WM_NCMOUSELEAVE: {
      ...
      if (bd->MouseTrackedArea == area) {
        if (bd->MouseHwnd == hwnd) bd->MouseHwnd = nullptr;
    ...
    case WM_XBUTTONDBLCLK: {
      ...
      if (bd->MouseButtonsDown != 0 &&
          hwnd_with_capture != hwnd)  // Did we externally lost capture?
        bd->MouseButtonsDown = 0;
      if (bd->MouseButtonsDown == 0 && hwnd_with_capture == nullptr)
        ::SetCapture(hwnd);  // Allow us to read mouse coordinates when dragging mouse outside of
                             // our window bounds.
    ...
    case WM_SYSKEYUP: {
      ...
      if (wParam < 256) {
        ...
        // Special behavior for VK_SNAPSHOT / ImGuiKey_PrintScreen as Windows doesn't emit the key
        // down event.
        if (key == ImGuiKey_PrintScreen && !is_key_down)
          ImGui_ImplWin32_AddKeyEvent(io, key, true, vk, scancode);

        // Submit key event
        if (key != ImGuiKey_None) ImGui_ImplWin32_AddKeyEvent(io, key, is_key_down, vk, scancode);

        // Submit individual left/right modifier events
        if (vk == VK_SHIFT) {
          // Important: Shift keys tend to get stuck when pressed together, missing key-up events
          // are corrected in ImGui_ImplWin32_ProcessKeyEventsWorkarounds()
          if (IsVkDown(VK_LSHIFT) == is_key_down) {
            ImGui_ImplWin32_AddKeyEvent(io, ImGuiKey_LeftShift, is_key_down, VK_LSHIFT, scancode);
          }
          if (IsVkDown(VK_RSHIFT) == is_key_down) {
            ImGui_ImplWin32_AddKeyEvent(io, ImGuiKey_RightShift, is_key_down, VK_RSHIFT, scancode);
          }
        } else if (vk == VK_CONTROL) {
          if (IsVkDown(VK_LCONTROL) == is_key_down) {
            ImGui_ImplWin32_AddKeyEvent(io, ImGuiKey_LeftCtrl, is_key_down, VK_LCONTROL, scancode);
          }
          if (IsVkDown(VK_RCONTROL) == is_key_down) {
            ImGui_ImplWin32_AddKeyEvent(io, ImGuiKey_RightCtrl, is_key_down, VK_RCONTROL, scancode);
          }
        } else if (vk == VK_MENU) {
          if (IsVkDown(VK_LMENU) == is_key_down) {
            ImGui_ImplWin32_AddKeyEvent(io, ImGuiKey_LeftAlt, is_key_down, VK_LMENU, scancode);
          }
          if (IsVkDown(VK_RMENU) == is_key_down) {
            ImGui_ImplWin32_AddKeyEvent(io, ImGuiKey_RightAlt, is_key_down, VK_RMENU, scancode);
          }
        }
      }
      return 0;
    ...
    case WM_CHAR:
      ...
      return 0;
    case WM_SETCURSOR:
      // This is required to restore cursor when transitioning from e.g resize borders to client
      // area.
...
// by main.cpp, we don't call it automatically. If you are trying to implement your own backend for
// your own engine, you may ignore that noise.
//---------------------------------------------------------------------------------------------------------

// Perform our own check with RtlVerifyVersionInfo() instead of using functions from
// <VersionHelpers.h> as they require a manifest to be functional for checks above 8.1. See
...
static BOOL _IsWindowsVersionOrGreater(WORD major, WORD minor, WORD) {
  ...
  if (RtlVerifyVersionInfoFn == nullptr) return FALSE;

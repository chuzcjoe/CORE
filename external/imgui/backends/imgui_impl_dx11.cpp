...
//  [X] Renderer: User texture binding. Use 'ID3D11ShaderResourceView*' as texture identifier. Read
//  the FAQ about ImTextureID/ImTextureRef! [X] Renderer: Large meshes support (64k+ vertices) even
...
//  2025-06-11: DirectX11: Added support for ImGuiBackendFlags_RendererHasTextures, for dynamic font
//  atlas. 2025-05-07: DirectX11: Honor draw_data->FramebufferScale to allow for custom backends and
//  experiment using it (consistently with other renderer backends, even though in normal condition
//  it is not set under Windows). 2025-01-06: DirectX11: Expose VertexConstantBuffer in
//  ImGui_ImplDX11_RenderState. Reset projection matrix in ImDrawCallback_ResetRenderState handler.
//  2024-10-07: DirectX11: Changed default texture sampler to Clamp instead of Repeat/Wrap.
//  2024-10-07: DirectX11: Expose selected render state in ImGui_ImplDX11_RenderState, which you can
//  access in 'void* platform_io.Renderer_RenderState' during draw callbacks. 2022-10-11: Using
//  'nullptr' instead of 'NULL' as per our switch to C++11. 2021-06-29: Reorganized backend to pull
//  data from a single structure to facilitate usage with multiple-contexts (all g_XXXX access
//  changed to bd->XXXX). 2021-05-19: DirectX11: Replaced direct access to ImDrawCmd::TextureId with
//  a call to ImDrawCmd::GetTexID(). (will become a requirement) 2021-02-18: DirectX11: Change
//  blending equation to preserve alpha in output buffer. 2019-08-01: DirectX11: Fixed code querying
//  the Geometry Shader state (would generally error with Debug layer enabled). 2019-07-21:
//  DirectX11: Backup, clear and restore Geometry Shader is any is bound when calling
//  ImGui_ImplDX11_RenderDrawData. Clearing Hull/Domain/Compute shaders without backup/restore.
//  2019-05-29: DirectX11: Added support for large mesh (64K+ vertices), enable
//  ImGuiBackendFlags_RendererHasVtxOffset flag. 2019-04-30: DirectX11: Added support for special
//  ImDrawCallback_ResetRenderState callback to reset render state. 2018-12-03: Misc: Added #pragma
//  comment statement to automatically link with d3dcompiler.lib when using D3DCompile().
//  2018-11-30: Misc: Setting up io.BackendRendererName so it can be displayed in the About Window.
//  2018-08-01: DirectX11: Querying for IDXGIFactory instead of IDXGIFactory1 to increase
//  compatibility. 2018-07-13: DirectX11: Fixed unreleased resources in Init and Shutdown functions.
//  2018-06-08: Misc: Extracted imgui_impl_dx11.cpp/.h away from the old combined DX11+Win32
//  example. 2018-06-08: DirectX11: Use draw_data->DisplayPos and draw_data->DisplaySize to setup
//  projection matrix and clipping rectangle. 2018-02-16: Misc: Obsoleted the io.RenderDrawListsFn
//  callback and exposed ImGui_ImplDX11_RenderDrawData() in the .h file so you can call it yourself.
//  2018-02-06: Misc: Removed call to ImGui::Shutdown() which is not available from 1.60 WIP, user
//  needs to call CreateContext/DestroyContext themselves. 2016-05-07: DirectX11: Disabling
//  depth-write.
...
static ImGui_ImplDX11_Data* ImGui_ImplDX11_GetBackendData() {
  return ImGui::GetCurrentContext() ? (ImGui_ImplDX11_Data*)ImGui::GetIO().BackendRendererUserData
                                    : nullptr;
}

// Functions
static void ImGui_ImplDX11_SetupRenderState(const ImDrawData* draw_data,
  ...
  if (device_ctx->Map(bd->pVertexConstantBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mapped_resource) ==
      S_OK) {
  ...
  device_ctx->IASetIndexBuffer(
      bd->pIB, sizeof(ImDrawIdx) == 2 ? DXGI_FORMAT_R16_UINT : DXGI_FORMAT_R32_UINT, 0);
...
void ImGui_ImplDX11_RenderDrawData(ImDrawData* draw_data) {
  ...
  if (draw_data->Textures != nullptr)
    for (ImTextureData* tex : *draw_data->Textures)
      if (tex->Status != ImTextureStatus_OK) ImGui_ImplDX11_UpdateTexture(tex);
  ...
  // Backup DX state that will be modified to restore it afterwards (unfortunately this is very ugly
  // looking and verbose. Close your eyes!)
  ...
  device->IAGetVertexBuffers(0, 1, &old.VertexBuffer, &old.VertexBufferStride,
  ...
  for (const ImDrawList* draw_list : draw_data->CmdLists) {
    for (int cmd_i = 0; cmd_i < draw_list->CmdBuffer.Size; cmd_i++) {
      ...
      if (pcmd->UserCallback != nullptr) {
        ...
        const D3D11_RECT r = {(LONG)clip_min.x, (LONG)clip_min.y, (LONG)clip_max.x,
                              (LONG)clip_max.y};
        ...
        device->DrawIndexed(pcmd->ElemCount, pcmd->IdxOffset + global_idx_offset,
  ...
  device->IASetVertexBuffers(0, 1, &old.VertexBuffer, &old.VertexBufferStride,
                             &old.VertexBufferOffset);
...
void ImGui_ImplDX11_UpdateTexture(ImTextureData* tex) {
  ...
  if (tex->Status == ImTextureStatus_WantCreate) {
    ...
    for (ImTextureRect& r : tex->Updates) {
      D3D11_BOX box = {(UINT)r.x,         (UINT)r.y,         (UINT)0,
                       (UINT)(r.x + r.w), (UINT)(r.y + r.h), (UINT)1};
      bd->pd3dDeviceContext->UpdateSubresource(
          backend_tex->pTexture, 0, &box, tex->GetPixelsAt(r.x, r.y), (UINT)tex->GetPitch(), 0);
    }
    tex->SetStatus(ImTextureStatus_OK);
  }
...
bool ImGui_ImplDX11_CreateDeviceObjects() {
  ImGui_ImplDX11_Data* bd = ImGui_ImplDX11_GetBackendData();
  if (!bd->pd3dDevice) return false;
  ImGui_ImplDX11_InvalidateDeviceObjects();

  // By using D3DCompile() from <d3dcompiler.h> / d3dcompiler.lib, we introduce a dependency to a
  // given version of d3dcompiler_XX.dll (see D3DCOMPILER_DLL_A) If you would like to use this DX11
  ...
  // Create the vertex shader
  {
    ...
    if (FAILED(D3DCompile(vertexShader, strlen(vertexShader), nullptr, nullptr, nullptr, "main",
                          "vs_4_0", 0, 0, &vertexShaderBlob, nullptr)))
  ...
  // Create the pixel shader
  {
    ...
    if (FAILED(D3DCompile(pixelShader, strlen(pixelShader), nullptr, nullptr, nullptr, "main",
                          "ps_4_0", 0, 0, &pixelShaderBlob, nullptr)))
      return false;  // NB: Pass ID3DBlob* pErrorBlob to D3DCompile() to get error showing in (const
                     // char*)pErrorBlob->GetBufferPointer(). Make sure to Release() the blob!
...
void ImGui_ImplDX11_InvalidateDeviceObjects() {
  ImGui_ImplDX11_Data* bd = ImGui_ImplDX11_GetBackendData();
  if (!bd->pd3dDevice) return;

  // Destroy all textures
  for (ImTextureData* tex : ImGui::GetPlatformIO().Textures)
    if (tex->RefCount == 1) ImGui_ImplDX11_DestroyTexture(tex);

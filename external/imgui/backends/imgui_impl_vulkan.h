...
// (2) Call ImGui_ImplVulkan_LoadFunctions() before ImGui_ImplVulkan_Init() with your custom
...
// Backend uses a small number of descriptors per font atlas + as many as additional calls done to
...
//     and must contain a pool size large enough to hold a small number of
...
//   PipelineRenderingCreateInfo structure.
struct ImGui_ImplVulkan_InitInfo {
  uint32_t ApiVersion;  // Fill with API version of Instance, e.g. VK_API_VERSION_1_3 or your value
  ...
  VkQueue Queue;
  VkDescriptorPool
      DescriptorPool;  // See requirements in note above; ignored if using DescriptorPoolSize > 0
  uint32_t DescriptorPoolSize;    // Optional: set to create internal descriptor pool automatically
  ...
  // VkRenderPass                  RenderPass;                 // --> Since 2025/09/26: set
  // 'PipelineInfoMain.RenderPass' instead uint32_t                      Subpass; // --> Since
  // 2025/09/26: set 'PipelineInfoMain.Subpass' instead VkSampleCountFlagBits         MSAASamples;
  // // --> Since 2025/09/26: set 'PipelineInfoMain.MSAASamples' instead
  // VkPipelineRenderingCreateInfoKHR PipelineRenderingCreateInfo; // Since 2025/09/26: set
  // 'PipelineInfoMain.PipelineRenderingCreateInfo' instead

  // (Optional) Dynamic Rendering
  // Need to explicitly enable VK_KHR_dynamic_rendering extension to use this, even for Vulkan 1.3 +
  // setup PipelineInfoMain.PipelineRenderingCreateInfo.
  ...
  // - if .sType == VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO we use specified structs, otherwise
  // we use defaults.
  // - Shader inputs/outputs need to match ours. Code/data pointed to by the structure needs to
  // survive for whole during of backend usage.
...
// (Advanced) Use e.g. if you need to recreate pipeline without reinitializing the backend (see
// #8110, #8111) The main window pipeline will be created by ImGui_ImplVulkan_Init() if possible (==
// RenderPass xor (UseDynamicRendering && PipelineRenderingCreateInfo->sType ==
// VK_STRUCTURE_TYPE_PIPELINE_RENDERING_CREATE_INFO_KHR)) Else, the pipeline can be created, or
// re-created, using ImGui_ImplVulkan_CreateMainPipeline() before rendering.
...
IMGUI_IMPL_API VkDescriptorSet ImGui_ImplVulkan_AddTexture(VkSampler sampler,
...
// ImGui_ImplVulkan_RenderDrawData() call. (Please open an issue if you feel you need access to more
// data)
...
IMGUI_IMPL_API void ImGui_ImplVulkanH_CreateOrResizeWindow(
    ...
    int w, int h, uint32_t min_image_count, VkImageUsageFlags image_usage);

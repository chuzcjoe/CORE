...
//  ImGui_ImplVulkan_AddTexture() to register one. Read the FAQ about ImTextureID/ImTextureRef +
//  https://github.com/ocornut/imgui/pull/914 for discussions. [X] Renderer: Large meshes support
//  (64k+ vertices) even with 16-bit indices (ImGuiBackendFlags_RendererHasVtxOffset). [X] Renderer:
//  Texture updates support for dynamic font atlas (ImGuiBackendFlags_RendererHasTextures). [X]
//  Renderer: Expose selected render state for draw callbacks to use. Access in
...
//  2025-09-26: *BREAKING CHANGE*: moved some fields in ImGui_ImplVulkan_InitInfo:
//  init_info.RenderPass --> init_info.PipelineInfoMain.RenderPass, init_info.Subpass -->
//  init_info.PipelineInfoMain.Subpass, init_info.MSAASamples -->
//  init_info.PipelineInfoMain.MSAASamples, init_info.PipelineRenderingCreateInfo -->
//  init_info.PipelineInfoMain.PipelineRenderingCreateInfo. 2025-09-26: *BREAKING CHANGE*: renamed
//  ImGui_ImplVulkan_MainPipelineCreateInfo to ImGui_ImplVulkan_PipelineInfo. Introduced very
//  recently so shouldn't affect many users. 2025-09-26: *BREAKING CHANGE*: helper
//  ImGui_ImplVulkanH_CreateOrResizeWindow() added a VkImageUsageFlags image_usage` argument,
//  default to VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT if 0. 2025-09-26: Vulkan: Added a way to
//  customize shaders by filling
//  ImGui_ImplVulkan_InitInfo::CustomShaderVertCreateInfo/CustomShaderFragCreateInfo. (#8585)
//  2025-09-18: Call platform_io.ClearRendererHandlers() on shutdown.
//  2025-09-04: Vulkan: Added ImGui_ImplVulkan_CreateMainPipeline(). (#8110, #8111)
//  2025-07-27: Vulkan: Fixed texture update corruption introduced on 2025-06-11. (#8801, #8755,
//  #8840) 2025-07-07: Vulkan: Fixed texture synchronization issue introduced on 2025-06-11. (#8772)
//  2025-06-27: Vulkan: Fixed validation errors during texture upload/update by aligning upload size
//  to 'nonCoherentAtomSize'. (#8743, #8744) 2025-06-11: Vulkan: Added support for
//  ImGuiBackendFlags_RendererHasTextures, for dynamic font atlas. Removed
//  ImGui_ImplVulkan_CreateFontsTexture() and ImGui_ImplVulkan_DestroyFontsTexture(). 2025-05-07:
//  Vulkan: Fixed validation errors during window detach in multi-viewport mode. (#8600, #8176)
//  2025-05-07: Vulkan: Load dynamic rendering functions using vkGetDeviceProcAddr() + try both
//  non-KHR and KHR versions. (#8600, #8326, #8365) 2025-04-07: Vulkan: Deep-copy
//  ImGui_ImplVulkan_InitInfo::PipelineRenderingCreateInfo's pColorAttachmentFormats buffer when
//  set, in order to reduce common user-error of specifying a pointer to data that gets out of
//  scope. (#8282) 2025-02-14: *BREAKING CHANGE*: Added uint32_t api_version to
//  ImGui_ImplVulkan_LoadFunctions(). 2025-02-13: Vulkan: Added ApiVersion field in
//  ImGui_ImplVulkan_InitInfo. Default to header version if unspecified. Dynamic rendering path
//  loads "vkCmdBeginRendering/vkCmdEndRendering" (without -KHR suffix) on API 1.3. (#8326)
//  2025-01-09: Vulkan: Added IMGUI_IMPL_VULKAN_MINIMUM_IMAGE_SAMPLER_POOL_SIZE to clarify how many
//  image sampler descriptors are expected to be available in descriptor pool. (#6642) 2025-01-06:
//  Vulkan: Added more ImGui_ImplVulkanH_XXXX helper functions to simplify our examples. 2024-12-11:
//  Vulkan: Fixed setting VkSwapchainCreateInfoKHR::preTransform for platforms not supporting
//  VK_SURFACE_TRANSFORM_IDENTITY_BIT_KHR. (#8222) 2024-11-27: Vulkan: Make user-provided descriptor
//  pool optional. As a convenience, when setting init_info->DescriptorPoolSize the backend will
//  create one itself. (#8172, #4867) 2024-10-07: Vulkan: Changed default texture sampler to Clamp
//  instead of Repeat/Wrap. 2024-10-07: Vulkan: Expose selected render state in
//  ImGui_ImplVulkan_RenderState, which you can access in 'void* platform_io.Renderer_RenderState'
//  during draw callbacks. 2024-10-07: Vulkan: Compiling with '#define ImTextureID=ImU64' is
//  unnecessary now that dear imgui defaults ImTextureID to u64 instead of void*. 2024-04-19:
//  Vulkan: Added convenience support for Volk via IMGUI_IMPL_VULKAN_USE_VOLK define (you can also
//  use IMGUI_IMPL_VULKAN_NO_PROTOTYPES + wrap Volk via ImGui_ImplVulkan_LoadFunctions().)
//  2024-02-14: *BREAKING CHANGE*: Moved RenderPass parameter from ImGui_ImplVulkan_Init() function
//  to ImGui_ImplVulkan_InitInfo structure. Not required when using dynamic rendering. 2024-02-12:
//  *BREAKING CHANGE*: Dynamic rendering now require filling PipelineRenderingCreateInfo structure.
//  2024-01-19: Vulkan: Fixed vkAcquireNextImageKHR() validation errors in VulkanSDK 1.3.275 by
//  allocating one extra semaphore than in-flight frames. (#7236) 2024-01-11: Vulkan: Fixed
//  vkMapMemory() calls unnecessarily using full buffer size (#3957). Fixed MinAllocationSize
//  handing (#7189). 2024-01-03: Vulkan: Added MinAllocationSize field in ImGui_ImplVulkan_InitInfo
//  to workaround zealous "best practice" validation layer. (#7189, #4238) 2024-01-03: Vulkan:
//  Stopped creating command pools with VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT as we don't
//  reset them. 2023-11-29: Vulkan: Fixed mismatching allocator passed to vkCreateCommandPool() vs
//  vkDestroyCommandPool(). (#7075) 2023-11-10: *BREAKING CHANGE*: Removed parameter from
//  ImGui_ImplVulkan_CreateFontsTexture(): backend now creates its own command-buffer to upload
//  fonts.
//              *BREAKING CHANGE*: Removed ImGui_ImplVulkan_DestroyFontUploadObjects() which is now
//              unnecessary as we create and destroy those objects in the backend.
//              ImGui_ImplVulkan_CreateFontsTexture() is automatically called by NewFrame() the
//              first time. You can call ImGui_ImplVulkan_CreateFontsTexture() again to recreate the
//              font atlas texture. Added ImGui_ImplVulkan_DestroyFontsTexture() but you probably
//              never need to call this.
//  2023-07-04: Vulkan: Added optional support for VK_KHR_dynamic_rendering. User needs to set
//  init_info->UseDynamicRendering = true and init_info->ColorAttachmentFormat. 2023-01-02: Vulkan:
//  Fixed sampler passed to ImGui_ImplVulkan_AddTexture() not being honored + removed a bunch of
//  duplicate code. 2022-10-11: Using 'nullptr' instead of 'NULL' as per our switch to C++11.
//  2022-10-04: Vulkan: Added experimental ImGui_ImplVulkan_RemoveTexture() for api symmetry. (#914,
//  #5738). 2022-01-20: Vulkan: Added support for ImTextureID as VkDescriptorSet. User need to call
//  ImGui_ImplVulkan_AddTexture(). Building for 32-bit targets requires '#define ImTextureID ImU64'.
//  (#914). 2021-10-15: Vulkan: Call vkCmdSetScissor() at the end of render a full-viewport to
//  reduce likelihood of issues with people using VK_DYNAMIC_STATE_SCISSOR in their app without
//  calling vkCmdSetScissor() explicitly every frame. 2021-06-29: Reorganized backend to pull data
//  from a single structure to facilitate usage with multiple-contexts (all g_XXXX access changed to
//  bd->XXXX). 2021-03-22: Vulkan: Fix mapped memory validation error when buffer sizes are not
//  multiple of VkPhysicalDeviceLimits::nonCoherentAtomSize. 2021-02-18: Vulkan: Change blending
//  equation to preserve alpha in output buffer. 2021-01-27: Vulkan: Added support for custom
//  function load and IMGUI_IMPL_VULKAN_NO_PROTOTYPES by using ImGui_ImplVulkan_LoadFunctions().
//  2020-11-11: Vulkan: Added support for specifying which subpass to reference during VkPipeline
//  creation. 2020-09-07: Vulkan: Added VkPipeline parameter to ImGui_ImplVulkan_RenderDrawData
//  (default to one passed to ImGui_ImplVulkan_Init). 2020-05-04: Vulkan: Fixed crash if initial
//  frame has no vertices. 2020-04-26: Vulkan: Fixed edge case where render callbacks wouldn't be
//  called if the ImDrawData didn't have vertices. 2019-08-01: Vulkan: Added support for specifying
//  multisample count. Set ImGui_ImplVulkan_InitInfo::MSAASamples to one of the
//  VkSampleCountFlagBits values to use, default is non-multisampled as before. 2019-05-29: Vulkan:
//  Added support for large mesh (64K+ vertices), enable ImGuiBackendFlags_RendererHasVtxOffset
//  flag. 2019-04-30: Vulkan: Added support for special ImDrawCallback_ResetRenderState callback to
//  reset render state. 2019-04-04: *BREAKING CHANGE*: Vulkan: Added ImageCount/MinImageCount fields
//  in ImGui_ImplVulkan_InitInfo, required for initialization (was previously a hard #define
//  IMGUI_VK_QUEUED_FRAMES 2). Added ImGui_ImplVulkan_SetMinImageCount(). 2019-04-04: Vulkan: Added
//  VkInstance argument to ImGui_ImplVulkanH_CreateWindow() optional helper. 2019-04-04: Vulkan:
//  Avoid passing negative coordinates to vkCmdSetScissor, which debug validation layers do not
//  like. 2019-04-01: Vulkan: Support for 32-bit index buffer (#define ImDrawIdx unsigned int).
//  2019-02-16: Vulkan: Viewport and clipping rectangles correctly using draw_data->FramebufferScale
//  to allow retina display. 2018-11-30: Misc: Setting up io.BackendRendererName so it can be
//  displayed in the About Window. 2018-08-25: Vulkan: Fixed mishandled
//  VkSurfaceCapabilitiesKHR::maxImageCount=0 case. 2018-06-22: Inverted the parameters to
//  ImGui_ImplVulkan_RenderDrawData() to be consistent with other backends. 2018-06-08: Misc:
//  Extracted imgui_impl_vulkan.cpp/.h away from the old combined GLFW+Vulkan example. 2018-06-08:
//  Vulkan: Use draw_data->DisplayPos and draw_data->DisplaySize to setup projection matrix and
//  clipping rectangle. 2018-03-03: Vulkan: Various refactor, created a couple of
//  ImGui_ImplVulkanH_XXX helper that the example can use and that viewport support will use.
//  2018-03-01: Vulkan: Renamed ImGui_ImplVulkan_Init_Info to ImGui_ImplVulkan_InitInfo and fields
//  to match more closely Vulkan terminology. 2018-02-16: Misc: Obsoleted the io.RenderDrawListsFn
//  callback, ImGui_ImplVulkan_Render() calls ImGui_ImplVulkan_RenderDrawData() itself. 2018-02-06:
//  Misc: Removed call to ImGui::Shutdown() which is not available from 1.60 WIP, user needs to call
//  CreateContext/DestroyContext themselves. 2017-05-15: Vulkan: Fix scissor offset being negative.
//  Fix new Vulkan validation warnings. Set required depth member for buffer image copy. 2016-11-13:
//  Vulkan: Fix validation layer warnings and errors and redeclare gl_PerVertex. 2016-10-18: Vulkan:
//  Add location decorators & change to use structs as in/out in glsl, update embedded spv (produced
//  with glslangValidator -x). Null the released resources. 2016-08-27: Vulkan: Fix Vulkan example
//  for use when a depth buffer is active.
...
void ImGui_ImplVulkan_DestroyFrameRenderBuffers(VkDevice device,
                                                ImGui_ImplVulkan_FrameRenderBuffers* buffers,
                                                const VkAllocationCallbacks* allocator);
void ImGui_ImplVulkan_DestroyWindowRenderBuffers(VkDevice device,
                                                 ImGui_ImplVulkan_WindowRenderBuffers* buffers,
                                                 const VkAllocationCallbacks* allocator);
...
struct ImGui_ImplVulkan_Data {
  ...
  ImVector<VkFormat>
      PipelineRenderingCreateInfoColorAttachmentFormats;  // Deep copy of format array
...
static ImGui_ImplVulkan_Data* ImGui_ImplVulkan_GetBackendData() {
  return ImGui::GetCurrentContext() ? (ImGui_ImplVulkan_Data*)ImGui::GetIO().BackendRendererUserData
                                    : nullptr;
}
...
static void CreateOrResizeBuffer(VkBuffer& buffer, VkDeviceMemory& buffer_memory,
                                 VkDeviceSize& buffer_size, VkDeviceSize new_size,
  ...
  VkDeviceSize buffer_size_aligned =
      AlignBufferSize(IM_MAX(v->MinAllocationSize, new_size), bd->BufferMemoryAlignment);
  ...
  alloc_info.memoryTypeIndex =
      ImGui_ImplVulkan_MemoryType(VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT, req.memoryTypeBits);
...
static void ImGui_ImplVulkan_SetupRenderState(ImDrawData* draw_data, VkPipeline pipeline,
...
void ImGui_ImplVulkan_RenderDrawData(ImDrawData* draw_data, VkCommandBuffer command_buffer,
  ...
  // status, aka nothing to do. (This almost always points to ImGui::GetPlatformIO().Textures[] but
  // is part of ImDrawData to allow overriding or disabling texture updates).
  ...
  if (draw_data->TotalVtxCount > 0) {
    ...
    VkDeviceSize vertex_size =
        AlignBufferSize(draw_data->TotalVtxCount * sizeof(ImDrawVert), bd->BufferMemoryAlignment);
    VkDeviceSize index_size =
        AlignBufferSize(draw_data->TotalIdxCount * sizeof(ImDrawIdx), bd->BufferMemoryAlignment);
    ...
    VkResult err =
        vkMapMemory(v->Device, rb->VertexBufferMemory, 0, vertex_size, 0, (void**)&vtx_dst);
    check_vk_result(err);
    ...
    for (int cmd_i = 0; cmd_i < draw_list->CmdBuffer.Size; cmd_i++) {
      ...
      if (pcmd->UserCallback != nullptr) {
        ...
        if (pcmd->UserCallback == ImDrawCallback_ResetRenderState)
          ImGui_ImplVulkan_SetupRenderState(draw_data, pipeline, command_buffer, rb, fb_width,
                                            fb_height);
        else
  ...
  // responsible for setting the values before rendering. In theory we should aim to backup/restore
  // those values but I am not sure this is possible. We perform a call to vkCmdSetScissor() to set
  // back a full viewport which is likely to fix things for 99% users but technically this is not
  // perfect. (See github #4644)
...
void ImGui_ImplVulkan_UpdateTexture(ImTextureData* tex) {
  ...
  if (tex->Status == ImTextureStatus_WantCreate) {
    ...
    backend_tex->DescriptorSet = ImGui_ImplVulkan_AddTexture(
        bd->TexSamplerLinear, backend_tex->ImageView, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
  ...
  if (tex->Status == ImTextureStatus_WantCreate || tex->Status == ImTextureStatus_WantUpdates) {
    ...
    // Update full texture or selected blocks. We only ever write to textures regions which have
    // never been used before! This backend choose to use tex->UpdateRect but you can use
    // tex->Updates[] to upload individual regions. We could use the smaller rect on _WantCreate but
    // using the full rect allows us to clear the texture.
    const int upload_x = (tex->Status == ImTextureStatus_WantCreate) ? 0 : tex->UpdateRect.x;
    const int upload_y = (tex->Status == ImTextureStatus_WantCreate) ? 0 : tex->UpdateRect.y;
    const int upload_w =
        (tex->Status == ImTextureStatus_WantCreate) ? tex->Width : tex->UpdateRect.w;
    const int upload_h =
    ...
    // Upload to Buffer:
    {
      ...
      for (int y = 0; y < upload_h; y++)
        memcpy(map + upload_pitch * y, tex->GetPixelsAt(upload_x, upload_y + y),
               (size_t)upload_pitch);
    ...
    // Copy to Image:
    {
      ...
      vkCmdPipelineBarrier(
          bd->TexCommandBuffer, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT | VK_PIPELINE_STAGE_HOST_BIT,
          VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 0, nullptr, 1, upload_barrier, 1, copy_barrier);
      ...
      vkCmdCopyBufferToImage(bd->TexCommandBuffer, upload_buffer, backend_tex->Image,
      ...
      vkCmdPipelineBarrier(bd->TexCommandBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT,
  ...
  if (tex->Status == ImTextureStatus_WantDestroy &&
      tex->UnusedFrames >= (int)bd->VulkanInitInfo.ImageCount)
...
static void ImGui_ImplVulkan_CreateShaderModules(VkDevice device,
  ...
  if (bd->ShaderModuleVert == VK_NULL_HANDLE) {
    ...
    VkShaderModuleCreateInfo* p_vert_info =
        (v->CustomShaderVertCreateInfo.sType == VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO)
            ? &v->CustomShaderVertCreateInfo
            : &default_vert_info;
  ...
  if (bd->ShaderModuleFrag == VK_NULL_HANDLE) {
    ...
    VkShaderModuleCreateInfo* p_frag_info =
        (v->CustomShaderFragCreateInfo.sType == VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO)
            ? &v->CustomShaderFragCreateInfo
            : &default_frag_info;
...
static VkPipeline ImGui_ImplVulkan_CreatePipeline(VkDevice device,
                                                  const VkAllocationCallbacks* allocator,
  ...
  if (bd->VulkanInitInfo.UseDynamicRendering) {
    IM_ASSERT(info->PipelineRenderingCreateInfo.sType ==
                  VK_STRUCTURE_TYPE_PIPELINE_RENDERING_CREATE_INFO_KHR &&
              "PipelineRenderingCreateInfo::sType must be "
              "VK_STRUCTURE_TYPE_PIPELINE_RENDERING_CREATE_INFO_KHR");
    IM_ASSERT(info->PipelineRenderingCreateInfo.pNext == nullptr &&
  ...
  VkPipeline pipeline;
  VkResult err =
      vkCreateGraphicsPipelines(device, pipelineCache, 1, &create_info, allocator, &pipeline);
  check_vk_result(err);
  return pipeline;
}
...
bool ImGui_ImplVulkan_CreateDeviceObjects() {
  ...
  create_main_pipeline |=
      (v->UseDynamicRendering && v->PipelineInfoMain.PipelineRenderingCreateInfo.sType ==
                                     VK_STRUCTURE_TYPE_PIPELINE_RENDERING_CREATE_INFO_KHR);
#endif
  if (create_main_pipeline) ImGui_ImplVulkan_CreateMainPipeline(&v->PipelineInfoMain);
...
void ImGui_ImplVulkan_CreateMainPipeline(const ImGui_ImplVulkan_PipelineInfo* pipeline_info_in) {
  ...
  if (pipeline_info != pipeline_info_in) *pipeline_info = *pipeline_info_in;

#ifdef IMGUI_IMPL_VULKAN_HAS_DYNAMIC_RENDERING
  VkPipelineRenderingCreateInfoKHR* pipeline_rendering_create_info =
      &pipeline_info->PipelineRenderingCreateInfo;
  if (v->UseDynamicRendering && pipeline_rendering_create_info->pColorAttachmentFormats != NULL) {
    ...
    memcpy(formats.Data, pipeline_rendering_create_info->pColorAttachmentFormats,
           (size_t)formats.size_in_bytes());
...
#endif
  bd->Pipeline =
      ImGui_ImplVulkan_CreatePipeline(v->Device, v->Allocator, v->PipelineCache, pipeline_info);
}

void ImGui_ImplVulkan_DestroyDeviceObjects() {
  ImGui_ImplVulkan_Data* bd = ImGui_ImplVulkan_GetBackendData();
  ImGui_ImplVulkan_InitInfo* v = &bd->VulkanInitInfo;
  ImGui_ImplVulkan_DestroyWindowRenderBuffers(v->Device, &bd->MainWindowRenderBuffers,
                                              v->Allocator);

  // Destroy all textures
  for (ImTextureData* tex : ImGui::GetPlatformIO().Textures)
    if (tex->RefCount == 1) ImGui_ImplVulkan_DestroyTexture(tex);
  ...
  if (bd->TexSamplerLinear) {
    vkDestroySampler(v->Device, bd->TexSamplerLinear, v->Allocator);
    bd->TexSamplerLinear = VK_NULL_HANDLE;
  }
...
#ifdef IMGUI_IMPL_VULKAN_HAS_DYNAMIC_RENDERING
static void ImGui_ImplVulkan_LoadDynamicRenderingFunctions(
  ...
  // - Try loading core (non-KHR) versions first (this will work for Vulkan 1.3+ and the device
  // supports dynamic rendering)
  ImGuiImplVulkanFuncs_vkCmdBeginRenderingKHR =
      reinterpret_cast<PFN_vkCmdBeginRenderingKHR>(loader_func("vkCmdBeginRendering", user_data));
  ImGuiImplVulkanFuncs_vkCmdEndRenderingKHR =
      reinterpret_cast<PFN_vkCmdEndRenderingKHR>(loader_func("vkCmdEndRendering", user_data));

  // - Fallback to KHR versions if core not available (this will work if KHR extension is available
  // and enabled and also the device supports dynamic rendering)
  if (ImGuiImplVulkanFuncs_vkCmdBeginRenderingKHR == nullptr ||
      ImGuiImplVulkanFuncs_vkCmdEndRenderingKHR == nullptr) {
    ImGuiImplVulkanFuncs_vkCmdBeginRenderingKHR = reinterpret_cast<PFN_vkCmdBeginRenderingKHR>(
        loader_func("vkCmdBeginRenderingKHR", user_data));
    ImGuiImplVulkanFuncs_vkCmdEndRenderingKHR =
        reinterpret_cast<PFN_vkCmdEndRenderingKHR>(loader_func("vkCmdEndRenderingKHR", user_data));
  }
}
#endif

// If unspecified by user, assume that ApiVersion == HeaderVersion
// We don't care about other versions than 1.3 for our checks, so don't need to make this exhaustive
// (e.g. with all #ifdef VK_VERSION_1_X checks)
...
bool ImGui_ImplVulkan_LoadFunctions(uint32_t api_version,
  ...
  if (api_version == 0) api_version = ImGui_ImplVulkan_GetDefaultApiVersion();
...
bool ImGui_ImplVulkan_Init(ImGui_ImplVulkan_InitInfo* info) {
  IM_ASSERT(g_FunctionsLoaded &&
            "Need to call ImGui_ImplVulkan_LoadFunctions() if IMGUI_IMPL_VULKAN_NO_PROTOTYPES or "
            "VK_NO_PROTOTYPES are set!");

  if (info->ApiVersion == 0) info->ApiVersion = ImGui_ImplVulkan_GetDefaultApiVersion();
...
#else
    IM_ASSERT(0 &&
              "Can't use dynamic rendering when neither VK_VERSION_1_3 or VK_KHR_dynamic_rendering "
              "is defined.");
#endif
  }
  ...
  if (info->DescriptorPool !=
      VK_NULL_HANDLE)  // Either DescriptorPool or DescriptorPoolSize must be set, not both!
    IM_ASSERT(info->DescriptorPoolSize == 0);
  else
    IM_ASSERT(info->DescriptorPoolSize > 0);
  if (info->UseDynamicRendering) IM_ASSERT(info->PipelineInfoMain.RenderPass == VK_NULL_HANDLE);
...
void ImGui_ImplVulkan_NewFrame() {
  ...
  IM_ASSERT(bd != nullptr &&
            "Context or backend not initialized! Did you call ImGui_ImplVulkan_Init()?");
  IM_UNUSED(bd);
}
...
// FIXME: This is experimental in the sense that we are unsure how to best design/tackle this
// problem, please post to https://github.com/ocornut/imgui/pull/914 if you have suggestions.
...
//
// Your engine/app will likely _already_ have code to setup all that stuff (swap chain, render pass,
// frame buffers, etc.). You may read this code to learn about Vulkan, but it is recommended you use
// your own custom tailored code to do equivalent work. (The ImGui_ImplVulkanH_XXX functions do not
// interact with any of the state used by the regular ImGui_ImplVulkan_XXX functions)
//-------------------------------------------------------------------------
...
VkSurfaceFormatKHR ImGui_ImplVulkanH_SelectSurfaceFormat(VkPhysicalDevice physical_device,
  ...
  // was set at image creation Assuming that the default behavior is without setting this bit, there
  // is no need for separate Swapchain image and image view format Additionally several new color
  // spaces were introduced with Vulkan Spec v1.0.40, hence we must make sure that a format with the
  // mostly available color space, VK_COLOR_SPACE_SRGB_NONLINEAR_KHR, is found and used.
...
VkPresentModeKHR ImGui_ImplVulkanH_SelectPresentMode(VkPhysicalDevice physical_device,
  ...
  vkGetPhysicalDeviceSurfacePresentModesKHR(physical_device, surface, &avail_count,
                                            avail_modes.Data);
  // for (uint32_t avail_i = 0; avail_i < avail_count; avail_i++)
  ...
  for (int request_i = 0; request_i < request_modes_count; request_i++)
    for (uint32_t avail_i = 0; avail_i < avail_count; avail_i++)
      if (request_modes[request_i] == avail_modes[avail_i]) return request_modes[request_i];
...
uint32_t ImGui_ImplVulkanH_SelectQueueFamilyIndex(VkPhysicalDevice physical_device) {
  ...
  for (uint32_t i = 0; i < count; i++)
    if (queues_properties[i].queueFlags & VK_QUEUE_GRAPHICS_BIT) return i;
  return (uint32_t)-1;
}
...
void ImGui_ImplVulkanH_CreateWindowSwapChain(VkPhysicalDevice physical_device, VkDevice device,
  ...
  // We don't use ImGui_ImplVulkanH_DestroyWindow() because we want to preserve the old swapchain to
  // create the new one. Destroy old Framebuffer
  for (uint32_t i = 0; i < wd->ImageCount; i++)
  ...
  // Create Swapchain
  {
    ...
    for (uint32_t i = 0; i < wd->ImageCount; i++) wd->Frames[i].Backbuffer = backbuffers[i];
  }
  ...
  if (wd->UseDynamicRendering == false) {
    ...
    // VK_SAMPLE_COUNT_1_BIT, &wd->Pipeline, v->Subpass);
  }
...
// - 2025/09/26: v1.92.4 added a trailing 'VkImageUsageFlags image_usage' parameter which is usually
// VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT.
void ImGui_ImplVulkanH_CreateOrResizeWindow(VkInstance instance, VkPhysicalDevice physical_device,
                                            VkDevice device, ImGui_ImplVulkanH_Window* wd,
                                            uint32_t queue_family,
                                            const VkAllocationCallbacks* allocator, int width,
  ...
  IM_ASSERT(g_FunctionsLoaded &&
            ...
            "VK_NO_PROTOTYPES are set!");
  (void)instance;
  ImGui_ImplVulkanH_CreateWindowSwapChain(physical_device, device, wd, allocator, width, height,
                                          min_image_count, image_usage);
  ImGui_ImplVulkanH_CreateWindowCommandBuffers(physical_device, device, wd, queue_family,
                                               allocator);

  // FIXME: to submit the command buffer, we need a queue. In the examples folder, the
  // ImGui_ImplVulkanH_CreateOrResizeWindow function is called before the ImGui_ImplVulkan_Init
  // function, so we don't have access to the queue yet. Here we have the queue_family that we can
  // use to grab a queue from the device and submit the command buffer. It would be better to have
  // access to the queue as suggested in the FIXME below.

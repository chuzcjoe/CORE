...
//  [X] Platform: Partial keyboard support. Since 1.87 we are using the io.AddKeyEvent() function.
//  Pass ImGuiKey values to all key functions e.g. ImGui::IsKeyPressed(ImGuiKey_Space). [Legacy GLUT
...
//  2023-04-17: BREAKING: Removed call to ImGui::NewFrame() from ImGui_ImplGLUT_NewFrame(). Needs to
//  be called from the main application loop, like with every other backends. 2022-09-26: Inputs:
//  Renamed ImGuiKey_ModXXX introduced in 1.87 to ImGuiMod_XXX (old names still supported).
//  2022-01-26: Inputs: replaced short-lived io.AddKeyModsEvent() (added two weeks ago) with
//  io.AddKeyEvent() using ImGuiKey_ModXXX flags. Sorry for the confusion. 2022-01-17: Inputs:
...
//  io.SetKeyEventNativeData() API (1.87+). Support for full ImGuiKey range. 2019-04-03: Misc:
//  Renamed imgui_impl_freeglut.cpp/.h to imgui_impl_glut.cpp/.h. 2019-03-25: Misc: Made
//  io.DeltaTime always above zero. 2018-11-30: Misc: Setting up io.BackendPlatformName so it can be
...
// Glut has one function for characters and one for "special keys". We map the characters in the
// 0..255 range and the keys above.

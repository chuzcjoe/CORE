...
// (Info: Allegro 5 is a cross-platform general purpose library for handling windows, inputs,
// graphics, etc.)

// Implemented features:
//  [X] Renderer: User texture binding. Use 'ALLEGRO_BITMAP*' as texture identifier. Read the FAQ
//  about ImTextureID/ImTextureRef! [X] Renderer: Texture updates support for dynamic font atlas
//  (ImGuiBackendFlags_RendererHasTextures). [X] Platform: Keyboard support. Since 1.87 we are using
//  the io.AddKeyEvent() function. Pass ImGuiKey values to all key functions e.g.
//  ImGui::IsKeyPressed(ImGuiKey_Space). [Legacy ALLEGRO_KEY_* values are obsolete since 1.87 and
//  not supported since 1.91.5] [X] Platform: Clipboard support (from Allegro 5.1.12). [X] Platform:
//  Mouse cursor shape and visibility (ImGuiBackendFlags_HasMouseCursors). Disable with
//  'io.ConfigFlags |= ImGuiConfigFlags_NoMouseCursorChange'.
// Missing features or Issues:
//  [ ] Renderer: The renderer is suboptimal as we need to unindex our buffers and convert vertices
...
//  2025-09-18: Call platform_io.ClearRendererHandlers() and platform_io.ClearPlatformHandlers() on
//  shutdown. 2025-08-12: Inputs: fixed missing support for ImGuiKey_PrintScreen under Windows, as
//  raw Allegro 5 does not receive it. 2025-08-12: Added ImGui_ImplAllegro5_SetDisplay() function to
//  change current ALLEGRO_DISPLAY, as Allegro applications often need to do that. 2025-07-07: Fixed
//  texture update broken on some platforms where ALLEGRO_LOCK_WRITEONLY needed all texels to be
//  rewritten. 2025-06-11: Added support for ImGuiBackendFlags_RendererHasTextures, for dynamic font
//  atlas. Removed ImGui_ImplSDLGPU3_CreateFontsTexture() and
//  ImGui_ImplSDLGPU3_DestroyFontsTexture(). 2025-02-18: Added ImGuiMouseCursor_Wait and
//  ImGuiMouseCursor_Progress mouse cursor support. 2025-01-06: Avoid calling al_set_mouse_cursor()
//  repeatedly since it appears to leak on on X11 (#8256). 2024-08-22: moved some OS/backend related
...
//  supported). 2022-01-26: Inputs: replaced short-lived io.AddKeyModsEvent() (added two weeks ago)
//  with io.AddKeyEvent() using ImGuiKey_ModXXX flags. Sorry for the confusion. 2022-01-17: Inputs:
//  calling new io.AddMousePosEvent(), io.AddMouseButtonEvent(), io.AddMouseWheelEvent() API
//  (1.87+). 2022-01-17: Inputs: always calling io.AddKeyModsEvent() next and before key event (not
//  in NewFrame) to fix input queue with very low framerates. 2022-01-10: Inputs: calling new
//  io.AddKeyEvent(), io.AddKeyModsEvent() + io.SetKeyEventNativeData() API (1.87+). Support for
//  full ImGuiKey range. 2021-12-08: Renderer: Fixed mishandling of the ImDrawCmd::IdxOffset field!
//  This is an old bug but it never had an effect until some internal rendering changes in 1.86.
//  2021-08-17: Calling io.AddFocusEvent() on
//  ALLEGRO_EVENT_DISPLAY_SWITCH_OUT/ALLEGRO_EVENT_DISPLAY_SWITCH_IN events. 2021-06-29: Reorganized
//  backend to pull data from a single structure to facilitate usage with multiple-contexts (all
//  g_XXXX access changed to bd->XXXX). 2021-05-19: Renderer: Replaced direct access to
...
//  2019-05-11: Inputs: Don't filter character value from ALLEGRO_EVENT_KEY_CHAR before calling
//  AddInputCharacter(). 2019-04-30: Renderer: Added support for special
//  ImDrawCallback_ResetRenderState callback to reset render state. 2018-11-30: Platform: Added
//  touchscreen support. 2018-11-30: Misc: Setting up io.BackendPlatformName/io.BackendRendererName
//  so they can be displayed in the About Window. 2018-06-13: Platform: Added clipboard support
//  (from Allegro 5.1.12). 2018-06-13: Renderer: Use draw_data->DisplayPos and
//  draw_data->DisplaySize to setup projection matrix and clipping rectangle. 2018-06-13: Renderer:
//  Stopped using al_draw_indexed_prim() as it is buggy in Allegro's DX9 backend. 2018-06-13:
//  Renderer: Backup/restore transform and clipping rectangle. 2018-06-11: Misc: Setup
//  io.BackendFlags ImGuiBackendFlags_HasMouseCursors flag + honor
//  ImGuiConfigFlags_NoMouseCursorChange flag. 2018-04-18: Misc: Renamed file from imgui_impl_a5.cpp
//  to imgui_impl_allegro5.cpp. 2018-04-18: Misc: Added support for 32-bit vertex indices to avoid
//  conversion at runtime. Added imconfig_allegro5.h to enforce 32-bit indices when included from
//  imgui.h. 2018-02-16: Misc: Obsoleted the io.RenderDrawListsFn callback and exposed
//  ImGui_ImplAllegro5_RenderDrawData() in the .h file so you can call it yourself. 2018-02-06:
...
#define ALLEGRO_HAS_DRAW_INDEXED_PRIM               \
  ((ALLEGRO_VERSION_INT & ~ALLEGRO_UNSTABLE_BIT) >= \
   ((5 << 24) | (2 << 16) |                         \
    (5 << 8)))  // DX9 implementation of al_draw_indexed_prim() got fixed in Allegro 5.2.5
...
// FIXME-OPT: Unfortunately Allegro doesn't support 32-bit packed colors so we have to convert them
// to 4 float as well..
...
static ImGui_ImplAllegro5_Data* ImGui_ImplAllegro5_GetBackendData() {
  return ImGui::GetCurrentContext()
             ? (ImGui_ImplAllegro5_Data*)ImGui::GetIO().BackendPlatformUserData
             : nullptr;
}

static void ImGui_ImplAllegro5_SetupRenderState(ImDrawData* draw_data) {
  // Setup blending
  al_set_separate_blender(ALLEGRO_ADD, ALLEGRO_ALPHA, ALLEGRO_INVERSE_ALPHA, ALLEGRO_ADD,
...
void ImGui_ImplAllegro5_RenderDrawData(ImDrawData* draw_data) {
  ...
  if (draw_data->Textures != nullptr)
    for (ImTextureData* tex : *draw_data->Textures)
      if (tex->Status != ImTextureStatus_OK) ImGui_ImplAllegro5_UpdateTexture(tex);
  ...
  for (const ImDrawList* draw_list : draw_data->CmdLists) {
    ...
    if (sizeof(ImDrawIdx) == 2) {
      ...
      // You can '#define ImDrawIdx int' in imconfig.h to request Dear ImGui to output 32-bit
      // indices. Otherwise, we convert them from 16-bit to 32-bit at runtime here, which works
      // perfectly but is a little wasteful.
...
#else
    // Allegro's implementation of al_draw_indexed_prim() for DX9 was broken until 5.2.5. Unindex
    // buffers ourselves while converting vertex format.
    ...
    for (int cmd_i = 0; cmd_i < draw_list->CmdBuffer.Size; cmd_i++) {
      ...
      if (pcmd->UserCallback) {
        ...
        al_set_clipping_rectangle(clip_min.x, clip_min.y, clip_max.x - clip_min.x,
                                  clip_max.y - clip_min.y);
#if ALLEGRO_HAS_DRAW_INDEXED_PRIM
        al_draw_indexed_prim(&vertices[0], bd->VertexDecl, texture, &indices[pcmd->IdxOffset],
                             pcmd->ElemCount, ALLEGRO_PRIM_TRIANGLE_LIST);
#else
        al_draw_prim(&vertices[0], bd->VertexDecl, texture, pcmd->IdxOffset,
                     pcmd->IdxOffset + pcmd->ElemCount, ALLEGRO_PRIM_TRIANGLE_LIST);
#endif
      }
    }
  }
...
void ImGui_ImplAllegro5_UpdateTexture(ImTextureData* tex) {
  if (tex->Status == ImTextureStatus_WantCreate) {
    ...
    ALLEGRO_LOCKED_REGION* locked_region =
        al_lock_bitmap(cpu_bitmap, al_get_bitmap_format(cpu_bitmap), ALLEGRO_LOCK_WRITEONLY);
    ...
    ALLEGRO_LOCKED_REGION* locked_region = al_lock_bitmap_region(
        gpu_bitmap, r.x, r.y, r.w, r.h, al_get_bitmap_format(gpu_bitmap), ALLEGRO_LOCK_WRITEONLY);
    IM_ASSERT(locked_region && "Backend failed to update texture!");
    for (int y = 0; y < r.h; y++)
      memcpy((unsigned char*)locked_region->data + locked_region->pitch * y,
             tex->GetPixelsAt(r.x, r.y + y), r.w * tex->BytesPerPixel);  // dst, src, block pitch
    al_unlock_bitmap(gpu_bitmap);
    tex->SetStatus(ImTextureStatus_OK);
  } else if (tex->Status == ImTextureStatus_WantDestroy) {
    ALLEGRO_BITMAP* backend_tex = (ALLEGRO_BITMAP*)(intptr_t)tex->TexID;
    if (backend_tex) al_destroy_bitmap(backend_tex);
...
static const char* ImGui_ImplAllegro5_GetClipboardText(ImGuiContext*) {
  ImGui_ImplAllegro5_Data* bd = ImGui_ImplAllegro5_GetBackendData();
  if (bd->ClipboardTextData) al_free(bd->ClipboardTextData);
...
void ImGui_ImplAllegro5_Shutdown() {
  ...
  if (bd->VertexDecl) al_destroy_vertex_decl(bd->VertexDecl);
  if (bd->ClipboardTextData) al_free(bd->ClipboardTextData);
...
void ImGui_ImplAllegro5_SetDisplay(ALLEGRO_DISPLAY* display) {
  ...
  if (bd->Display && !bd->VertexDecl) {
    ...
    // Unfortunately Allegro doesn't support 32-bits packed colors so we have to convert them to 4
    // floats. We still use a custom declaration to use 'ALLEGRO_PRIM_TEX_COORD' instead of
    // 'ALLEGRO_PRIM_TEX_COORD_PIXEL' else we can't do a reliable conversion.
...
static void ImGui_ImplAllegro5_UpdateKeyModifiers() {
  ...
  io.AddKeyEvent(ImGuiMod_Shift,
                 al_key_down(&keys, ALLEGRO_KEY_LSHIFT) || al_key_down(&keys, ALLEGRO_KEY_RSHIFT));
  io.AddKeyEvent(ImGuiMod_Alt,
                 al_key_down(&keys, ALLEGRO_KEY_ALT) || al_key_down(&keys, ALLEGRO_KEY_ALTGR));
  io.AddKeyEvent(ImGuiMod_Super,
                 al_key_down(&keys, ALLEGRO_KEY_LWIN) || al_key_down(&keys, ALLEGRO_KEY_RWIN));
}
...
bool ImGui_ImplAllegro5_ProcessEvent(ALLEGRO_EVENT* ev) {
  ...
  IM_ASSERT(bd != nullptr &&
            "Context or backend not initialized! Did you call ImGui_ImplAllegro5_Init()?");
  ImGuiIO& io = ImGui::GetIO();
  ...
  switch (ev->type) {
    ...
    case ALLEGRO_EVENT_TOUCH_MOVE:
      if (ev->touch.display == bd->Display) io.AddMousePosEvent(ev->touch.x, ev->touch.y);
      return true;
    ...
    case ALLEGRO_EVENT_MOUSE_LEAVE_DISPLAY:
      if (ev->mouse.display == bd->Display) io.AddMousePosEvent(-FLT_MAX, -FLT_MAX);
      return true;
    case ALLEGRO_EVENT_KEY_CHAR:
      if (ev->keyboard.display == bd->Display)
        if (ev->keyboard.unichar != 0) io.AddInputCharacter((unsigned int)ev->keyboard.unichar);
      return true;
    ...
    case ALLEGRO_EVENT_DISPLAY_SWITCH_OUT:
      if (ev->display.source == bd->Display) io.AddFocusEvent(false);
      return true;
...
static void ImGui_ImplAllegro5_UpdateMouseCursor() {
  ...
  if (bd->LastCursor == imgui_cursor) return;
  ...
  if (imgui_cursor == ImGuiMouseCursor_None) {
    ...
    switch (imgui_cursor) {
      ...
      case ImGuiMouseCursor_Wait:
        cursor_id = ALLEGRO_SYSTEM_MOUSE_CURSOR_BUSY;
        break;
      case ImGuiMouseCursor_Progress:
        cursor_id = ALLEGRO_SYSTEM_MOUSE_CURSOR_PROGRESS;
        break;
...
void ImGui_ImplAllegro5_NewFrame() {
  ...
  if (!bd->MouseCursorInvisible) ImGui_ImplAllegro5_CreateDeviceObjects();

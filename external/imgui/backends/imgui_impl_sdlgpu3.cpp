...
//  [X] Renderer: User texture binding. Use 'SDL_GPUTexture*' as texture identifier. Read the FAQ
//  about ImTextureID/ImTextureRef! **IMPORTANT** Before 2025/08/08, ImTextureID was a reference to
//  a SDL_GPUTextureSamplerBinding struct. [X] Renderer: Large meshes support (64k+ vertices) even
//  with 16-bit indices (ImGuiBackendFlags_RendererHasVtxOffset). [X] Renderer: Texture updates
//  support for dynamic font atlas (ImGuiBackendFlags_RendererHasTextures).
...
// Important note to the reader who wish to integrate imgui_impl_sdlgpu3.cpp/.h in their own
// engine/app.
// - Unlike other backends, the user must call the function ImGui_ImplSDLGPU3_PrepareDrawData()
// BEFORE issuing a SDL_GPURenderPass containing ImGui_ImplSDLGPU3_RenderDrawData.
//   Calling the function is MANDATORY, otherwise the ImGui will not upload neither the vertex nor
//   the index buffer for the GPU. See imgui_impl_sdlgpu3.cpp for more info.

// CHANGELOG
//  2025-09-18: Call platform_io.ClearRendererHandlers() on shutdown.
//  2025-08-20: Added ImGui_ImplSDLGPU3_InitInfo::SwapchainComposition and
//  ImGui_ImplSDLGPU3_InitInfo::PresentMode to configure how secondary viewports are created.
//  2025-08-08: *BREAKING* Changed ImTextureID type from SDL_GPUTextureSamplerBinding* to
//  SDL_GPUTexture*, which is more natural and easier for user to manage. If you need to change the
//  current sampler, you can access the ImGui_ImplSDLGPU3_RenderState struct. (#8866, #8163, #7998,
//  #7988) 2025-08-08: Expose SamplerDefault and SamplerCurrent in ImGui_ImplSDLGPU3_RenderState.
//  Allow callback to change sampler. 2025-06-25: Mapping transfer buffer for texture update use
//  cycle=true. Fixes artifacts e.g. on Metal backend. 2025-06-11: Added support for
//  ImGuiBackendFlags_RendererHasTextures, for dynamic font atlas. Removed
//  ImGui_ImplSDLGPU3_CreateFontsTexture() and ImGui_ImplSDLGPU3_DestroyFontsTexture(). 2025-04-28:
//  Added support for special ImDrawCallback_ResetRenderState callback to reset render state.
//  2025-03-30: Made ImGui_ImplSDLGPU3_PrepareDrawData() reuse GPU Transfer Buffers which were
//  unusually slow to recreate every frame. Much faster now. 2025-03-21: Fixed typo in function name
//  Imgui_ImplSDLGPU3_PrepareDrawData() -> ImGui_ImplSDLGPU3_PrepareDrawData(). 2025-01-16: Renamed
//  ImGui_ImplSDLGPU3_InitInfo::GpuDevice to Device. 2025-01-09: SDL_GPU: Added the SDL_GPU3
//  backend.
...
static ImGui_ImplSDLGPU3_Data* ImGui_ImplSDLGPU3_GetBackendData() {
  return ImGui::GetCurrentContext()
             ? (ImGui_ImplSDLGPU3_Data*)ImGui::GetIO().BackendRendererUserData
             : nullptr;
}

static void ImGui_ImplSDLGPU3_SetupRenderState(ImDrawData* draw_data,
                                               ImGui_ImplSDLGPU3_RenderState* render_state,
                                               SDL_GPUGraphicsPipeline* pipeline,
                                               SDL_GPUCommandBuffer* command_buffer,
                                               SDL_GPURenderPass* render_pass,
                                               ImGui_ImplSDLGPU3_FrameData* fd, uint32_t fb_width,
                                               uint32_t fb_height) {
  ...
  if (draw_data->TotalVtxCount > 0) {
    ...
    SDL_BindGPUIndexBuffer(
        render_pass, &index_buffer_binding,
...
static void CreateOrResizeBuffers(SDL_GPUBuffer** buffer, SDL_GPUTransferBuffer** transferbuffer,
  ...
  *transferbuffer = SDL_CreateGPUTransferBuffer(v->Device, &transferbuffer_info);
  IM_ASSERT(*transferbuffer != nullptr &&
            "Failed to create GPU Transfer Buffer, call SDL_GetError() for more information");
}

// SDL_GPU doesn't allow copy passes to occur while a render or compute pass is bound!
// The only way to allow a user to supply their own RenderPass (to render to a texture instead of
// the window for example), is to split the upload part of ImGui_ImplSDLGPU3_RenderDrawData() to
// another function that needs to be called by the user before rendering.
void ImGui_ImplSDLGPU3_PrepareDrawData(ImDrawData* draw_data,
                                       SDL_GPUCommandBuffer* command_buffer) {
  ...
  if (draw_data->Textures != nullptr)
    for (ImTextureData* tex : *draw_data->Textures)
      if (tex->Status != ImTextureStatus_OK) ImGui_ImplSDLGPU3_UpdateTexture(tex);
  ...
  if (fd->VertexBuffer == nullptr || fd->VertexBufferSize < vertex_size)
    CreateOrResizeBuffers(&fd->VertexBuffer, &fd->VertexTransferBuffer, &fd->VertexBufferSize,
                          vertex_size, SDL_GPU_BUFFERUSAGE_VERTEX);
  if (fd->IndexBuffer == nullptr || fd->IndexBufferSize < index_size)
    CreateOrResizeBuffers(&fd->IndexBuffer, &fd->IndexTransferBuffer, &fd->IndexBufferSize,
                          index_size, SDL_GPU_BUFFERUSAGE_INDEX);

  ImDrawVert* vtx_dst =
      (ImDrawVert*)SDL_MapGPUTransferBuffer(v->Device, fd->VertexTransferBuffer, true);
  ImDrawIdx* idx_dst =
      (ImDrawIdx*)SDL_MapGPUTransferBuffer(v->Device, fd->IndexTransferBuffer, true);
...
void ImGui_ImplSDLGPU3_RenderDrawData(ImDrawData* draw_data, SDL_GPUCommandBuffer* command_buffer,
  ...
  if (pipeline == nullptr) pipeline = bd->Pipeline;
  ...
  ImGui_ImplSDLGPU3_SetupRenderState(draw_data, &render_state, pipeline, command_buffer,
  ...
  for (const ImDrawList* draw_list : draw_data->CmdLists) {
    for (int cmd_i = 0; cmd_i < draw_list->CmdBuffer.Size; cmd_i++) {
      ...
      if (pcmd->UserCallback != nullptr) {
        ...
        // Draw
        // **IF YOU GET A CRASH HERE** In 1.92.2 on 2025/08/08 we have changed ImTextureID to store
        // 'SDL_GPUTexture*' instead of storing 'SDL_GPUTextureSamplerBinding'. Any code loading
        // custom texture using this backend needs to be updated.
        SDL_DrawGPUIndexedPrimitives(render_pass, pcmd->ElemCount, 1,
  ...
  // Our last values will leak into user/application rendering if you forgot to call
  // SDL_SetGPUViewport() and SDL_SetGPUScissor() yourself to explicitly set that state In theory we
  // should aim to backup/restore those values but I am not sure this is possible. We perform a call
  // to SDL_SetGPUScissor() to set back a full viewport which is likely to fix things for 99% users
  // but technically this is not perfect. (See github #4644)
...
void ImGui_ImplSDLGPU3_UpdateTexture(ImTextureData* tex) {
  ...
  if (tex->Status == ImTextureStatus_WantCreate || tex->Status == ImTextureStatus_WantUpdates) {
    ...
    if (bd->TexTransferBufferSize < upload_size) {
      ...
      IM_ASSERT(bd->TexTransferBuffer != nullptr &&
                "Failed to create transfer buffer, call SDL_GetError() for more information");
    }
    ...
    // Copy to transfer buffer
    {
      ...
      for (int y = 0; y < upload_h; y++)
        memcpy((void*)((uintptr_t)texture_ptr + y * upload_pitch),
               tex->GetPixelsAt(upload_x, upload_y + y), upload_pitch);
...
void ImGui_ImplSDLGPU3_CreateDeviceObjects() {
  ...
  if (bd->TexSamplerLinear == nullptr) {
    ...
    IM_ASSERT(bd->TexSamplerLinear != nullptr &&
              "Failed to create sampler, call SDL_GetError() for more information");
  }
...
void ImGui_ImplSDLGPU3_DestroyDeviceObjects() {
  ...
  for (ImTextureData* tex : ImGui::GetPlatformIO().Textures)
    if (tex->RefCount == 1) ImGui_ImplSDLGPU3_DestroyTexture(tex);
  if (bd->TexTransferBuffer) {
    SDL_ReleaseGPUTransferBuffer(v->Device, bd->TexTransferBuffer);
    bd->TexTransferBuffer = nullptr;
  }
  if (bd->VertexShader) {
    SDL_ReleaseGPUShader(v->Device, bd->VertexShader);
    bd->VertexShader = nullptr;
  }
  if (bd->FragmentShader) {
    SDL_ReleaseGPUShader(v->Device, bd->FragmentShader);
    bd->FragmentShader = nullptr;
  }
  if (bd->TexSamplerLinear) {
    SDL_ReleaseGPUSampler(v->Device, bd->TexSamplerLinear);
    bd->TexSamplerLinear = nullptr;
  }
  if (bd->Pipeline) {
    SDL_ReleaseGPUGraphicsPipeline(v->Device, bd->Pipeline);
    bd->Pipeline = nullptr;
  }
}
...
void ImGui_ImplSDLGPU3_NewFrame() {
  ...
  IM_ASSERT(bd != nullptr &&
            "Context or backend not initialized! Did you call ImGui_ImplSDLGPU3_Init()?");

  if (!bd->TexSamplerLinear) ImGui_ImplSDLGPU3_CreateDeviceObjects();
}

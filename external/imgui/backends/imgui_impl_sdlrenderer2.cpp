...
// You can copy and use unmodified imgui_impl_* files in your project. See examples/ folder for
// examples of using this. Prefer including the entire imgui/ repository into your project (either
// as a copy or as a submodule), and only build the backends you need. Learn about Dear ImGui:
...
//  Removed ImGui_ImplSDLRenderer2_CreateFontsTexture() and
//  ImGui_ImplSDLRenderer2_DestroyFontsTexture(). 2025-01-18: Use endian-dependent RGBA32 texture
//  format, to match SDL_Color. 2024-10-09: Expose selected render state in
//  ImGui_ImplSDLRenderer2_RenderState, which you can access in 'void*
//  platform_io.Renderer_RenderState' during draw callbacks. 2024-05-14: *BREAKING CHANGE*
//  ImGui_ImplSDLRenderer3_RenderDrawData() requires SDL_Renderer* passed as parameter. 2023-05-30:
//  Renamed imgui_impl_sdlrenderer.h/.cpp to imgui_impl_sdlrenderer2.h/.cpp to accommodate for
//  upcoming SDL3. 2022-10-11: Using 'nullptr' instead of 'NULL' as per our switch to C++11.
//  2021-12-21: Update SDL_RenderGeometryRaw() format to work with SDL 2.0.19.
//  2021-12-03: Added support for large mesh (64K+ vertices), enable
//  ImGuiBackendFlags_RendererHasVtxOffset flag. 2021-10-06: Backup and restore modified
//  ClipRect/Viewport. 2021-09-21: Initial version.
...
static ImGui_ImplSDLRenderer2_Data* ImGui_ImplSDLRenderer2_GetBackendData() {
  return ImGui::GetCurrentContext()
             ? (ImGui_ImplSDLRenderer2_Data*)ImGui::GetIO().BackendRendererUserData
             : nullptr;
}
...
static void ImGui_ImplSDLRenderer2_SetupRenderState(SDL_Renderer* renderer) {
  ...
  // FIXME: Technically speaking there are lots of other things we could backup/setup/restore during
  // our render process.
...
void ImGui_ImplSDLRenderer2_NewFrame() {
  ...
  IM_ASSERT(bd != nullptr &&
            "Context or backend not initialized! Did you call ImGui_ImplSDLRenderer2_Init()?");
  IM_UNUSED(bd);
}

void ImGui_ImplSDLRenderer2_RenderDrawData(ImDrawData* draw_data, SDL_Renderer* renderer) {
  // If there's a scale factor set by the user, use that instead
  // If the user has specified a scale factor to SDL_Renderer already via SDL_RenderSetScale(), SDL
  // will scale whatever we pass to SDL_RenderGeometryRaw() by that scale factor. In that case we
  // don't want to be also scaling it ourselves here.
  float rsx = 1.0f;
  float rsy = 1.0f;
  ...
  if (draw_data->Textures != nullptr)
    for (ImTextureData* tex : *draw_data->Textures)
      if (tex->Status != ImTextureStatus_OK) ImGui_ImplSDLRenderer2_UpdateTexture(tex);
  ...
  for (const ImDrawList* draw_list : draw_data->CmdLists) {
    ...
    for (int cmd_i = 0; cmd_i < draw_list->CmdBuffer.Size; cmd_i++) {
      ...
      if (pcmd->UserCallback) {
        ...
        const float* xy = (const float*)(const void*)((const char*)(vtx_buffer + pcmd->VtxOffset) +
                                                      offsetof(ImDrawVert, pos));
        const float* uv = (const float*)(const void*)((const char*)(vtx_buffer + pcmd->VtxOffset) +
...
#else
        const int* color =
            (const int*)(const void*)((const char*)(vtx_buffer + pcmd->VtxOffset) +
                                      offsetof(ImDrawVert, col));  // SDL 2.0.17 and 2.0.18
#endif

        // Bind texture, Draw
        SDL_Texture* tex = (SDL_Texture*)pcmd->GetTexID();
        SDL_RenderGeometryRaw(renderer, tex, xy, (int)sizeof(ImDrawVert), color,
...
void ImGui_ImplSDLRenderer2_UpdateTexture(ImTextureData* tex) {
  ...
  if (tex->Status == ImTextureStatus_WantCreate) {
    ...
    SDL_Texture* sdl_texture = SDL_CreateTexture(bd->Renderer, SDL_PIXELFORMAT_RGBA32,
                                                 SDL_TEXTUREACCESS_STATIC, tex->Width, tex->Height);

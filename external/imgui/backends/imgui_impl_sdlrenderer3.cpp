...
//  Removed ImGui_ImplSDLRenderer3_CreateFontsTexture() and
//  ImGui_ImplSDLRenderer3_DestroyFontsTexture(). 2025-01-18: Use endian-dependent RGBA32 texture
//  format, to match SDL_Color. 2024-10-09: Expose selected render state in
//  ImGui_ImplSDLRenderer3_RenderState, which you can access in 'void*
//  platform_io.Renderer_RenderState' during draw callbacks. 2024-07-01: Update for SDL3 api
//  changes: SDL_RenderGeometryRaw() uint32 version was removed (SDL#9009). 2024-05-14: *BREAKING
//  CHANGE* ImGui_ImplSDLRenderer3_RenderDrawData() requires SDL_Renderer* passed as parameter.
...
void ImGui_ImplSDLRenderer3_RenderDrawData(ImDrawData* draw_data, SDL_Renderer* renderer) {
  ...
  if (draw_data->Textures != nullptr)
    for (ImTextureData* tex : *draw_data->Textures)
      if (tex->Status != ImTextureStatus_OK) ImGui_ImplSDLRenderer3_UpdateTexture(tex);
  ...
  for (const ImDrawList* draw_list : draw_data->CmdLists) {
    ...
    for (int cmd_i = 0; cmd_i < draw_list->CmdBuffer.Size; cmd_i++) {
      ...
      if (pcmd->UserCallback) {
        ...
        SDL_RenderGeometryRaw8BitColor(renderer, bd->ColorBuffer, tex, xy, (int)sizeof(ImDrawVert),
                                       color, (int)sizeof(ImDrawVert), uv, (int)sizeof(ImDrawVert),

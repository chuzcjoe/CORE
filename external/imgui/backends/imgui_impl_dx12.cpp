...
//  [X] Renderer: User texture binding. Use 'D3D12_GPU_DESCRIPTOR_HANDLE' as texture identifier.
//  Read the FAQ about ImTextureID/ImTextureRef! [X] Renderer: Large meshes support (64k+ vertices)
//  even with 16-bit indices (ImGuiBackendFlags_RendererHasVtxOffset). [X] Renderer: Texture updates
//  support for dynamic font atlas (ImGuiBackendFlags_RendererHasTextures). [X] Renderer: Expose
//  selected render state for draw callbacks to use. Access in
...
//  2025-09-29: DirectX12: Enable swapchain tearing to eliminate viewports framerate throttling.
//  (#8965) 2025-09-29: DirectX12: Reuse a command list and allocator for texture uploads instead of
//  recreating them each time. (#8963) 2025-09-18: Call platform_io.ClearRendererHandlers() on
//  shutdown. 2025-06-19: Fixed build on MinGW. (#8702, #4594) 2025-06-11: DirectX12: Added support
//  for ImGuiBackendFlags_RendererHasTextures, for dynamic font atlas. 2025-05-07: DirectX12: Honor
//  draw_data->FramebufferScale to allow for custom backends and experiment using it (consistently
//  with other renderer backends, even though in normal condition it is not set under Windows).
//  2025-02-24: DirectX12: Fixed an issue where ImGui_ImplDX12_Init() signature change from
//  2024-11-15 combined with change from 2025-01-15 made legacy ImGui_ImplDX12_Init() crash. (#8429)
//  2025-01-15: DirectX12: Texture upload use the command queue provided in ImGui_ImplDX12_InitInfo
//  instead of creating its own. 2024-12-09: DirectX12: Let user specifies the DepthStencilView
//  format by setting ImGui_ImplDX12_InitInfo::DSVFormat. 2024-11-15: DirectX12: *BREAKING CHANGE*
//  Changed ImGui_ImplDX12_Init() signature to take a ImGui_ImplDX12_InitInfo struct. Legacy
//  ImGui_ImplDX12_Init() signature is still supported (will obsolete). 2024-11-15: DirectX12:
//  *BREAKING CHANGE* User is now required to pass function pointers to allocate/free SRV
//  Descriptors. We provide convenience legacy fields to pass a single descriptor, matching the old
//  API, but upcoming features will want multiple. 2024-10-23: DirectX12: Unmap() call specify
//  written range. The range is informational and may be used by debug tools. 2024-10-07: DirectX12:
//  Changed default texture sampler to Clamp instead of Repeat/Wrap. 2024-10-07: DirectX12: Expose
//  selected render state in ImGui_ImplDX12_RenderState, which you can access in 'void*
//  platform_io.Renderer_RenderState' during draw callbacks. 2024-10-07: DirectX12: Compiling with
//  '#define ImTextureID=ImU64' is unnecessary now that dear imgui defaults ImTextureID to u64
//  instead of void*. 2022-10-11: Using 'nullptr' instead of 'NULL' as per our switch to C++11.
//  2021-06-29: Reorganized backend to pull data from a single structure to facilitate usage with
//  multiple-contexts (all g_XXXX access changed to bd->XXXX). 2021-05-19: DirectX12: Replaced
//  direct access to ImDrawCmd::TextureId with a call to ImDrawCmd::GetTexID(). (will become a
//  requirement) 2021-02-18: DirectX12: Change blending equation to preserve alpha in output buffer.
//  2021-01-11: DirectX12: Improve Windows 7 compatibility (for D3D12On7) by loading d3d12.dll
//  dynamically. 2020-09-16: DirectX12: Avoid rendering calls with zero-sized scissor rectangle
//  since it generates a validation layer warning. 2020-09-08: DirectX12: Clarified support for
//  building on 32-bit systems by redefining ImTextureID. 2019-10-18: DirectX12: *BREAKING CHANGE*
//  Added extra ID3D12DescriptorHeap parameter to ImGui_ImplDX12_Init() function. 2019-05-29:
//  DirectX12: Added support for large mesh (64K+ vertices), enable
//  ImGuiBackendFlags_RendererHasVtxOffset flag. 2019-04-30: DirectX12: Added support for special
//  ImDrawCallback_ResetRenderState callback to reset render state. 2019-03-29: Misc: Various minor
//  tidying up. 2018-12-03: Misc: Added #pragma comment statement to automatically link with
//  d3dcompiler.lib when using D3DCompile(). 2018-11-30: Misc: Setting up io.BackendRendererName so
//  it can be displayed in the About Window. 2018-06-12: DirectX12: Moved the
//  ID3D12GraphicsCommandList* parameter from NewFrame() to RenderDrawData(). 2018-06-08: Misc:
...
#pragma comment( \
    lib,         \
    "d3dcompiler")  // Automatically link with d3dcompiler.lib as we are using D3DCompile() below.
#endif
...
static ImGui_ImplDX12_Data* ImGui_ImplDX12_GetBackendData() {
  return ImGui::GetCurrentContext() ? (ImGui_ImplDX12_Data*)ImGui::GetIO().BackendRendererUserData
                                    : nullptr;
}
...
void ImGui_ImplDX12_RenderDrawData(ImDrawData* draw_data, ID3D12GraphicsCommandList* command_list) {
  ...
  if (draw_data->Textures != nullptr)
    for (ImTextureData* tex : *draw_data->Textures)
      if (tex->Status != ImTextureStatus_OK) ImGui_ImplDX12_UpdateTexture(tex);
  ...
  if (fr->VertexBuffer == nullptr || fr->VertexBufferSize < draw_data->TotalVtxCount) {
    ...
    if (bd->pd3dDevice->CreateCommittedResource(&props, D3D12_HEAP_FLAG_NONE, &desc,
  ...
  // During Unmap() we specify the written range (as per DX12 API, this is informational and for
  // tooling only)
  ...
  for (const ImDrawList* draw_list : draw_data->CmdLists) {
    for (int cmd_i = 0; cmd_i < draw_list->CmdBuffer.Size; cmd_i++) {
      ...
      if (pcmd->UserCallback != nullptr) {
        ...
        const D3D12_RECT r = {(LONG)clip_min.x, (LONG)clip_min.y, (LONG)clip_max.x,
                              (LONG)clip_max.y};
        ...
        command_list->DrawIndexedInstanced(pcmd->ElemCount, 1, pcmd->IdxOffset + global_idx_offset,
...
static void ImGui_ImplDX12_DestroyTexture(ImTextureData* tex) {
  if (ImGui_ImplDX12_Texture* backend_tex = (ImGui_ImplDX12_Texture*)tex->BackendUserData) {
    ...
    ImGui_ImplDX12_Data* bd = ImGui_ImplDX12_GetBackendData();
    bd->InitInfo.SrvDescriptorFreeFn(&bd->InitInfo, backend_tex->hFontSrvCpuDescHandle,
                                     backend_tex->hFontSrvGpuDescHandle);
...
void ImGui_ImplDX12_UpdateTexture(ImTextureData* tex) {
  ...
  if (tex->Status == ImTextureStatus_WantCreate) {
    ...
    bd->InitInfo.SrvDescriptorAllocFn(
        &bd->InitInfo, &backend_tex->hFontSrvCpuDescHandle,
        &backend_tex->hFontSrvGpuDescHandle);  // Allocate a desctriptor handle
    ...
    bd->pd3dDevice->CreateShaderResourceView(pTexture, &srvDesc,
                                             backend_tex->hFontSrvCpuDescHandle);
    SafeRelease(backend_tex->pTextureResource);
    ...
    need_barrier_before_copy =
        false;  // Because this is a newly-created texture it will be in D3D12_RESOURCE_STATE_COMMON
                // and thus we don't need a barrier
  ...
  if (tex->Status == ImTextureStatus_WantCreate || tex->Status == ImTextureStatus_WantUpdates) {
    ...
    // We could use the smaller rect on _WantCreate but using the full rect allows us to clear the
    // texture.
    // FIXME-OPT: Uploading single box even when using ImTextureStatus_WantUpdates. Could use
    // tex->Updates[]
    ...
    // tex->Updates[] to upload individual regions.
    UINT upload_pitch_src = upload_w * tex->BytesPerPixel;
    UINT upload_pitch_dst = (upload_pitch_src + D3D12_TEXTURE_DATA_PITCH_ALIGNMENT - 1u) &
    ...
    if (bd->pTexUploadBuffer == nullptr || upload_size > bd->pTexUploadBufferSize) {
      ...
      HRESULT hr = bd->pd3dDevice->CreateCommittedResource(
          &props, D3D12_HEAP_FLAG_NONE, &desc, D3D12_RESOURCE_STATE_GENERIC_READ, nullptr,
          IID_PPV_ARGS(&bd->pTexUploadBuffer));
    ...
    for (int y = 0; y < upload_h; y++)
      memcpy((void*)((uintptr_t)bd->pTexUploadBufferMapped + y * upload_pitch_dst),
             tex->GetPixelsAt(upload_x, upload_y + y), upload_pitch_src);
    ...
    // - To remove this may need to create NumFramesInFlight x ImGui_ImplDX12_FrameContext in
    // backend data (mimick docking version)
...
bool ImGui_ImplDX12_CreateDeviceObjects() {
  ...
  bd->pdxgiFactory->CheckFeatureSupport(DXGI_FEATURE_PRESENT_ALLOW_TEARING, &allow_tearing,
  ...
  // Create the root signature
  {
    ...
    // Load d3d12.dll and D3D12SerializeRootSignature() function address dynamically to facilitate
    // using with D3D12On7. See if any version of d3d12.dll is already loaded in the process. If so,
    // give preference to that.
    static HINSTANCE d3d12_dll = ::GetModuleHandleA("d3d12.dll");
    if (d3d12_dll == nullptr) {
      // Attempt to load d3d12.dll from local directories. This will only succeed if
      // (1) the current OS is Windows 7, and
      // (2) there exists a version of d3d12.dll for Windows 7 (D3D12On7) in one of the following
      // directories. See https://github.com/ocornut/imgui/pull/3696 for details.
      const char* localD3d12Paths[] = {
          ".\\d3d12.dll", ".\\d3d12on7\\d3d12.dll",
          ".\\12on7\\d3d12.dll"};  // A. current directory, B. used by some games, C. used in
                                   // Microsoft D3D12On7 sample
    ...
    _PFN_D3D12_SERIALIZE_ROOT_SIGNATURE D3D12SerializeRootSignatureFn =
        (_PFN_D3D12_SERIALIZE_ROOT_SIGNATURE)(void*)::GetProcAddress(d3d12_dll,
    ...
    bd->pd3dDevice->CreateRootSignature(0, blob->GetBufferPointer(), blob->GetBufferSize(),
  ...
  //  1) compile once, save the compiled shader blobs into a file or source code and assign them to
  //  psoDesc.VS/PS [preferred solution] 2) use code to detect any version of the DLL and grab a
  //  pointer to D3DCompile from the DLL.
  ...
  // Create the vertex shader
  {
    ...
    if (FAILED(D3DCompile(vertexShader, strlen(vertexShader), nullptr, nullptr, nullptr, "main",
    ...
    static D3D12_INPUT_ELEMENT_DESC local_layout[] = {
        {"POSITION", 0, DXGI_FORMAT_R32G32_FLOAT, 0, (UINT)offsetof(ImDrawVert, pos),
         D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0},
        {"TEXCOORD", 0, DXGI_FORMAT_R32G32_FLOAT, 0, (UINT)offsetof(ImDrawVert, uv),
         D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0},
        {"COLOR", 0, DXGI_FORMAT_R8G8B8A8_UNORM, 0, (UINT)offsetof(ImDrawVert, col),
         D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0},
    };
  ...
  // Create the pixel shader
  {
    ...
    if (FAILED(D3DCompile(pixelShader, strlen(pixelShader), nullptr, nullptr, nullptr, "main",
  ...
  HRESULT result_pipeline_state =
      bd->pd3dDevice->CreateGraphicsPipelineState(&psoDesc, IID_PPV_ARGS(&bd->pPipelineState));
  vertexShaderBlob->Release();
  ...
  hr = bd->pd3dDevice->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_DIRECT,
                                              IID_PPV_ARGS(&bd->pTexCmdAllocator));
  IM_ASSERT(SUCCEEDED(hr));
  hr = bd->pd3dDevice->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, bd->pTexCmdAllocator,
                                         nullptr, IID_PPV_ARGS(&bd->pTexCmdList));
  IM_ASSERT(SUCCEEDED(hr));
...
void ImGui_ImplDX12_InvalidateDeviceObjects() {
  ...
  if (bd->commandQueueOwned) SafeRelease(bd->pCommandQueue);
  ...
  for (ImTextureData* tex : ImGui::GetPlatformIO().Textures)
    if (tex->RefCount == 1) ImGui_ImplDX12_DestroyTexture(tex);
...
static void ImGui_ImplDX12_InitLegacySingleDescriptorMode(ImGui_ImplDX12_InitInfo* init_info) {
  ...
  IM_ASSERT(init_info->LegacySingleSrvCpuDescriptor.ptr != 0 &&
            init_info->LegacySingleSrvGpuDescriptor.ptr != 0);
...
#endif
  IM_ASSERT(init_info->SrvDescriptorAllocFn != nullptr &&
...
// font_srv_cpu_desc_handle and font_srv_gpu_desc_handle are handles to a single SRV descriptor to
// use for the internal font texture, they must be in 'srv_descriptor_heap'
bool ImGui_ImplDX12_Init(ID3D12Device* device, int num_frames_in_flight, DXGI_FORMAT rtv_format,
  ...
  ImGuiIO& io = ImGui::GetIO();
  io.BackendFlags &=
      ~ImGuiBackendFlags_RendererHasTextures;  // Using legacy ImGui_ImplDX12_Init() call with 1 SRV
                                               // descriptor we cannot support multiple textures.

...
// (Please note that WebGPU is currently experimental, will not run on non-beta browsers, and may
// break.)

// Implemented features:
//  [X] Renderer: User texture binding. Use 'WGPUTextureView' as ImTextureID. Read the FAQ about
//  ImTextureID/ImTextureRef! [X] Renderer: Large meshes support (64k+ vertices) even with 16-bit
//  indices (ImGuiBackendFlags_RendererHasVtxOffset). [X] Renderer: Expose selected render state for
//  draw callbacks to use. Access in
//  '(ImGui_ImplXXXX_RenderState*)GetPlatformIO().Renderer_RenderState'. [X] Renderer: Texture
//  updates support for dynamic font system (ImGuiBackendFlags_RendererHasTextures).
...
//  2025-06-12: Added support for ImGuiBackendFlags_RendererHasTextures, for dynamic font atlas.
//  (#8465) 2025-02-26: Recreate image bind groups during render. (#8426, #8046, #7765, #8027) +
//  Update for latest webgpu-native changes. 2024-10-14: Update Dawn support for change of string
//  usages. (#8082, #8083) 2024-10-07: Expose selected render state in ImGui_ImplWGPU_RenderState,
...
//  2024-09-16: Added support for optional IMGUI_IMPL_WEBGPU_BACKEND_DAWN /
//  IMGUI_IMPL_WEBGPU_BACKEND_WGPU define to handle ever-changing native implementations. (#7977)
//  2024-01-22: Added configurable PipelineMultisampleState struct. (#7240)
//  2024-01-22: (Breaking) ImGui_ImplWGPU_Init() now takes a ImGui_ImplWGPU_InitInfo structure
//  instead of variety of parameters, allowing for easier further changes. 2024-01-22: Fixed
//  pipeline layout leak. (#7245) 2024-01-17: Explicitly fill all of WGPUDepthStencilState since
//  standard removed defaults. 2023-07-13: Use WGPUShaderModuleWGSLDescriptor's code instead of
//  source. use WGPUMipmapFilterMode_Linear instead of WGPUFilterMode_Linear. (#6602) 2023-04-11:
//  Align buffer sizes. Use WGSL shaders instead of precompiled SPIR-V. 2023-04-11: Reorganized
//  backend to pull data from a single structure to facilitate usage with multiple-contexts (all
//  g_XXXX access changed to bd->XXXX). 2023-01-25: Revert automatic pipeline layout generation (see
//  https://github.com/gpuweb/gpuweb/issues/2470) 2022-11-24: Fixed validation error with default
//  depth buffer settings. 2022-11-10: Fixed rendering when a depth buffer is enabled. Added
//  'WGPUTextureFormat depth_format' parameter to ImGui_ImplWGPU_Init(). 2022-10-11: Using 'nullptr'
//  instead of 'NULL' as per our switch to C++11. 2021-11-29: Passing explicit buffer sizes to
//  wgpuRenderPassEncoderSetVertexBuffer()/wgpuRenderPassEncoderSetIndexBuffer(). 2021-08-24: Fixed
//  for latest specs. 2021-05-24: Add support for draw_data->FramebufferScale. 2021-05-19: Replaced
//  direct access to ImDrawCmd::TextureId with a call to ImDrawCmd::GetTexID(). (will become a
//  requirement) 2021-05-16: Update to latest WebGPU specs (compatible with Emscripten 2.0.20 and
//  Chrome Canary 92). 2021-02-18: Change blending equation to preserve alpha in output buffer.
...
// Dawn renamed WGPUProgrammableStageDescriptor to WGPUComputeState (see:
// https://github.com/webgpu-native/webgpu-headers/pull/413) Using type alias until WGPU adopts the
// same naming convention (#8369)
...
#define MEMALIGN(_SIZE, _ALIGN) \
  (((_SIZE) + ((_ALIGN) - 1)) & ~((_ALIGN) - 1))  // Memory align (copied from IM_ALIGN() macro).
...
struct RenderResources {
  ...
  ImGuiStorage ImageBindGroups;  // Resources bind-group to bind the font/image resources to
                                 // pipeline (this is a key->value map)
  WGPUBindGroupLayout ImageBindGroupLayout =
      nullptr;  // Cache layout used for the image bind group. Avoids allocating unnecessary JS
...
static void SafeRelease(WGPUBindGroupLayout& res) {
  if (res) wgpuBindGroupLayoutRelease(res);
  res = nullptr;
}
static void SafeRelease(WGPUBindGroup& res) {
  if (res) wgpuBindGroupRelease(res);
  res = nullptr;
}
static void SafeRelease(WGPUBuffer& res) {
  if (res) wgpuBufferRelease(res);
  res = nullptr;
}
static void SafeRelease(WGPUPipelineLayout& res) {
  if (res) wgpuPipelineLayoutRelease(res);
  res = nullptr;
}
static void SafeRelease(WGPURenderPipeline& res) {
  if (res) wgpuRenderPipelineRelease(res);
  res = nullptr;
}
static void SafeRelease(WGPUSampler& res) {
  if (res) wgpuSamplerRelease(res);
  res = nullptr;
}
static void SafeRelease(WGPUShaderModule& res) {
  if (res) wgpuShaderModuleRelease(res);
  res = nullptr;
}
...
static WGPUBindGroup ImGui_ImplWGPU_CreateImageBindGroup(WGPUBindGroupLayout layout,
...
static void ImGui_ImplWGPU_SetupRenderState(ImDrawData* draw_data, WGPURenderPassEncoder ctx,
                                            FrameResources* fr) {
  ImGui_ImplWGPU_Data* bd = ImGui_ImplWGPU_GetBackendData();
  ...
  // draw_data->DisplayPos+data_data->DisplaySize (bottom right).
  {
    ...
    wgpuQueueWriteBuffer(bd->defaultQueue, bd->renderResources.Uniforms, offsetof(Uniforms, MVP),
                         mvp, sizeof(Uniforms::MVP));
    float gamma;
    ...
    wgpuQueueWriteBuffer(bd->defaultQueue, bd->renderResources.Uniforms, offsetof(Uniforms, Gamma),
                         &gamma, sizeof(Uniforms::Gamma));
  }

  // Setup viewport
  wgpuRenderPassEncoderSetViewport(ctx, 0, 0,
                                   draw_data->FramebufferScale.x * draw_data->DisplaySize.x,
...
void ImGui_ImplWGPU_RenderDrawData(ImDrawData* draw_data, WGPURenderPassEncoder pass_encoder) {
  ...
  if (fb_width <= 0 || fb_height <= 0 || draw_data->CmdLists.Size == 0) return;
  ...
  if (fr->VertexBuffer == nullptr || fr->VertexBufferSize < draw_data->TotalVtxCount) {
    ...
    WGPUBufferDescriptor vb_desc = {nullptr,
  ...
  if (fr->IndexBuffer == nullptr || fr->IndexBufferSize < draw_data->TotalIdxCount) {
    ...
    WGPUBufferDescriptor ib_desc = {nullptr,
    ...
    for (int cmd_i = 0; cmd_i < draw_list->CmdBuffer.Size; cmd_i++) {
      ...
      if (pcmd->UserCallback != nullptr) {
        ...
        WGPUBindGroup bind_group =
            (WGPUBindGroup)bd->renderResources.ImageBindGroups.GetVoidPtr(tex_id_hash);
        if (!bind_group) {
          bind_group = ImGui_ImplWGPU_CreateImageBindGroup(bd->renderResources.ImageBindGroupLayout,
        ...
        // Clamp to viewport as wgpuRenderPassEncoderSetScissorRect() won't accept values that are
        // off bounds
        if (clip_min.x < 0.0f) {
          clip_min.x = 0.0f;
        }
...
void ImGui_ImplWGPU_UpdateTexture(ImTextureData* tex) {
  ...
  if (tex->Status == ImTextureStatus_WantCreate || tex->Status == ImTextureStatus_WantUpdates) {
    ...
    wgpuQueueWriteTexture(bd->defaultQueue, &dst_view, tex->GetPixelsAt(upload_x, upload_y),
                          (uint32_t)(tex->Width * upload_h * tex->BytesPerPixel), &layout,
                          &write_size);
...
static void ImGui_ImplWGPU_CreateUniformBuffer() {
  ImGui_ImplWGPU_Data* bd = ImGui_ImplWGPU_GetBackendData();
  WGPUBufferDescriptor ub_desc = {nullptr,
...
bool ImGui_ImplWGPU_CreateDeviceObjects() {
  ImGui_ImplWGPU_Data* bd = ImGui_ImplWGPU_GetBackendData();
  if (!bd->wgpuDevice) return false;
  if (bd->pipelineState) ImGui_ImplWGPU_InvalidateDeviceObjects();
...
void ImGui_ImplWGPU_InvalidateDeviceObjects() {
  ...
  for (ImTextureData* tex : ImGui::GetPlatformIO().Textures)
    if (tex->RefCount == 1) ImGui_ImplWGPU_DestroyTexture(tex);

  for (unsigned int i = 0; i < bd->numFramesInFlight; i++) SafeRelease(bd->pFrameResources[i]);
}

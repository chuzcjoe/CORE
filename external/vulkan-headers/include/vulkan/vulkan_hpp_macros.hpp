...
#if defined(__cpp_lib_three_way_comparison) && (201907 <= __cpp_lib_three_way_comparison) && \
...
#if defined(__cpp_lib_modules) && !defined(VULKAN_HPP_STD_MODULE) && \
    defined(VULKAN_HPP_ENABLE_STD_MODULE)
...
// 32-bit vulkan is not typesafe for non-dispatchable handles, so don't allow copy constructors on
// this platform by default. To enable this feature on 32-bit platforms please #define
// VULKAN_HPP_TYPESAFE_CONVERSION 1 To disable this feature on 64-bit platforms please #define
...
#define VULKAN_HPP_RAII_NAMESPACE_STRING \
  VULKAN_HPP_STRINGIFY(VULKAN_HPP_NAMESPACE::VULKAN_HPP_RAII_NAMESPACE)
#endif

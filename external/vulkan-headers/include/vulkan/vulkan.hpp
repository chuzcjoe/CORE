...
extern "C" __declspec(dllimport) FARPROC __stdcall GetProcAddress(HINSTANCE hModule,
...
// XLib.h defines True/False, which collides with our
// VULKAN_HPP_NAMESPACE::True/VULKAN_HPP_NAMESPACE::False
...
std::strong_ordering operator<=>(ArrayWrapper1D<char, N> const &lhs,
                                 ArrayWrapper1D<char, M> const &rhs) VULKAN_HPP_NOEXCEPT {
...
#else
template <size_t N, size_t M>
bool operator<(ArrayWrapper1D<char, N> const &lhs,
               ArrayWrapper1D<char, M> const &rhs) VULKAN_HPP_NOEXCEPT {
...
#if defined(VULKAN_HPP_HAS_SPACESHIP_OPERATOR)
template <size_t N>
std::strong_ordering operator<=>(std::string const &lhs,
...
template <size_t N>
std::strong_ordering operator<=>(ArrayWrapper1D<char, N> const &lhs,
                                 std::string const &rhs) VULKAN_HPP_NOEXCEPT {
  return lhs.data() <=> rhs;
}
#else
template <size_t N>
...
class ArrayWrapper2D : public std::array<ArrayWrapper1D<T, M>, N> {
 public:
  ...
  VULKAN_HPP_CONSTEXPR ArrayWrapper2D(std::array<std::array<T, M>, N> const &data)
      VULKAN_HPP_NOEXCEPT
      : std::array<ArrayWrapper1D<T, M>, N>(
...
class ArrayProxy {
 public:
  ...
  VULKAN_HPP_CONSTEXPR ArrayProxy(std::nullptr_t) VULKAN_HPP_NOEXCEPT : m_count(0),
                                                                        m_ptr(nullptr) {}
...
#pragma GCC diagnostic pop
  ...
  template <typename V,
            typename std::enable_if<
                ...
                * = nullptr>
  ArrayProxy(V const &v) VULKAN_HPP_NOEXCEPT : m_count(static_cast<uint32_t>(v.size())),
                                               m_ptr(v.data()) {}
...
class ArrayProxyNoTemporaries {
 public:
  ...
  template <typename B = T, typename std::enable_if<std::is_convertible<B, T>::value &&
                                                        std::is_lvalue_reference<B>::value,
                                                    int>::type = 0>
  ArrayProxyNoTemporaries(B &&value) VULKAN_HPP_NOEXCEPT : m_count(1), m_ptr(&value) {}

  ArrayProxyNoTemporaries(uint32_t count, T *ptr) VULKAN_HPP_NOEXCEPT : m_count(count),
                                                                        m_ptr(ptr) {}

  template <std::size_t C>
  ArrayProxyNoTemporaries(T (&ptr)[C]) VULKAN_HPP_NOEXCEPT : m_count(C), m_ptr(ptr) {}

  template <std::size_t C>
  ArrayProxyNoTemporaries(T (&&ptr)[C]) = delete;

  // Any l-value reference with a .data() return type implicitly convertible to T*, and a .size()
  // return type implicitly convertible to size_t.
  template <typename V,
            typename std::enable_if<
                !std::is_convertible<decltype(std::declval<V>().begin()), T *>::value &&
                    std::is_convertible<decltype(std::declval<V>().data()), T *>::value &&
                    std::is_convertible<decltype(std::declval<V>().size()), std::size_t>::value &&
                    std::is_lvalue_reference<V>::value,
                int>::type = 0>
  ArrayProxyNoTemporaries(V &&v) VULKAN_HPP_NOEXCEPT : m_count(static_cast<uint32_t>(v.size())),
                                                       m_ptr(v.data()) {}

  // Any l-value reference with a .begin() return type implicitly convertible to T*, and a .size()
  // return type implicitly convertible to size_t.
  template <typename V,
            typename std::enable_if<
                std::is_convertible<decltype(std::declval<V>().begin()), T *>::value &&
...
class Optional {
 public:
  ...
  RefType const *get() const VULKAN_HPP_NOEXCEPT { return m_ptr; }
...
struct IsPartOfStructureChain<Type, Head, Tail...> {
  static const bool valid =
      std::is_same<Type, Head>::value || IsPartOfStructureChain<Type, Tail...>::valid;
};
...
struct StructureChainContains {
  static const bool value =
      std::is_same<T,
                   typename std::tuple_element<Index, std::tuple<ChainElements...>>::type>::value ||
...
struct StructureChainValidation {
  ...
  static const bool valid =
      StructExtends<TestType,
                    typename std::tuple_element<0, std::tuple<ChainElements...>>::type>::value &&
...
class StructureChain : public std::tuple<ChainElements...> {
  // Note: StructureChain has no move constructor or move assignment operator, as it is not supposed
  // to contain movable containers.
  //       In order to get a copy-operation on a move-operations, those functions are neither
  //       deleted nor defaulted.
 public:
  ...
  template <typename T = typename std::tuple_element<0, std::tuple<ChainElements...>>::type,
            size_t Which = 0>
 ...
 private:
  ...
  struct ChainElementIndex<Index, T, Which,
  ...
  struct ChainElementIndex<Index, T, 0,
                           typename std::enable_if<std::is_same<T, First>::value, void>::type,
                           First, Types...> : std::integral_constant<int, Index> {};
  ...
  void link(void *dstBase, void const *srcBase, VkBaseOutStructure *dst,
    ...
    while (src->pNext) {
      ...
      dst->pNext =
          reinterpret_cast<VkBaseOutStructure *>(reinterpret_cast<char *>(dstBase) + offset);
      dst = dst->pNext;
      src = src->pNext;
    }
    dst->pNext = nullptr;
...
// interupt the VULKAN_HPP_NAMESPACE for a moment to add specializations of std::tuple_size and
// std::tuple_element for the StructureChain!
}
...
class UniqueHandle : public UniqueHandleTraits<Type, Dispatch>::deleter {
 ...
 public:
  ...
  operator Type() const VULKAN_HPP_NOEXCEPT { return m_value; }
#endif
...
class DispatchLoaderBase {
 ...
 private:
  ...
  bool m_valid = true;
    ...
    (defined(VULKAN_HPP_DISPATCH_LOADER_DYNAMIC) && (VULKAN_HPP_DISPATCH_LOADER_DYNAMIC == 0))
class DispatchLoaderStatic : public DispatchLoaderBase {
 public:
  ...
  void vkGetPhysicalDeviceQueueFamilyProperties(
    ...
    return ::vkGetPhysicalDeviceQueueFamilyProperties(physicalDevice, pQueueFamilyPropertyCount,
  ...
  VkResult vkEnumerateInstanceLayerProperties(
      uint32_t *pPropertyCount, VkLayerProperties *pProperties) const VULKAN_HPP_NOEXCEPT {
  ...
  void vkGetImageSparseMemoryRequirements(
    ...
    return ::vkGetImageSparseMemoryRequirements(device, image, pSparseMemoryRequirementCount,
  ...
  VkResult vkWaitForFences(VkDevice device, uint32_t fenceCount, const VkFence *pFences,
                           VkBool32 waitAll, uint64_t timeout) const VULKAN_HPP_NOEXCEPT {
  ...
  VkResult vkGetPipelineCacheData(VkDevice device, VkPipelineCache pipelineCache, size_t *pDataSize,
                                  void *pData) const VULKAN_HPP_NOEXCEPT {
    return ::vkGetPipelineCacheData(device, pipelineCache, pDataSize, pData);
  }

  VkResult vkMergePipelineCaches(VkDevice device, VkPipelineCache dstCache, uint32_t srcCacheCount,
  ...
  void vkCmdBindDescriptorSets(VkCommandBuffer commandBuffer, VkPipelineBindPoint pipelineBindPoint,
    ...
    return ::vkCmdBindDescriptorSets(commandBuffer, pipelineBindPoint, layout, firstSet,
  ...
  void vkCmdDraw(VkCommandBuffer commandBuffer, uint32_t vertexCount, uint32_t instanceCount,
                 uint32_t firstVertex, uint32_t firstInstance) const VULKAN_HPP_NOEXCEPT {
  ...
  void vkCmdDrawIndexed(VkCommandBuffer commandBuffer, uint32_t indexCount, uint32_t instanceCount,
    ...
    return ::vkCmdDrawIndexed(commandBuffer, indexCount, instanceCount, firstIndex, vertexOffset,
                              firstInstance);
  }
  ...
  void vkCmdDrawIndexedIndirect(VkCommandBuffer commandBuffer, VkBuffer buffer, VkDeviceSize offset,
                                uint32_t drawCount, uint32_t stride) const VULKAN_HPP_NOEXCEPT {
  ...
  void vkCmdCopyImage(VkCommandBuffer commandBuffer, VkImage srcImage, VkImageLayout srcImageLayout,
    ...
    return ::vkCmdCopyImage(commandBuffer, srcImage, srcImageLayout, dstImage, dstImageLayout,
                            regionCount, pRegions);
  }

  void vkCmdBlitImage(VkCommandBuffer commandBuffer, VkImage srcImage, VkImageLayout srcImageLayout,
                      VkImage dstImage, VkImageLayout dstImageLayout, uint32_t regionCount,
                      const VkImageBlit *pRegions, VkFilter filter) const VULKAN_HPP_NOEXCEPT {
    return ::vkCmdBlitImage(commandBuffer, srcImage, srcImageLayout, dstImage, dstImageLayout,
  ...
  void vkCmdCopyBufferToImage(VkCommandBuffer commandBuffer, VkBuffer srcBuffer, VkImage dstImage,
    ...
    return ::vkCmdCopyBufferToImage(commandBuffer, srcBuffer, dstImage, dstImageLayout, regionCount,
                                    pRegions);
  }
  ...
  void vkCmdCopyImageToBuffer(VkCommandBuffer commandBuffer, VkImage srcImage,
                              ...
                              uint32_t regionCount,
                              const VkBufferImageCopy *pRegions) const VULKAN_HPP_NOEXCEPT {
    return ::vkCmdCopyImageToBuffer(commandBuffer, srcImage, srcImageLayout, dstBuffer, regionCount,
                                    pRegions);
  }

  void vkCmdUpdateBuffer(VkCommandBuffer commandBuffer, VkBuffer dstBuffer, VkDeviceSize dstOffset,
                         VkDeviceSize dataSize, const void *pData) const VULKAN_HPP_NOEXCEPT {
    return ::vkCmdUpdateBuffer(commandBuffer, dstBuffer, dstOffset, dataSize, pData);
  }

  void vkCmdFillBuffer(VkCommandBuffer commandBuffer, VkBuffer dstBuffer, VkDeviceSize dstOffset,
                       VkDeviceSize size, uint32_t data) const VULKAN_HPP_NOEXCEPT {
  ...
  void vkCmdClearDepthStencilImage(
      ...
      const VkImageSubresourceRange *pRanges) const VULKAN_HPP_NOEXCEPT {
    return ::vkCmdClearDepthStencilImage(commandBuffer, image, imageLayout, pDepthStencil,
                                         rangeCount, pRanges);
  }
  ...
  void vkCmdResolveImage(VkCommandBuffer commandBuffer, VkImage srcImage,
    ...
    return ::vkCmdResolveImage(commandBuffer, srcImage, srcImageLayout, dstImage, dstImageLayout,
                               regionCount, pRegions);
  }
  ...
  void vkCmdWaitEvents(VkCommandBuffer commandBuffer, uint32_t eventCount, const VkEvent *pEvents,
    ...
    return ::vkCmdWaitEvents(commandBuffer, eventCount, pEvents, srcStageMask, dstStageMask,
  ...
  void vkCmdPipelineBarrier(
      ...
      uint32_t imageMemoryBarrierCount,
      const VkImageMemoryBarrier *pImageMemoryBarriers) const VULKAN_HPP_NOEXCEPT {
    return ::vkCmdPipelineBarrier(commandBuffer, srcStageMask, dstStageMask, dependencyFlags,
                                  memoryBarrierCount, pMemoryBarriers, bufferMemoryBarrierCount,
  ...
  void vkCmdResetQueryPool(VkCommandBuffer commandBuffer, VkQueryPool queryPool,
                           uint32_t firstQuery, uint32_t queryCount) const VULKAN_HPP_NOEXCEPT {
  ...
  void vkCmdCopyQueryPoolResults(VkCommandBuffer commandBuffer, VkQueryPool queryPool,
                                 ...
                                 VkQueryResultFlags flags) const VULKAN_HPP_NOEXCEPT {
    return ::vkCmdCopyQueryPoolResults(commandBuffer, queryPool, firstQuery, queryCount, dstBuffer,
  ...
  void vkGetDeviceGroupPeerMemoryFeatures(
    ...
    return ::vkGetDeviceGroupPeerMemoryFeatures(device, heapIndex, localDeviceIndex,
  ...
  void vkCmdDispatchBase(VkCommandBuffer commandBuffer, uint32_t baseGroupX, uint32_t baseGroupY,
                         ...
                         uint32_t groupCountZ) const VULKAN_HPP_NOEXCEPT {
    return ::vkCmdDispatchBase(commandBuffer, baseGroupX, baseGroupY, baseGroupZ, groupCountX,
  ...
  void vkGetImageSparseMemoryRequirements2(
    ...
    return ::vkGetImageSparseMemoryRequirements2(device, pInfo, pSparseMemoryRequirementCount,
                                                 pSparseMemoryRequirements);
  }
  ...
  void vkGetPhysicalDeviceQueueFamilyProperties2(
    ...
    return ::vkGetPhysicalDeviceQueueFamilyProperties2(physicalDevice, pQueueFamilyPropertyCount,
                                                       pQueueFamilyProperties);
  }
  ...
  void vkGetPhysicalDeviceSparseImageFormatProperties2(
    ...
    return ::vkGetPhysicalDeviceSparseImageFormatProperties2(physicalDevice, pFormatInfo,
                                                             pPropertyCount, pProperties);
  }

  void vkTrimCommandPool(VkDevice device, VkCommandPool commandPool,
  ...
  void vkUpdateDescriptorSetWithTemplate(VkDevice device, VkDescriptorSet descriptorSet,
    ...
    return ::vkUpdateDescriptorSetWithTemplate(device, descriptorSet, descriptorUpdateTemplate,
                                               pData);
  }
  ...
  void vkGetPhysicalDeviceExternalBufferProperties(
    ...
    return ::vkGetPhysicalDeviceExternalBufferProperties(physicalDevice, pExternalBufferInfo,
  ...
  void vkGetPhysicalDeviceExternalFenceProperties(
    ...
    return ::vkGetPhysicalDeviceExternalFenceProperties(physicalDevice, pExternalFenceInfo,
  ...
  void vkGetPhysicalDeviceExternalSemaphoreProperties(
    ...
    return ::vkGetPhysicalDeviceExternalSemaphoreProperties(physicalDevice, pExternalSemaphoreInfo,
  ...
  void vkCmdDrawIndirectCount(VkCommandBuffer commandBuffer, VkBuffer buffer, VkDeviceSize offset,
    ...
    return ::vkCmdDrawIndirectCount(commandBuffer, buffer, offset, countBuffer, countBufferOffset,
                                    maxDrawCount, stride);
  }
  ...
  void vkCmdDrawIndexedIndirectCount(VkCommandBuffer commandBuffer, VkBuffer buffer,
    ...
    return ::vkCmdDrawIndexedIndirectCount(commandBuffer, buffer, offset, countBuffer,
  ...
  void vkCmdNextSubpass2(VkCommandBuffer commandBuffer, const VkSubpassBeginInfo *pSubpassBeginInfo,
  ...
  VkResult vkGetSemaphoreCounterValue(VkDevice device, VkSemaphore semaphore,
  ...
  void vkCmdWaitEvents2(VkCommandBuffer commandBuffer, uint32_t eventCount, const VkEvent *pEvents,
  ...
  void vkCmdBindVertexBuffers2(VkCommandBuffer commandBuffer, uint32_t firstBinding,
    ...
    return ::vkCmdBindVertexBuffers2(commandBuffer, firstBinding, bindingCount, pBuffers, pOffsets,
                                     pSizes, pStrides);
  }
  ...
  void vkGetDeviceImageSparseMemoryRequirements(
      ...
      uint32_t *pSparseMemoryRequirementCount,
      VkSparseImageMemoryRequirements2 *pSparseMemoryRequirements) const VULKAN_HPP_NOEXCEPT {
    return ::vkGetDeviceImageSparseMemoryRequirements(device, pInfo, pSparseMemoryRequirementCount,
                                                      pSparseMemoryRequirements);
  }
  ...
  VkResult vkMapMemory2(VkDevice device, const VkMemoryMapInfo *pMemoryMapInfo,
                        void **ppData) const VULKAN_HPP_NOEXCEPT {
    return ::vkMapMemory2(device, pMemoryMapInfo, ppData);
  }

  VkResult vkUnmapMemory2(VkDevice device,
                          const VkMemoryUnmapInfo *pMemoryUnmapInfo) const VULKAN_HPP_NOEXCEPT {
  ...
  void vkCmdBindIndexBuffer2(VkCommandBuffer commandBuffer, VkBuffer buffer, VkDeviceSize offset,
                             VkDeviceSize size, VkIndexType indexType) const VULKAN_HPP_NOEXCEPT {
  ...
  void vkCmdPushDescriptorSet(VkCommandBuffer commandBuffer, VkPipelineBindPoint pipelineBindPoint,
                              VkPipelineLayout layout, uint32_t set, uint32_t descriptorWriteCount,
                              const VkWriteDescriptorSet *pDescriptorWrites) const
      VULKAN_HPP_NOEXCEPT {
    return ::vkCmdPushDescriptorSet(commandBuffer, pipelineBindPoint, layout, set,
  ...
  void vkCmdPushDescriptorSetWithTemplate(VkCommandBuffer commandBuffer,
                                          ...
                                          const void *pData) const VULKAN_HPP_NOEXCEPT {
    return ::vkCmdPushDescriptorSetWithTemplate(commandBuffer, descriptorUpdateTemplate, layout,
                                                set, pData);
  }
  ...
  void vkCmdSetRenderingInputAttachmentIndices(
      VkCommandBuffer commandBuffer,
      const VkRenderingInputAttachmentIndexInfo *pInputAttachmentIndexInfo) const
      VULKAN_HPP_NOEXCEPT {
  ...
  void vkCmdPushConstants2(VkCommandBuffer commandBuffer,
                           const VkPushConstantsInfo *pPushConstantsInfo) const
      VULKAN_HPP_NOEXCEPT {
  ...
  VkResult vkQueuePresentKHR(VkQueue queue,
                             const VkPresentInfoKHR *pPresentInfo) const VULKAN_HPP_NOEXCEPT {
  ...
  VkResult vkGetPhysicalDeviceDisplayPlanePropertiesKHR(
    ...
    return ::vkGetPhysicalDeviceDisplayPlanePropertiesKHR(physicalDevice, pPropertyCount,
                                                          pProperties);
  }
  ...
  VkResult vkGetDisplayPlaneSupportedDisplaysKHR(
    ...
    return ::vkGetDisplayPlaneSupportedDisplaysKHR(physicalDevice, planeIndex, pDisplayCount,
                                                   pDisplays);
  }
  ...
  VkResult vkCreateSharedSwapchainsKHR(VkDevice device, uint32_t swapchainCount,
    ...
    return ::vkCreateSharedSwapchainsKHR(device, swapchainCount, pCreateInfos, pAllocator,
                                         pSwapchains);
  }
  ...
  VkBool32 vkGetPhysicalDeviceXlibPresentationSupportKHR(
    ...
    return ::vkGetPhysicalDeviceXlibPresentationSupportKHR(physicalDevice, queueFamilyIndex, dpy,
                                                           visualID);
  }
  ...
  VkBool32 vkGetPhysicalDeviceXcbPresentationSupportKHR(
    ...
    return ::vkGetPhysicalDeviceXcbPresentationSupportKHR(physicalDevice, queueFamilyIndex,
  ...
  VkBool32 vkGetPhysicalDeviceWaylandPresentationSupportKHR(
    ...
    return ::vkGetPhysicalDeviceWaylandPresentationSupportKHR(physicalDevice, queueFamilyIndex,
                                                              display);
  }
  ...
  VkBool32 vkGetPhysicalDeviceWin32PresentationSupportKHR(
      VkPhysicalDevice physicalDevice, uint32_t queueFamilyIndex) const VULKAN_HPP_NOEXCEPT {
  ...
  void vkDebugReportMessageEXT(VkInstance instance, VkDebugReportFlagsEXT flags,
    ...
    return ::vkDebugReportMessageEXT(instance, flags, objectType, object, location, messageCode,
  ...
  VkResult vkDebugMarkerSetObjectTagEXT(
      VkDevice device, const VkDebugMarkerObjectTagInfoEXT *pTagInfo) const VULKAN_HPP_NOEXCEPT {
    return ::vkDebugMarkerSetObjectTagEXT(device, pTagInfo);
  }

  VkResult vkDebugMarkerSetObjectNameEXT(
      VkDevice device, const VkDebugMarkerObjectNameInfoEXT *pNameInfo) const VULKAN_HPP_NOEXCEPT {
  ...
  VkResult vkCreateVideoSessionKHR(VkDevice device, const VkVideoSessionCreateInfoKHR *pCreateInfo,
                                   const VkAllocationCallbacks *pAllocator,
  ...
  VkResult vkBindVideoSessionMemoryKHR(
    ...
    return ::vkBindVideoSessionMemoryKHR(device, videoSession, bindSessionMemoryInfoCount,
  ...
  void vkCmdDecodeVideoKHR(VkCommandBuffer commandBuffer,
                           const VkVideoDecodeInfoKHR *pDecodeInfo) const VULKAN_HPP_NOEXCEPT {
  ...
  void vkCmdBindTransformFeedbackBuffersEXT(VkCommandBuffer commandBuffer, uint32_t firstBinding,
    ...
    return ::vkCmdBindTransformFeedbackBuffersEXT(commandBuffer, firstBinding, bindingCount,
  ...
  void vkCmdBeginTransformFeedbackEXT(VkCommandBuffer commandBuffer, uint32_t firstCounterBuffer,
      ...
      VULKAN_HPP_NOEXCEPT {
    return ::vkCmdBeginTransformFeedbackEXT(commandBuffer, firstCounterBuffer, counterBufferCount,
  ...
  void vkCmdEndTransformFeedbackEXT(VkCommandBuffer commandBuffer, uint32_t firstCounterBuffer,
                                    ...
                                    const VkDeviceSize *pCounterBufferOffsets) const
      VULKAN_HPP_NOEXCEPT {
    return ::vkCmdEndTransformFeedbackEXT(commandBuffer, firstCounterBuffer, counterBufferCount,
                                          pCounterBuffers, pCounterBufferOffsets);
  }
  ...
  void vkCmdEndQueryIndexedEXT(VkCommandBuffer commandBuffer, VkQueryPool queryPool, uint32_t query,
                               uint32_t index) const VULKAN_HPP_NOEXCEPT {
  ...
  void vkCmdDrawIndirectByteCountEXT(VkCommandBuffer commandBuffer, uint32_t instanceCount,
    ...
    return ::vkCmdDrawIndirectByteCountEXT(commandBuffer, instanceCount, firstInstance,
  ...
  uint64_t vkGetImageViewHandle64NVX(VkDevice device, const VkImageViewHandleInfoNVX *pInfo) const
      VULKAN_HPP_NOEXCEPT {
  ...
  void vkCmdDrawIndirectCountAMD(VkCommandBuffer commandBuffer, VkBuffer buffer,
                                 ...
                                 VkDeviceSize countBufferOffset, uint32_t maxDrawCount,
                                 uint32_t stride) const VULKAN_HPP_NOEXCEPT {
    return ::vkCmdDrawIndirectCountAMD(commandBuffer, buffer, offset, countBuffer,
                                       countBufferOffset, maxDrawCount, stride);
  }
  ...
  void vkCmdDrawIndexedIndirectCountAMD(VkCommandBuffer commandBuffer, VkBuffer buffer,
                                        ...
                                        VkDeviceSize countBufferOffset, uint32_t maxDrawCount,
                                        uint32_t stride) const VULKAN_HPP_NOEXCEPT {
    return ::vkCmdDrawIndexedIndirectCountAMD(commandBuffer, buffer, offset, countBuffer,
                                              countBufferOffset, maxDrawCount, stride);
  }
  ...
  VkResult vkGetShaderInfoAMD(VkDevice device, VkPipeline pipeline,
                              ...
                              size_t *pInfoSize, void *pInfo) const VULKAN_HPP_NOEXCEPT {
  ...
  VkResult vkGetPhysicalDeviceImageFormatProperties2KHR(
      VkPhysicalDevice physicalDevice, const VkPhysicalDeviceImageFormatInfo2 *pImageFormatInfo,
      VkImageFormatProperties2 *pImageFormatProperties) const VULKAN_HPP_NOEXCEPT {
    return ::vkGetPhysicalDeviceImageFormatProperties2KHR(physicalDevice, pImageFormatInfo,
                                                          pImageFormatProperties);
  }
  ...
  void vkGetPhysicalDeviceQueueFamilyProperties2KHR(
      VkPhysicalDevice physicalDevice, uint32_t *pQueueFamilyPropertyCount,
      VkQueueFamilyProperties2 *pQueueFamilyProperties) const VULKAN_HPP_NOEXCEPT {
    return ::vkGetPhysicalDeviceQueueFamilyProperties2KHR(physicalDevice, pQueueFamilyPropertyCount,
                                                          pQueueFamilyProperties);
  }
  ...
  void vkGetPhysicalDeviceSparseImageFormatProperties2KHR(
      ...
      uint32_t *pPropertyCount,
      VkSparseImageFormatProperties2 *pProperties) const VULKAN_HPP_NOEXCEPT {
    return ::vkGetPhysicalDeviceSparseImageFormatProperties2KHR(physicalDevice, pFormatInfo,
                                                                pPropertyCount, pProperties);
  }
  ...
  void vkGetDeviceGroupPeerMemoryFeaturesKHR(
      VkDevice device, uint32_t heapIndex, uint32_t localDeviceIndex, uint32_t remoteDeviceIndex,
      VkPeerMemoryFeatureFlags *pPeerMemoryFeatures) const VULKAN_HPP_NOEXCEPT {
    return ::vkGetDeviceGroupPeerMemoryFeaturesKHR(device, heapIndex, localDeviceIndex,
                                                   remoteDeviceIndex, pPeerMemoryFeatures);
  }
  ...
  void vkCmdDispatchBaseKHR(VkCommandBuffer commandBuffer, uint32_t baseGroupX, uint32_t baseGroupY,
                            uint32_t baseGroupZ, uint32_t groupCountX, uint32_t groupCountY,
                            uint32_t groupCountZ) const VULKAN_HPP_NOEXCEPT {
    return ::vkCmdDispatchBaseKHR(commandBuffer, baseGroupX, baseGroupY, baseGroupZ, groupCountX,
                                  groupCountY, groupCountZ);
  }
  ...
  VkResult vkEnumeratePhysicalDeviceGroupsKHR(
      VkInstance instance, uint32_t *pPhysicalDeviceGroupCount,
      VkPhysicalDeviceGroupProperties *pPhysicalDeviceGroupProperties) const VULKAN_HPP_NOEXCEPT {
    return ::vkEnumeratePhysicalDeviceGroupsKHR(instance, pPhysicalDeviceGroupCount,
                                                pPhysicalDeviceGroupProperties);
  }
  ...
  void vkGetPhysicalDeviceExternalBufferPropertiesKHR(
      ...
      const VkPhysicalDeviceExternalBufferInfo *pExternalBufferInfo,
      VkExternalBufferProperties *pExternalBufferProperties) const VULKAN_HPP_NOEXCEPT {
    return ::vkGetPhysicalDeviceExternalBufferPropertiesKHR(physicalDevice, pExternalBufferInfo,
                                                            pExternalBufferProperties);
  }
  ...
  VkResult vkGetMemoryFdKHR(VkDevice device, const VkMemoryGetFdInfoKHR *pGetFdInfo,
  ...
  void vkCmdPushDescriptorSetKHR(
    ...
    return ::vkCmdPushDescriptorSetKHR(commandBuffer, pipelineBindPoint, layout, set,
                                       descriptorWriteCount, pDescriptorWrites);
  }
  ...
  void vkCmdPushDescriptorSetWithTemplateKHR(VkCommandBuffer commandBuffer,
                                             ...
                                             VkPipelineLayout layout, uint32_t set,
                                             const void *pData) const VULKAN_HPP_NOEXCEPT {
    return ::vkCmdPushDescriptorSetWithTemplateKHR(commandBuffer, descriptorUpdateTemplate, layout,
                                                   set, pData);
  }
  ...
  void vkCmdBeginConditionalRenderingEXT(VkCommandBuffer commandBuffer,
                                         const VkConditionalRenderingBeginInfoEXT *
                                             pConditionalRenderingBegin) const VULKAN_HPP_NOEXCEPT {
  ...
  VkResult vkCreateDescriptorUpdateTemplateKHR(
      ...
      const VkAllocationCallbacks *pAllocator,
      VkDescriptorUpdateTemplate *pDescriptorUpdateTemplate) const VULKAN_HPP_NOEXCEPT {
    return ::vkCreateDescriptorUpdateTemplateKHR(device, pCreateInfo, pAllocator,
                                                 pDescriptorUpdateTemplate);
  }
  ...
  void vkUpdateDescriptorSetWithTemplateKHR(VkDevice device, VkDescriptorSet descriptorSet,
                                            VkDescriptorUpdateTemplate descriptorUpdateTemplate,
                                            const void *pData) const VULKAN_HPP_NOEXCEPT {
    return ::vkUpdateDescriptorSetWithTemplateKHR(device, descriptorSet, descriptorUpdateTemplate,
                                                  pData);
  }
  ...
  void vkCmdSetViewportWScalingNV(
    ...
    return ::vkCmdSetViewportWScalingNV(commandBuffer, firstViewport, viewportCount,
                                        pViewportWScalings);
  }
  ...
  VkResult vkGetPastPresentationTimingGOOGLE(
    ...
    return ::vkGetPastPresentationTimingGOOGLE(device, swapchain, pPresentationTimingCount,
  ...
  void vkCmdSetDiscardRectangleEnableEXT(
      VkCommandBuffer commandBuffer, VkBool32 discardRectangleEnable) const VULKAN_HPP_NOEXCEPT {
  ...
  void vkGetPhysicalDeviceExternalFencePropertiesKHR(
      VkPhysicalDevice physicalDevice, const VkPhysicalDeviceExternalFenceInfo *pExternalFenceInfo,
      VkExternalFenceProperties *pExternalFenceProperties) const VULKAN_HPP_NOEXCEPT {
    return ::vkGetPhysicalDeviceExternalFencePropertiesKHR(physicalDevice, pExternalFenceInfo,
                                                           pExternalFenceProperties);
  }
  ...
  VkResult vkImportFenceWin32HandleKHR(VkDevice device,
                                       const VkImportFenceWin32HandleInfoKHR
                                           *pImportFenceWin32HandleInfo) const VULKAN_HPP_NOEXCEPT {
  ...
  VkResult vkImportFenceFdKHR(
      VkDevice device, const VkImportFenceFdInfoKHR *pImportFenceFdInfo) const VULKAN_HPP_NOEXCEPT {
    return ::vkImportFenceFdKHR(device, pImportFenceFdInfo);
  }

  VkResult vkGetFenceFdKHR(VkDevice device, const VkFenceGetFdInfoKHR *pGetFdInfo,
                           int *pFd) const VULKAN_HPP_NOEXCEPT {
  ...
  VkResult vkAcquireProfilingLockKHR(
      VkDevice device, const VkAcquireProfilingLockInfoKHR *pInfo) const VULKAN_HPP_NOEXCEPT {
  ...
  VkResult vkGetPhysicalDeviceSurfaceCapabilities2KHR(
    ...
    return ::vkGetPhysicalDeviceSurfaceCapabilities2KHR(physicalDevice, pSurfaceInfo,
                                                        pSurfaceCapabilities);
  }
  ...
  VkResult vkGetPhysicalDeviceSurfaceFormats2KHR(
    ...
    return ::vkGetPhysicalDeviceSurfaceFormats2KHR(physicalDevice, pSurfaceInfo,
  ...
  VkResult vkGetPhysicalDeviceDisplayPlaneProperties2KHR(
    ...
    return ::vkGetPhysicalDeviceDisplayPlaneProperties2KHR(physicalDevice, pPropertyCount,
                                                           pProperties);
  }
  ...
  VkResult vkCreateIOSSurfaceMVK(VkInstance instance, const VkIOSSurfaceCreateInfoMVK *pCreateInfo,
                                 const VkAllocationCallbacks *pAllocator,
                                 VkSurfaceKHR *pSurface) const VULKAN_HPP_NOEXCEPT {
  ...
  VkResult vkSetDebugUtilsObjectNameEXT(
      VkDevice device, const VkDebugUtilsObjectNameInfoEXT *pNameInfo) const VULKAN_HPP_NOEXCEPT {
    return ::vkSetDebugUtilsObjectNameEXT(device, pNameInfo);
  }

  VkResult vkSetDebugUtilsObjectTagEXT(
      VkDevice device, const VkDebugUtilsObjectTagInfoEXT *pTagInfo) const VULKAN_HPP_NOEXCEPT {
  ...
  VkResult vkCreateExecutionGraphPipelinesAMDX(
    ...
    return ::vkCreateExecutionGraphPipelinesAMDX(device, pipelineCache, createInfoCount,
  ...
  VkResult vkGetExecutionGraphPipelineNodeIndexAMDX(
    ...
    return ::vkGetExecutionGraphPipelineNodeIndexAMDX(device, executionGraph, pNodeInfo,
                                                      pNodeIndex);
  }
  ...
  void vkCmdInitializeGraphScratchMemoryAMDX(VkCommandBuffer commandBuffer,
    ...
    return ::vkCmdInitializeGraphScratchMemoryAMDX(commandBuffer, executionGraph, scratch,
                                                   scratchSize);
  }
  ...
  void vkCmdDispatchGraphIndirectCountAMDX(VkCommandBuffer commandBuffer, VkDeviceAddress scratch,
  ...
  void vkGetPhysicalDeviceMultisamplePropertiesEXT(
    ...
    return ::vkGetPhysicalDeviceMultisamplePropertiesEXT(physicalDevice, samples,
  ...
  void vkGetImageSparseMemoryRequirements2KHR(
      ...
      uint32_t *pSparseMemoryRequirementCount,
      VkSparseImageMemoryRequirements2 *pSparseMemoryRequirements) const VULKAN_HPP_NOEXCEPT {
    return ::vkGetImageSparseMemoryRequirements2KHR(device, pInfo, pSparseMemoryRequirementCount,
                                                    pSparseMemoryRequirements);
  }
  ...
  void vkCmdBuildAccelerationStructuresKHR(
      ...
      VULKAN_HPP_NOEXCEPT {
    return ::vkCmdBuildAccelerationStructuresKHR(commandBuffer, infoCount, pInfos,
                                                 ppBuildRangeInfos);
  }
  ...
  void vkCmdBuildAccelerationStructuresIndirectKHR(
    ...
    return ::vkCmdBuildAccelerationStructuresIndirectKHR(commandBuffer, infoCount, pInfos,
  ...
  VkResult vkBuildAccelerationStructuresKHR(
      ...
      const VkAccelerationStructureBuildRangeInfoKHR *const *ppBuildRangeInfos) const
      VULKAN_HPP_NOEXCEPT {
    return ::vkBuildAccelerationStructuresKHR(device, deferredOperation, infoCount, pInfos,
                                              ppBuildRangeInfos);
  }
  ...
  VkResult vkWriteAccelerationStructuresPropertiesKHR(
      ...
      size_t dataSize, void *pData, size_t stride) const VULKAN_HPP_NOEXCEPT {
    return ::vkWriteAccelerationStructuresPropertiesKHR(device, accelerationStructureCount,
  ...
  void vkCmdWriteAccelerationStructuresPropertiesKHR(
    ...
    return ::vkCmdWriteAccelerationStructuresPropertiesKHR(
        commandBuffer, accelerationStructureCount, pAccelerationStructures, queryType, queryPool,
        firstQuery);
  }
  ...
  void vkGetAccelerationStructureBuildSizesKHR(
    ...
    return ::vkGetAccelerationStructureBuildSizesKHR(device, buildType, pBuildInfo,
  ...
  void vkCmdTraceRaysKHR(VkCommandBuffer commandBuffer,
    ...
    return ::vkCmdTraceRaysKHR(commandBuffer, pRaygenShaderBindingTable, pMissShaderBindingTable,
  ...
  VkResult vkCreateRayTracingPipelinesKHR(VkDevice device, VkDeferredOperationKHR deferredOperation,
                                          ...
                                          const VkAllocationCallbacks *pAllocator,
                                          VkPipeline *pPipelines) const VULKAN_HPP_NOEXCEPT {
    return ::vkCreateRayTracingPipelinesKHR(device, deferredOperation, pipelineCache,
  ...
  VkResult vkGetRayTracingShaderGroupHandlesKHR(VkDevice device, VkPipeline pipeline,
                                                ...
                                                size_t dataSize,
                                                void *pData) const VULKAN_HPP_NOEXCEPT {
    return ::vkGetRayTracingShaderGroupHandlesKHR(device, pipeline, firstGroup, groupCount,
                                                  dataSize, pData);
  }
  ...
  VkResult vkGetRayTracingCaptureReplayShaderGroupHandlesKHR(
    ...
    return ::vkGetRayTracingCaptureReplayShaderGroupHandlesKHR(device, pipeline, firstGroup,
  ...
  void vkCmdTraceRaysIndirectKHR(VkCommandBuffer commandBuffer,
    ...
    return ::vkCmdTraceRaysIndirectKHR(commandBuffer, pRaygenShaderBindingTable,
  ...
  void vkCmdSetRayTracingPipelineStackSizeKHR(
      VkCommandBuffer commandBuffer, uint32_t pipelineStackSize) const VULKAN_HPP_NOEXCEPT {
  ...
  void vkDestroySamplerYcbcrConversionKHR(VkDevice device, VkSamplerYcbcrConversion ycbcrConversion,
                                          const VkAllocationCallbacks *pAllocator) const
      VULKAN_HPP_NOEXCEPT {
  ...
  void vkCmdSetViewportShadingRatePaletteNV(
    ...
    return ::vkCmdSetViewportShadingRatePaletteNV(commandBuffer, firstViewport, viewportCount,
  ...
  void vkCmdBuildAccelerationStructureNV(VkCommandBuffer commandBuffer,
    ...
    return ::vkCmdBuildAccelerationStructureNV(commandBuffer, pInfo, instanceData, instanceOffset,
  ...
  void vkCmdTraceRaysNV(VkCommandBuffer commandBuffer, VkBuffer raygenShaderBindingTableBuffer,
  ...
  VkResult vkCreateRayTracingPipelinesNV(VkDevice device, VkPipelineCache pipelineCache,
                                         ...
                                         const VkAllocationCallbacks *pAllocator,
                                         VkPipeline *pPipelines) const VULKAN_HPP_NOEXCEPT {
    return ::vkCreateRayTracingPipelinesNV(device, pipelineCache, createInfoCount, pCreateInfos,
                                           pAllocator, pPipelines);
  }
  ...
  VkResult vkGetRayTracingShaderGroupHandlesNV(VkDevice device, VkPipeline pipeline,
                                               ...
                                               size_t dataSize,
                                               void *pData) const VULKAN_HPP_NOEXCEPT {
    return ::vkGetRayTracingShaderGroupHandlesNV(device, pipeline, firstGroup, groupCount, dataSize,
                                                 pData);
  }
  ...
  void vkCmdWriteAccelerationStructuresPropertiesNV(
      ...
      VkQueryPool queryPool, uint32_t firstQuery) const VULKAN_HPP_NOEXCEPT {
    return ::vkCmdWriteAccelerationStructuresPropertiesNV(commandBuffer, accelerationStructureCount,
                                                          pAccelerationStructures, queryType,
  ...
  void vkCmdDrawIndirectCountKHR(VkCommandBuffer commandBuffer, VkBuffer buffer,
                                 ...
                                 VkDeviceSize countBufferOffset, uint32_t maxDrawCount,
                                 uint32_t stride) const VULKAN_HPP_NOEXCEPT {
    return ::vkCmdDrawIndirectCountKHR(commandBuffer, buffer, offset, countBuffer,
                                       countBufferOffset, maxDrawCount, stride);
  }
  ...
  void vkCmdDrawIndexedIndirectCountKHR(VkCommandBuffer commandBuffer, VkBuffer buffer,
                                        ...
                                        VkDeviceSize countBufferOffset, uint32_t maxDrawCount,
                                        uint32_t stride) const VULKAN_HPP_NOEXCEPT {
    return ::vkCmdDrawIndexedIndirectCountKHR(commandBuffer, buffer, offset, countBuffer,
                                              countBufferOffset, maxDrawCount, stride);
  }
  ...
  VkResult vkGetMemoryHostPointerPropertiesEXT(
    ...
    return ::vkGetMemoryHostPointerPropertiesEXT(device, handleType, pHostPointer,
  ...
  VkResult vkGetPhysicalDeviceCalibrateableTimeDomainsEXT(
    ...
    return ::vkGetPhysicalDeviceCalibrateableTimeDomainsEXT(physicalDevice, pTimeDomainCount,
                                                            pTimeDomains);
  }
  ...
  VkResult vkGetCalibratedTimestampsEXT(VkDevice device, uint32_t timestampCount,
    ...
    return ::vkGetCalibratedTimestampsEXT(device, timestampCount, pTimestampInfos, pTimestamps,
                                          pMaxDeviation);
  }
  ...
  void vkCmdDrawMeshTasksIndirectCountNV(VkCommandBuffer commandBuffer, VkBuffer buffer,
                                         ...
                                         VkDeviceSize countBufferOffset, uint32_t maxDrawCount,
                                         uint32_t stride) const VULKAN_HPP_NOEXCEPT {
    return ::vkCmdDrawMeshTasksIndirectCountNV(commandBuffer, buffer, offset, countBuffer,
                                               countBufferOffset, maxDrawCount, stride);
  }
  ...
  void vkCmdSetExclusiveScissorEnableNV(
    ...
    return ::vkCmdSetExclusiveScissorEnableNV(commandBuffer, firstExclusiveScissor,
  ...
  void vkCmdSetExclusiveScissorNV(VkCommandBuffer commandBuffer, uint32_t firstExclusiveScissor,
    ...
    return ::vkCmdSetExclusiveScissorNV(commandBuffer, firstExclusiveScissor, exclusiveScissorCount,
                                        pExclusiveScissors);
  }
  ...
  void vkCmdSetCheckpointNV(VkCommandBuffer commandBuffer,
                            const void *pCheckpointMarker) const VULKAN_HPP_NOEXCEPT {
  ...
  VkResult vkGetSemaphoreCounterValueKHR(VkDevice device, VkSemaphore semaphore,
                                         uint64_t *pValue) const VULKAN_HPP_NOEXCEPT {
  ...
  VkResult vkSignalSemaphoreKHR(VkDevice device, const VkSemaphoreSignalInfo *pSignalInfo) const
      VULKAN_HPP_NOEXCEPT {
  ...
  VkResult vkReleasePerformanceConfigurationINTEL(
      VkDevice device, VkPerformanceConfigurationINTEL configuration) const VULKAN_HPP_NOEXCEPT {
    return ::vkReleasePerformanceConfigurationINTEL(device, configuration);
  }

  VkResult vkQueueSetPerformanceConfigurationINTEL(
      VkQueue queue, VkPerformanceConfigurationINTEL configuration) const VULKAN_HPP_NOEXCEPT {
  ...
  VkResult vkGetPhysicalDeviceFragmentShadingRatesKHR(
    ...
    return ::vkGetPhysicalDeviceFragmentShadingRatesKHR(physicalDevice, pFragmentShadingRateCount,
  ...
  VkResult vkGetPhysicalDeviceCooperativeMatrixPropertiesNV(
    ...
    return ::vkGetPhysicalDeviceCooperativeMatrixPropertiesNV(physicalDevice, pPropertyCount,
                                                              pProperties);
  }
  ...
  VkResult vkGetPhysicalDeviceSurfacePresentModes2EXT(
    ...
    return ::vkGetPhysicalDeviceSurfacePresentModes2EXT(physicalDevice, pSurfaceInfo,
                                                        pPresentModeCount, pPresentModes);
  }

  VkResult vkAcquireFullScreenExclusiveModeEXT(VkDevice device,
                                               VkSwapchainKHR swapchain) const VULKAN_HPP_NOEXCEPT {
    return ::vkAcquireFullScreenExclusiveModeEXT(device, swapchain);
  }

  VkResult vkReleaseFullScreenExclusiveModeEXT(VkDevice device,
                                               VkSwapchainKHR swapchain) const VULKAN_HPP_NOEXCEPT {
  ...
  void vkCmdBindVertexBuffers2EXT(VkCommandBuffer commandBuffer, uint32_t firstBinding,
                                  ...
                                  const VkDeviceSize *pOffsets, const VkDeviceSize *pSizes,
                                  const VkDeviceSize *pStrides) const VULKAN_HPP_NOEXCEPT {
    return ::vkCmdBindVertexBuffers2EXT(commandBuffer, firstBinding, bindingCount, pBuffers,
  ...
  uint32_t vkGetDeferredOperationMaxConcurrencyKHR(
      VkDevice device, VkDeferredOperationKHR operation) const VULKAN_HPP_NOEXCEPT {
    return ::vkGetDeferredOperationMaxConcurrencyKHR(device, operation);
  }

  VkResult vkGetDeferredOperationResultKHR(VkDevice device, VkDeferredOperationKHR operation) const
      VULKAN_HPP_NOEXCEPT {
  ...
  VkResult vkGetPipelineExecutablePropertiesKHR(
    ...
    return ::vkGetPipelineExecutablePropertiesKHR(device, pPipelineInfo, pExecutableCount,
                                                  pProperties);
  }
  ...
  VkResult vkGetPipelineExecutableStatisticsKHR(
    ...
    return ::vkGetPipelineExecutableStatisticsKHR(device, pExecutableInfo, pStatisticCount,
                                                  pStatistics);
  }
  ...
  VkResult vkMapMemory2KHR(VkDevice device, const VkMemoryMapInfo *pMemoryMapInfo,
                           void **ppData) const VULKAN_HPP_NOEXCEPT {
  ...
  void vkCmdSetDepthBias2EXT(VkCommandBuffer commandBuffer,
                             const VkDepthBiasInfoEXT *pDepthBiasInfo) const VULKAN_HPP_NOEXCEPT {
  ...
  VkResult vkGetEncodedVideoSessionParametersKHR(
      ...
      void *pData) const VULKAN_HPP_NOEXCEPT {
    return ::vkGetEncodedVideoSessionParametersKHR(device, pVideoSessionParametersInfo,
  ...
  VkResult vkCreateCudaModuleNV(VkDevice device, const VkCudaModuleCreateInfoNV *pCreateInfo,
                                const VkAllocationCallbacks *pAllocator,
                                VkCudaModuleNV *pModule) const VULKAN_HPP_NOEXCEPT {
    return ::vkCreateCudaModuleNV(device, pCreateInfo, pAllocator, pModule);
  }

  VkResult vkGetCudaModuleCacheNV(VkDevice device, VkCudaModuleNV module, size_t *pCacheSize,
                                  void *pCacheData) const VULKAN_HPP_NOEXCEPT {
    return ::vkGetCudaModuleCacheNV(device, module, pCacheSize, pCacheData);
  }

  VkResult vkCreateCudaFunctionNV(VkDevice device, const VkCudaFunctionCreateInfoNV *pCreateInfo,
                                  const VkAllocationCallbacks *pAllocator,
  ...
  void vkCmdCudaLaunchKernelNV(VkCommandBuffer commandBuffer,
                               const VkCudaLaunchInfoNV *pLaunchInfo) const VULKAN_HPP_NOEXCEPT {
  ...
  void vkCmdDispatchTileQCOM(VkCommandBuffer commandBuffer,
                             const VkDispatchTileInfoQCOM *pDispatchTileInfo) const
      VULKAN_HPP_NOEXCEPT {
    return ::vkCmdDispatchTileQCOM(commandBuffer, pDispatchTileInfo);
  }

  void vkCmdBeginPerTileExecutionQCOM(VkCommandBuffer commandBuffer,
  ...
  void vkCmdEndPerTileExecutionQCOM(VkCommandBuffer commandBuffer,
  ...
  void vkExportMetalObjectsEXT(
      VkDevice device, VkExportMetalObjectsInfoEXT *pMetalObjectsInfo) const VULKAN_HPP_NOEXCEPT {
  ...
  void vkGetDescriptorEXT(VkDevice device, const VkDescriptorGetInfoEXT *pDescriptorInfo,
                          size_t dataSize, void *pDescriptor) const VULKAN_HPP_NOEXCEPT {
  ...
  void vkCmdSetDescriptorBufferOffsetsEXT(VkCommandBuffer commandBuffer,
                                          ...
                                          const VkDeviceSize *pOffsets) const VULKAN_HPP_NOEXCEPT {
    return ::vkCmdSetDescriptorBufferOffsetsEXT(commandBuffer, pipelineBindPoint, layout, firstSet,
  ...
  void vkCmdBindDescriptorBufferEmbeddedSamplersEXT(VkCommandBuffer commandBuffer,
    ...
    return ::vkCmdBindDescriptorBufferEmbeddedSamplersEXT(commandBuffer, pipelineBindPoint, layout,
                                                          set);
  }
  ...
  void vkCmdDrawMeshTasksIndirectCountEXT(VkCommandBuffer commandBuffer, VkBuffer buffer,
                                          ...
                                          VkDeviceSize countBufferOffset, uint32_t maxDrawCount,
                                          uint32_t stride) const VULKAN_HPP_NOEXCEPT {
    return ::vkCmdDrawMeshTasksIndirectCountEXT(commandBuffer, buffer, offset, countBuffer,
                                                countBufferOffset, maxDrawCount, stride);
  }
  ...
  VkResult vkAcquireWinrtDisplayNV(VkPhysicalDevice physicalDevice,
                                   VkDisplayKHR display) const VULKAN_HPP_NOEXCEPT {
  ...
  VkBool32 vkGetPhysicalDeviceDirectFBPresentationSupportEXT(
    ...
    return ::vkGetPhysicalDeviceDirectFBPresentationSupportEXT(physicalDevice, queueFamilyIndex,
                                                               dfb);
  }
  ...
  void vkCmdSetVertexInputEXT(VkCommandBuffer commandBuffer, uint32_t vertexBindingDescriptionCount,
                              const VkVertexInputBindingDescription2EXT *pVertexBindingDescriptions,
                              uint32_t vertexAttributeDescriptionCount,
                              const VkVertexInputAttributeDescription2EXT
                                  *pVertexAttributeDescriptions) const VULKAN_HPP_NOEXCEPT {
    return ::vkCmdSetVertexInputEXT(commandBuffer, vertexBindingDescriptionCount,
  ...
  VkResult vkGetMemoryZirconHandlePropertiesFUCHSIA(
      ...
      VULKAN_HPP_NOEXCEPT {
    return ::vkGetMemoryZirconHandlePropertiesFUCHSIA(device, handleType, zirconHandle,
  ...
  VkResult vkSetBufferCollectionImageConstraintsFUCHSIA(
    ...
    return ::vkSetBufferCollectionImageConstraintsFUCHSIA(device, collection,
  ...
  VkResult vkSetBufferCollectionBufferConstraintsFUCHSIA(
    ...
    return ::vkSetBufferCollectionBufferConstraintsFUCHSIA(device, collection,
  ...
  VkResult vkGetDeviceSubpassShadingMaxWorkgroupSizeHUAWEI(VkDevice device, VkRenderPass renderpass,
  ...
  void vkCmdSetRasterizerDiscardEnableEXT(
      VkCommandBuffer commandBuffer, VkBool32 rasterizerDiscardEnable) const VULKAN_HPP_NOEXCEPT {
  ...
  void vkCmdSetPrimitiveRestartEnableEXT(
      VkCommandBuffer commandBuffer, VkBool32 primitiveRestartEnable) const VULKAN_HPP_NOEXCEPT {
  ...
  VkBool32 vkGetPhysicalDeviceScreenPresentationSupportQNX(
    ...
    return ::vkGetPhysicalDeviceScreenPresentationSupportQNX(physicalDevice, queueFamilyIndex,
                                                             window);
  }
  ...
  void vkCmdDrawMultiEXT(VkCommandBuffer commandBuffer, uint32_t drawCount,
                         ...
                         uint32_t firstInstance, uint32_t stride) const VULKAN_HPP_NOEXCEPT {
    return ::vkCmdDrawMultiEXT(commandBuffer, drawCount, pVertexInfo, instanceCount, firstInstance,
                               stride);
  }
  ...
  void vkCmdDrawMultiIndexedEXT(VkCommandBuffer commandBuffer, uint32_t drawCount,
    ...
    return ::vkCmdDrawMultiIndexedEXT(commandBuffer, drawCount, pIndexInfo, instanceCount,
  ...
  void vkCmdWriteMicromapsPropertiesEXT(VkCommandBuffer commandBuffer, uint32_t micromapCount,
    ...
    return ::vkCmdWriteMicromapsPropertiesEXT(commandBuffer, micromapCount, pMicromaps, queryType,
                                              queryPool, firstQuery);
  }
  ...
  void vkCmdCopyMemoryIndirectNV(VkCommandBuffer commandBuffer, VkDeviceAddress copyBufferAddress,
                                 uint32_t copyCount, uint32_t stride) const VULKAN_HPP_NOEXCEPT {
  ...
  void vkCmdCopyMemoryToImageIndirectNV(
    ...
    return ::vkCmdCopyMemoryToImageIndirectNV(commandBuffer, copyBufferAddress, copyCount, stride,
  ...
  void vkCmdSetDepthClampEnableEXT(VkCommandBuffer commandBuffer,
                                   VkBool32 depthClampEnable) const VULKAN_HPP_NOEXCEPT {
  ...
  void vkCmdSetAlphaToCoverageEnableEXT(VkCommandBuffer commandBuffer,
                                        VkBool32 alphaToCoverageEnable) const VULKAN_HPP_NOEXCEPT {
  ...
  void vkCmdSetAlphaToOneEnableEXT(VkCommandBuffer commandBuffer,
                                   VkBool32 alphaToOneEnable) const VULKAN_HPP_NOEXCEPT {
  ...
  void vkCmdSetColorBlendEnableEXT(VkCommandBuffer commandBuffer, uint32_t firstAttachment,
    ...
    return ::vkCmdSetColorBlendEnableEXT(commandBuffer, firstAttachment, attachmentCount,
                                         pColorBlendEnables);
  }
  ...
  void vkCmdSetColorBlendEquationEXT(
    ...
    return ::vkCmdSetColorBlendEquationEXT(commandBuffer, firstAttachment, attachmentCount,
  ...
  void vkCmdSetColorWriteMaskEXT(
    ...
    return ::vkCmdSetColorWriteMaskEXT(commandBuffer, firstAttachment, attachmentCount,
                                       pColorWriteMasks);
  }
  ...
  void vkCmdSetExtraPrimitiveOverestimationSizeEXT(VkCommandBuffer commandBuffer,
      ...
      VULKAN_HPP_NOEXCEPT {
    return ::vkCmdSetExtraPrimitiveOverestimationSizeEXT(commandBuffer,
                                                         extraPrimitiveOverestimationSize);
  }

  void vkCmdSetDepthClipEnableEXT(VkCommandBuffer commandBuffer,
                                  VkBool32 depthClipEnable) const VULKAN_HPP_NOEXCEPT {
  ...
  void vkCmdSetSampleLocationsEnableEXT(VkCommandBuffer commandBuffer,
                                        VkBool32 sampleLocationsEnable) const VULKAN_HPP_NOEXCEPT {
  ...
  void vkCmdSetColorBlendAdvancedEXT(
    ...
    return ::vkCmdSetColorBlendAdvancedEXT(commandBuffer, firstAttachment, attachmentCount,
                                           pColorBlendAdvanced);
  }
  ...
  void vkCmdSetLineStippleEnableEXT(VkCommandBuffer commandBuffer,
                                    VkBool32 stippledLineEnable) const VULKAN_HPP_NOEXCEPT {
  ...
  void vkCmdSetDepthClipNegativeOneToOneEXT(VkCommandBuffer commandBuffer,
                                            VkBool32 negativeOneToOne) const VULKAN_HPP_NOEXCEPT {
  ...
  void vkCmdSetViewportWScalingEnableNV(VkCommandBuffer commandBuffer,
                                        VkBool32 viewportWScalingEnable) const VULKAN_HPP_NOEXCEPT {
  ...
  void vkCmdSetViewportSwizzleNV(
    ...
    return ::vkCmdSetViewportSwizzleNV(commandBuffer, firstViewport, viewportCount,
                                       pViewportSwizzles);
  }

  void vkCmdSetCoverageToColorEnableNV(VkCommandBuffer commandBuffer,
                                       VkBool32 coverageToColorEnable) const VULKAN_HPP_NOEXCEPT {
  ...
  void vkCmdSetCoverageModulationTableNV(
    ...
    return ::vkCmdSetCoverageModulationTableNV(commandBuffer, coverageModulationTableCount,
                                               pCoverageModulationTable);
  }

  void vkCmdSetShadingRateImageEnableNV(VkCommandBuffer commandBuffer,
                                        VkBool32 shadingRateImageEnable) const VULKAN_HPP_NOEXCEPT {
  ...
  void vkCmdSetRepresentativeFragmentTestEnableNV(VkCommandBuffer commandBuffer,
      ...
      VULKAN_HPP_NOEXCEPT {
    return ::vkCmdSetRepresentativeFragmentTestEnableNV(commandBuffer,
  ...
  void vkGetDeviceTensorMemoryRequirementsARM(
      VkDevice device, const VkDeviceTensorMemoryRequirementsARM *pInfo,
      VkMemoryRequirements2 *pMemoryRequirements) const VULKAN_HPP_NOEXCEPT {
    return ::vkGetDeviceTensorMemoryRequirementsARM(device, pInfo, pMemoryRequirements);
  }

  void vkCmdCopyTensorARM(VkCommandBuffer commandBuffer,
                          const VkCopyTensorInfoARM *pCopyTensorInfo) const VULKAN_HPP_NOEXCEPT {
  ...
  void vkGetPhysicalDeviceExternalTensorPropertiesARM(
    ...
    return ::vkGetPhysicalDeviceExternalTensorPropertiesARM(physicalDevice, pExternalTensorInfo,
                                                            pExternalTensorProperties);
  }

  VkResult vkGetTensorOpaqueCaptureDescriptorDataARM(
      VkDevice device, const VkTensorCaptureDescriptorDataInfoARM *pInfo,
      void *pData) const VULKAN_HPP_NOEXCEPT {
  ...
  VkResult vkGetTensorViewOpaqueCaptureDescriptorDataARM(
      VkDevice device, const VkTensorViewCaptureDescriptorDataInfoARM *pInfo,
      void *pData) const VULKAN_HPP_NOEXCEPT {
  ...
  void vkCmdBindIndexBuffer2KHR(VkCommandBuffer commandBuffer, VkBuffer buffer, VkDeviceSize offset,
                                VkDeviceSize size,
                                VkIndexType indexType) const VULKAN_HPP_NOEXCEPT {
  ...
  void vkAntiLagUpdateAMD(VkDevice device,
                          const VkAntiLagDataAMD *pData) const VULKAN_HPP_NOEXCEPT {
    return ::vkAntiLagUpdateAMD(device, pData);
  }

  //=== VK_KHR_present_wait2 ===

  VkResult vkWaitForPresent2KHR(VkDevice device, VkSwapchainKHR swapchain,
  ...
  void vkCmdSetDepthClampRangeEXT(VkCommandBuffer commandBuffer, VkDepthClampModeEXT depthClampMode,
  ...
  VkResult vkCreatePipelineBinariesKHR(
      VkDevice device, const VkPipelineBinaryCreateInfoKHR *pCreateInfo,
      const VkAllocationCallbacks *pAllocator,
  ...
  void vkDestroyPipelineBinaryKHR(VkDevice device, VkPipelineBinaryKHR pipelineBinary,
                                  const VkAllocationCallbacks *pAllocator) const
      VULKAN_HPP_NOEXCEPT {
    return ::vkDestroyPipelineBinaryKHR(device, pipelineBinary, pAllocator);
  }

  VkResult vkGetPipelineKeyKHR(VkDevice device, const VkPipelineCreateInfoKHR *pPipelineCreateInfo,
  ...
  VkResult vkGetPipelineBinaryDataKHR(VkDevice device, const VkPipelineBinaryDataInfoKHR *pInfo,
                                      VkPipelineBinaryKeyKHR *pPipelineBinaryKey,
                                      size_t *pPipelineBinaryDataSize,
                                      void *pPipelineBinaryData) const VULKAN_HPP_NOEXCEPT {
    return ::vkGetPipelineBinaryDataKHR(device, pInfo, pPipelineBinaryKey, pPipelineBinaryDataSize,
                                        pPipelineBinaryData);
  }

  VkResult vkReleaseCapturedPipelineDataKHR(
      VkDevice device, const VkReleaseCapturedPipelineDataInfoKHR *pInfo,
      const VkAllocationCallbacks *pAllocator) const VULKAN_HPP_NOEXCEPT {
  ...
  VkResult vkGetPhysicalDeviceCooperativeVectorPropertiesNV(
    ...
    return ::vkGetPhysicalDeviceCooperativeVectorPropertiesNV(physicalDevice, pPropertyCount,
                                                              pProperties);
  }
  ...
  VkResult vkSetLatencySleepModeNV(VkDevice device, VkSwapchainKHR swapchain,
  ...
  void vkQueueNotifyOutOfBandNV(
      VkQueue queue, const VkOutOfBandQueueTypeInfoNV *pQueueTypeInfo) const VULKAN_HPP_NOEXCEPT {
  ...
  VkResult vkGetPhysicalDeviceCooperativeMatrixPropertiesKHR(
    ...
    return ::vkGetPhysicalDeviceCooperativeMatrixPropertiesKHR(physicalDevice, pPropertyCount,
                                                               pProperties);
  }

  //=== VK_ARM_data_graph ===

  VkResult vkCreateDataGraphPipelinesARM(VkDevice device, VkDeferredOperationKHR deferredOperation,
                                         VkPipelineCache pipelineCache, uint32_t createInfoCount,
                                         const VkDataGraphPipelineCreateInfoARM *pCreateInfos,
                                         const VkAllocationCallbacks *pAllocator,
                                         VkPipeline *pPipelines) const VULKAN_HPP_NOEXCEPT {
    return ::vkCreateDataGraphPipelinesARM(device, deferredOperation, pipelineCache,
                                           createInfoCount, pCreateInfos, pAllocator, pPipelines);
  }
  ...
  void vkDestroyDataGraphPipelineSessionARM(VkDevice device, VkDataGraphPipelineSessionARM session,
                                            const VkAllocationCallbacks *pAllocator) const
      VULKAN_HPP_NOEXCEPT {
  ...
  VkResult vkGetDataGraphPipelinePropertiesARM(
    ...
    return ::vkGetDataGraphPipelinePropertiesARM(device, pPipelineInfo, propertiesCount,
                                                 pProperties);
  }
  ...
  VkResult vkGetPhysicalDeviceQueueFamilyDataGraphPropertiesARM(
    ...
    return ::vkGetPhysicalDeviceQueueFamilyDataGraphPropertiesARM(
        physicalDevice, queueFamilyIndex, pQueueFamilyDataGraphPropertyCount,
  ...
  void vkGetPhysicalDeviceQueueFamilyDataGraphProcessingEnginePropertiesARM(
    ...
    return ::vkGetPhysicalDeviceQueueFamilyDataGraphProcessingEnginePropertiesARM(
        physicalDevice, pQueueFamilyDataGraphProcessingEngineInfo,
  ...
  VkResult vkGetScreenBufferPropertiesQNX(VkDevice device, const struct _screen_buffer *buffer,
  ...
  VkResult vkGetPhysicalDeviceCalibrateableTimeDomainsKHR(
      VkPhysicalDevice physicalDevice, uint32_t *pTimeDomainCount,
      VkTimeDomainKHR *pTimeDomains) const VULKAN_HPP_NOEXCEPT {
    return ::vkGetPhysicalDeviceCalibrateableTimeDomainsKHR(physicalDevice, pTimeDomainCount,
                                                            pTimeDomains);
  }
  ...
  VkResult vkGetCalibratedTimestampsKHR(VkDevice device, uint32_t timestampCount,
                                        ...
                                        uint64_t *pTimestamps,
                                        uint64_t *pMaxDeviation) const VULKAN_HPP_NOEXCEPT {
    return ::vkGetCalibratedTimestampsKHR(device, timestampCount, pTimestampInfos, pTimestamps,
                                          pMaxDeviation);
  }
  ...
  void vkCmdPushDescriptorSetWithTemplate2KHR(
      ...
      const VkPushDescriptorSetWithTemplateInfo *pPushDescriptorSetWithTemplateInfo) const
      VULKAN_HPP_NOEXCEPT {
    return ::vkCmdPushDescriptorSetWithTemplate2KHR(commandBuffer,
  ...
  void vkCmdBindTileMemoryQCOM(VkCommandBuffer commandBuffer,
  ...
  VkResult vkCreateExternalComputeQueueNV(
      VkDevice device, const VkExternalComputeQueueCreateInfoNV *pCreateInfo,
      const VkAllocationCallbacks *pAllocator,
  ...
  void vkDestroyExternalComputeQueueNV(VkDevice device, VkExternalComputeQueueNV externalQueue,
                                       const VkAllocationCallbacks *pAllocator) const
      VULKAN_HPP_NOEXCEPT {
  ...
  void vkCmdPreprocessGeneratedCommandsEXT(
    ...
    return ::vkCmdPreprocessGeneratedCommandsEXT(commandBuffer, pGeneratedCommandsInfo,
                                                 stateCommandBuffer);
  }
  ...
  void vkCmdExecuteGeneratedCommandsEXT(
    ...
    return ::vkCmdExecuteGeneratedCommandsEXT(commandBuffer, isPreprocessed,
  ...
  VkResult vkCreateIndirectCommandsLayoutEXT(
    ...
    return ::vkCreateIndirectCommandsLayoutEXT(device, pCreateInfo, pAllocator,
                                               pIndirectCommandsLayout);
  }
  ...
  VkResult vkCreateIndirectExecutionSetEXT(
    ...
    return ::vkCreateIndirectExecutionSetEXT(device, pCreateInfo, pAllocator,
  ...
  void vkUpdateIndirectExecutionSetPipelineEXT(
    ...
    return ::vkUpdateIndirectExecutionSetPipelineEXT(device, indirectExecutionSet,
  ...
  void vkUpdateIndirectExecutionSetShaderEXT(
    ...
    return ::vkUpdateIndirectExecutionSetShaderEXT(device, indirectExecutionSet,
                                                   executionSetWriteCount, pExecutionSetWrites);
  }
  ...
  VkResult vkCreateSurfaceOHOS(VkInstance instance, const VkSurfaceCreateInfoOHOS *pCreateInfo,
                               const VkAllocationCallbacks *pAllocator,
                               VkSurfaceKHR *pSurface) const VULKAN_HPP_NOEXCEPT {
  ...
  VkResult vkGetMemoryMetalHandleEXT(VkDevice device,
  ...
  VkResult vkGetMemoryMetalHandlePropertiesEXT(
    ...
    return ::vkGetMemoryMetalHandlePropertiesEXT(device, handleType, pHandle,
  ...
  void vkCmdEndRendering2EXT(VkCommandBuffer commandBuffer,
...
class ObjectRelease {
 public:
  ...
  ObjectRelease(OwnerType owner, Dispatch const &dispatch VULKAN_HPP_DEFAULT_DISPATCHER_ASSIGNMENT)
      VULKAN_HPP_NOEXCEPT : m_owner(owner),
                            m_dispatch(&dispatch) {}
...
[[noreturn]] VULKAN_HPP_INLINE void throwResultException(Result result, char const *message) {
  switch (result) {
    ...
    case Result::eErrorNotPermitted:
      throw NotPermittedError(message);
    ...
    case Result::eErrorNotEnoughSpaceKHR:
      throw NotEnoughSpaceKHRError(message);
    default:
...
struct ResultValue {
  ...
  bool has_value() const VULKAN_HPP_NOEXCEPT { return result == vk::Result::eSuccess; }
...
struct ResultValue<UniqueHandle<Type, Dispatch>> {
  ...
  VULKAN_HPP_DEPRECATED(
      "asTuple() on an l-value is deprecated, as it implicitly moves the UniqueHandle out of the "
      "ResultValue. Use asTuple() on an r-value instead, requiring to explicitly move the "
      "UniqueHandle.")
...
VULKAN_HPP_CONSTEXPR_INLINE uint32_t CompressedTriangleFormatDgf1ByteAlignmentAMDX =
    VK_COMPRESSED_TRIANGLE_FORMAT_DGF1_BYTE_ALIGNMENT_AMDX;
VULKAN_HPP_CONSTEXPR_INLINE uint32_t CompressedTriangleFormatDgf1ByteStrideAMDX =
    VK_COMPRESSED_TRIANGLE_FORMAT_DGF1_BYTE_STRIDE_AMDX;
#endif /*VK_ENABLE_BETA_EXTENSIONS*/

//=== VK_KHR_pipeline_binary ===
VULKAN_HPP_CONSTEXPR_INLINE uint32_t MaxPipelineBinaryKeySizeKHR =
    VK_MAX_PIPELINE_BINARY_KEY_SIZE_KHR;

//=== VK_ARM_data_graph ===
VULKAN_HPP_CONSTEXPR_INLINE uint32_t MaxPhysicalDeviceDataGraphOperationSetNameSizeARM =
    VK_MAX_PHYSICAL_DEVICE_DATA_GRAPH_OPERATION_SET_NAME_SIZE_ARM;

//=== VK_KHR_video_decode_av1 ===
VULKAN_HPP_CONSTEXPR_INLINE uint32_t MaxVideoAv1ReferencesPerFrameKHR =
    VK_MAX_VIDEO_AV1_REFERENCES_PER_FRAME_KHR;

//=== VK_KHR_video_decode_vp9 ===
VULKAN_HPP_CONSTEXPR_INLINE uint32_t MaxVideoVp9ReferencesPerFrameKHR =
    VK_MAX_VIDEO_VP9_REFERENCES_PER_FRAME_KHR;

//=== VK_NV_partitioned_acceleration_structure ===
VULKAN_HPP_CONSTEXPR_INLINE uint32_t PartitionedAccelerationStructurePartitionIndexGlobalNV =
...
template <typename T, typename = typename std::enable_if<std::is_integral<T>::value>::type>
VULKAN_HPP_CONSTEXPR uint32_t makeApiVersion(T const variant, T const major, T const minor,
                                             T const patch) {
...
//=== VK_EXT_debug_report ===
VULKAN_HPP_DEPRECATED(
    "The VK_EXT_debug_report extension has been deprecated by VK_EXT_debug_utils.")
...
VULKAN_HPP_DEPRECATED(
    "The VK_NV_dedicated_allocation extension has been deprecated by VK_KHR_dedicated_allocation.")
...
VULKAN_HPP_DEPRECATED(
    "The VK_AMD_negative_viewport_height extension has been obsoleted by VK_KHR_maintenance1.")
...
VULKAN_HPP_DEPRECATED(
    "The VK_AMD_gpu_shader_half_float extension has been deprecated by VK_KHR_shader_float16_int8.")
...
//=== VK_NV_external_memory_capabilities ===
VULKAN_HPP_DEPRECATED(
    "The VK_NV_external_memory_capabilities extension has been deprecated by "
    "VK_KHR_external_memory_capabilities.")
...
VULKAN_HPP_DEPRECATED(
    "The VK_NV_external_memory extension has been deprecated by VK_KHR_external_memory.")
...
//=== VK_NV_external_memory_win32 ===
VULKAN_HPP_DEPRECATED(
    "The VK_NV_external_memory_win32 extension has been deprecated by "
    "VK_KHR_external_memory_win32.")
...
VULKAN_HPP_DEPRECATED(
    "The VK_EXT_validation_flags extension has been deprecated by VK_EXT_layer_settings.")
...
VULKAN_HPP_DEPRECATED(
    "The VK_EXT_shader_subgroup_ballot extension has been deprecated by VK_VERSION_1_2.")
...
VULKAN_HPP_DEPRECATED(
    "The VK_EXT_shader_subgroup_vote extension has been deprecated by VK_VERSION_1_1.")
...
//=== VK_MVK_ios_surface ===
VULKAN_HPP_DEPRECATED(
    "The VK_MVK_ios_surface extension has been deprecated by VK_EXT_metal_surface.")
...
//=== VK_MVK_macos_surface ===
VULKAN_HPP_DEPRECATED(
    "The VK_MVK_macos_surface extension has been deprecated by VK_EXT_metal_surface.")
...
#if defined(VK_USE_PLATFORM_ANDROID_KHR)
//=== VK_ANDROID_external_memory_android_hardware_buffer ===
VULKAN_HPP_CONSTEXPR_INLINE auto ANDROIDExternalMemoryAndroidHardwareBufferExtensionName =
    VK_ANDROID_EXTERNAL_MEMORY_ANDROID_HARDWARE_BUFFER_EXTENSION_NAME;
VULKAN_HPP_CONSTEXPR_INLINE auto ANDROIDExternalMemoryAndroidHardwareBufferSpecVersion =
...
VULKAN_HPP_DEPRECATED(
    "The VK_AMD_gpu_shader_int16 extension has been deprecated by VK_KHR_shader_float16_int8.")
...
VULKAN_HPP_CONSTEXPR_INLINE auto KHRShaderBfloat16ExtensionName =
...
//=== VK_NV_ray_tracing ===
VULKAN_HPP_DEPRECATED(
    "The VK_NV_ray_tracing extension has been deprecated by VK_KHR_ray_tracing_pipeline.")
...
//=== VK_KHR_dynamic_rendering_local_read ===
VULKAN_HPP_CONSTEXPR_INLINE auto KHRDynamicRenderingLocalReadExtensionName =
    VK_KHR_DYNAMIC_RENDERING_LOCAL_READ_EXTENSION_NAME;
VULKAN_HPP_CONSTEXPR_INLINE auto KHRDynamicRenderingLocalReadSpecVersion =
...
VULKAN_HPP_CONSTEXPR_INLINE auto KHRShaderQuadControlExtensionName =
    VK_KHR_SHADER_QUAD_CONTROL_EXTENSION_NAME;
VULKAN_HPP_CONSTEXPR_INLINE auto KHRShaderQuadControlSpecVersion =
...
//=== VK_EXT_buffer_device_address ===
VULKAN_HPP_DEPRECATED(
    "The VK_EXT_buffer_device_address extension has been deprecated by "
    "VK_KHR_buffer_device_address.")
...
VULKAN_HPP_DEPRECATED(
    "The VK_EXT_validation_features extension has been deprecated by VK_EXT_layer_settings.")
...
VULKAN_HPP_CONSTEXPR_INLINE auto EXTMapMemoryPlacedExtensionName =
    VK_EXT_MAP_MEMORY_PLACED_EXTENSION_NAME;
VULKAN_HPP_CONSTEXPR_INLINE auto EXTMapMemoryPlacedSpecVersion =
...
VULKAN_HPP_CONSTEXPR_INLINE auto KHRShaderUntypedPointersExtensionName =
...
VULKAN_HPP_DEPRECATED(
    "The VK_NV_displacement_micromap extension has been deprecated by "
    "VK_NV_cluster_acceleration_structure.")
...
VULKAN_HPP_CONSTEXPR_INLINE auto NVRayTracingLinearSweptSpheresExtensionName =
...
VULKAN_HPP_CONSTEXPR_INLINE auto KHRShaderMaximalReconvergenceExtensionName =
    VK_KHR_SHADER_MAXIMAL_RECONVERGENCE_EXTENSION_NAME;
VULKAN_HPP_CONSTEXPR_INLINE auto KHRShaderMaximalReconvergenceSpecVersion =
...
//=== VK_KHR_present_wait2 ===
VULKAN_HPP_CONSTEXPR_INLINE auto KHRPresentWait2ExtensionName =
...
//=== VK_KHR_pipeline_binary ===
VULKAN_HPP_CONSTEXPR_INLINE auto KHRPipelineBinaryExtensionName =
...
VULKAN_HPP_CONSTEXPR_INLINE auto KHRSurfaceMaintenance1ExtensionName =
    VK_KHR_SURFACE_MAINTENANCE_1_EXTENSION_NAME;
VULKAN_HPP_CONSTEXPR_INLINE auto KHRSurfaceMaintenance1SpecVersion =
    VK_KHR_SURFACE_MAINTENANCE_1_SPEC_VERSION;

//=== VK_KHR_swapchain_maintenance1 ===
VULKAN_HPP_CONSTEXPR_INLINE auto KHRSwapchainMaintenance1ExtensionName =
    VK_KHR_SWAPCHAIN_MAINTENANCE_1_EXTENSION_NAME;
VULKAN_HPP_CONSTEXPR_INLINE auto KHRSwapchainMaintenance1SpecVersion =
...
//=== VK_NV_cooperative_vector ===
VULKAN_HPP_CONSTEXPR_INLINE auto NVCooperativeVectorExtensionName =
    VK_NV_COOPERATIVE_VECTOR_EXTENSION_NAME;
VULKAN_HPP_CONSTEXPR_INLINE auto NVCooperativeVectorSpecVersion =
...
VULKAN_HPP_CONSTEXPR_INLINE auto EXTLegacyVertexAttributesExtensionName =
    VK_EXT_LEGACY_VERTEX_ATTRIBUTES_EXTENSION_NAME;
VULKAN_HPP_CONSTEXPR_INLINE auto EXTLegacyVertexAttributesSpecVersion =
...
VULKAN_HPP_CONSTEXPR_INLINE auto KHRComputeShaderDerivativesExtensionName =
...
//=== VK_KHR_video_decode_av1 ===
VULKAN_HPP_CONSTEXPR_INLINE auto KHRVideoDecodeAv1ExtensionName =
    VK_KHR_VIDEO_DECODE_AV1_EXTENSION_NAME;
VULKAN_HPP_CONSTEXPR_INLINE auto KHRVideoDecodeAv1SpecVersion =
...
VULKAN_HPP_CONSTEXPR_INLINE auto KHRVideoEncodeAv1SpecVersion =
    VK_KHR_VIDEO_ENCODE_AV1_SPEC_VERSION;

//=== VK_KHR_video_decode_vp9 ===
VULKAN_HPP_CONSTEXPR_INLINE auto KHRVideoDecodeVp9ExtensionName =
    VK_KHR_VIDEO_DECODE_VP9_EXTENSION_NAME;
VULKAN_HPP_CONSTEXPR_INLINE auto KHRVideoDecodeVp9SpecVersion =
...
VULKAN_HPP_CONSTEXPR_INLINE auto KHRLoadStoreOpNoneSpecVersion =
...
VULKAN_HPP_CONSTEXPR_INLINE auto KHRShaderFloatControls2ExtensionName =
...
VULKAN_HPP_CONSTEXPR_INLINE auto KHRIndexTypeUint8ExtensionName =
...
VULKAN_HPP_CONSTEXPR_INLINE auto KHRLineRasterizationSpecVersion =
...
VULKAN_HPP_CONSTEXPR_INLINE auto KHRShaderExpectAssumeSpecVersion =
...
//=== VK_QCOM_tile_memory_heap ===
VULKAN_HPP_CONSTEXPR_INLINE auto QCOMTileMemoryHeapExtensionName =
    VK_QCOM_TILE_MEMORY_HEAP_EXTENSION_NAME;
VULKAN_HPP_CONSTEXPR_INLINE auto QCOMTileMemoryHeapSpecVersion =
...
VULKAN_HPP_CONSTEXPR_INLINE auto KHRVideoEncodeIntraRefreshExtensionName =
    VK_KHR_VIDEO_ENCODE_INTRA_REFRESH_EXTENSION_NAME;
VULKAN_HPP_CONSTEXPR_INLINE auto KHRVideoEncodeIntraRefreshSpecVersion =
    VK_KHR_VIDEO_ENCODE_INTRA_REFRESH_SPEC_VERSION;

//=== VK_KHR_video_encode_quantization_map ===
VULKAN_HPP_CONSTEXPR_INLINE auto KHRVideoEncodeQuantizationMapExtensionName =
    VK_KHR_VIDEO_ENCODE_QUANTIZATION_MAP_EXTENSION_NAME;
VULKAN_HPP_CONSTEXPR_INLINE auto KHRVideoEncodeQuantizationMapSpecVersion =
    VK_KHR_VIDEO_ENCODE_QUANTIZATION_MAP_SPEC_VERSION;

//=== VK_NV_raw_access_chains ===
VULKAN_HPP_CONSTEXPR_INLINE auto NVRawAccessChainsExtensionName =
    VK_NV_RAW_ACCESS_CHAINS_EXTENSION_NAME;
VULKAN_HPP_CONSTEXPR_INLINE auto NVRawAccessChainsSpecVersion =
    VK_NV_RAW_ACCESS_CHAINS_SPEC_VERSION;

//=== VK_NV_external_compute_queue ===
VULKAN_HPP_CONSTEXPR_INLINE auto NVExternalComputeQueueExtensionName =
    VK_NV_EXTERNAL_COMPUTE_QUEUE_EXTENSION_NAME;
VULKAN_HPP_CONSTEXPR_INLINE auto NVExternalComputeQueueSpecVersion =
    VK_NV_EXTERNAL_COMPUTE_QUEUE_SPEC_VERSION;

//=== VK_KHR_shader_relaxed_extended_instruction ===
VULKAN_HPP_CONSTEXPR_INLINE auto KHRShaderRelaxedExtendedInstructionExtensionName =
    VK_KHR_SHADER_RELAXED_EXTENDED_INSTRUCTION_EXTENSION_NAME;
VULKAN_HPP_CONSTEXPR_INLINE auto KHRShaderRelaxedExtendedInstructionSpecVersion =
    VK_KHR_SHADER_RELAXED_EXTENDED_INSTRUCTION_SPEC_VERSION;

//=== VK_NV_command_buffer_inheritance ===
VULKAN_HPP_CONSTEXPR_INLINE auto NVCommandBufferInheritanceExtensionName =
    VK_NV_COMMAND_BUFFER_INHERITANCE_EXTENSION_NAME;
VULKAN_HPP_CONSTEXPR_INLINE auto NVCommandBufferInheritanceSpecVersion =
...
VULKAN_HPP_CONSTEXPR_INLINE auto NVShaderAtomicFloat16VectorExtensionName =
    VK_NV_SHADER_ATOMIC_FLOAT16_VECTOR_EXTENSION_NAME;
VULKAN_HPP_CONSTEXPR_INLINE auto NVShaderAtomicFloat16VectorSpecVersion =
    VK_NV_SHADER_ATOMIC_FLOAT16_VECTOR_SPEC_VERSION;

//=== VK_EXT_shader_replicated_composites ===
VULKAN_HPP_CONSTEXPR_INLINE auto EXTShaderReplicatedCompositesExtensionName =
...
VULKAN_HPP_CONSTEXPR_INLINE auto NVRayTracingValidationExtensionName =
    VK_NV_RAY_TRACING_VALIDATION_EXTENSION_NAME;
VULKAN_HPP_CONSTEXPR_INLINE auto NVRayTracingValidationSpecVersion =
    VK_NV_RAY_TRACING_VALIDATION_SPEC_VERSION;

//=== VK_NV_cluster_acceleration_structure ===
VULKAN_HPP_CONSTEXPR_INLINE auto NVClusterAccelerationStructureExtensionName =
    VK_NV_CLUSTER_ACCELERATION_STRUCTURE_EXTENSION_NAME;
VULKAN_HPP_CONSTEXPR_INLINE auto NVClusterAccelerationStructureSpecVersion =
    VK_NV_CLUSTER_ACCELERATION_STRUCTURE_SPEC_VERSION;

//=== VK_NV_partitioned_acceleration_structure ===
VULKAN_HPP_CONSTEXPR_INLINE auto NVPartitionedAccelerationStructureExtensionName =
    VK_NV_PARTITIONED_ACCELERATION_STRUCTURE_EXTENSION_NAME;
VULKAN_HPP_CONSTEXPR_INLINE auto NVPartitionedAccelerationStructureSpecVersion =
    VK_NV_PARTITIONED_ACCELERATION_STRUCTURE_SPEC_VERSION;

//=== VK_EXT_device_generated_commands ===
VULKAN_HPP_CONSTEXPR_INLINE auto EXTDeviceGeneratedCommandsExtensionName =
...
VULKAN_HPP_CONSTEXPR_INLINE auto MESAImageAlignmentControlExtensionName =
    VK_MESA_IMAGE_ALIGNMENT_CONTROL_EXTENSION_NAME;
VULKAN_HPP_CONSTEXPR_INLINE auto MESAImageAlignmentControlSpecVersion =
    VK_MESA_IMAGE_ALIGNMENT_CONTROL_SPEC_VERSION;

//=== VK_EXT_depth_clamp_control ===
VULKAN_HPP_CONSTEXPR_INLINE auto EXTDepthClampControlExtensionName =
    VK_EXT_DEPTH_CLAMP_CONTROL_EXTENSION_NAME;
VULKAN_HPP_CONSTEXPR_INLINE auto EXTDepthClampControlSpecVersion =
...
VULKAN_HPP_CONSTEXPR_INLINE auto KHRVideoMaintenance2ExtensionName =
    VK_KHR_VIDEO_MAINTENANCE_2_EXTENSION_NAME;
VULKAN_HPP_CONSTEXPR_INLINE auto KHRVideoMaintenance2SpecVersion =
...
//=== VK_NV_cooperative_matrix2 ===
VULKAN_HPP_CONSTEXPR_INLINE auto NVCooperativeMatrix2ExtensionName =
    VK_NV_COOPERATIVE_MATRIX_2_EXTENSION_NAME;
VULKAN_HPP_CONSTEXPR_INLINE auto NVCooperativeMatrix2SpecVersion =
    VK_NV_COOPERATIVE_MATRIX_2_SPEC_VERSION;

//=== VK_ARM_pipeline_opacity_micromap ===
VULKAN_HPP_CONSTEXPR_INLINE auto ARMPipelineOpacityMicromapExtensionName =
    VK_ARM_PIPELINE_OPACITY_MICROMAP_EXTENSION_NAME;
VULKAN_HPP_CONSTEXPR_INLINE auto ARMPipelineOpacityMicromapSpecVersion =
    VK_ARM_PIPELINE_OPACITY_MICROMAP_SPEC_VERSION;

#if defined(VK_USE_PLATFORM_METAL_EXT)
//=== VK_EXT_external_memory_metal ===
VULKAN_HPP_CONSTEXPR_INLINE auto EXTExternalMemoryMetalExtensionName =
    VK_EXT_EXTERNAL_MEMORY_METAL_EXTENSION_NAME;
VULKAN_HPP_CONSTEXPR_INLINE auto EXTExternalMemoryMetalSpecVersion =
    VK_EXT_EXTERNAL_MEMORY_METAL_SPEC_VERSION;
#endif /*VK_USE_PLATFORM_METAL_EXT*/

//=== VK_KHR_depth_clamp_zero_one ===
VULKAN_HPP_CONSTEXPR_INLINE auto KHRDepthClampZeroOneExtensionName =
...
VULKAN_HPP_CONSTEXPR_INLINE auto EXTVertexAttributeRobustnessExtensionName =
...
VULKAN_HPP_CONSTEXPR_INLINE auto VALVEFragmentDensityMapLayeredExtensionName =
    VK_VALVE_FRAGMENT_DENSITY_MAP_LAYERED_EXTENSION_NAME;
VULKAN_HPP_CONSTEXPR_INLINE auto VALVEFragmentDensityMapLayeredSpecVersion =
...
VULKAN_HPP_CONSTEXPR_INLINE auto NVPresentMeteringExtensionName =
...
//=== VK_EXT_fragment_density_map_offset ===
VULKAN_HPP_CONSTEXPR_INLINE auto EXTFragmentDensityMapOffsetExtensionName =
...
VULKAN_HPP_CONSTEXPR_INLINE auto KHRPresentModeFifoLatestReadyExtensionName =
    VK_KHR_PRESENT_MODE_FIFO_LATEST_READY_EXTENSION_NAME;
VULKAN_HPP_CONSTEXPR_INLINE auto KHRPresentModeFifoLatestReadySpecVersion =
    VK_KHR_PRESENT_MODE_FIFO_LATEST_READY_SPEC_VERSION;

//=== VK_SEC_pipeline_cache_incremental_mode ===
VULKAN_HPP_CONSTEXPR_INLINE auto SECPipelineCacheIncrementalModeExtensionName =
    VK_SEC_PIPELINE_CACHE_INCREMENTAL_MODE_EXTENSION_NAME;
VULKAN_HPP_CONSTEXPR_INLINE auto SECPipelineCacheIncrementalModeSpecVersion =
...
template <>
struct StructExtends<PipelineRasterizationLineStateCreateInfo,
...
template <>
struct StructExtends<PipelineVertexInputDivisorStateCreateInfo,
...
//=== VK_NV_ray_tracing_linear_swept_spheres ===
template <>
struct StructExtends<PhysicalDeviceRayTracingLinearSweptSpheresFeaturesNV,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <>
struct StructExtends<AccelerationStructureGeometryLinearSweptSpheresDataNV,
...
//=== VK_EXT_rasterization_order_attachment_access ===
template <>
struct StructExtends<PhysicalDeviceRasterizationOrderAttachmentAccessFeaturesEXT,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <>
struct StructExtends<AccelerationStructureDenseGeometryFormatTrianglesDataAMDX,
                     AccelerationStructureGeometryKHR> {
  enum { value = true };
};
#endif /*VK_ENABLE_BETA_EXTENSIONS*/
...
template <>
struct StructExtends<DataGraphPipelineCompilerControlCreateInfoARM,
...
template <>
struct StructExtends<DataGraphPipelineConstantTensorSemiStructuredSparsityInfoARM,
...
template <>
struct StructExtends<PhysicalDeviceComputeShaderDerivativesPropertiesKHR,
                     PhysicalDeviceProperties2> {
  enum { value = true };
};
...
template <>
struct StructExtends<VideoDecodeAV1SessionParametersCreateInfoKHR,
                     VideoSessionParametersCreateInfoKHR> {
  enum { value = true };
};
...
template <>
struct StructExtends<VideoEncodeAV1QualityLevelPropertiesKHR,
                     VideoEncodeQualityLevelPropertiesKHR> {
  enum { value = true };
};
...
template <>
struct StructExtends<VideoEncodeAV1SessionParametersCreateInfoKHR,
                     VideoSessionParametersCreateInfoKHR> {
  enum { value = true };
};
...
//=== VK_EXT_attachment_feedback_loop_dynamic_state ===
template <>
struct StructExtends<PhysicalDeviceAttachmentFeedbackLoopDynamicStateFeaturesEXT,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <>
struct StructExtends<VideoEncodeQuantizationMapSessionParametersCreateInfoKHR,
                     VideoSessionParametersCreateInfoKHR> {
  enum { value = true };
};
...
//=== VK_KHR_shader_relaxed_extended_instruction ===
template <>
struct StructExtends<PhysicalDeviceShaderRelaxedExtendedInstructionFeaturesKHR,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <>
struct StructExtends<PhysicalDeviceLayeredApiVulkanPropertiesKHR,
...
//=== VK_NV_cluster_acceleration_structure ===
template <>
struct StructExtends<PhysicalDeviceClusterAccelerationStructureFeaturesNV,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <>
struct StructExtends<PhysicalDeviceClusterAccelerationStructurePropertiesNV,
                     PhysicalDeviceProperties2> {
  enum { value = true };
};

template <>
struct StructExtends<RayTracingPipelineClusterAccelerationStructureCreateInfoNV,
...
//=== VK_NV_partitioned_acceleration_structure ===
template <>
struct StructExtends<PhysicalDevicePartitionedAccelerationStructureFeaturesNV,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <>
struct StructExtends<PhysicalDevicePartitionedAccelerationStructurePropertiesNV,
                     PhysicalDeviceProperties2> {
  enum { value = true };
};

template <>
struct StructExtends<PartitionedAccelerationStructureFlagsNV,
...
template <>
struct StructExtends<PhysicalDeviceDeviceGeneratedCommandsPropertiesEXT,
                     PhysicalDeviceProperties2> {
  enum { value = true };
};
...
template <>
struct StructExtends<PipelineViewportDepthClampControlCreateInfoEXT,
                     PipelineViewportStateCreateInfo> {
  enum { value = true };
};
...
//=== VK_VALVE_fragment_density_map_layered ===
template <>
struct StructExtends<PhysicalDeviceFragmentDensityMapLayeredFeaturesVALVE,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
template <>
struct StructExtends<PhysicalDeviceFragmentDensityMapLayeredPropertiesVALVE,
                     PhysicalDeviceProperties2> {
  enum { value = true };
};
...
template <>
struct StructExtends<PhysicalDeviceFragmentDensityMapOffsetPropertiesEXT,
                     PhysicalDeviceProperties2> {
  enum { value = true };
};
...
//=== VK_SEC_pipeline_cache_incremental_mode ===
template <>
struct StructExtends<PhysicalDevicePipelineCacheIncrementalModeFeaturesSEC,
                     PhysicalDeviceFeatures2> {
  enum { value = true };
};
...
#error unsupported platform
      ...
      // modern versions of macOS don't search /usr/local/lib automatically contrary to what man
      // dlopen says Vulkan SDK uses this as the system-wide installation location, so we're going
      // to fallback to this if all else fails
...
#ifndef VULKAN_HPP_NO_EXCEPTIONS
    if (!m_library) {
      // NOTE there should be an InitializationFailedError, but msvc insists on the symbol does not
      // exist within the scope of this function.
...
class DispatchLoaderDynamic : public DispatchLoaderBase {
 ...
 public:
  ...
  DispatchLoaderDynamic(VkInstance instance, PFN_vkGetInstanceProcAddr getInstanceProcAddr,
                        VkDevice device = {},
  ...
  void init(Instance instanceCpp) VULKAN_HPP_NOEXCEPT {
    ...
    vkBindBufferMemory =
        PFN_vkBindBufferMemory(vkGetInstanceProcAddr(instance, "vkBindBufferMemory"));
    ...
    vkDestroySemaphore =
        PFN_vkDestroySemaphore(vkGetInstanceProcAddr(instance, "vkDestroySemaphore"));
    ...
    vkDestroyQueryPool =
        PFN_vkDestroyQueryPool(vkGetInstanceProcAddr(instance, "vkDestroyQueryPool"));
    vkGetQueryPoolResults =
        PFN_vkGetQueryPoolResults(vkGetInstanceProcAddr(instance, "vkGetQueryPoolResults"));
    vkCreateBuffer = PFN_vkCreateBuffer(vkGetInstanceProcAddr(instance, "vkCreateBuffer"));
    vkDestroyBuffer = PFN_vkDestroyBuffer(vkGetInstanceProcAddr(instance, "vkDestroyBuffer"));
    vkCreateBufferView =
        PFN_vkCreateBufferView(vkGetInstanceProcAddr(instance, "vkCreateBufferView"));
    vkDestroyBufferView =
        PFN_vkDestroyBufferView(vkGetInstanceProcAddr(instance, "vkDestroyBufferView"));
    ...
    vkDestroyImageView =
        PFN_vkDestroyImageView(vkGetInstanceProcAddr(instance, "vkDestroyImageView"));
    vkCreateShaderModule =
        PFN_vkCreateShaderModule(vkGetInstanceProcAddr(instance, "vkCreateShaderModule"));
    vkDestroyShaderModule =
        PFN_vkDestroyShaderModule(vkGetInstanceProcAddr(instance, "vkDestroyShaderModule"));
    vkCreatePipelineCache =
        PFN_vkCreatePipelineCache(vkGetInstanceProcAddr(instance, "vkCreatePipelineCache"));
    ...
    vkMergePipelineCaches =
        PFN_vkMergePipelineCaches(vkGetInstanceProcAddr(instance, "vkMergePipelineCaches"));
    ...
    vkResetDescriptorPool =
        PFN_vkResetDescriptorPool(vkGetInstanceProcAddr(instance, "vkResetDescriptorPool"));
    vkAllocateDescriptorSets =
        PFN_vkAllocateDescriptorSets(vkGetInstanceProcAddr(instance, "vkAllocateDescriptorSets"));
    vkFreeDescriptorSets =
        PFN_vkFreeDescriptorSets(vkGetInstanceProcAddr(instance, "vkFreeDescriptorSets"));
    vkUpdateDescriptorSets =
        PFN_vkUpdateDescriptorSets(vkGetInstanceProcAddr(instance, "vkUpdateDescriptorSets"));
    vkCreateFramebuffer =
        PFN_vkCreateFramebuffer(vkGetInstanceProcAddr(instance, "vkCreateFramebuffer"));
    vkDestroyFramebuffer =
        PFN_vkDestroyFramebuffer(vkGetInstanceProcAddr(instance, "vkDestroyFramebuffer"));
    vkCreateRenderPass =
        PFN_vkCreateRenderPass(vkGetInstanceProcAddr(instance, "vkCreateRenderPass"));
    vkDestroyRenderPass =
        PFN_vkDestroyRenderPass(vkGetInstanceProcAddr(instance, "vkDestroyRenderPass"));
    ...
    vkDestroyCommandPool =
        PFN_vkDestroyCommandPool(vkGetInstanceProcAddr(instance, "vkDestroyCommandPool"));
    vkResetCommandPool =
        PFN_vkResetCommandPool(vkGetInstanceProcAddr(instance, "vkResetCommandPool"));
    vkAllocateCommandBuffers =
        PFN_vkAllocateCommandBuffers(vkGetInstanceProcAddr(instance, "vkAllocateCommandBuffers"));
    vkFreeCommandBuffers =
        PFN_vkFreeCommandBuffers(vkGetInstanceProcAddr(instance, "vkFreeCommandBuffers"));
    vkBeginCommandBuffer =
        PFN_vkBeginCommandBuffer(vkGetInstanceProcAddr(instance, "vkBeginCommandBuffer"));
    vkEndCommandBuffer =
        PFN_vkEndCommandBuffer(vkGetInstanceProcAddr(instance, "vkEndCommandBuffer"));
    vkResetCommandBuffer =
        PFN_vkResetCommandBuffer(vkGetInstanceProcAddr(instance, "vkResetCommandBuffer"));
    ...
    vkCmdSetDepthBounds =
        PFN_vkCmdSetDepthBounds(vkGetInstanceProcAddr(instance, "vkCmdSetDepthBounds"));
    ...
    vkCmdBindIndexBuffer =
        PFN_vkCmdBindIndexBuffer(vkGetInstanceProcAddr(instance, "vkCmdBindIndexBuffer"));
    ...
    vkCmdDispatchIndirect =
        PFN_vkCmdDispatchIndirect(vkGetInstanceProcAddr(instance, "vkCmdDispatchIndirect"));
    ...
    vkCmdClearColorImage =
        PFN_vkCmdClearColorImage(vkGetInstanceProcAddr(instance, "vkCmdClearColorImage"));
    ...
    vkCmdClearAttachments =
        PFN_vkCmdClearAttachments(vkGetInstanceProcAddr(instance, "vkCmdClearAttachments"));
    ...
    vkCmdPipelineBarrier =
        PFN_vkCmdPipelineBarrier(vkGetInstanceProcAddr(instance, "vkCmdPipelineBarrier"));
    ...
    vkCmdResetQueryPool =
        PFN_vkCmdResetQueryPool(vkGetInstanceProcAddr(instance, "vkCmdResetQueryPool"));
    vkCmdWriteTimestamp =
        PFN_vkCmdWriteTimestamp(vkGetInstanceProcAddr(instance, "vkCmdWriteTimestamp"));
    vkCmdCopyQueryPoolResults =
        PFN_vkCmdCopyQueryPoolResults(vkGetInstanceProcAddr(instance, "vkCmdCopyQueryPoolResults"));
    vkCmdPushConstants =
        PFN_vkCmdPushConstants(vkGetInstanceProcAddr(instance, "vkCmdPushConstants"));
    vkCmdBeginRenderPass =
        PFN_vkCmdBeginRenderPass(vkGetInstanceProcAddr(instance, "vkCmdBeginRenderPass"));
    vkCmdNextSubpass = PFN_vkCmdNextSubpass(vkGetInstanceProcAddr(instance, "vkCmdNextSubpass"));
    vkCmdEndRenderPass =
        PFN_vkCmdEndRenderPass(vkGetInstanceProcAddr(instance, "vkCmdEndRenderPass"));
    vkCmdExecuteCommands =
        PFN_vkCmdExecuteCommands(vkGetInstanceProcAddr(instance, "vkCmdExecuteCommands"));

    //=== VK_VERSION_1_1 ===
    vkBindBufferMemory2 =
        PFN_vkBindBufferMemory2(vkGetInstanceProcAddr(instance, "vkBindBufferMemory2"));
    vkBindImageMemory2 =
        PFN_vkBindImageMemory2(vkGetInstanceProcAddr(instance, "vkBindImageMemory2"));
    ...
    vkCmdSetDeviceMask =
        PFN_vkCmdSetDeviceMask(vkGetInstanceProcAddr(instance, "vkCmdSetDeviceMask"));
    ...
    vkCreateRenderPass2 =
        PFN_vkCreateRenderPass2(vkGetInstanceProcAddr(instance, "vkCreateRenderPass2"));
    ...
    vkCmdEndRenderPass2 =
        PFN_vkCmdEndRenderPass2(vkGetInstanceProcAddr(instance, "vkCmdEndRenderPass2"));
    ...
    vkCmdPipelineBarrier2 =
        PFN_vkCmdPipelineBarrier2(vkGetInstanceProcAddr(instance, "vkCmdPipelineBarrier2"));
    vkCmdWriteTimestamp2 =
        PFN_vkCmdWriteTimestamp2(vkGetInstanceProcAddr(instance, "vkCmdWriteTimestamp2"));
    ...
    vkCmdResolveImage2 =
        PFN_vkCmdResolveImage2(vkGetInstanceProcAddr(instance, "vkCmdResolveImage2"));
    vkCmdBeginRendering =
        PFN_vkCmdBeginRendering(vkGetInstanceProcAddr(instance, "vkCmdBeginRendering"));
    ...
    vkCmdBindIndexBuffer2 =
        PFN_vkCmdBindIndexBuffer2(vkGetInstanceProcAddr(instance, "vkCmdBindIndexBuffer2"));
    vkGetRenderingAreaGranularity = PFN_vkGetRenderingAreaGranularity(
        vkGetInstanceProcAddr(instance, "vkGetRenderingAreaGranularity"));
    vkGetDeviceImageSubresourceLayout = PFN_vkGetDeviceImageSubresourceLayout(
        vkGetInstanceProcAddr(instance, "vkGetDeviceImageSubresourceLayout"));
    vkGetImageSubresourceLayout2 = PFN_vkGetImageSubresourceLayout2(
        vkGetInstanceProcAddr(instance, "vkGetImageSubresourceLayout2"));
    vkCmdPushDescriptorSet =
        PFN_vkCmdPushDescriptorSet(vkGetInstanceProcAddr(instance, "vkCmdPushDescriptorSet"));
    vkCmdPushDescriptorSetWithTemplate = PFN_vkCmdPushDescriptorSetWithTemplate(
        vkGetInstanceProcAddr(instance, "vkCmdPushDescriptorSetWithTemplate"));
    vkCmdSetRenderingAttachmentLocations = PFN_vkCmdSetRenderingAttachmentLocations(
        vkGetInstanceProcAddr(instance, "vkCmdSetRenderingAttachmentLocations"));
    vkCmdSetRenderingInputAttachmentIndices = PFN_vkCmdSetRenderingInputAttachmentIndices(
        vkGetInstanceProcAddr(instance, "vkCmdSetRenderingInputAttachmentIndices"));
    vkCmdBindDescriptorSets2 =
        PFN_vkCmdBindDescriptorSets2(vkGetInstanceProcAddr(instance, "vkCmdBindDescriptorSets2"));
    vkCmdPushConstants2 =
        PFN_vkCmdPushConstants2(vkGetInstanceProcAddr(instance, "vkCmdPushConstants2"));
    vkCmdPushDescriptorSet2 =
        PFN_vkCmdPushDescriptorSet2(vkGetInstanceProcAddr(instance, "vkCmdPushDescriptorSet2"));
    vkCmdPushDescriptorSetWithTemplate2 = PFN_vkCmdPushDescriptorSetWithTemplate2(
        vkGetInstanceProcAddr(instance, "vkCmdPushDescriptorSetWithTemplate2"));
    vkCopyMemoryToImage =
        PFN_vkCopyMemoryToImage(vkGetInstanceProcAddr(instance, "vkCopyMemoryToImage"));
    vkCopyImageToMemory =
        PFN_vkCopyImageToMemory(vkGetInstanceProcAddr(instance, "vkCopyImageToMemory"));
    vkCopyImageToImage =
        PFN_vkCopyImageToImage(vkGetInstanceProcAddr(instance, "vkCopyImageToImage"));
    vkTransitionImageLayout =
        PFN_vkTransitionImageLayout(vkGetInstanceProcAddr(instance, "vkTransitionImageLayout"));
    ...
    vkCreateSwapchainKHR =
        PFN_vkCreateSwapchainKHR(vkGetInstanceProcAddr(instance, "vkCreateSwapchainKHR"));
    vkDestroySwapchainKHR =
        PFN_vkDestroySwapchainKHR(vkGetInstanceProcAddr(instance, "vkDestroySwapchainKHR"));
    vkGetSwapchainImagesKHR =
        PFN_vkGetSwapchainImagesKHR(vkGetInstanceProcAddr(instance, "vkGetSwapchainImagesKHR"));
    vkAcquireNextImageKHR =
        PFN_vkAcquireNextImageKHR(vkGetInstanceProcAddr(instance, "vkAcquireNextImageKHR"));
    ...
    //=== VK_KHR_video_decode_queue ===
    vkCmdDecodeVideoKHR =
        PFN_vkCmdDecodeVideoKHR(vkGetInstanceProcAddr(instance, "vkCmdDecodeVideoKHR"));
    ...
    //=== VK_NVX_binary_import ===
    vkCreateCuModuleNVX =
        PFN_vkCreateCuModuleNVX(vkGetInstanceProcAddr(instance, "vkCreateCuModuleNVX"));
    vkCreateCuFunctionNVX =
        PFN_vkCreateCuFunctionNVX(vkGetInstanceProcAddr(instance, "vkCreateCuFunctionNVX"));
    vkDestroyCuModuleNVX =
        PFN_vkDestroyCuModuleNVX(vkGetInstanceProcAddr(instance, "vkDestroyCuModuleNVX"));
    ...
    vkGetImageViewHandle64NVX =
        PFN_vkGetImageViewHandle64NVX(vkGetInstanceProcAddr(instance, "vkGetImageViewHandle64NVX"));
    ...
    //=== VK_AMD_shader_info ===
    vkGetShaderInfoAMD =
        PFN_vkGetShaderInfoAMD(vkGetInstanceProcAddr(instance, "vkGetShaderInfoAMD"));
    ...
    vkCmdSetDeviceMaskKHR =
        PFN_vkCmdSetDeviceMaskKHR(vkGetInstanceProcAddr(instance, "vkCmdSetDeviceMaskKHR"));
    ...
    //=== VK_NN_vi_surface ===
    vkCreateViSurfaceNN =
        PFN_vkCreateViSurfaceNN(vkGetInstanceProcAddr(instance, "vkCreateViSurfaceNN"));
#endif /*VK_USE_PLATFORM_VI_NN*/

    //=== VK_KHR_maintenance1 ===
    vkTrimCommandPoolKHR =
        PFN_vkTrimCommandPoolKHR(vkGetInstanceProcAddr(instance, "vkTrimCommandPoolKHR"));
    ...
    //=== VK_EXT_direct_mode_display ===
    vkReleaseDisplayEXT =
        PFN_vkReleaseDisplayEXT(vkGetInstanceProcAddr(instance, "vkReleaseDisplayEXT"));
    ...
    //=== VK_EXT_hdr_metadata ===
    vkSetHdrMetadataEXT =
        PFN_vkSetHdrMetadataEXT(vkGetInstanceProcAddr(instance, "vkSetHdrMetadataEXT"));
    ...
    //=== VK_KHR_external_fence_fd ===
    vkImportFenceFdKHR =
        PFN_vkImportFenceFdKHR(vkGetInstanceProcAddr(instance, "vkImportFenceFdKHR"));
    vkGetFenceFdKHR = PFN_vkGetFenceFdKHR(vkGetInstanceProcAddr(instance, "vkGetFenceFdKHR"));

    //=== VK_KHR_performance_query ===
    vkEnumeratePhysicalDeviceQueueFamilyPerformanceQueryCountersKHR =
        PFN_vkEnumeratePhysicalDeviceQueueFamilyPerformanceQueryCountersKHR(vkGetInstanceProcAddr(
    ...
    vkCreateIOSSurfaceMVK =
        PFN_vkCreateIOSSurfaceMVK(vkGetInstanceProcAddr(instance, "vkCreateIOSSurfaceMVK"));
#endif /*VK_USE_PLATFORM_IOS_MVK*/
    ...
    vkCreateExecutionGraphPipelinesAMDX = PFN_vkCreateExecutionGraphPipelinesAMDX(
        vkGetInstanceProcAddr(instance, "vkCreateExecutionGraphPipelinesAMDX"));
    vkGetExecutionGraphPipelineScratchSizeAMDX = PFN_vkGetExecutionGraphPipelineScratchSizeAMDX(
        vkGetInstanceProcAddr(instance, "vkGetExecutionGraphPipelineScratchSizeAMDX"));
    vkGetExecutionGraphPipelineNodeIndexAMDX = PFN_vkGetExecutionGraphPipelineNodeIndexAMDX(
        vkGetInstanceProcAddr(instance, "vkGetExecutionGraphPipelineNodeIndexAMDX"));
    vkCmdInitializeGraphScratchMemoryAMDX = PFN_vkCmdInitializeGraphScratchMemoryAMDX(
        vkGetInstanceProcAddr(instance, "vkCmdInitializeGraphScratchMemoryAMDX"));
    vkCmdDispatchGraphAMDX =
        PFN_vkCmdDispatchGraphAMDX(vkGetInstanceProcAddr(instance, "vkCmdDispatchGraphAMDX"));
    ...
    vkCmdDispatchGraphIndirectCountAMDX = PFN_vkCmdDispatchGraphIndirectCountAMDX(
        vkGetInstanceProcAddr(instance, "vkCmdDispatchGraphIndirectCountAMDX"));
#endif /*VK_ENABLE_BETA_EXTENSIONS*/
    ...
    vkBindImageMemory2KHR =
        PFN_vkBindImageMemory2KHR(vkGetInstanceProcAddr(instance, "vkBindImageMemory2KHR"));
    ...
    vkCmdDrawMeshTasksNV =
        PFN_vkCmdDrawMeshTasksNV(vkGetInstanceProcAddr(instance, "vkCmdDrawMeshTasksNV"));
    ...
    vkCmdSetCheckpointNV =
        PFN_vkCmdSetCheckpointNV(vkGetInstanceProcAddr(instance, "vkCmdSetCheckpointNV"));
    ...
    vkWaitSemaphoresKHR =
        PFN_vkWaitSemaphoresKHR(vkGetInstanceProcAddr(instance, "vkWaitSemaphoresKHR"));
    if (!vkWaitSemaphores) vkWaitSemaphores = vkWaitSemaphoresKHR;
    vkSignalSemaphoreKHR =
        PFN_vkSignalSemaphoreKHR(vkGetInstanceProcAddr(instance, "vkSignalSemaphoreKHR"));
    ...
    vkCmdSetRenderingInputAttachmentIndicesKHR = PFN_vkCmdSetRenderingInputAttachmentIndicesKHR(
        vkGetInstanceProcAddr(instance, "vkCmdSetRenderingInputAttachmentIndicesKHR"));
    ...
    //=== VK_KHR_present_wait ===
    vkWaitForPresentKHR =
        PFN_vkWaitForPresentKHR(vkGetInstanceProcAddr(instance, "vkWaitForPresentKHR"));
    ...
    vkGetPhysicalDeviceSupportedFramebufferMixedSamplesCombinationsNV =
        PFN_vkGetPhysicalDeviceSupportedFramebufferMixedSamplesCombinationsNV(vkGetInstanceProcAddr(
    ...
    //=== VK_EXT_host_query_reset ===
    vkResetQueryPoolEXT =
        PFN_vkResetQueryPoolEXT(vkGetInstanceProcAddr(instance, "vkResetQueryPoolEXT"));
    if (!vkResetQueryPool) vkResetQueryPool = vkResetQueryPoolEXT;

    //=== VK_EXT_extended_dynamic_state ===
    vkCmdSetCullModeEXT =
        PFN_vkCmdSetCullModeEXT(vkGetInstanceProcAddr(instance, "vkCmdSetCullModeEXT"));
    if (!vkCmdSetCullMode) vkCmdSetCullMode = vkCmdSetCullModeEXT;
    vkCmdSetFrontFaceEXT =
        PFN_vkCmdSetFrontFaceEXT(vkGetInstanceProcAddr(instance, "vkCmdSetFrontFaceEXT"));
    ...
    vkCmdSetStencilOpEXT =
        PFN_vkCmdSetStencilOpEXT(vkGetInstanceProcAddr(instance, "vkCmdSetStencilOpEXT"));
    ...
    vkCopyMemoryToImageEXT =
        PFN_vkCopyMemoryToImageEXT(vkGetInstanceProcAddr(instance, "vkCopyMemoryToImageEXT"));
    if (!vkCopyMemoryToImage) vkCopyMemoryToImage = vkCopyMemoryToImageEXT;
    vkCopyImageToMemoryEXT =
        PFN_vkCopyImageToMemoryEXT(vkGetInstanceProcAddr(instance, "vkCopyImageToMemoryEXT"));
    if (!vkCopyImageToMemory) vkCopyImageToMemory = vkCopyImageToMemoryEXT;
    vkCopyImageToImageEXT =
        PFN_vkCopyImageToImageEXT(vkGetInstanceProcAddr(instance, "vkCopyImageToImageEXT"));
    ...
    if (!vkReleaseSwapchainImagesKHR) vkReleaseSwapchainImagesKHR = vkReleaseSwapchainImagesEXT;
    ...
    vkCmdSetDepthBias2EXT =
        PFN_vkCmdSetDepthBias2EXT(vkGetInstanceProcAddr(instance, "vkCmdSetDepthBias2EXT"));

    //=== VK_EXT_acquire_drm_display ===
    vkAcquireDrmDisplayEXT =
        PFN_vkAcquireDrmDisplayEXT(vkGetInstanceProcAddr(instance, "vkAcquireDrmDisplayEXT"));
    vkGetDrmDisplayEXT =
        PFN_vkGetDrmDisplayEXT(vkGetInstanceProcAddr(instance, "vkGetDrmDisplayEXT"));
    ...
    vkSetPrivateDataEXT =
        PFN_vkSetPrivateDataEXT(vkGetInstanceProcAddr(instance, "vkSetPrivateDataEXT"));
    if (!vkSetPrivateData) vkSetPrivateData = vkSetPrivateDataEXT;
    vkGetPrivateDataEXT =
        PFN_vkGetPrivateDataEXT(vkGetInstanceProcAddr(instance, "vkGetPrivateDataEXT"));
    ...
    vkGetEncodedVideoSessionParametersKHR = PFN_vkGetEncodedVideoSessionParametersKHR(
        vkGetInstanceProcAddr(instance, "vkGetEncodedVideoSessionParametersKHR"));
    vkCmdEncodeVideoKHR =
        PFN_vkCmdEncodeVideoKHR(vkGetInstanceProcAddr(instance, "vkCmdEncodeVideoKHR"));

#if defined(VK_ENABLE_BETA_EXTENSIONS)
    //=== VK_NV_cuda_kernel_launch ===
    vkCreateCudaModuleNV =
        PFN_vkCreateCudaModuleNV(vkGetInstanceProcAddr(instance, "vkCreateCudaModuleNV"));
    vkGetCudaModuleCacheNV =
        PFN_vkGetCudaModuleCacheNV(vkGetInstanceProcAddr(instance, "vkGetCudaModuleCacheNV"));
    vkCreateCudaFunctionNV =
        PFN_vkCreateCudaFunctionNV(vkGetInstanceProcAddr(instance, "vkCreateCudaFunctionNV"));
    vkDestroyCudaModuleNV =
        PFN_vkDestroyCudaModuleNV(vkGetInstanceProcAddr(instance, "vkDestroyCudaModuleNV"));
    vkDestroyCudaFunctionNV =
        PFN_vkDestroyCudaFunctionNV(vkGetInstanceProcAddr(instance, "vkDestroyCudaFunctionNV"));
    vkCmdCudaLaunchKernelNV =
        PFN_vkCmdCudaLaunchKernelNV(vkGetInstanceProcAddr(instance, "vkCmdCudaLaunchKernelNV"));
#endif /*VK_ENABLE_BETA_EXTENSIONS*/

    //=== VK_QCOM_tile_shading ===
    vkCmdDispatchTileQCOM =
        PFN_vkCmdDispatchTileQCOM(vkGetInstanceProcAddr(instance, "vkCmdDispatchTileQCOM"));
    vkCmdBeginPerTileExecutionQCOM = PFN_vkCmdBeginPerTileExecutionQCOM(
        vkGetInstanceProcAddr(instance, "vkCmdBeginPerTileExecutionQCOM"));
    vkCmdEndPerTileExecutionQCOM = PFN_vkCmdEndPerTileExecutionQCOM(
        vkGetInstanceProcAddr(instance, "vkCmdEndPerTileExecutionQCOM"));

#if defined(VK_USE_PLATFORM_METAL_EXT)
    //=== VK_EXT_metal_objects ===
    vkExportMetalObjectsEXT =
        PFN_vkExportMetalObjectsEXT(vkGetInstanceProcAddr(instance, "vkExportMetalObjectsEXT"));
#endif /*VK_USE_PLATFORM_METAL_EXT*/
    ...
    vkCmdResetEvent2KHR =
        PFN_vkCmdResetEvent2KHR(vkGetInstanceProcAddr(instance, "vkCmdResetEvent2KHR"));
    if (!vkCmdResetEvent2) vkCmdResetEvent2 = vkCmdResetEvent2KHR;
    vkCmdWaitEvents2KHR =
        PFN_vkCmdWaitEvents2KHR(vkGetInstanceProcAddr(instance, "vkCmdWaitEvents2KHR"));
    ...
    vkGetDescriptorEXT =
        PFN_vkGetDescriptorEXT(vkGetInstanceProcAddr(instance, "vkGetDescriptorEXT"));
    ...
    vkCmdBindDescriptorBufferEmbeddedSamplersEXT = PFN_vkCmdBindDescriptorBufferEmbeddedSamplersEXT(
        vkGetInstanceProcAddr(instance, "vkCmdBindDescriptorBufferEmbeddedSamplersEXT"));
    vkGetBufferOpaqueCaptureDescriptorDataEXT = PFN_vkGetBufferOpaqueCaptureDescriptorDataEXT(
        vkGetInstanceProcAddr(instance, "vkGetBufferOpaqueCaptureDescriptorDataEXT"));
    vkGetImageOpaqueCaptureDescriptorDataEXT = PFN_vkGetImageOpaqueCaptureDescriptorDataEXT(
        vkGetInstanceProcAddr(instance, "vkGetImageOpaqueCaptureDescriptorDataEXT"));
    vkGetImageViewOpaqueCaptureDescriptorDataEXT = PFN_vkGetImageViewOpaqueCaptureDescriptorDataEXT(
        vkGetInstanceProcAddr(instance, "vkGetImageViewOpaqueCaptureDescriptorDataEXT"));
    vkGetSamplerOpaqueCaptureDescriptorDataEXT = PFN_vkGetSamplerOpaqueCaptureDescriptorDataEXT(
        vkGetInstanceProcAddr(instance, "vkGetSamplerOpaqueCaptureDescriptorDataEXT"));
    vkGetAccelerationStructureOpaqueCaptureDescriptorDataEXT =
        PFN_vkGetAccelerationStructureOpaqueCaptureDescriptorDataEXT(vkGetInstanceProcAddr(
    ...
    vkCmdDrawMeshTasksEXT =
        PFN_vkCmdDrawMeshTasksEXT(vkGetInstanceProcAddr(instance, "vkCmdDrawMeshTasksEXT"));
    ...
    //=== VK_KHR_copy_commands2 ===
    vkCmdCopyBuffer2KHR =
        PFN_vkCmdCopyBuffer2KHR(vkGetInstanceProcAddr(instance, "vkCmdCopyBuffer2KHR"));
    if (!vkCmdCopyBuffer2) vkCmdCopyBuffer2 = vkCmdCopyBuffer2KHR;
    vkCmdCopyImage2KHR =
        PFN_vkCmdCopyImage2KHR(vkGetInstanceProcAddr(instance, "vkCmdCopyImage2KHR"));
    ...
    vkCmdBlitImage2KHR =
        PFN_vkCmdBlitImage2KHR(vkGetInstanceProcAddr(instance, "vkCmdBlitImage2KHR"));
    if (!vkCmdBlitImage2) vkCmdBlitImage2 = vkCmdBlitImage2KHR;
    vkCmdResolveImage2KHR =
        PFN_vkCmdResolveImage2KHR(vkGetInstanceProcAddr(instance, "vkCmdResolveImage2KHR"));
    if (!vkCmdResolveImage2) vkCmdResolveImage2 = vkCmdResolveImage2KHR;

    //=== VK_EXT_device_fault ===
    vkGetDeviceFaultInfoEXT =
        PFN_vkGetDeviceFaultInfoEXT(vkGetInstanceProcAddr(instance, "vkGetDeviceFaultInfoEXT"));
    ...
    vkGetWinrtDisplayNV =
        PFN_vkGetWinrtDisplayNV(vkGetInstanceProcAddr(instance, "vkGetWinrtDisplayNV"));
#endif /*VK_USE_PLATFORM_WIN32_KHR*/
    ...
    vkCmdSetLogicOpEXT =
        PFN_vkCmdSetLogicOpEXT(vkGetInstanceProcAddr(instance, "vkCmdSetLogicOpEXT"));
    ...
    //=== VK_EXT_opacity_micromap ===
    vkCreateMicromapEXT =
        PFN_vkCreateMicromapEXT(vkGetInstanceProcAddr(instance, "vkCreateMicromapEXT"));
    vkDestroyMicromapEXT =
        PFN_vkDestroyMicromapEXT(vkGetInstanceProcAddr(instance, "vkDestroyMicromapEXT"));
    vkCmdBuildMicromapsEXT =
        PFN_vkCmdBuildMicromapsEXT(vkGetInstanceProcAddr(instance, "vkCmdBuildMicromapsEXT"));
    vkBuildMicromapsEXT =
        PFN_vkBuildMicromapsEXT(vkGetInstanceProcAddr(instance, "vkBuildMicromapsEXT"));
    vkCopyMicromapEXT = PFN_vkCopyMicromapEXT(vkGetInstanceProcAddr(instance, "vkCopyMicromapEXT"));
    vkCopyMicromapToMemoryEXT =
        PFN_vkCopyMicromapToMemoryEXT(vkGetInstanceProcAddr(instance, "vkCopyMicromapToMemoryEXT"));
    vkCopyMemoryToMicromapEXT =
        PFN_vkCopyMemoryToMicromapEXT(vkGetInstanceProcAddr(instance, "vkCopyMemoryToMicromapEXT"));
    ...
    vkCmdCopyMicromapEXT =
        PFN_vkCmdCopyMicromapEXT(vkGetInstanceProcAddr(instance, "vkCmdCopyMicromapEXT"));
    ...
    vkGetDeviceMicromapCompatibilityEXT = PFN_vkGetDeviceMicromapCompatibilityEXT(
        vkGetInstanceProcAddr(instance, "vkGetDeviceMicromapCompatibilityEXT"));
    vkGetMicromapBuildSizesEXT = PFN_vkGetMicromapBuildSizesEXT(
        vkGetInstanceProcAddr(instance, "vkGetMicromapBuildSizesEXT"));

    //=== VK_HUAWEI_cluster_culling_shader ===
    vkCmdDrawClusterHUAWEI =
        PFN_vkCmdDrawClusterHUAWEI(vkGetInstanceProcAddr(instance, "vkCmdDrawClusterHUAWEI"));
    ...
    vkCmdCopyMemoryIndirectNV =
        PFN_vkCmdCopyMemoryIndirectNV(vkGetInstanceProcAddr(instance, "vkCmdCopyMemoryIndirectNV"));
    ...
    vkCmdDecompressMemoryNV =
        PFN_vkCmdDecompressMemoryNV(vkGetInstanceProcAddr(instance, "vkCmdDecompressMemoryNV"));
    vkCmdDecompressMemoryIndirectCountNV = PFN_vkCmdDecompressMemoryIndirectCountNV(
        vkGetInstanceProcAddr(instance, "vkCmdDecompressMemoryIndirectCountNV"));
    ...
    vkCmdUpdatePipelineIndirectBufferNV = PFN_vkCmdUpdatePipelineIndirectBufferNV(
        vkGetInstanceProcAddr(instance, "vkCmdUpdatePipelineIndirectBufferNV"));
    vkGetPipelineIndirectDeviceAddressNV = PFN_vkGetPipelineIndirectDeviceAddressNV(
        vkGetInstanceProcAddr(instance, "vkGetPipelineIndirectDeviceAddressNV"));
    ...
    vkCmdSetPolygonModeEXT =
        PFN_vkCmdSetPolygonModeEXT(vkGetInstanceProcAddr(instance, "vkCmdSetPolygonModeEXT"));
    ...
    vkCmdSetSampleMaskEXT =
        PFN_vkCmdSetSampleMaskEXT(vkGetInstanceProcAddr(instance, "vkCmdSetSampleMaskEXT"));
    ...
    vkCmdSetLogicOpEnableEXT =
        PFN_vkCmdSetLogicOpEnableEXT(vkGetInstanceProcAddr(instance, "vkCmdSetLogicOpEnableEXT"));
    ...
    vkCmdSetColorWriteMaskEXT =
        PFN_vkCmdSetColorWriteMaskEXT(vkGetInstanceProcAddr(instance, "vkCmdSetColorWriteMaskEXT"));
    vkCmdSetTessellationDomainOriginEXT = PFN_vkCmdSetTessellationDomainOriginEXT(
        vkGetInstanceProcAddr(instance, "vkCmdSetTessellationDomainOriginEXT"));
    ...
    vkCmdSetConservativeRasterizationModeEXT = PFN_vkCmdSetConservativeRasterizationModeEXT(
        vkGetInstanceProcAddr(instance, "vkCmdSetConservativeRasterizationModeEXT"));
    vkCmdSetExtraPrimitiveOverestimationSizeEXT = PFN_vkCmdSetExtraPrimitiveOverestimationSizeEXT(
        vkGetInstanceProcAddr(instance, "vkCmdSetExtraPrimitiveOverestimationSizeEXT"));
    ...
    vkCmdSetDepthClipNegativeOneToOneEXT = PFN_vkCmdSetDepthClipNegativeOneToOneEXT(
        vkGetInstanceProcAddr(instance, "vkCmdSetDepthClipNegativeOneToOneEXT"));
    ...
    vkCmdSetViewportSwizzleNV =
        PFN_vkCmdSetViewportSwizzleNV(vkGetInstanceProcAddr(instance, "vkCmdSetViewportSwizzleNV"));
    ...
    vkCmdSetRepresentativeFragmentTestEnableNV = PFN_vkCmdSetRepresentativeFragmentTestEnableNV(
        vkGetInstanceProcAddr(instance, "vkCmdSetRepresentativeFragmentTestEnableNV"));
    ...
    vkDestroyTensorARM =
        PFN_vkDestroyTensorARM(vkGetInstanceProcAddr(instance, "vkDestroyTensorARM"));
    vkCreateTensorViewARM =
        PFN_vkCreateTensorViewARM(vkGetInstanceProcAddr(instance, "vkCreateTensorViewARM"));
    vkDestroyTensorViewARM =
        PFN_vkDestroyTensorViewARM(vkGetInstanceProcAddr(instance, "vkDestroyTensorViewARM"));
    vkGetTensorMemoryRequirementsARM = PFN_vkGetTensorMemoryRequirementsARM(
        vkGetInstanceProcAddr(instance, "vkGetTensorMemoryRequirementsARM"));
    ...
    vkCmdCopyTensorARM =
        PFN_vkCmdCopyTensorARM(vkGetInstanceProcAddr(instance, "vkCmdCopyTensorARM"));
    vkGetPhysicalDeviceExternalTensorPropertiesARM =
        PFN_vkGetPhysicalDeviceExternalTensorPropertiesARM(
            vkGetInstanceProcAddr(instance, "vkGetPhysicalDeviceExternalTensorPropertiesARM"));
    vkGetTensorOpaqueCaptureDescriptorDataARM = PFN_vkGetTensorOpaqueCaptureDescriptorDataARM(
        vkGetInstanceProcAddr(instance, "vkGetTensorOpaqueCaptureDescriptorDataARM"));
    vkGetTensorViewOpaqueCaptureDescriptorDataARM =
        PFN_vkGetTensorViewOpaqueCaptureDescriptorDataARM(
            vkGetInstanceProcAddr(instance, "vkGetTensorViewOpaqueCaptureDescriptorDataARM"));
    ...
    vkGetShaderModuleCreateInfoIdentifierEXT = PFN_vkGetShaderModuleCreateInfoIdentifierEXT(
        vkGetInstanceProcAddr(instance, "vkGetShaderModuleCreateInfoIdentifierEXT"));
    ...
    vkCmdOpticalFlowExecuteNV =
        PFN_vkCmdOpticalFlowExecuteNV(vkGetInstanceProcAddr(instance, "vkCmdOpticalFlowExecuteNV"));

    //=== VK_KHR_maintenance5 ===
    vkCmdBindIndexBuffer2KHR =
        PFN_vkCmdBindIndexBuffer2KHR(vkGetInstanceProcAddr(instance, "vkCmdBindIndexBuffer2KHR"));
    ...
    vkGetDeviceImageSubresourceLayoutKHR = PFN_vkGetDeviceImageSubresourceLayoutKHR(
        vkGetInstanceProcAddr(instance, "vkGetDeviceImageSubresourceLayoutKHR"));
    ...
    //=== VK_AMD_anti_lag ===
    vkAntiLagUpdateAMD =
        PFN_vkAntiLagUpdateAMD(vkGetInstanceProcAddr(instance, "vkAntiLagUpdateAMD"));
    ...
    //=== VK_EXT_shader_object ===
    vkCreateShadersEXT =
        PFN_vkCreateShadersEXT(vkGetInstanceProcAddr(instance, "vkCreateShadersEXT"));
    vkDestroyShaderEXT =
        PFN_vkDestroyShaderEXT(vkGetInstanceProcAddr(instance, "vkDestroyShaderEXT"));
    vkGetShaderBinaryDataEXT =
        PFN_vkGetShaderBinaryDataEXT(vkGetInstanceProcAddr(instance, "vkGetShaderBinaryDataEXT"));
    vkCmdBindShadersEXT =
        PFN_vkCmdBindShadersEXT(vkGetInstanceProcAddr(instance, "vkCmdBindShadersEXT"));
    vkCmdSetDepthClampRangeEXT = PFN_vkCmdSetDepthClampRangeEXT(
        vkGetInstanceProcAddr(instance, "vkCmdSetDepthClampRangeEXT"));

    //=== VK_KHR_pipeline_binary ===
    vkCreatePipelineBinariesKHR = PFN_vkCreatePipelineBinariesKHR(
        vkGetInstanceProcAddr(instance, "vkCreatePipelineBinariesKHR"));
    vkDestroyPipelineBinaryKHR = PFN_vkDestroyPipelineBinaryKHR(
        vkGetInstanceProcAddr(instance, "vkDestroyPipelineBinaryKHR"));
    vkGetPipelineKeyKHR =
        PFN_vkGetPipelineKeyKHR(vkGetInstanceProcAddr(instance, "vkGetPipelineKeyKHR"));
    vkGetPipelineBinaryDataKHR = PFN_vkGetPipelineBinaryDataKHR(
        vkGetInstanceProcAddr(instance, "vkGetPipelineBinaryDataKHR"));
    vkReleaseCapturedPipelineDataKHR = PFN_vkReleaseCapturedPipelineDataKHR(
        vkGetInstanceProcAddr(instance, "vkReleaseCapturedPipelineDataKHR"));
    ...
    vkGetDynamicRenderingTilePropertiesQCOM = PFN_vkGetDynamicRenderingTilePropertiesQCOM(
        vkGetInstanceProcAddr(instance, "vkGetDynamicRenderingTilePropertiesQCOM"));
    ...
    vkSetLatencyMarkerNV =
        PFN_vkSetLatencyMarkerNV(vkGetInstanceProcAddr(instance, "vkSetLatencyMarkerNV"));
    vkGetLatencyTimingsNV =
        PFN_vkGetLatencyTimingsNV(vkGetInstanceProcAddr(instance, "vkGetLatencyTimingsNV"));
    vkQueueNotifyOutOfBandNV =
        PFN_vkQueueNotifyOutOfBandNV(vkGetInstanceProcAddr(instance, "vkQueueNotifyOutOfBandNV"));
    ...
    vkCreateDataGraphPipelinesARM = PFN_vkCreateDataGraphPipelinesARM(
        vkGetInstanceProcAddr(instance, "vkCreateDataGraphPipelinesARM"));
    vkCreateDataGraphPipelineSessionARM = PFN_vkCreateDataGraphPipelineSessionARM(
        vkGetInstanceProcAddr(instance, "vkCreateDataGraphPipelineSessionARM"));
    vkGetDataGraphPipelineSessionBindPointRequirementsARM =
        PFN_vkGetDataGraphPipelineSessionBindPointRequirementsARM(vkGetInstanceProcAddr(
            instance, "vkGetDataGraphPipelineSessionBindPointRequirementsARM"));
    vkGetDataGraphPipelineSessionMemoryRequirementsARM =
        PFN_vkGetDataGraphPipelineSessionMemoryRequirementsARM(
            vkGetInstanceProcAddr(instance, "vkGetDataGraphPipelineSessionMemoryRequirementsARM"));
    vkBindDataGraphPipelineSessionMemoryARM = PFN_vkBindDataGraphPipelineSessionMemoryARM(
        vkGetInstanceProcAddr(instance, "vkBindDataGraphPipelineSessionMemoryARM"));
    vkDestroyDataGraphPipelineSessionARM = PFN_vkDestroyDataGraphPipelineSessionARM(
        vkGetInstanceProcAddr(instance, "vkDestroyDataGraphPipelineSessionARM"));
    vkCmdDispatchDataGraphARM =
        PFN_vkCmdDispatchDataGraphARM(vkGetInstanceProcAddr(instance, "vkCmdDispatchDataGraphARM"));
    ...
    vkGetDataGraphPipelinePropertiesARM = PFN_vkGetDataGraphPipelinePropertiesARM(
        vkGetInstanceProcAddr(instance, "vkGetDataGraphPipelinePropertiesARM"));
    vkGetPhysicalDeviceQueueFamilyDataGraphPropertiesARM =
        PFN_vkGetPhysicalDeviceQueueFamilyDataGraphPropertiesARM(vkGetInstanceProcAddr(
    ...
    vkCmdSetAttachmentFeedbackLoopEnableEXT = PFN_vkCmdSetAttachmentFeedbackLoopEnableEXT(
        vkGetInstanceProcAddr(instance, "vkCmdSetAttachmentFeedbackLoopEnableEXT"));
    ...
    vkCmdSetLineStippleKHR =
        PFN_vkCmdSetLineStippleKHR(vkGetInstanceProcAddr(instance, "vkCmdSetLineStippleKHR"));
    ...
    vkCmdPushConstants2KHR =
        PFN_vkCmdPushConstants2KHR(vkGetInstanceProcAddr(instance, "vkCmdPushConstants2KHR"));
    ...
    vkCmdPushDescriptorSetWithTemplate2KHR = PFN_vkCmdPushDescriptorSetWithTemplate2KHR(
        vkGetInstanceProcAddr(instance, "vkCmdPushDescriptorSetWithTemplate2KHR"));
    ...
    vkCmdBindDescriptorBufferEmbeddedSamplers2EXT =
        PFN_vkCmdBindDescriptorBufferEmbeddedSamplers2EXT(
            vkGetInstanceProcAddr(instance, "vkCmdBindDescriptorBufferEmbeddedSamplers2EXT"));

    //=== VK_QCOM_tile_memory_heap ===
    vkCmdBindTileMemoryQCOM =
        PFN_vkCmdBindTileMemoryQCOM(vkGetInstanceProcAddr(instance, "vkCmdBindTileMemoryQCOM"));

    //=== VK_NV_external_compute_queue ===
    vkCreateExternalComputeQueueNV = PFN_vkCreateExternalComputeQueueNV(
        vkGetInstanceProcAddr(instance, "vkCreateExternalComputeQueueNV"));
    vkDestroyExternalComputeQueueNV = PFN_vkDestroyExternalComputeQueueNV(
        vkGetInstanceProcAddr(instance, "vkDestroyExternalComputeQueueNV"));
    vkGetExternalComputeQueueDataNV = PFN_vkGetExternalComputeQueueDataNV(
        vkGetInstanceProcAddr(instance, "vkGetExternalComputeQueueDataNV"));

    //=== VK_NV_cluster_acceleration_structure ===
    vkGetClusterAccelerationStructureBuildSizesNV =
        PFN_vkGetClusterAccelerationStructureBuildSizesNV(
            vkGetInstanceProcAddr(instance, "vkGetClusterAccelerationStructureBuildSizesNV"));
    vkCmdBuildClusterAccelerationStructureIndirectNV =
        PFN_vkCmdBuildClusterAccelerationStructureIndirectNV(
            vkGetInstanceProcAddr(instance, "vkCmdBuildClusterAccelerationStructureIndirectNV"));

    //=== VK_NV_partitioned_acceleration_structure ===
    vkGetPartitionedAccelerationStructuresBuildSizesNV =
        PFN_vkGetPartitionedAccelerationStructuresBuildSizesNV(
            vkGetInstanceProcAddr(instance, "vkGetPartitionedAccelerationStructuresBuildSizesNV"));
    vkCmdBuildPartitionedAccelerationStructuresNV =
        PFN_vkCmdBuildPartitionedAccelerationStructuresNV(
            vkGetInstanceProcAddr(instance, "vkCmdBuildPartitionedAccelerationStructuresNV"));
    ...
    vkCmdPreprocessGeneratedCommandsEXT = PFN_vkCmdPreprocessGeneratedCommandsEXT(
        vkGetInstanceProcAddr(instance, "vkCmdPreprocessGeneratedCommandsEXT"));
    vkCmdExecuteGeneratedCommandsEXT = PFN_vkCmdExecuteGeneratedCommandsEXT(
        vkGetInstanceProcAddr(instance, "vkCmdExecuteGeneratedCommandsEXT"));
    vkCreateIndirectCommandsLayoutEXT = PFN_vkCreateIndirectCommandsLayoutEXT(
        vkGetInstanceProcAddr(instance, "vkCreateIndirectCommandsLayoutEXT"));
    vkDestroyIndirectCommandsLayoutEXT = PFN_vkDestroyIndirectCommandsLayoutEXT(
        vkGetInstanceProcAddr(instance, "vkDestroyIndirectCommandsLayoutEXT"));
    vkCreateIndirectExecutionSetEXT = PFN_vkCreateIndirectExecutionSetEXT(
        vkGetInstanceProcAddr(instance, "vkCreateIndirectExecutionSetEXT"));
    vkDestroyIndirectExecutionSetEXT = PFN_vkDestroyIndirectExecutionSetEXT(
        vkGetInstanceProcAddr(instance, "vkDestroyIndirectExecutionSetEXT"));
    vkUpdateIndirectExecutionSetPipelineEXT = PFN_vkUpdateIndirectExecutionSetPipelineEXT(
        vkGetInstanceProcAddr(instance, "vkUpdateIndirectExecutionSetPipelineEXT"));
    vkUpdateIndirectExecutionSetShaderEXT = PFN_vkUpdateIndirectExecutionSetShaderEXT(
        vkGetInstanceProcAddr(instance, "vkUpdateIndirectExecutionSetShaderEXT"));
    ...
    vkGetMemoryMetalHandleEXT =
        PFN_vkGetMemoryMetalHandleEXT(vkGetInstanceProcAddr(instance, "vkGetMemoryMetalHandleEXT"));
    vkGetMemoryMetalHandlePropertiesEXT = PFN_vkGetMemoryMetalHandlePropertiesEXT(
        vkGetInstanceProcAddr(instance, "vkGetMemoryMetalHandlePropertiesEXT"));
#endif /*VK_USE_PLATFORM_METAL_EXT*/

    //=== VK_EXT_fragment_density_map_offset ===
    vkCmdEndRendering2EXT =
        PFN_vkCmdEndRendering2EXT(vkGetInstanceProcAddr(instance, "vkCmdEndRendering2EXT"));
  }
  ...
  void init(Device deviceCpp) VULKAN_HPP_NOEXCEPT {
    ...
    vkGetQueryPoolResults =
        PFN_vkGetQueryPoolResults(vkGetDeviceProcAddr(device, "vkGetQueryPoolResults"));
    vkCreateBuffer = PFN_vkCreateBuffer(vkGetDeviceProcAddr(device, "vkCreateBuffer"));
    vkDestroyBuffer = PFN_vkDestroyBuffer(vkGetDeviceProcAddr(device, "vkDestroyBuffer"));
    vkCreateBufferView = PFN_vkCreateBufferView(vkGetDeviceProcAddr(device, "vkCreateBufferView"));
    vkDestroyBufferView =
        PFN_vkDestroyBufferView(vkGetDeviceProcAddr(device, "vkDestroyBufferView"));
    ...
    vkCreatePipelineCache =
        PFN_vkCreatePipelineCache(vkGetDeviceProcAddr(device, "vkCreatePipelineCache"));
    vkDestroyPipelineCache =
        PFN_vkDestroyPipelineCache(vkGetDeviceProcAddr(device, "vkDestroyPipelineCache"));
    vkGetPipelineCacheData =
        PFN_vkGetPipelineCacheData(vkGetDeviceProcAddr(device, "vkGetPipelineCacheData"));
    vkMergePipelineCaches =
        PFN_vkMergePipelineCaches(vkGetDeviceProcAddr(device, "vkMergePipelineCaches"));
    vkCreateGraphicsPipelines =
    ...
    vkResetDescriptorPool =
        PFN_vkResetDescriptorPool(vkGetDeviceProcAddr(device, "vkResetDescriptorPool"));
    vkAllocateDescriptorSets =
        PFN_vkAllocateDescriptorSets(vkGetDeviceProcAddr(device, "vkAllocateDescriptorSets"));
    vkFreeDescriptorSets =
        PFN_vkFreeDescriptorSets(vkGetDeviceProcAddr(device, "vkFreeDescriptorSets"));
    vkUpdateDescriptorSets =
    ...
    vkResetCommandBuffer =
        PFN_vkResetCommandBuffer(vkGetDeviceProcAddr(device, "vkResetCommandBuffer"));
    ...
    vkCmdSetBlendConstants =
        PFN_vkCmdSetBlendConstants(vkGetDeviceProcAddr(device, "vkCmdSetBlendConstants"));
    vkCmdSetDepthBounds =
    ...
    vkCmdDispatchIndirect =
        PFN_vkCmdDispatchIndirect(vkGetDeviceProcAddr(device, "vkCmdDispatchIndirect"));
    vkCmdCopyBuffer = PFN_vkCmdCopyBuffer(vkGetDeviceProcAddr(device, "vkCmdCopyBuffer"));
    vkCmdCopyImage = PFN_vkCmdCopyImage(vkGetDeviceProcAddr(device, "vkCmdCopyImage"));
    vkCmdBlitImage = PFN_vkCmdBlitImage(vkGetDeviceProcAddr(device, "vkCmdBlitImage"));
    vkCmdCopyBufferToImage =
        PFN_vkCmdCopyBufferToImage(vkGetDeviceProcAddr(device, "vkCmdCopyBufferToImage"));
    vkCmdCopyImageToBuffer =
        PFN_vkCmdCopyImageToBuffer(vkGetDeviceProcAddr(device, "vkCmdCopyImageToBuffer"));
    vkCmdUpdateBuffer = PFN_vkCmdUpdateBuffer(vkGetDeviceProcAddr(device, "vkCmdUpdateBuffer"));
    vkCmdFillBuffer = PFN_vkCmdFillBuffer(vkGetDeviceProcAddr(device, "vkCmdFillBuffer"));
    vkCmdClearColorImage =
        PFN_vkCmdClearColorImage(vkGetDeviceProcAddr(device, "vkCmdClearColorImage"));
    vkCmdClearDepthStencilImage =
        PFN_vkCmdClearDepthStencilImage(vkGetDeviceProcAddr(device, "vkCmdClearDepthStencilImage"));
    vkCmdClearAttachments =
        PFN_vkCmdClearAttachments(vkGetDeviceProcAddr(device, "vkCmdClearAttachments"));
    vkCmdResolveImage = PFN_vkCmdResolveImage(vkGetDeviceProcAddr(device, "vkCmdResolveImage"));
    vkCmdSetEvent = PFN_vkCmdSetEvent(vkGetDeviceProcAddr(device, "vkCmdSetEvent"));
    vkCmdResetEvent = PFN_vkCmdResetEvent(vkGetDeviceProcAddr(device, "vkCmdResetEvent"));
    vkCmdWaitEvents = PFN_vkCmdWaitEvents(vkGetDeviceProcAddr(device, "vkCmdWaitEvents"));
    vkCmdPipelineBarrier =
        PFN_vkCmdPipelineBarrier(vkGetDeviceProcAddr(device, "vkCmdPipelineBarrier"));
    vkCmdBeginQuery = PFN_vkCmdBeginQuery(vkGetDeviceProcAddr(device, "vkCmdBeginQuery"));
    vkCmdEndQuery = PFN_vkCmdEndQuery(vkGetDeviceProcAddr(device, "vkCmdEndQuery"));
    vkCmdResetQueryPool =
        PFN_vkCmdResetQueryPool(vkGetDeviceProcAddr(device, "vkCmdResetQueryPool"));
    vkCmdWriteTimestamp =
        PFN_vkCmdWriteTimestamp(vkGetDeviceProcAddr(device, "vkCmdWriteTimestamp"));
    vkCmdCopyQueryPoolResults =
    ...
    vkCmdExecuteCommands =
        PFN_vkCmdExecuteCommands(vkGetDeviceProcAddr(device, "vkCmdExecuteCommands"));
    ...
    //=== VK_VERSION_1_2 ===
    vkCmdDrawIndirectCount =
        PFN_vkCmdDrawIndirectCount(vkGetDeviceProcAddr(device, "vkCmdDrawIndirectCount"));
    vkCmdDrawIndexedIndirectCount = PFN_vkCmdDrawIndexedIndirectCount(
        vkGetDeviceProcAddr(device, "vkCmdDrawIndexedIndirectCount"));
    vkCreateRenderPass2 =
        PFN_vkCreateRenderPass2(vkGetDeviceProcAddr(device, "vkCreateRenderPass2"));
    vkCmdBeginRenderPass2 =
        PFN_vkCmdBeginRenderPass2(vkGetDeviceProcAddr(device, "vkCmdBeginRenderPass2"));
    vkCmdNextSubpass2 = PFN_vkCmdNextSubpass2(vkGetDeviceProcAddr(device, "vkCmdNextSubpass2"));
    vkCmdEndRenderPass2 =
        PFN_vkCmdEndRenderPass2(vkGetDeviceProcAddr(device, "vkCmdEndRenderPass2"));
    ...
    vkCmdWriteTimestamp2 =
        PFN_vkCmdWriteTimestamp2(vkGetDeviceProcAddr(device, "vkCmdWriteTimestamp2"));
    ...
    vkCmdSetDepthCompareOp =
        PFN_vkCmdSetDepthCompareOp(vkGetDeviceProcAddr(device, "vkCmdSetDepthCompareOp"));
    vkCmdSetDepthBoundsTestEnable = PFN_vkCmdSetDepthBoundsTestEnable(
    ...
    //=== VK_VERSION_1_4 ===
    vkCmdSetLineStipple =
        PFN_vkCmdSetLineStipple(vkGetDeviceProcAddr(device, "vkCmdSetLineStipple"));
    ...
    vkCmdBindIndexBuffer2 =
        PFN_vkCmdBindIndexBuffer2(vkGetDeviceProcAddr(device, "vkCmdBindIndexBuffer2"));
    vkGetRenderingAreaGranularity = PFN_vkGetRenderingAreaGranularity(
        vkGetDeviceProcAddr(device, "vkGetRenderingAreaGranularity"));
    vkGetDeviceImageSubresourceLayout = PFN_vkGetDeviceImageSubresourceLayout(
        vkGetDeviceProcAddr(device, "vkGetDeviceImageSubresourceLayout"));
    vkGetImageSubresourceLayout2 = PFN_vkGetImageSubresourceLayout2(
        vkGetDeviceProcAddr(device, "vkGetImageSubresourceLayout2"));
    vkCmdPushDescriptorSet =
        PFN_vkCmdPushDescriptorSet(vkGetDeviceProcAddr(device, "vkCmdPushDescriptorSet"));
    vkCmdPushDescriptorSetWithTemplate = PFN_vkCmdPushDescriptorSetWithTemplate(
        vkGetDeviceProcAddr(device, "vkCmdPushDescriptorSetWithTemplate"));
    vkCmdSetRenderingAttachmentLocations = PFN_vkCmdSetRenderingAttachmentLocations(
        vkGetDeviceProcAddr(device, "vkCmdSetRenderingAttachmentLocations"));
    vkCmdSetRenderingInputAttachmentIndices = PFN_vkCmdSetRenderingInputAttachmentIndices(
        vkGetDeviceProcAddr(device, "vkCmdSetRenderingInputAttachmentIndices"));
    vkCmdBindDescriptorSets2 =
        PFN_vkCmdBindDescriptorSets2(vkGetDeviceProcAddr(device, "vkCmdBindDescriptorSets2"));
    vkCmdPushConstants2 =
        PFN_vkCmdPushConstants2(vkGetDeviceProcAddr(device, "vkCmdPushConstants2"));
    vkCmdPushDescriptorSet2 =
        PFN_vkCmdPushDescriptorSet2(vkGetDeviceProcAddr(device, "vkCmdPushDescriptorSet2"));
    vkCmdPushDescriptorSetWithTemplate2 = PFN_vkCmdPushDescriptorSetWithTemplate2(
        vkGetDeviceProcAddr(device, "vkCmdPushDescriptorSetWithTemplate2"));
    vkCopyMemoryToImage =
        PFN_vkCopyMemoryToImage(vkGetDeviceProcAddr(device, "vkCopyMemoryToImage"));
    vkCopyImageToMemory =
        PFN_vkCopyImageToMemory(vkGetDeviceProcAddr(device, "vkCopyImageToMemory"));
    vkCopyImageToImage = PFN_vkCopyImageToImage(vkGetDeviceProcAddr(device, "vkCopyImageToImage"));
    vkTransitionImageLayout =
        PFN_vkTransitionImageLayout(vkGetDeviceProcAddr(device, "vkTransitionImageLayout"));
    ...
    //=== VK_NVX_binary_import ===
    vkCreateCuModuleNVX =
        PFN_vkCreateCuModuleNVX(vkGetDeviceProcAddr(device, "vkCreateCuModuleNVX"));
    vkCreateCuFunctionNVX =
        PFN_vkCreateCuFunctionNVX(vkGetDeviceProcAddr(device, "vkCreateCuFunctionNVX"));
    vkDestroyCuModuleNVX =
        PFN_vkDestroyCuModuleNVX(vkGetDeviceProcAddr(device, "vkDestroyCuModuleNVX"));
    vkDestroyCuFunctionNVX =
        PFN_vkDestroyCuFunctionNVX(vkGetDeviceProcAddr(device, "vkDestroyCuFunctionNVX"));
    vkCmdCuLaunchKernelNVX =
    ...
    vkGetImageViewHandle64NVX =
        PFN_vkGetImageViewHandle64NVX(vkGetDeviceProcAddr(device, "vkGetImageViewHandle64NVX"));
    vkGetImageViewAddressNVX =
    ...
    if (!vkGetDeviceGroupPeerMemoryFeatures)
      vkGetDeviceGroupPeerMemoryFeatures = vkGetDeviceGroupPeerMemoryFeaturesKHR;
    vkCmdSetDeviceMaskKHR =
        PFN_vkCmdSetDeviceMaskKHR(vkGetDeviceProcAddr(device, "vkCmdSetDeviceMaskKHR"));
    if (!vkCmdSetDeviceMask) vkCmdSetDeviceMask = vkCmdSetDeviceMaskKHR;
    vkCmdDispatchBaseKHR =
        PFN_vkCmdDispatchBaseKHR(vkGetDeviceProcAddr(device, "vkCmdDispatchBaseKHR"));
    if (!vkCmdDispatchBase) vkCmdDispatchBase = vkCmdDispatchBaseKHR;
    ...
    if (!vkCmdNextSubpass2) vkCmdNextSubpass2 = vkCmdNextSubpass2KHR;
    vkCmdEndRenderPass2KHR =
        PFN_vkCmdEndRenderPass2KHR(vkGetDeviceProcAddr(device, "vkCmdEndRenderPass2KHR"));
    if (!vkCmdEndRenderPass2) vkCmdEndRenderPass2 = vkCmdEndRenderPass2KHR;
    ...
    vkGetExecutionGraphPipelineNodeIndexAMDX = PFN_vkGetExecutionGraphPipelineNodeIndexAMDX(
        vkGetDeviceProcAddr(device, "vkGetExecutionGraphPipelineNodeIndexAMDX"));
    vkCmdInitializeGraphScratchMemoryAMDX = PFN_vkCmdInitializeGraphScratchMemoryAMDX(
        vkGetDeviceProcAddr(device, "vkCmdInitializeGraphScratchMemoryAMDX"));
    vkCmdDispatchGraphAMDX =
        PFN_vkCmdDispatchGraphAMDX(vkGetDeviceProcAddr(device, "vkCmdDispatchGraphAMDX"));
    vkCmdDispatchGraphIndirectAMDX = PFN_vkCmdDispatchGraphIndirectAMDX(
    ...
    vkCompileDeferredNV =
        PFN_vkCompileDeferredNV(vkGetDeviceProcAddr(device, "vkCompileDeferredNV"));
    ...
    //=== VK_NV_device_diagnostic_checkpoints ===
    vkCmdSetCheckpointNV =
        PFN_vkCmdSetCheckpointNV(vkGetDeviceProcAddr(device, "vkCmdSetCheckpointNV"));
    ...
    if (!vkWaitSemaphores) vkWaitSemaphores = vkWaitSemaphoresKHR;
    vkSignalSemaphoreKHR =
        PFN_vkSignalSemaphoreKHR(vkGetDeviceProcAddr(device, "vkSignalSemaphoreKHR"));
    if (!vkSignalSemaphore) vkSignalSemaphore = vkSignalSemaphoreKHR;
    ...
    //=== VK_AMD_display_native_hdr ===
    vkSetLocalDimmingAMD =
        PFN_vkSetLocalDimmingAMD(vkGetDeviceProcAddr(device, "vkSetLocalDimmingAMD"));
    ...
    if (!vkCmdSetRenderingAttachmentLocations)
      vkCmdSetRenderingAttachmentLocations = vkCmdSetRenderingAttachmentLocationsKHR;
    vkCmdSetRenderingInputAttachmentIndicesKHR = PFN_vkCmdSetRenderingInputAttachmentIndicesKHR(
        vkGetDeviceProcAddr(device, "vkCmdSetRenderingInputAttachmentIndicesKHR"));
    if (!vkCmdSetRenderingInputAttachmentIndices)
      vkCmdSetRenderingInputAttachmentIndices = vkCmdSetRenderingInputAttachmentIndicesKHR;
    ...
    //=== VK_EXT_line_rasterization ===
    vkCmdSetLineStippleEXT =
        PFN_vkCmdSetLineStippleEXT(vkGetDeviceProcAddr(device, "vkCmdSetLineStippleEXT"));
    if (!vkCmdSetLineStipple) vkCmdSetLineStipple = vkCmdSetLineStippleEXT;

    //=== VK_EXT_host_query_reset ===
    vkResetQueryPoolEXT =
        PFN_vkResetQueryPoolEXT(vkGetDeviceProcAddr(device, "vkResetQueryPoolEXT"));
    if (!vkResetQueryPool) vkResetQueryPool = vkResetQueryPoolEXT;

    //=== VK_EXT_extended_dynamic_state ===
    vkCmdSetCullModeEXT =
        PFN_vkCmdSetCullModeEXT(vkGetDeviceProcAddr(device, "vkCmdSetCullModeEXT"));
    if (!vkCmdSetCullMode) vkCmdSetCullMode = vkCmdSetCullModeEXT;
    vkCmdSetFrontFaceEXT =
        PFN_vkCmdSetFrontFaceEXT(vkGetDeviceProcAddr(device, "vkCmdSetFrontFaceEXT"));
    if (!vkCmdSetFrontFace) vkCmdSetFrontFace = vkCmdSetFrontFaceEXT;
    vkCmdSetPrimitiveTopologyEXT = PFN_vkCmdSetPrimitiveTopologyEXT(
    ...
    if (!vkCmdSetStencilTestEnable) vkCmdSetStencilTestEnable = vkCmdSetStencilTestEnableEXT;
    vkCmdSetStencilOpEXT =
        PFN_vkCmdSetStencilOpEXT(vkGetDeviceProcAddr(device, "vkCmdSetStencilOpEXT"));
    if (!vkCmdSetStencilOp) vkCmdSetStencilOp = vkCmdSetStencilOpEXT;
    ...
    //=== VK_EXT_host_image_copy ===
    vkCopyMemoryToImageEXT =
        PFN_vkCopyMemoryToImageEXT(vkGetDeviceProcAddr(device, "vkCopyMemoryToImageEXT"));
    if (!vkCopyMemoryToImage) vkCopyMemoryToImage = vkCopyMemoryToImageEXT;
    vkCopyImageToMemoryEXT =
        PFN_vkCopyImageToMemoryEXT(vkGetDeviceProcAddr(device, "vkCopyImageToMemoryEXT"));
    if (!vkCopyImageToMemory) vkCopyImageToMemory = vkCopyImageToMemoryEXT;
    vkCopyImageToImageEXT =
        PFN_vkCopyImageToImageEXT(vkGetDeviceProcAddr(device, "vkCopyImageToImageEXT"));
    if (!vkCopyImageToImage) vkCopyImageToImage = vkCopyImageToImageEXT;
    vkTransitionImageLayoutEXT =
        PFN_vkTransitionImageLayoutEXT(vkGetDeviceProcAddr(device, "vkTransitionImageLayoutEXT"));
    if (!vkTransitionImageLayout) vkTransitionImageLayout = vkTransitionImageLayoutEXT;
    vkGetImageSubresourceLayout2EXT = PFN_vkGetImageSubresourceLayout2EXT(
    ...
    vkReleaseSwapchainImagesEXT =
        PFN_vkReleaseSwapchainImagesEXT(vkGetDeviceProcAddr(device, "vkReleaseSwapchainImagesEXT"));
    if (!vkReleaseSwapchainImagesKHR) vkReleaseSwapchainImagesKHR = vkReleaseSwapchainImagesEXT;
    ...
    //=== VK_EXT_depth_bias_control ===
    vkCmdSetDepthBias2EXT =
        PFN_vkCmdSetDepthBias2EXT(vkGetDeviceProcAddr(device, "vkCmdSetDepthBias2EXT"));
    ...
    if (!vkDestroyPrivateDataSlot) vkDestroyPrivateDataSlot = vkDestroyPrivateDataSlotEXT;
    vkSetPrivateDataEXT =
        PFN_vkSetPrivateDataEXT(vkGetDeviceProcAddr(device, "vkSetPrivateDataEXT"));
    if (!vkSetPrivateData) vkSetPrivateData = vkSetPrivateDataEXT;
    vkGetPrivateDataEXT =
        PFN_vkGetPrivateDataEXT(vkGetDeviceProcAddr(device, "vkGetPrivateDataEXT"));
    if (!vkGetPrivateData) vkGetPrivateData = vkGetPrivateDataEXT;

    //=== VK_KHR_video_encode_queue ===
    vkGetEncodedVideoSessionParametersKHR = PFN_vkGetEncodedVideoSessionParametersKHR(
        vkGetDeviceProcAddr(device, "vkGetEncodedVideoSessionParametersKHR"));
    vkCmdEncodeVideoKHR =
    ...
    //=== VK_NV_cuda_kernel_launch ===
    vkCreateCudaModuleNV =
        PFN_vkCreateCudaModuleNV(vkGetDeviceProcAddr(device, "vkCreateCudaModuleNV"));
    vkGetCudaModuleCacheNV =
        PFN_vkGetCudaModuleCacheNV(vkGetDeviceProcAddr(device, "vkGetCudaModuleCacheNV"));
    vkCreateCudaFunctionNV =
        PFN_vkCreateCudaFunctionNV(vkGetDeviceProcAddr(device, "vkCreateCudaFunctionNV"));
    vkDestroyCudaModuleNV =
        PFN_vkDestroyCudaModuleNV(vkGetDeviceProcAddr(device, "vkDestroyCudaModuleNV"));
    vkDestroyCudaFunctionNV =
        PFN_vkDestroyCudaFunctionNV(vkGetDeviceProcAddr(device, "vkDestroyCudaFunctionNV"));
    vkCmdCudaLaunchKernelNV =
        PFN_vkCmdCudaLaunchKernelNV(vkGetDeviceProcAddr(device, "vkCmdCudaLaunchKernelNV"));
#endif /*VK_ENABLE_BETA_EXTENSIONS*/

    //=== VK_QCOM_tile_shading ===
    vkCmdDispatchTileQCOM =
        PFN_vkCmdDispatchTileQCOM(vkGetDeviceProcAddr(device, "vkCmdDispatchTileQCOM"));
    vkCmdBeginPerTileExecutionQCOM = PFN_vkCmdBeginPerTileExecutionQCOM(
        vkGetDeviceProcAddr(device, "vkCmdBeginPerTileExecutionQCOM"));
    vkCmdEndPerTileExecutionQCOM = PFN_vkCmdEndPerTileExecutionQCOM(
    ...
    if (!vkCmdSetEvent2) vkCmdSetEvent2 = vkCmdSetEvent2KHR;
    vkCmdResetEvent2KHR =
        PFN_vkCmdResetEvent2KHR(vkGetDeviceProcAddr(device, "vkCmdResetEvent2KHR"));
    if (!vkCmdResetEvent2) vkCmdResetEvent2 = vkCmdResetEvent2KHR;
    vkCmdWaitEvents2KHR =
        PFN_vkCmdWaitEvents2KHR(vkGetDeviceProcAddr(device, "vkCmdWaitEvents2KHR"));
    if (!vkCmdWaitEvents2) vkCmdWaitEvents2 = vkCmdWaitEvents2KHR;
    vkCmdPipelineBarrier2KHR =
    ...
    vkCmdBindDescriptorBufferEmbeddedSamplersEXT = PFN_vkCmdBindDescriptorBufferEmbeddedSamplersEXT(
        vkGetDeviceProcAddr(device, "vkCmdBindDescriptorBufferEmbeddedSamplersEXT"));
    vkGetBufferOpaqueCaptureDescriptorDataEXT = PFN_vkGetBufferOpaqueCaptureDescriptorDataEXT(
        vkGetDeviceProcAddr(device, "vkGetBufferOpaqueCaptureDescriptorDataEXT"));
    vkGetImageOpaqueCaptureDescriptorDataEXT = PFN_vkGetImageOpaqueCaptureDescriptorDataEXT(
        vkGetDeviceProcAddr(device, "vkGetImageOpaqueCaptureDescriptorDataEXT"));
    vkGetImageViewOpaqueCaptureDescriptorDataEXT = PFN_vkGetImageViewOpaqueCaptureDescriptorDataEXT(
        vkGetDeviceProcAddr(device, "vkGetImageViewOpaqueCaptureDescriptorDataEXT"));
    vkGetSamplerOpaqueCaptureDescriptorDataEXT = PFN_vkGetSamplerOpaqueCaptureDescriptorDataEXT(
        vkGetDeviceProcAddr(device, "vkGetSamplerOpaqueCaptureDescriptorDataEXT"));
    vkGetAccelerationStructureOpaqueCaptureDescriptorDataEXT =
        PFN_vkGetAccelerationStructureOpaqueCaptureDescriptorDataEXT(vkGetDeviceProcAddr(
    ...
    //=== VK_KHR_copy_commands2 ===
    vkCmdCopyBuffer2KHR =
        PFN_vkCmdCopyBuffer2KHR(vkGetDeviceProcAddr(device, "vkCmdCopyBuffer2KHR"));
    if (!vkCmdCopyBuffer2) vkCmdCopyBuffer2 = vkCmdCopyBuffer2KHR;
    ...
    if (!vkCmdBlitImage2) vkCmdBlitImage2 = vkCmdBlitImage2KHR;
    vkCmdResolveImage2KHR =
        PFN_vkCmdResolveImage2KHR(vkGetDeviceProcAddr(device, "vkCmdResolveImage2KHR"));
    if (!vkCmdResolveImage2) vkCmdResolveImage2 = vkCmdResolveImage2KHR;
    ...
    vkCmdTraceRaysIndirect2KHR =
        PFN_vkCmdTraceRaysIndirect2KHR(vkGetDeviceProcAddr(device, "vkCmdTraceRaysIndirect2KHR"));
    ...
    vkDestroyMicromapEXT =
        PFN_vkDestroyMicromapEXT(vkGetDeviceProcAddr(device, "vkDestroyMicromapEXT"));
    vkCmdBuildMicromapsEXT =
        PFN_vkCmdBuildMicromapsEXT(vkGetDeviceProcAddr(device, "vkCmdBuildMicromapsEXT"));
    vkBuildMicromapsEXT =
        PFN_vkBuildMicromapsEXT(vkGetDeviceProcAddr(device, "vkBuildMicromapsEXT"));
    vkCopyMicromapEXT = PFN_vkCopyMicromapEXT(vkGetDeviceProcAddr(device, "vkCopyMicromapEXT"));
    vkCopyMicromapToMemoryEXT =
        PFN_vkCopyMicromapToMemoryEXT(vkGetDeviceProcAddr(device, "vkCopyMicromapToMemoryEXT"));
    vkCopyMemoryToMicromapEXT =
        PFN_vkCopyMemoryToMicromapEXT(vkGetDeviceProcAddr(device, "vkCopyMemoryToMicromapEXT"));
    vkWriteMicromapsPropertiesEXT = PFN_vkWriteMicromapsPropertiesEXT(
        vkGetDeviceProcAddr(device, "vkWriteMicromapsPropertiesEXT"));
    vkCmdCopyMicromapEXT =
        PFN_vkCmdCopyMicromapEXT(vkGetDeviceProcAddr(device, "vkCmdCopyMicromapEXT"));
    vkCmdCopyMicromapToMemoryEXT = PFN_vkCmdCopyMicromapToMemoryEXT(
    ...
    //=== VK_HUAWEI_cluster_culling_shader ===
    vkCmdDrawClusterHUAWEI =
        PFN_vkCmdDrawClusterHUAWEI(vkGetDeviceProcAddr(device, "vkCmdDrawClusterHUAWEI"));
    vkCmdDrawClusterIndirectHUAWEI = PFN_vkCmdDrawClusterIndirectHUAWEI(
    ...
    //=== VK_VALVE_descriptor_set_host_mapping ===
    vkGetDescriptorSetLayoutHostMappingInfoVALVE = PFN_vkGetDescriptorSetLayoutHostMappingInfoVALVE(
        vkGetDeviceProcAddr(device, "vkGetDescriptorSetLayoutHostMappingInfoVALVE"));
    vkGetDescriptorSetHostMappingVALVE = PFN_vkGetDescriptorSetHostMappingVALVE(
        vkGetDeviceProcAddr(device, "vkGetDescriptorSetHostMappingVALVE"));

    //=== VK_NV_copy_memory_indirect ===
    vkCmdCopyMemoryIndirectNV =
        PFN_vkCmdCopyMemoryIndirectNV(vkGetDeviceProcAddr(device, "vkCmdCopyMemoryIndirectNV"));
    vkCmdCopyMemoryToImageIndirectNV = PFN_vkCmdCopyMemoryToImageIndirectNV(
        vkGetDeviceProcAddr(device, "vkCmdCopyMemoryToImageIndirectNV"));

    //=== VK_NV_memory_decompression ===
    vkCmdDecompressMemoryNV =
        PFN_vkCmdDecompressMemoryNV(vkGetDeviceProcAddr(device, "vkCmdDecompressMemoryNV"));
    vkCmdDecompressMemoryIndirectCountNV = PFN_vkCmdDecompressMemoryIndirectCountNV(
        vkGetDeviceProcAddr(device, "vkCmdDecompressMemoryIndirectCountNV"));

    //=== VK_NV_device_generated_commands_compute ===
    vkGetPipelineIndirectMemoryRequirementsNV = PFN_vkGetPipelineIndirectMemoryRequirementsNV(
        vkGetDeviceProcAddr(device, "vkGetPipelineIndirectMemoryRequirementsNV"));
    vkCmdUpdatePipelineIndirectBufferNV = PFN_vkCmdUpdatePipelineIndirectBufferNV(
        vkGetDeviceProcAddr(device, "vkCmdUpdatePipelineIndirectBufferNV"));
    vkGetPipelineIndirectDeviceAddressNV = PFN_vkGetPipelineIndirectDeviceAddressNV(
        vkGetDeviceProcAddr(device, "vkGetPipelineIndirectDeviceAddressNV"));

    //=== VK_EXT_extended_dynamic_state3 ===
    vkCmdSetDepthClampEnableEXT =
        PFN_vkCmdSetDepthClampEnableEXT(vkGetDeviceProcAddr(device, "vkCmdSetDepthClampEnableEXT"));
    vkCmdSetPolygonModeEXT =
    ...
    vkCmdSetAlphaToOneEnableEXT =
        PFN_vkCmdSetAlphaToOneEnableEXT(vkGetDeviceProcAddr(device, "vkCmdSetAlphaToOneEnableEXT"));
    vkCmdSetLogicOpEnableEXT =
        PFN_vkCmdSetLogicOpEnableEXT(vkGetDeviceProcAddr(device, "vkCmdSetLogicOpEnableEXT"));
    ...
    vkCmdSetConservativeRasterizationModeEXT = PFN_vkCmdSetConservativeRasterizationModeEXT(
        vkGetDeviceProcAddr(device, "vkCmdSetConservativeRasterizationModeEXT"));
    vkCmdSetExtraPrimitiveOverestimationSizeEXT = PFN_vkCmdSetExtraPrimitiveOverestimationSizeEXT(
        vkGetDeviceProcAddr(device, "vkCmdSetExtraPrimitiveOverestimationSizeEXT"));
    vkCmdSetDepthClipEnableEXT =
        PFN_vkCmdSetDepthClipEnableEXT(vkGetDeviceProcAddr(device, "vkCmdSetDepthClipEnableEXT"));
    vkCmdSetSampleLocationsEnableEXT = PFN_vkCmdSetSampleLocationsEnableEXT(
    ...
    vkCmdSetDepthClipNegativeOneToOneEXT = PFN_vkCmdSetDepthClipNegativeOneToOneEXT(
        vkGetDeviceProcAddr(device, "vkCmdSetDepthClipNegativeOneToOneEXT"));
    vkCmdSetViewportWScalingEnableNV = PFN_vkCmdSetViewportWScalingEnableNV(
        vkGetDeviceProcAddr(device, "vkCmdSetViewportWScalingEnableNV"));
    vkCmdSetViewportSwizzleNV =
        PFN_vkCmdSetViewportSwizzleNV(vkGetDeviceProcAddr(device, "vkCmdSetViewportSwizzleNV"));
    vkCmdSetCoverageToColorEnableNV = PFN_vkCmdSetCoverageToColorEnableNV(
    ...
    vkCmdSetCoverageModulationTableEnableNV = PFN_vkCmdSetCoverageModulationTableEnableNV(
        vkGetDeviceProcAddr(device, "vkCmdSetCoverageModulationTableEnableNV"));
    vkCmdSetCoverageModulationTableNV = PFN_vkCmdSetCoverageModulationTableNV(
    ...
    vkCreateTensorViewARM =
        PFN_vkCreateTensorViewARM(vkGetDeviceProcAddr(device, "vkCreateTensorViewARM"));
    vkDestroyTensorViewARM =
        PFN_vkDestroyTensorViewARM(vkGetDeviceProcAddr(device, "vkDestroyTensorViewARM"));
    vkGetTensorMemoryRequirementsARM = PFN_vkGetTensorMemoryRequirementsARM(
        vkGetDeviceProcAddr(device, "vkGetTensorMemoryRequirementsARM"));
    vkBindTensorMemoryARM =
        PFN_vkBindTensorMemoryARM(vkGetDeviceProcAddr(device, "vkBindTensorMemoryARM"));
    vkGetDeviceTensorMemoryRequirementsARM = PFN_vkGetDeviceTensorMemoryRequirementsARM(
    ...
    vkGetTensorOpaqueCaptureDescriptorDataARM = PFN_vkGetTensorOpaqueCaptureDescriptorDataARM(
        vkGetDeviceProcAddr(device, "vkGetTensorOpaqueCaptureDescriptorDataARM"));
    vkGetTensorViewOpaqueCaptureDescriptorDataARM =
        PFN_vkGetTensorViewOpaqueCaptureDescriptorDataARM(
            vkGetDeviceProcAddr(device, "vkGetTensorViewOpaqueCaptureDescriptorDataARM"));
    ...
    vkCmdOpticalFlowExecuteNV =
        PFN_vkCmdOpticalFlowExecuteNV(vkGetDeviceProcAddr(device, "vkCmdOpticalFlowExecuteNV"));

    //=== VK_KHR_maintenance5 ===
    vkCmdBindIndexBuffer2KHR =
        PFN_vkCmdBindIndexBuffer2KHR(vkGetDeviceProcAddr(device, "vkCmdBindIndexBuffer2KHR"));
    if (!vkCmdBindIndexBuffer2) vkCmdBindIndexBuffer2 = vkCmdBindIndexBuffer2KHR;
    vkGetRenderingAreaGranularityKHR = PFN_vkGetRenderingAreaGranularityKHR(
    ...
    vkCmdSetDepthClampRangeEXT =
        PFN_vkCmdSetDepthClampRangeEXT(vkGetDeviceProcAddr(device, "vkCmdSetDepthClampRangeEXT"));

    //=== VK_KHR_pipeline_binary ===
    vkCreatePipelineBinariesKHR =
        PFN_vkCreatePipelineBinariesKHR(vkGetDeviceProcAddr(device, "vkCreatePipelineBinariesKHR"));
    vkDestroyPipelineBinaryKHR =
        PFN_vkDestroyPipelineBinaryKHR(vkGetDeviceProcAddr(device, "vkDestroyPipelineBinaryKHR"));
    vkGetPipelineKeyKHR =
        PFN_vkGetPipelineKeyKHR(vkGetDeviceProcAddr(device, "vkGetPipelineKeyKHR"));
    vkGetPipelineBinaryDataKHR =
        PFN_vkGetPipelineBinaryDataKHR(vkGetDeviceProcAddr(device, "vkGetPipelineBinaryDataKHR"));
    vkReleaseCapturedPipelineDataKHR = PFN_vkReleaseCapturedPipelineDataKHR(
        vkGetDeviceProcAddr(device, "vkReleaseCapturedPipelineDataKHR"));

    //=== VK_QCOM_tile_properties ===
    vkGetFramebufferTilePropertiesQCOM = PFN_vkGetFramebufferTilePropertiesQCOM(
        vkGetDeviceProcAddr(device, "vkGetFramebufferTilePropertiesQCOM"));
    vkGetDynamicRenderingTilePropertiesQCOM = PFN_vkGetDynamicRenderingTilePropertiesQCOM(
        vkGetDeviceProcAddr(device, "vkGetDynamicRenderingTilePropertiesQCOM"));

    //=== VK_KHR_swapchain_maintenance1 ===
    vkReleaseSwapchainImagesKHR =
        PFN_vkReleaseSwapchainImagesKHR(vkGetDeviceProcAddr(device, "vkReleaseSwapchainImagesKHR"));

    //=== VK_NV_cooperative_vector ===
    vkConvertCooperativeVectorMatrixNV = PFN_vkConvertCooperativeVectorMatrixNV(
        vkGetDeviceProcAddr(device, "vkConvertCooperativeVectorMatrixNV"));
    vkCmdConvertCooperativeVectorMatrixNV = PFN_vkCmdConvertCooperativeVectorMatrixNV(
        vkGetDeviceProcAddr(device, "vkCmdConvertCooperativeVectorMatrixNV"));

    //=== VK_NV_low_latency2 ===
    vkSetLatencySleepModeNV =
        PFN_vkSetLatencySleepModeNV(vkGetDeviceProcAddr(device, "vkSetLatencySleepModeNV"));
    vkLatencySleepNV = PFN_vkLatencySleepNV(vkGetDeviceProcAddr(device, "vkLatencySleepNV"));
    vkSetLatencyMarkerNV =
        PFN_vkSetLatencyMarkerNV(vkGetDeviceProcAddr(device, "vkSetLatencyMarkerNV"));
    vkGetLatencyTimingsNV =
        PFN_vkGetLatencyTimingsNV(vkGetDeviceProcAddr(device, "vkGetLatencyTimingsNV"));
    vkQueueNotifyOutOfBandNV =
        PFN_vkQueueNotifyOutOfBandNV(vkGetDeviceProcAddr(device, "vkQueueNotifyOutOfBandNV"));

    //=== VK_ARM_data_graph ===
    vkCreateDataGraphPipelinesARM = PFN_vkCreateDataGraphPipelinesARM(
        vkGetDeviceProcAddr(device, "vkCreateDataGraphPipelinesARM"));
    vkCreateDataGraphPipelineSessionARM = PFN_vkCreateDataGraphPipelineSessionARM(
        vkGetDeviceProcAddr(device, "vkCreateDataGraphPipelineSessionARM"));
    vkGetDataGraphPipelineSessionBindPointRequirementsARM =
        PFN_vkGetDataGraphPipelineSessionBindPointRequirementsARM(
            vkGetDeviceProcAddr(device, "vkGetDataGraphPipelineSessionBindPointRequirementsARM"));
    vkGetDataGraphPipelineSessionMemoryRequirementsARM =
        PFN_vkGetDataGraphPipelineSessionMemoryRequirementsARM(
            vkGetDeviceProcAddr(device, "vkGetDataGraphPipelineSessionMemoryRequirementsARM"));
    vkBindDataGraphPipelineSessionMemoryARM = PFN_vkBindDataGraphPipelineSessionMemoryARM(
        vkGetDeviceProcAddr(device, "vkBindDataGraphPipelineSessionMemoryARM"));
    vkDestroyDataGraphPipelineSessionARM = PFN_vkDestroyDataGraphPipelineSessionARM(
        vkGetDeviceProcAddr(device, "vkDestroyDataGraphPipelineSessionARM"));
    vkCmdDispatchDataGraphARM =
        PFN_vkCmdDispatchDataGraphARM(vkGetDeviceProcAddr(device, "vkCmdDispatchDataGraphARM"));
    vkGetDataGraphPipelineAvailablePropertiesARM = PFN_vkGetDataGraphPipelineAvailablePropertiesARM(
        vkGetDeviceProcAddr(device, "vkGetDataGraphPipelineAvailablePropertiesARM"));
    vkGetDataGraphPipelinePropertiesARM = PFN_vkGetDataGraphPipelinePropertiesARM(
        vkGetDeviceProcAddr(device, "vkGetDataGraphPipelinePropertiesARM"));

    //=== VK_EXT_attachment_feedback_loop_dynamic_state ===
    vkCmdSetAttachmentFeedbackLoopEnableEXT = PFN_vkCmdSetAttachmentFeedbackLoopEnableEXT(
        vkGetDeviceProcAddr(device, "vkCmdSetAttachmentFeedbackLoopEnableEXT"));
    ...
    //=== VK_KHR_line_rasterization ===
    vkCmdSetLineStippleKHR =
        PFN_vkCmdSetLineStippleKHR(vkGetDeviceProcAddr(device, "vkCmdSetLineStippleKHR"));
    if (!vkCmdSetLineStipple) vkCmdSetLineStipple = vkCmdSetLineStippleKHR;
    ...
    vkCmdBindDescriptorSets2KHR =
        PFN_vkCmdBindDescriptorSets2KHR(vkGetDeviceProcAddr(device, "vkCmdBindDescriptorSets2KHR"));
    if (!vkCmdBindDescriptorSets2) vkCmdBindDescriptorSets2 = vkCmdBindDescriptorSets2KHR;
    vkCmdPushConstants2KHR =
        PFN_vkCmdPushConstants2KHR(vkGetDeviceProcAddr(device, "vkCmdPushConstants2KHR"));
    if (!vkCmdPushConstants2) vkCmdPushConstants2 = vkCmdPushConstants2KHR;
    vkCmdPushDescriptorSet2KHR =
        PFN_vkCmdPushDescriptorSet2KHR(vkGetDeviceProcAddr(device, "vkCmdPushDescriptorSet2KHR"));
    if (!vkCmdPushDescriptorSet2) vkCmdPushDescriptorSet2 = vkCmdPushDescriptorSet2KHR;
    vkCmdPushDescriptorSetWithTemplate2KHR = PFN_vkCmdPushDescriptorSetWithTemplate2KHR(
        vkGetDeviceProcAddr(device, "vkCmdPushDescriptorSetWithTemplate2KHR"));
    if (!vkCmdPushDescriptorSetWithTemplate2)
      vkCmdPushDescriptorSetWithTemplate2 = vkCmdPushDescriptorSetWithTemplate2KHR;
    vkCmdSetDescriptorBufferOffsets2EXT = PFN_vkCmdSetDescriptorBufferOffsets2EXT(
    ...
    vkCmdBindDescriptorBufferEmbeddedSamplers2EXT =
        PFN_vkCmdBindDescriptorBufferEmbeddedSamplers2EXT(
            vkGetDeviceProcAddr(device, "vkCmdBindDescriptorBufferEmbeddedSamplers2EXT"));

    //=== VK_QCOM_tile_memory_heap ===
    vkCmdBindTileMemoryQCOM =
        PFN_vkCmdBindTileMemoryQCOM(vkGetDeviceProcAddr(device, "vkCmdBindTileMemoryQCOM"));

    //=== VK_NV_external_compute_queue ===
    vkCreateExternalComputeQueueNV = PFN_vkCreateExternalComputeQueueNV(
        vkGetDeviceProcAddr(device, "vkCreateExternalComputeQueueNV"));
    vkDestroyExternalComputeQueueNV = PFN_vkDestroyExternalComputeQueueNV(
        vkGetDeviceProcAddr(device, "vkDestroyExternalComputeQueueNV"));
    vkGetExternalComputeQueueDataNV = PFN_vkGetExternalComputeQueueDataNV(
        vkGetDeviceProcAddr(device, "vkGetExternalComputeQueueDataNV"));

    //=== VK_NV_cluster_acceleration_structure ===
    vkGetClusterAccelerationStructureBuildSizesNV =
        PFN_vkGetClusterAccelerationStructureBuildSizesNV(
            vkGetDeviceProcAddr(device, "vkGetClusterAccelerationStructureBuildSizesNV"));
    vkCmdBuildClusterAccelerationStructureIndirectNV =
        PFN_vkCmdBuildClusterAccelerationStructureIndirectNV(
            vkGetDeviceProcAddr(device, "vkCmdBuildClusterAccelerationStructureIndirectNV"));

    //=== VK_NV_partitioned_acceleration_structure ===
    vkGetPartitionedAccelerationStructuresBuildSizesNV =
        PFN_vkGetPartitionedAccelerationStructuresBuildSizesNV(
            vkGetDeviceProcAddr(device, "vkGetPartitionedAccelerationStructuresBuildSizesNV"));
    vkCmdBuildPartitionedAccelerationStructuresNV =
        PFN_vkCmdBuildPartitionedAccelerationStructuresNV(
    ...
    vkUpdateIndirectExecutionSetShaderEXT = PFN_vkUpdateIndirectExecutionSetShaderEXT(
        vkGetDeviceProcAddr(device, "vkUpdateIndirectExecutionSetShaderEXT"));

#if defined(VK_USE_PLATFORM_METAL_EXT)
    //=== VK_EXT_external_memory_metal ===
    vkGetMemoryMetalHandleEXT =
        PFN_vkGetMemoryMetalHandleEXT(vkGetDeviceProcAddr(device, "vkGetMemoryMetalHandleEXT"));
    vkGetMemoryMetalHandlePropertiesEXT = PFN_vkGetMemoryMetalHandlePropertiesEXT(
        vkGetDeviceProcAddr(device, "vkGetMemoryMetalHandlePropertiesEXT"));
#endif /*VK_USE_PLATFORM_METAL_EXT*/

    //=== VK_EXT_fragment_density_map_offset ===
    vkCmdEndRendering2EXT =
        PFN_vkCmdEndRendering2EXT(vkGetDeviceProcAddr(device, "vkCmdEndRendering2EXT"));
  }

  template <typename DynamicLoader>
  void init(Instance const &instance, Device const &device,
            DynamicLoader const &dl) VULKAN_HPP_NOEXCEPT {
    PFN_vkGetInstanceProcAddr getInstanceProcAddr =
        dl.template getProcAddress<PFN_vkGetInstanceProcAddr>("vkGetInstanceProcAddr");
    PFN_vkGetDeviceProcAddr getDeviceProcAddr =
        dl.template getProcAddress<PFN_vkGetDeviceProcAddr>("vkGetDeviceProcAddr");
    init(static_cast<VkInstance>(instance), getInstanceProcAddr, static_cast<VkDevice>(device),
